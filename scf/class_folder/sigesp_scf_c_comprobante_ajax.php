...
***********************************************************************************/
	...
	switch($ls_proceso)
	{
		case "NUEVO":
			...
			if($lb_valido)
			{			
				$arrResultado=$io_comprobante->io_intscg->uf_obtener_comprobante($io_comprobante->ls_codemp,$ls_procede,$ls_comprobante,$ld_fecha,$ls_codban,$ls_ctaban,$ls_tipdes,$ls_cedbene,$ls_codpro);
				$ld_fecha = $arrResultado['adt_fecha'];
				$ls_tipdes = $arrResultado['as_tipo_destino'];
		...
		case "LOADCOMPROBANTE":
			$arrResultado=$io_comprobante->uf_load_cuentas_contables($ls_procede,$ls_comprobante,$ld_fecha,$ls_codban,$ls_ctaban,$io_ds_scgcuentas);
			$io_ds_scgcuentas = $arrResultado['io_ds_scgcuentas'];
	...
	{	
		...
		if($rs_cuentas == NULL)
		{
			...
			$arrResultado=$io_comprobante->uf_obtener_cuentas_contables($ls_procede,$ls_comprobante,$ld_fecha,$ls_codban,$ls_ctaban,$rs_cuentas);
			$rs_cuentas = $arrResultado['rs_data'];
			$lb_valido = $arrResultado['lb_valido'];
		}
