...
com.sigesp.vista.comCatalogoPersonal = function(options) {
	...
	this.buscarDataCatalogo = function() {
		...
		var valorCampo = '';
		var cuentaNoVacio = 0;
		var codper  = Ext.getCmp('codper'+options.idComponente).getValue();
		var cedper  = Ext.getCmp('cedper'+options.idComponente).getValue();
		var nomper  = Ext.getCmp('nomper'+options.idComponente).getValue();
		var apeper  = Ext.getCmp('apeper'+options.idComponente).getValue();
		var esBeneficiario = '';
		if(codper!='')
		{
			cuentaNoVacio++;
		...
		if(options.valoresBeneficiario!='')
		{
			esBeneficiario=options.valoresBeneficiario;
		}
		
		nuevosparamentros = nuevosparamentros +",'mcodper':'"+codper+"','mcedper':'"+cedper+"','mnomper':'"+nomper+"','mapeper':'"+apeper+"','mesbeneficiario':'"+esBeneficiario+"'";
	...
	this.formBusquedaCat = new Ext.FormPanel({
		...
		items: [{
			...
			items:[{
				...
				labelSeparator :'',
				fieldLabel: 'C&#233;dula',
				id: 'cedper'+options.idComponente,									
				width: 120,
				autoCreate: {tag: 'input', type: 'text', size: '100', autocomplete: 'off', maxlength: '10', onkeypress: "return keyRestrict(event,'0123456789');"},
				changeCheck: this.filtrarStore.createDelegate(this, ['cedper'], 0), 
				initEvents: function(){
					AgregarKeyPress(this);
				}
			},{
				...
				labelSeparator :'',
				fieldLabel: 'Nombre',
				id: 'nomper'+options.idComponente,
				width: 450,
				changeCheck: this.filtrarStore.createDelegate(this, ['nomper'], 0),
				initEvents: function(){
					AgregarKeyPress(this);
				}	
			},{
				...
				labelSeparator :'',
				fieldLabel: 'Apellido',
				id: 'apeper'+options.idComponente,
				width: 450,
				changeCheck: this.filtrarStore.createDelegate(this, ['apeper'], 0), 
				initEvents: function(){
					AgregarKeyPress(this);
				}
			},{
				...
				id: 'catbtnBuscar',
				text: 'Buscar',
				style: 'position:absolute;left:520px;top:140px',
				iconCls: 'menubuscar',
	...
	var cmProveedor = new Ext.grid.ColumnModel([
		 {header: "<H1 align='center'>C&#243;digo</H1>", width: 20, sortable: true,   dataIndex: 'codper'},
		 {header: "<H1 align='center'>C&#233;dula</H1>", width: 30, sortable: true, dataIndex: 'cedper'},
		 {header: "<H1 align='center'>Nombre</H1>", width: 30, sortable: true, dataIndex: 'nomper'},
		 {header: "<H1 align='center'>Apellido</H1>", width: 30, sortable: true, dataIndex: 'apeper'}
	]);
	...
	this.cerrarVentana = function(){
		...
		Ext.getCmp('codper'+options.idComponente).reset();
		Ext.getCmp('cedper'+options.idComponente).reset();
		Ext.getCmp('nomper'+options.idComponente).reset();
		Ext.getCmp('apeper'+options.idComponente).reset();
	...
	this.vencatalogo = new Ext.Window({
		title: "<H1 align='center'>Cat&#225;logo de Personal</H1>",
		autoScroll:true,
        width:670,
        height:580,
        modal: true,
