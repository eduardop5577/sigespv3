...
com.sigesp.vista.comCatalogoBancos = function(options) {
	...
	this.buscarDataCatalogo = function() {
		...
		var valorCampo = '';
		var cuentaNoVacio = 0;
		var codban  = Ext.getCmp('catcodban'+options.idComponente).getValue();
		var nomban  = Ext.getCmp('catnomban'+options.idComponente).getValue();
		if(codban!=''){
			cuentaNoVacio++;
		}
		if(nomban!=''){
	...
	this.formBusquedaCat = new Ext.FormPanel({
		...
		items: [{
			...
			items:[{
				...
				id: 'catnomban'+options.idComponente,
				width: 330,
				changeCheck: this.filtrarStore.createDelegate(this, ['nomban'], 0),
				initEvents: function(){
					AgregarKeyPress(this);
				}	
			}]  
	...
	var cmBanco = new Ext.grid.ColumnModel([
		 {header: "<H1 align='center'>C�digo</H1>", width: 10, sortable: true,   dataIndex: 'codban'},
		 {header: "<H1 align='center'>Denominaci�n</H1>", width: 18, sortable: true, dataIndex: 'nomban'}
	]);
	...
	this.cerrarVentana = function(){
		...
		Ext.getCmp('catcodban'+options.idComponente).reset();
		Ext.getCmp('catnomban'+options.idComponente).reset();
	...
	this.vencatalogo = new Ext.Window({
		title: "<H1 align='center'>Cat�logo de Bancos</H1>",
		autoScroll:true,
        width:500,
        height:450,
        modal: true,
