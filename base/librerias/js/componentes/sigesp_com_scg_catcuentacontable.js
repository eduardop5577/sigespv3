...
//	this.opcionAceptar     = options.opcionaceptar;
	...
	this.buscarDataCatalogo = function() {
		...
		var valorCampo = '';
		var cuentaNoVacio = 0;
		var sc_cuenta  = Ext.getCmp('cuenta'+options.idComponente).getValue();
		...
		nuevosparamentros = nuevosparamentros +",'sc_cuenta':'"+sc_cuenta+"','denominacion':'"+denominacion+"','status':'"+status+"'";
	...
	var cmCuentaContable = new Ext.grid.ColumnModel([
		 {header: "<H1 align='center'>Cuenta</H1>", width: 30, sortable: true,   dataIndex: 'sc_cuenta'},
		 {header: "<H1 align='center'>Denominaci&#243;n</H1>", width: 90, sortable: true, dataIndex: 'denominacion'}
	]);
	...
	this.gridcatalogo.getView().getRowClass = function(record, index)
	{
		...
		var color = 'celda_blanca';
		if(this.validarCuenta)
		{
			if(record.data.status!='S')
			{
				color = 'celda-azul';
			}
		}
	...
	this.setDataCampo = function()
	{
		...
		this.validarCuentaMayor = true;
		if(options.validarCuentaMayor!=undefined)
		{
			this.validarCuentaMayor = options.validarCuentaMayor;
		}
		var registrocat = this.gridcatalogo.getSelectionModel().getSelected();
		if(registrocat!= undefined)
		{
			if (((registrocat.get('status')!='S') && (this.validarCuenta))||(!this.validarCuentaMayor))
			{
			...
			else
			{
				Ext.MessageBox.show({
					title:'Mensaje',
					msg:'Debe seleccionar un registro disponible a procesar',
					buttons: Ext.Msg.OK,
					icon: Ext.MessageBox.INFO,
				});
			}
	...
	this.vencatalogo = new Ext.Window({
		title: "<H1 align='center'>Cat&#225;logo de Cuentas Contables</H1>",
		autoScroll:true,
        width:670,
        height:520,
        modal: true,
