...
com.sigesp.vista.comCatalogoProcedencia = function(options) {
	...
	this.buscarDataCatalogo = function() {
		...
		var valorCampo = '';
		var cuentaNoVacio = 0;
		var procede  = Ext.getCmp('procede'+options.idComponente).getValue();
		...
		nuevosparamentros = nuevosparamentros +",'procede':'"+procede+"','descripcion':'"+descripcion+"'";
	...
	this.formBusquedaCat = new Ext.FormPanel({
		...
		items: [{
			...
			items:[{
				...
				labelSeparator :'',
				fieldLabel: 'Procede',
				id: 'procede'+options.idComponente,									
				width: 120,
				autoCreate: {tag: 'input', type: 'text', size: '100', autocomplete: 'off', maxlength: '6', onkeypress: "return keyRestrict(event,'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ');"},
				changeCheck: this.filtrarStore.createDelegate(this, ['procede'], 0), 
				initEvents: function(){
				...
				width: 330,
				changeCheck: this.filtrarStore.createDelegate(this, ['desproc'], 0),
				initEvents: function(){
					AgregarKeyPress(this);
				}	
			}]  
	...
	var cmCtaBanco = new Ext.grid.ColumnModel([
		 {header: "<H1 align='center'>Procede</H1>", width: 30, sortable: true, dataIndex: 'procede'},
		 {header: "<H1 align='center'>Descripci&#243;n</H1>", width: 150, sortable: true, dataIndex: 'desproc'}
	]);
	...
	this.cerrarVentana = function(){
		...
		Ext.getCmp('procede'+options.idComponente).reset();
	...
	this.vencatalogo = new Ext.Window({
		title: "<H1 align='center'>Cat�logo de Procedencia</H1>",
		autoScroll:true,
        width:560,
        height:430,
        modal: true,
