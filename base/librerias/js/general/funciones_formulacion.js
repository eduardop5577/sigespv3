	...
	if (banderaNuevo) {
		...
		Ext.Ajax.request({
			...
			{ 
				...
				{
					Ext.MessageBox.alert('Mensaje', 'No se pudo realizar la operaci�n');
					LimpiarCampos();
				}
			},
			failure: function ( result, request) { 
...
/*******************************************************************************
 * @Funci�n que valida un dato de acuerdo a varios tipos de validaci�n.
 * @Par�metros: id: propiedad id del objeto del formulario a validar. long
 *              longitud del campo, tipoVal: tipo de validaci�n.
	...
	for (i=0;i<arVal.length;i++)
	{
		...
		case 'nombre': // solo letras
			...
			else
			{
				...
				{
					ch=val.charAt(r);					  
					if(validos.search(ch) == -1) // busca en la cadena
						// validos el caracter
						// ch
					{
		...
		case 'telefono':
			val = obj.value;	
			var er_tlf = /^\d{4}-\d{7}$/; // expresi�n regular para
			// telefono con formato ejm:
			// 0251-5555555
			if(!er_tlf.test(val))
			{
		...
		case 'vacioemail':
			...
			if ((longitud <= long) && longitud>0)
			{			
				var filtro=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/; // expresi�n
				// regular
				// para
				// emails
				if (!filtro.test(val)) // test compara la cadena val con la
					// de la expresi�n regular
				{
		...
		case 'numero': // para solo numeros
			...
			if (longitud <= long)
			{			
				var er_numero=/^\d+$/; // expresi�n regular para solo
				// digitos
...
//--------------------------------------------------------
//Funci�n que valida que solo se incluyan n�meros(1234567890),guiones(-) y
//Espacios en blanco
//--------------------------------------------------------
...
//----------------------------------------------------------------------------------------------
//Funci�n que valida para que se incluyan datos alfanumericos y guiones(-) para
//los codigos
//----------------------------------------------------------------------------------------------
