...
class class_datastore
{
	...
	function getRowCount($column)
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////
		//Verifico si ya se han creado indices<br />
		$a=0;
		if(count((array)$this->data)>0)
		{
			...
			if(array_key_exists($valor,$this->data))
			{
				$a=count((array)array_keys($this->data[$column]));
			}
			else
			{
				$a=0;
	...
	function  deleteRow($column,$pos)
	{
		...
		$a=$this->getRowCount($valor);//Obtiene el total de filas del datastore.
		$arr=array_keys($this->data);//Obtiene un arreglo con las columnas del datastore.
		$totkeys=count((array)$arr);//Obtiene el total de columnas del datastore.
		if($pos<=$a)
		{
	...
	function findValues($values,$column)
	{
		...
		$rows=$this->getRowCount($valor);//Obtiene el total de columnas del datastore.
		$totkeys=count((array)$values);
		$valor=-1;		
		for($y=1;$y<=$rows;$y++)
		{
			$lb_valido=true;		
	...
	function group_by($aa_items,$sum,$column)
	{
		...
		$li_total=$this->getRowCount($valor);//Obtengo el total de filas.
		$pos=0;//Variable de comparacion de registro encontrado.
		$totcols=count((array)$aa_items);//Arreglo de claves del group.
		$arrcol=array_keys($this->data);//Arreglo de claves(Columnas).
		$totcolsdata=count((array)$arrcol);//Total de columnas.
	...
	function uf_cuadrar_montos($columna)
	{
		...
		$li_total=$this->getRowCount($valor);		
		for($li_i=1;$li_i<=$li_total;$li_i++)
		{
