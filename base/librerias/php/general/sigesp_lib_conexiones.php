...
class conexiones {
	...
	{		
			global $ruta;
			if(!isset($ruta)){$ruta="../";}
			require_once($ruta."base/librerias/php/general/sigesp_lib_mensajes.php");
	...
	function nombre_form(){
		...
		$cantidad = count((array)$formulario);
	...
	function conexion($query_rs,$opciones,$informacion = '',$modo='select',$campo_id = '',$base_de_datos = '',$gestor = '',$usuario=''){
			...
			if($_SESSION["ls_gestor"] == 'POSTGRES' and $modo=='insert'){$query_rs = $query_rs.' ';}
			...
			switch($modo){
					  ...
					  case "insert":
								if($_SESSION["ls_gestor"] == 'MYSQLT' or $_SESSION["ls_gestor"] == 'MYSQLI'){ return mysql_insert_id();}
	...
	function formatea_fecha_bd($var_fecha){// CONVIERTE LA FECHA A MYSQL o POSTGRES
			...
			preg_match( "([0-9]{1,2})/([0-9]{1,2})/([0-9]{2,4})", $var_fecha, $lugarx);
	...
	function formatea_fecha_normal($fechax){// CONVIERTE LA FECHA A MYSQL A NORMAL.
		preg_match( "([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})", $fechax, $lugarx);
