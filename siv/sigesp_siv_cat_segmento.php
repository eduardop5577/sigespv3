    ...
    if($rs_data===false)
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_tiposegmento=$row["tipo"];
				switch ($ls_tipo)
				{
					case"":                               
                                            print "<td><a href=\"javascript: aceptar('$ls_codseg','$ls_denominacion','$ls_tiposegmento');\">".$ls_codseg."</a></td>";
                                            print "<td>".$ls_denominacion."</td>";
                                            print "</tr>";			
					break;
					case"generico":
                                            print "<td><a href=\"javascript: aceptar_generico('$ls_codseg','$ls_denominacion');\">".$ls_codseg."</a></td>";
                                            print "<td>".$ls_denominacion."</td>";
                                            print "</tr>";			
					break;
                                }
...
<script >
	function aceptar(ls_codseg,ls_denominacion,ls_tipo)
	{
        ...
        function aceptar_generico(ls_codseg,ls_denominacion)
	{
		opener.document.form1.txtcodseg.value=ls_codseg;	
		opener.document.form1.txtcodseg.readOnly=true;
		opener.document.form1.txtdesseg.value=ls_denominacion;
		opener.document.form1.txtdesseg.readOnly=true;
		close();
	}
        ...
        function ue_search()
	{
		...
		f.action="sigesp_siv_cat_segmento.php?tipo=<?PHP print $ls_tipo;?>";
		f.submit();
	}
</script>
</html>
