...
<?php
	...
	if($ls_operacion=="BUSCAR")
	{
		$ls_sql="SELECT siv_articulo.*,".
				...
				"        WHERE siv_articulo.codemp=siv_articulo.codemp".
				"		   AND siv_articulo.codartpri=siv_articulo.codart) as denartpri,".
				...
				"        WHERE siv_dt_movimiento.codart=siv_articulo.codart) as movimiento".
				"  FROM siv_articulo".
		...
		while(!$rs_data->EOF)
		{
			...
			$ls_estproter= $rs_data->fields["estproter"];
			$li_movimiento=     $rs_data->fields["movimiento"];
			if ($ls_tipart=="1")
			{
			...
			$li_reoart= number_format($li_reoart,2,",",".");
				...
				print "'$ls_lote','$ls_carcom','$ls_cod_pro','$ls_nom_pro','$ls_sccuentainv','$ls_scdencuentainv','$ls_estproter','$li_movimiento');\">".$ls_codart."</a></td>";
...
<script >
	function aceptar(ls_codart,ls_denart,ls_codtipart,ls_dentipart,ls_codunimed,ls_denunimed,ld_feccreart,ls_obsart,li_exiart,
					 ...
					 ls_tipart,ls_estartgen,ls_codartpri,ls_denartpri,ls_lote,ls_carcom,ls_cod_pro,ls_nom_pro,ls_sccuentainv,
					 ls_scdencuentainv,ls_estproter,li_movimiento)
	{
		...
		opener.document.form1.hidmovimiento.value=li_movimiento;
		if(ls_estproter==1)
		{
			opener.document.form1.chkestproter.checked=true;
		}
		...
		opener.document.form1.btnmateriales.disabled=false;
