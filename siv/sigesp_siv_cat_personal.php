...
$li_row=$io_fun_siv->uf_obtenervalor_get("row","");
if ($li_row=="")
   {
	 $li_row=$io_fun_siv->uf_obtenervalor("hidrow","");
   }
$operacion  = $io_fun_siv->uf_obteneroperacion();
$ls_destino = $io_fun_siv->uf_obtenervalor_get("destino","");
if($operacion=="BUSCAR")
{$ls_destino=$io_fun_siv->uf_obtenervalor("destino","");}
...
{
	opener.document.formulario.txtcodrespri.value=ls_codper;
	opener.document.formulario.txtdenrespri.value=ls_nomper+" "+ls_apeper;
	close();
}

function ue_usorep(ls_codper,ls_cedper,ls_nomper,ls_apeper)
{
	opener.document.formulario.txtcodresuso.value=ls_codper;
	opener.document.formulario.txtdenresuso.value=ls_nomper+" "+ls_apeper;
	close();
}
...
function ue_search()
{
	f=document.form1;
  	...
  	f.action="sigesp_siv_cat_personal.php";
  	f.submit();
}
