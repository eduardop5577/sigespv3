...
class sigesp_siv_c_recepcion
{
	...
	function uf_siv_insert_recepcion($as_codemp,$as_numordcom,$as_codpro,$as_codalm,$ad_fecrec,$as_obsrec,$as_codusu,$as_estpro,$as_estrec,$as_numconrec,$aa_seguridad) 
	{
		...
		$arrResultado['as_numconrec']=$as_numconrec;
	...
	function uf_siv_select_dt_recepcion($as_codemp,$as_numordcom)
	{
		...
		{
			case "MYSQLT":
				$ls_cadena="CONCAT(numordcom,' ',numconrec)";
				break;
			case "MYSQLI":
				$ls_cadena="CONCAT(numordcom,' ',numconrec)";
				break;
			case "POSTGRES":
				$ls_cadena="numordcom||' '||numconrec";
				break;
			case "oci8po":
				$ls_cadena=$this->con->Concat('numordcom','numconrec');
				break;
		}
		$ls_sql="SELECT * FROM siv_dt_recepcion".
				" WHERE codemp='". $as_codemp ."'".
				"   AND $ls_cadena NOT IN (SELECT $ls_cadena FROM siv_recepcion WHERE estrevrec='0')".
				"   AND numordcom='". $as_numordcom ."'";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
	...
	function uf_siv_obtener_dt_pendiente($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				{
					$li_total=count((array)$la_alternos);
					if($li_total>0)
					{
			  ...
			  $ls_sql="SELECT siv_dt_recepcion.*,siv_articulo.codunimed,siv_articulo.denart, ".
				...
				else
				{
					while($row=$this->io_sql->fetch_row($rs_data1))
					{
						...
						$li_canart=    "";
						$li_montotart= "";
						$arrResultado=$this->uf_select_articulosrelacionados($as_codemp,$as_numordcom,$ls_codart,$ls_codartaux,$li_adicionales);
						$li_adicionales = $arrResultado['ai_adicionales'];
						...
						if($li_penart!=0.00)
						{
							...
							else
							{
								...
								$li_canoriart=number_format($li_canoriart,3,",",".");
								$li_penart=number_format($li_penart,3,",",".");
							}
							...
							$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."'  readonly>";
							$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."'  $ls_funcion onBlur='javascript: ue_calcularpendiente(".$ai_totrows.");'>";
							$ao_object[$ai_totrows][6]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_penart."'  onKeyPress=return(ue_formatonumero(this,'.',',',event)); readonly><input name='hidpendiente".$ai_totrows."' type='hidden' id='hidpendiente".$ai_totrows."' value='".$li_penartaux."'>";
...
//				"   AND siv_articulo.codart<>'".$ls_codartaux."'";
		...
		$arrResultado['ai_adicionales']=$ai_adicionales;
		$arrResultado['lb_valido']=$lb_valido;
		return $arrResultado;		
	...
	function uf_siv_obtener_dt_bienes($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."' readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."'  $ls_funcion  onBlur='javascript: ue_calcularpendiente(".$ai_totrows.");'>";
				$ao_object[$ai_totrows][6]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_penart."' readonly><input name='hidpendiente".$ai_totrows."' type='hidden' id='hidpendiente".$ai_totrows."' value='".$li_penartaux."'>";
				$ao_object[$ai_totrows][7]="<input name=txtpreuniart".$ai_totrows." type=text id=txtpreuniart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_preuniart,2,",",".")."' readonly>".
										   "<input name=txtpreuniartaux".$ai_totrows." type=hidden id=txtpreuniartaux".$ai_totrows." class=sin-borde size=20 maxlength=20 value='".$li_preuniart."' readonly>";
				$ao_object[$ai_totrows][8]="<input name=txtmontotart".$ai_totrows." type=text id=txtmontotart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".$li_montotart."' readonly>";
				$ao_object[$ai_totrows][9]="";
	...
	function uf_siv_obtener_dt_orden($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		...
		if($lb_existe)
		{
			$arrResultado=$this->uf_siv_obtener_dt_pendiente($as_codemp,$as_numordcom,$ai_totrows,$ao_object);
			$ai_totrows = $arrResultado['ai_totrows'];
			$ao_object = $arrResultado['ao_object'];
			$lb_valido = $arrResultado['lb_valido'];
		}
		else
		{
			$arrResultado = $this->uf_siv_obtener_dt_bienes($as_codemp,$as_numordcom,$ai_totrows,$ao_object);
			$ai_totrows = $arrResultado['ai_totrows'];
			$ao_object = $arrResultado['ao_object'];
			$lb_valido = $arrResultado['lb_valido'];
		}
	...
	function uf_siv_obtener_dt_recepcion($as_codemp,$as_numordcom,$as_numconrec,$ai_totrows,$ao_object,$ai_totentsum)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][3]="<input name=txtunidad".$ai_totrows."    type=text id=txtunidad".$ai_totrows."    class=sin-borde size=12 maxlength=12 value='".$ls_unidadaux."' readonly><input name='hidunidad".$ai_totrows."' type='hidden' id='hidunidad".$ai_totrows."' value='".$li_unidad."'>";
				$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."'  readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."' readonly>";
				$ao_object[$ai_totrows][6]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_penart."' readonly>";
				$ao_object[$ai_totrows][7]="<input name=txtpreuniart".$ai_totrows." type=text id=txtpreuniart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_preuniart,2,",",".")."' readonly>".
										   "<input name=txtpreuniartaux".$ai_totrows." type=hidden id=txtpreuniartaux".$ai_totrows." class=sin-borde size=20 maxlength=20 value='".$li_preuniart."' readonly>";
...
//		$this->io_sql->free_result($li_exec);
	...
	function uf_siv_update_recepcion($as_codemp,$as_numordcom,$as_codpro,$as_codalm,$as_obsrec,$as_estpro,$as_estrec,$as_numconrec,$aa_seguridad) 
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_siv_select_catalogo($ai_estnum,$ai_estcmp)
	{
		...
		$arrResultado['ai_estnum']=$ai_estnum;
		$arrResultado['ai_estcmp']=$ai_estcmp;
		$arrResultado['lb_valido']=$lb_valido;
		return $arrResultado;		
...
//				}
		...
		$arrResultado['ls_codartpri']=$ls_codartpri;
	...
	function uf_siv_validar_pendientes($as_codemp,$as_numordcom,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->recepcion MÉTODO->uf_siv_validar_pendientes 1 ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
			  ...
			  switch (strtoupper($ls_gestor))
		   	   {
					  case "OCI8PO":
						$ls_sql="SELECT * FROM".
								"(SELECT siv_dt_recepcion.*,siv_articulo.codunimed,siv_articulo.denart, ".
								  "     (SELECT unidad FROM siv_unidadmedida ".
								  "	      WHERE siv_unidadmedida.codunimed = siv_articulo.codunimed) AS unidades,".
								  "     (SELECT denart FROM siv_articulo ".
								  "       WHERE siv_articulo.codart=siv_articulo.codartpri) AS denartpri,".
								  "     (SELECT unidad FROM siv_articulo,siv_unidadmedida ".
								  "       WHERE siv_articulo.codart=siv_articulo.codartpri".
								  "         AND siv_articulo.codunimed=siv_unidadmedida.codunimed) AS unidadespri".
								  "  FROM siv_dt_recepcion, siv_recepcion, siv_articulo ".
								  " WHERE  siv_dt_recepcion.codemp=siv_recepcion.codemp".
								  "   AND siv_dt_recepcion.codart=siv_articulo.codart".
								  "   AND siv_dt_recepcion.numordcom=siv_recepcion.numordcom".
				...
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->recepcion MÉTODO->uf_siv_validar_pendientes 2 ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				}
				else
				{
					while($row=$this->io_sql->fetch_row($rs_data1))
...
/*******************************************************************************************************************************************/
	...
	function uf_siv_insert_dt_recepcion_lote($as_codemp,$as_numordcom,$as_codart,$as_unidad,$ai_canart,$ai_penart,
	                                    $ai_preuniart,$ai_monsubart,$ai_montotart,$ai_orden,$ai_canoriart,
										$as_numconrec,$aa_seguridad,$ls_artdes="",$ls_arthas="",$ls_codalm,$ld_fecrecbd)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_insert_dt_recepcion
		//         Access: public (sigesp_siv_p_recepcion)
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$lb_valido=$this->incluirLote($as_codart,$ls_codalm,$ls_artdes,$ls_arthas,$ld_fecrecbd,$aa_seguridad);
		if($lb_valido)
		{
			$ls_sql="INSERT INTO siv_dt_recepcion (codemp,numordcom,numconrec,codart,unidad,canart,penart,preuniart,monsubart,montotart,".
					"                              orden,canoriart,serartdes,serarthas)".
					" VALUES ('".$as_codemp."','".$as_numordcom."','".$as_numconrec."','".$as_codart."','".$as_unidad."',".$ai_canart.",".
					"         ".$ai_penart.",".$ai_preuniart.",".$ai_monsubart.",".$ai_montotart.",".$ai_orden.",".
					"         ".$ai_canoriart.",'".$ls_artdes."','".$ls_arthas."')"; 
			$li_row=$this->io_sql->execute($ls_sql);
			if($li_row===false)
			{print $this->io_sql->message;
				$this->io_msg->message("CLASE->recepcion MÉTODO->uf_siv_insert_dt_recepcion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	...
	function incluirLote($ls_codart,$ls_codalm,$ls_artdes,$ls_arthas,$ld_fecrecbd,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for($i=1;$i<=$totmat;$i++)
		{
			...
			if(!$this->validarExiste($ls_codart,$serial))
			{
				$lb_valido=$this->uf_siv_insert_dt_articulo($ls_codart,$ls_codalm,$serial,$ld_fecrecbd,$aa_seguridad);
			}
			$valor=$valor+1;
	...
	function validarExiste($codart,$serial)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->recepcion MÉTODO->validarExiste ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_siv_insert_dt_articulo($ls_codart,$ls_codalm,$ls_serial,$ld_fecrecbd,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_insert_dt_recepcion
		//         Access: public (sigesp_siv_p_recepcion)
		...
		$ls_codemp=$_SESSION["la_empresa"]["codemp"];
		$lb_valido=true;
			$ls_sql="INSERT INTO siv_dt_articulo (codemp,codart,coddetart,codalm,estdetart,fecregdet)".
					" VALUES ('".$ls_codemp."','".$ls_codart."','".$ls_serial."','".$ls_codalm."','R','".$ld_fecrecbd."')";
			$li_row=$this->io_sql->execute($ls_sql);
			if($li_row===false)
			{print $this->io_sql->message;
				$this->io_msg->message("CLASE->recepcion MÉTODO->uf_siv_insert_dt_articulo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
	...
	function uf_siv_obtener_dt_orden_lote($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_obtener_dt_orden
		//         Access: public (sigesp_siv_p_recepcion)
		...
		if($lb_existe)
		{
			$arrResultado=$this->uf_siv_obtener_dt_pendiente_lote($as_codemp,$as_numordcom,$ai_totrows,$ao_object);
			$ai_totrows = $arrResultado['ai_totrows'];
			$ao_object = $arrResultado['ao_object'];
			$lb_valido = $arrResultado['lb_valido'];
		}
		...
		{
			$arrResultado = $this->uf_siv_obtener_dt_bienes_lote($as_codemp,$as_numordcom,$ai_totrows,$ao_object);
			$ai_totrows = $arrResultado['ai_totrows'];
			$ao_object = $arrResultado['ao_object'];
			$lb_valido = $arrResultado['lb_valido'];
		}
	...
	function uf_siv_obtener_dt_pendiente_lote($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_obtener_dt_pendiente
		//         Access: private
			  ...
			  $ls_sql="SELECT siv_dt_recepcion.*,siv_articulo.codunimed,siv_articulo.denart, ".
				...
				else
				{
					while($row=$this->io_sql->fetch_row($rs_data1))
					{
						...
						$ls_artdes=    $row["serartdes"];
						$ls_arthas=    $row["serarthas"];
						if($li_penart!=0.00)
						{
							$li_penartaux=$li_penart;
							if($li_numdecper!="3")
							...
							$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."'  readonly>";
							$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."'  $ls_funcion onBlur='javascript: ue_calcularpendiente(".$ai_totrows.");'>";
							$ao_object[$ai_totrows][6]="<input name=txtartdes".$ai_totrows."    type=text id=txtartdes".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."'  >";
							$ao_object[$ai_totrows][7]="<input name=txtarthas".$ai_totrows."    type=text id=txtarthas".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$ls_artdes."'  >";
							$ao_object[$ai_totrows][8]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$ls_arthas."'  onKeyPress=return(ue_formatonumero(this,'.',',',event)); readonly><input name='hidpendiente".$ai_totrows."' type='hidden' id='hidpendiente".$ai_totrows."' value='".$li_penartaux."'>";
							$ao_object[$ai_totrows][9]="<input name=txtpreuniart".$ai_totrows." type=text id=txtpreuniart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_preuniart,2,",",".")."' readonly>".
													   "<input name=txtpreuniartaux".$ai_totrows." type=hidden id=txtpreuniartaux".$ai_totrows." class=sin-borde size=20 maxlength=20 value='".$li_preuniart."' readonly>";
							$ao_object[$ai_totrows][10]="<input name=txtmontotart".$ai_totrows." type=text id=txtmontotart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".$li_montotart."' readonly>";
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_siv_obtener_dt_bienes_lote($as_codemp,$as_numordcom,$ai_totrows,$ao_object)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_obtener_dt_bienes
		//         Access: private
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."' readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."'  $ls_funcion  onBlur='javascript: ue_calcularpendiente(".$ai_totrows.");'>";
				$ao_object[$ai_totrows][6]="<input name=txtartdes".$ai_totrows."    type=text id=txtartdes".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='' >";
				$ao_object[$ai_totrows][7]="<input name=txtarthas".$ai_totrows."    type=text id=txtarthas".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='' >";
				$ao_object[$ai_totrows][8]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_penart."' readonly><input name='hidpendiente".$ai_totrows."' type='hidden' id='hidpendiente".$ai_totrows."' value='".$li_penartaux."'>";
				$ao_object[$ai_totrows][9]="<input name=txtpreuniart".$ai_totrows." type=text id=txtpreuniart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_preuniart,2,",",".")."' readonly>".
										   "<input name=txtpreuniartaux".$ai_totrows." type=hidden id=txtpreuniartaux".$ai_totrows." class=sin-borde size=20 maxlength=20 value='".$li_preuniart."' readonly>";
				$ao_object[$ai_totrows][10]="<input name=txtmontotart".$ai_totrows." type=text id=txtmontotart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".$li_montotart."' readonly>";
				$ao_object[$ai_totrows][11]="";
	...
	function uf_siv_obtener_dt_recepcion_lote($as_codemp,$as_numordcom,$as_numconrec,$ai_totrows,$ao_object,$ai_totentsum)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_siv_obtener_dt_recepcion
		//         Access: public (sigesp_siv_p_recepcion)
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][4]="<input name=txtcanoriart".$ai_totrows." type=text id=txtcanoriart".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canoriart."'  readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtcanart".$ai_totrows."    type=text id=txtcanart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_canart."' readonly>";
				$ao_object[$ai_totrows][6]="<input name=txtartdes".$ai_totrows."    type=text id=txtartdes".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$ls_artdes."' readonly>";
				$ao_object[$ai_totrows][7]="<input name=txtarthas".$ai_totrows."    type=text id=txtarthas".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$ls_arthas."' readonly>";
				$ao_object[$ai_totrows][8]="<input name=txtpenart".$ai_totrows."    type=text id=txtpenart".$ai_totrows."    class=sin-borde size=10 maxlength=12 value='".$li_penart."' readonly>";
				$ao_object[$ai_totrows][9]="<input name=txtpreuniart".$ai_totrows." type=text id=txtpreuniart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_preuniart,2,",",".")."' readonly>".
										   "<input name=txtpreuniartaux".$ai_totrows." type=hidden id=txtpreuniartaux".$ai_totrows." class=sin-borde size=20 maxlength=20 value='".$li_preuniart."' readonly>";
				$ao_object[$ai_totrows][10]="<input name=txtmontotart".$ai_totrows." type=text id=txtmontotart".$ai_totrows." class=sin-borde size=14 maxlength=15 value='".number_format ($li_montotart,2,",",".")."' readonly>";
				$ao_object[$ai_totrows][11]="";
				$ao_object[$ai_totrows][12]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.gif alt=Aceptar width=15 height=15 border=0></a>";			
				$ao_object[$ai_totrows][11]="<a href=javascript:uf_dt_activo(".$ai_totrows.");><img src=../shared/imagebank/mas.gif alt=Agregar Seriales width=15 height=15 border=0></a><input name=hclasi".$ai_totrows." type=hidden id=hclasi".$ai_totrows." class=sin-borde size=20 maxlength=20  value='".$ls_clasi."' readonly>";	
