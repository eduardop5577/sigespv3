...
session_start();
	...
	{
		$ls_coddestino="txtcodalmdes";
		$ls_dendestino="txtnomfisdes";
	}
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		if($row=$io_sql->fetch_row($rs_cta))
		{
			...
			for($z=1;$z<=$totrow;$z++)
			{
				...
				print "<td><a href=\"javascript: aceptar('$ls_codalm','$ls_nomfisalm','$ls_status','$li_linea','$ls_coddestino','$ls_dendestino');\">".$ls_codalm."</a></td>";
...
<script >
	function aceptar(ls_codalm,ls_nomfisalm,hidstatus,li_linea,ls_coddestino,ls_dendestino)
	{
		obj=eval("opener.document.form1."+ls_coddestino+li_linea+"");
		obj.value=ls_codalm;
		obj1=eval("opener.document.form1."+ls_dendestino+"");
		obj1.value=ls_nomfisalm;
		close();
	}
