...
<table width="489" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <td height="22"><select name="cmbtipsep" id="cmbtipsep">
	  ...
	  <option value="S">Sin afectación presupuestaria</option>
    </select>
    </td>
    <td height="22">&nbsp;</td>
    <td height="22">&nbsp;</td>
...
<?php
	...
	if (array_key_exists("contable",$_GET))
	{
		$li_contable=$_GET["contable"];
	}
	else
	{
		if(array_key_exists("hidcontable",$_POST))
		{
			$li_contable=$_POST["hidcontable"];
		}
		else
		{
			$li_contable="0";
			...
			if (array_key_exists("txthasta",$_POST))
			{
				...
				if($lb_fechavalida)
				{
					...
					$ls_sql=" SELECT sep_solicitud.numsol,sep_solicitud.coduniadm,sep_solicitud.fecregsol,sep_solicitud.estsol,
					                 sep_solicitud.monto,spg_ep1.estint,trim(spg_ep1.sc_cuenta) as ctascgint,sep_solicitud.codtipsol,sep_solicitud.codfuefin, 
							         (SELECT denuniadm 
									 ...
									 WHERE sep_solicitud.codfuefin=sigesp_fuentefinanciamiento.codfuefin) AS denfuefin 
			  ...
			  while ($row=$io_sql->fetch_row($rs_data))
			        {
					  ...
					  {
					  	  print "<td style=text-align:center width=125><a href=\"javascript: aceptar_costo('$ls_numsol','$ls_fecsol','$ls_monto','$ls_coduniadm','$ls_denuniadm','$ls_estsol','$li_estint','$ls_ctascgint','$ls_codfuefin','$ls_denfuefin');\">".$ls_numsol."</a></td>";
						  print "<td style=text-align:center width=80>".$ls_fecsol."</td>";
						  print "<td style=text-align:right  width=95>".$ls_monto."</td>";
						  print "<td style=text-align:left   width=200 title='".$ls_denuniadm."'>".ltrim($ls_denuniadm)."</td>";
						  print "</tr>";
...
print "</table>";
?>
<input name="hidcontable" type="hidden" id="hidcontable" value="<?php print $li_contable?>">
</p>
</form>      
</body>
<script >
function aceptar(ls_numsol,ls_fecsol,ls_monto,ls_coduniadm,ls_denuniadm,ls_estsol,as_estint,as_ctascgint,ls_codfuefin,ls_denfuefin)
{    
...
function aceptar_costo(ls_numsol,ls_fecsol,ls_monto,ls_coduniadm,ls_denuniadm,ls_estsol,as_estint,as_ctascgint,ls_codfuefin,ls_denfuefin)
{    
	opener.document.form1.txtnumsol.value    = ls_numsol;
	opener.document.form1.txtcoduniadm.value = ls_coduniadm;
	opener.document.form1.txtdenuniadm.value = ls_denuniadm;
...
function ue_search()
{
	...
	contable=f.hidcontable.value;
	f.action="sigesp_catdinamic_sol_eje_pre.php?contable="+contable+"";
	f.submit();
}
