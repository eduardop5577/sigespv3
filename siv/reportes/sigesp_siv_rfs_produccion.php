...
***********************************************************************************/
	...
	$ld_fecemi=    $io_fun_inventario->uf_obtenervalor_get("fecemi","");*/
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=$io_report->uf_select_produccion($ls_codemp,$ls_numtra,"","",""); // Cargar el DS con los datos de la cabecera del reporte
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$lb_valido=$io_report->uf_select_dt_produccion1($ls_codemp,$ls_numtra); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
			...
			$lb_valido=$io_report->uf_select_dt_produccion2($ls_codemp,$ls_numtra); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				$la_data=Array();
				$li_totcan=0;
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
					...
					$li_unidad=     $io_report->dts_reporte->data["unidades"][$li_s];
					$li_totcan=$li_totcan + $li_cantidad;
					$li_total=$li_total + $li_costot;
					if($ls_unidad=="D")
					{
				...
				$lb_valido=$io_report->uf_select_dt_scg($ls_codemp,$ls_numtra); // Obtenemos el detalle del reporte
				if($lb_valido)
				{
					$la_data=Array();
					$li_totcan=0;
					...
					for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
					{
						...
						switch ($ls_debhab) 
						{
							...
							$la_data[$li_s]=array('articulo'=>$ls_denart,'cuenta'=>$ls_cuenta,'debhab'=>$ls_debhab,'monto'=>$li_monto);
					}
