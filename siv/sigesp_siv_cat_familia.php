		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($ls_tipo)
				{
					...
					break;
					case"generico":
                                            print "<td><a href=\"javascript: aceptar_generico('$ls_codfami','$ls_denominacion');\">".$ls_codseg."</a></td>";
                                            print "<td>".$ls_codfami."</td>";
                                            print "<td>".$ls_denominacion."</td>";
                                            print "</tr>";			
					break;
...
<script >
	function aceptar(ls_codseg,ls_denominacion,ls_codfami)
	{
        ...
        function aceptar_generico(ls_codfami,ls_denominacion)
	{
		opener.document.form1.txtcodfam.value=ls_codfami;
		opener.document.form1.txtcodfam.readOnly=true; 
		opener.document.form1.txtdesfam.value=ls_denominacion;
		opener.document.form1.txtdesfam.readOnly=true;
		close();
	}
	...
	function ue_search()
	{
		...
		f.action="sigesp_siv_cat_familia.php?tipo=<?PHP print $ls_tipo;?>&codseg=<?PHP print $ls_codsegmento;?>";
		f.submit();
	}
</script>
</html>
