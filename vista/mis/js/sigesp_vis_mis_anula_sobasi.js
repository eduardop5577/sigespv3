...
Ext.onReady(function(){
	...
	var cmSolicitud = new Ext.grid.ColumnModel([
	    ...
	    {header: "<CENTER>Concepto Anulaci&#243;n</CENTER>", width: 60, sortable: true, dataIndex: 'conanu',editor : new Ext.form.TextArea({allowBlank : false,autoCreate: {tag: 'textarea', type: 'text', onkeypress: "return keyRestrict(event,'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.;,!@%&/\()¿?¡-+*[]{}');"}})}
	]);

	//creando datastore y columnmodel para la grid de obras asignadas
	gridSolicitud = new Ext.grid.EditorGridPanel({
		width:570,
		height:240,
		frame:true,
		title:"<H1 align='center'>Asignaciones Contabilizadas</H1>",
...
//					tituloVentana: "<H1 align='center'>Informaci&#243;n del Comprobante</H1>",
	...
	fromAnulasobasi = new Ext.form.FieldSet({  
	...
	plAnulaSobasi = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Anulaci&#243;n de la Contabilizaci&#243;n de Asignaci&#243;n</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',

		items: [fromAnulasobasi,gridSolicitud]
	});	
	fromAnulasobasi.doLayout();
});
function irCancelar(){
	limpiarFormulario(fromAnulasobasi);
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'anular_sobasi','codsis':'"+sistema+"','nomven':'"+vista+"','fechaanula':'"+fecha+"','arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
		for(var i=0; i<total && valido==true; i++)
		{
			for (i=0; i<total; i++)
			{
				if (i==0) 
				{
					if(grid[i].get('conanu')!=undefined){
						cadenajson += "{'codasi':'"+grid[i].get('codasi')+"','fecasi':'"+grid[i].get('fecasi')+"'," +
			                           ...
			                           "'montotasi':'"+grid[i].get('montotasi')+"','conanu':'"+grid[i].get('conanu')+"'}";
					}
					else{
						Ext.MessageBox.show({
							title:'Mensaje',
							msg:'Debe llenar el concepto de anulaci&#243;n de la Asignaci&#243;n '+grid[i].get('codasi')+' !!!',
							buttons: Ext.Msg.OK,
			...
			Ext.Ajax.request({
				...
				{ 
					...
					var comResultado = new com.sigesp.vista.comResultadoIntegrador({
						tituloVentana: 'Resultado Anulaci&#243;n de Asignaciones',
						anchoLabel: 200,
						labelTotal:'Total asignaciones procesados',
						valorTotal: arrResultado[0],
						labelProcesada:'Total asignaciones anulados',
...
function irBuscar(){
	...
	if (fecasi!='')
	{
		fecasi   =Ext.getCmp('fecasi').getValue().format('Y/m/d');	
	}
