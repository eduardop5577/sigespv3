...
var gridMovEmpaquetado  = null //varibale para almacenar la instacia de objeto de grid de los movimientos 
var fromContEmpaquetado = null //varibale para almacenar la instacia de objeto de formulario 
...
Ext.onReady(function() {
	...
	var	fromContabilzarEmpaquetado = new Ext.FormPanel({
		applyTo: 'formularioMIS',
		width: 920,
		height: 480,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'>Contabilización de Empaquetado</H1>",
		frame: true,
		autoScroll:true,
		items: [fromContEmpaquetado,
		    	gridMovEmpaquetado
		        ]
	});
	
	fromContabilzarEmpaquetado.doLayout();
});
	...
	gridMovEmpaquetado = new Ext.grid.GridPanel({
		width:870,
		height:250,
		frame:true,
	...
	gridMovEmpaquetado.on({
		...
		var comDetalleEmpaquetado = new com.sigesp.vista.comDetalleComprobante({
			tituloVentana: 'Comprobante de Empaquetado',
			anchoVentana: 720,
			altoVentana: 500,
			anchoFormulario: 680,
			altoFormulario:150,
			...
			paramContable: 'ObjSon='+Ext.util.JSON.encode({'operacion': 'empaquetado_detalle_contable',
				'codemppro':registro.get('comprobante'),
				'fecemppro':registro.get('fecha')})
	...
	fromContEmpaquetado = new Ext.form.FieldSet({ 
		    title:'',
...
function irBuscar( )
{
	...
	var JSONObject = {
			'operacion' : 'empaquetado_por_contabilizar',
			'codemppro'    : codemppro,
			'fecemppro'    : fecemppro
}
			...
			if(objetoMovEmpaquetado!=''){
				if(objetoMovEmpaquetado!='0'){
					if(objetoMovEmpaquetado.raiz == null || objetoMovEmpaquetado.raiz ==''){
						...
						gridMovEmpaquetado.store.removeAll();
					}
					else{
						gridMovEmpaquetado.store.loadData(objetoMovEmpaquetado);
					}
				}
				else{
					Ext.MessageBox.show({
...
function irCancelar(){
	limpiarFormulario(fromContEmpaquetado);
	gridMovEmpaquetado.store.removeAll();
}
...
function irProcesar()
{
	...
	var cadenaJson = "{'operacion': 'contabilizar_empaquetado', 'codsis':'"+sistema+"', 'nomven':'"+vista+"', 'feccon': '"+feccon+"', 'arrDetalle':[";				
	var arrProduccion = gridMovEmpaquetado.getSelectionModel().getSelections();
	...
	if (total>0)
	{
		...
		{
			if (i==0) {
				cadenaJson = cadenaJson +"{'comprobante':'"+ arrProduccion[i].get('comprobante')+ "','fecemppro':'"+ arrProduccion[i].get('fecha')+ "'}";
			}
			else
			{
				cadenaJson = cadenaJson +",{'comprobante':'"+ arrProduccion[i].get('comprobante')+ "','fecemppro':'"+ arrProduccion[i].get('fecha')+ "'}";
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizaci&#243;n de Empaquetado',
					anchoLabel: 200,
					labelTotal:'Total Empaquetados procesados',
					valorTotal: arrResultado[0],
					labelProcesada:'Total Empaquetados contabilizados',
