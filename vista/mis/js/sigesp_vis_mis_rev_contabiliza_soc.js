...
//-----------------------------------------------------------------------------------------------------------------------	
	...
	var gridOrdenCompra = new Ext.grid.GridPanel({
			...
			frame:true,
			title:'',
			style: 'position:absolute;left:20px;top:240px',
			autoScroll:true,
     		border:true,
     		ds: dsOrdenCompra,
       		cm: cmOrdenCompra,
		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comDetalleOrdencompra = new com.sigesp.vista.comDetalleComprobante({
					tituloVentana: 'Reverso de Contabilización de Orden de Compra/Servicio',
					anchoVentana: 600,
					altoVentana: 500,
					anchoFormulario: 580,
					altoFormulario:150,
...
//-----------------------------------------------------------------------------------------------------------------------	
//creando store para la tipo compra (reverso)
	var tipcompra = 	[
                    	['-- Bienes/Servicios --','-'],
                    	['Bienes','B'],
                    	['Servicios','S']
...
//Creando el campo catalogo para llamar al proveedor (reverso)
...
//-------------------------------------------------------------------------------------------------------------------------	
	...
	var	fromBusquedaRevSOC = new Ext.form.FieldSet({ 
...
Ext.onReady(function() {
	...
	var	fromReversoContabilzarSOC = new Ext.FormPanel({
		applyTo: 'formulario_reversoSOC',
		width: 920,
		height: 500,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:35px;',
		title: "<H1 align='center'>Reverso de Contabilización de Ordenes de Compra/Servicio</H1>",
		frame: true,
		autoScroll:true,
		items: [fromBusquedaRevSOC,
		        gridOrdenCompra
		        ]
	});
	fromReversoContabilzarSOC.doLayout();
});
...
function irBuscar(){
	...
	var JSONObject = {
			'operacion'   : 'buscar_por_rev_contabilizacion',
			'numordcom' : numordcom,
			'cod_pro' : cod_pro,
			'estcondat' : estcondat,
			'fecaprord' : fecaprord,
...
function irCancelar(){
	limpiarFormulario(fromBusquedaRevSOC);
	gridOrdenCompra.store.removeAll();
}
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_contabilizacion','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	if (total>0)
	{			
		for (i=0; i<total; i++)
		{
			if (i==0) 
			{
				cadenajson += "{'numordcom':'"+arrOrdenCompra[i].get('numordcom')+"','estcondat':'"+arrOrdenCompra[i].get('estcondat')+"'}";                
			}
			else {
				cadenajson += ",{'numordcom':'"+arrOrdenCompra[i].get('numordcom')+"','estcondat':'"+arrOrdenCompra[i].get('estcondat')+"'}";                
			}
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: "<H1 align='center'>Resultado Reversar las Ordenes de Compra</H1>",
					anchoLabel: 200,
