...
var	fromRevContabilizaSobcon = null;
...
Ext.onReady(function(){
	...
	fromRevContabilizaSobcon = new Ext.form.FieldSet({
	...
	plRevContabilizaSobcon = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Reverso de Contabilizaci&#243;n de Contratos</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',

		items: [fromRevContabilizaSobcon,gridSolicitud]
	});	
	plRevContabilizaSobcon.doLayout();
});
function irCancelar(){
	limpiarFormulario(fromRevContabilizaSobcon);
...
function irProcesar()
{
	...
	cadenajson = "{'operacion':'rev_contabilizar_sobcon','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
			...
			for (i=0; i<total; i++)
			{
				if (i==0) 
				{
					cadenajson += "{'codasi':'"+grid[i].get('codasi')+"','codcon':'"+grid[i].get('codcon')+"'," +
							       ...
							       "'fechacontrato':'"+grid[i].get('fechacontrato')+"','fechaanulada':'"+grid[i].get('fechaanulada')+"'," +
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Reversar los Contratos',
					anchoLabel: 200,
