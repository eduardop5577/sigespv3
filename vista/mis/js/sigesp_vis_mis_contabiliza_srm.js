...
var gridMovCobranza  = null //varibale para almacenar la instacia de objeto de grid de los movimientos 
var fromContabilzarSRM = null //varibale para almacenar la instacia de objeto de formulario 
...
Ext.onReady(function() {
	...
	var	p1fromContabilzarSRM = new Ext.FormPanel({
		applyTo: 'formularioSRM',
		width: 935,
		height: 400,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'>Contabilización de resumen de Cobranzas</H1>",
		frame: true,
		autoScroll:true,
		items: [fromContabilzarSRM,
		        gridMovCobranza
		        ]
	});
	
	p1fromContabilzarSRM.doLayout();
});
	...
	gridMovCobranza = new Ext.grid.GridPanel({
		width:870,
		height:250,
		frame:true,
		title:'',
		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comDetalleModificacion = new com.sigesp.vista.comDetalleComprobante({
					tituloVentana: "<H1 align='center'>Rentas Municipales</H1>",
					anchoVentana: 720,
					altoVentana: 500,
					anchoFormulario: 680,
					altoFormulario:150,
	...
	fromContabilzarSRM = new Ext.form.FieldSet({ 
		    title:'',
			...
			if(objetoMovbco!=''){
				if(objetoMovbco!='0'){
					if(objetoMovbco.raiz == null || objetoMovbco.raiz ==''){
						...
						gridMovCobranza.store.removeAll();
					}
					else{
						gridMovCobranza.store.loadData(objetoMovbco);
					}
				}
				else{
					Ext.MessageBox.show({
...
function irCancelar(){
	limpiarFormulario(fromContabilzarSRM);
	gridMovCobranza.store.removeAll();
}

function irProcesar(){
	valido=true;
	var cadenaJson = "{'operacion': 'contabilizar', 'codsis':'"+sistema+"', 'nomven':'"+vista+"', 'arrDetalle':[";				
	var arrModificaciones = gridMovCobranza.getSelectionModel().getSelections();
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizaci&#243;n de Cobranzas',
					anchoLabel: 200,
					labelTotal:'Total de cobros procesados',
					valorTotal: arrResultado[0],
					labelProcesada:'Total de cobros contabilizados',
