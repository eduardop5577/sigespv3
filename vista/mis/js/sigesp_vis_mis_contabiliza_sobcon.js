...
var fromContabilizaSobcon= null;
...
Ext.onReady(function(){
	...
	var cmSolicitud = new Ext.grid.ColumnModel([
	    ...
	    {header: "<CENTER>Nro. Contrato</CENTER>", width: 30, sortable: true, dataIndex: 'codcon'},
	    {header: "<CENTER>Nro. Asignaci&#243;n</CENTER>", width: 30, sortable: true, dataIndex: 'codasi'},
	    {header: "<CENTER>Fecha</CENTER>", width: 30, sortable: true, dataIndex: 'fechacontrato'},
	    {header: "<CENTER>Observaci&#243;n</CENTER>", width: 60, sortable: true, dataIndex: 'obscon'},
	]);
	...
	gridSolicitud = new Ext.grid.GridPanel({
		...
		height:250,
		frame:true,
		title:"<H1 align='center'>Contratos por Contabilizar</H1>",
	...
	fromContabilizaSobcon = new  Ext.form.FieldSet({
	...
	plContabilizaSobcon = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Contabilizaci&#243;n de Contratos</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',
		items: [fromContabilizaSobcon,gridSolicitud]
	});	
	plContabilizaSobcon.doLayout();
	buscarConfig();
});

function irCancelar()
{
	limpiarFormulario(fromContabilizaSobcon);
	gridSolicitud.store.removeAll();
	Ext.getCmp('fecconta').disable();
}
...
function irProcesar()
{
	...
	cadenajson = "{'operacion':'contabilizar_sobcon','codsis':'"+sistema+"','nomven':'"+vista+"', 'feccon':'"+fecha+"', 'arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
			...
			for (i=0; i<total; i++)
			{
				if (i==0) 
				{
					cadenajson += "{'codasi':'"+grid[i].get('codasi')+"','codcon':'"+grid[i].get('codcon')+"'," +
							       "'fecvar':'"+grid[i].get('fecvar')+"','obscon':'"+grid[i].get('obscon')+"'," +
							       "'fechacontrato':'"+grid[i].get('fechacontrato')+"'," +
							       "'fechaanulada':'"+grid[i].get('fechaanulada')+"'," +
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizar los Contratos',
					anchoLabel: 200,
					labelTotal:'Total contratos procesados',
					valorTotal: arrResultado[0],
					labelProcesada:'Total contratos contabilizados',
