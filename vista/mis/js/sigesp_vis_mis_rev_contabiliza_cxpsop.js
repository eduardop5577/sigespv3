...
//-------------------------------------------------------------------------------------------------------------------------					
	...
	var	fromReversoCXP = new Ext.form.FieldSet({  
...
Ext.onReady(function() {
	...
	var	fromContabilzarCXP = new Ext.FormPanel({
		...
		title: "<H1 align='center'>Reverso de Contabilizacion de Solicitud de Orden de Pago</H1>",
		frame: true,
		autoScroll:true,
		items: [fromReversoCXP,
		        gridSolicitud
		        ]
	});
	
	fromReversoCXP.doLayout();
});
...
function irBuscar(){
	...
	var JSONObject = {
		'operacion' : 'buscar_por_rev_contabilizacion_sop',
		'numsol'    : numsol,
		'tipo'      : destino,
		'codigo'    : cod_pro,
		'fecreg'    : fecemisol,
...
function irCancelar(){
	limpiarFormulario(fromReversoCXP);
	gridSolicitud.store.removeAll();	
}

function irProcesar(){
	var cadenaJson = "{'operacion': 'rev_contabilizacion_sop', 'solicitudes':[";				
		...
		Ext.Ajax.request({
			...
			success: function (resultado, request) {
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Reverso de Contabilizaci&#243;n de Solicitudes de Orden de Pago',
					anchoLabel: 200,
