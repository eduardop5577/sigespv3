...
Ext.onReady(function(){
	...
	var	plAproSPI = new Ext.FormPanel({
		applyTo: 'formulario',
		width:618,
		height: 480,
		style: 'position:absolute;left:200px;top:80px',
		title: "<H1 align='center'>Aprobaci&#243;n de Modificaciones Presupuestarias de Ingreso</H1>",
		frame: true,
		autoScroll:true,
		items: [fromAproSPI,
		        gridSolicitud]
...
//	    {header: "Detalle", width: 20, sortable: true, dataIndex: 'detalle',editor : new Ext.form.TextField({allowBlank : true})}
	...
	fromAproSPI = new Ext.form.FieldSet({ 
		    title:'',
...
function irProcesar(){
	...
	fecha = Ext.getCmp('fechaaprob').getValue().format('Y/m/d');
	cadenajson = "{'operacion':'contabilizar_spi','codsis':'"+sistema+"','nomven':'"+vista+"','fecapro':'"+fecha+"','arrDetalle':[";
	total = grid.length;
	if (total>0)
	{			
		for (i=0; i<total; i++)
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizaci&#243;n de Modificaciones Presupuestarias de Ingreso',
					anchoLabel: 200,
