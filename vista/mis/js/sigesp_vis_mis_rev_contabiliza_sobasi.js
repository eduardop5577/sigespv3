...
var fromRevContabilizaSobasi = null;
//-------------------------------------------------------------------------------------------------------------------------	
Ext.onReady(function(){
	...
	fromRevContabilizaSobasi = new  Ext.form.FieldSet({ 
			...
			items:[{
					...
					items: [{
							...
							items: [{
									xtype: 'textfield',
									labelSeparator :'',
									fieldLabel: 'Número de Asignacion',
									id: 'codasi',
									width: 150,
									binding:true,
									hiddenvalue:'',
	...
	plrevcontabilizaSobasi = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Reverso Contabilizaci&#243;n de Asignaci&#243;n</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',

		items: [fromRevContabilizaSobasi,gridSolicitud]
	});	
	fromRevContabilizaSobasi.doLayout();
});
...
function irCancelar(){
	limpiarFormulario(fromRevContabilizaSobasi);
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_contabilizar_sobasi','codsis':'"+sistema+"','nomven':'"+vista+"', 'arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Reverso de Asignaciones',
					anchoLabel: 200,
