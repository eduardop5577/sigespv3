		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comDetalleOrdencompra = new com.sigesp.vista.comDetalleComprobante({
					tituloVentana: 'Contabilización de Orden de Compra/Servicio',
					anchoVentana: 720,
					altoVentana: 500,
					anchoFormulario: 680,
					altoFormulario:150,
...
//-----------------------------------------------------------------------------------------------------------------------	
//Creando el campo catalogo para llamar al proveedor
...
//creando funcion que construye formulario principal para contabilización
	var	fromBusquedaSOC = new Ext.form.FieldSet({
...
//--------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------
var fechacontabil = new Date(Ext.getCmp('fechaconta').getValue());
Ext.onReady(function() {
	Ext.BLANK_IMAGE_URL = '../../base/librerias/js/ext/resources/images/default/s.gif';
	var Xpos = ((screen.width/2)-(920/2));
	var	fromContabilzarSOC = new Ext.FormPanel({
		applyTo: 'formularioSOC',
		width: 920,
		height: 550,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:35px;',
		title: "<H1 align='center'>Contabilización de Ordenes de Compra/Servicio</H1>",
		frame: true,
		autoScroll:true,
		items: [fromBusquedaSOC,
		        gridOrdenCompra
		        ]
	});
	fromContabilzarSOC.doLayout();
});
...
function irCancelar(){
	limpiarFormulario(fromBusquedaSOC);
	gridOrdenCompra.store.removeAll();
}

function irProcesar(){
	valido=true;
	fecha = new String(Ext.get('fechaconta').getValue());
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: "<H1 align='center'>Resultado Contabilizaci&#243;n de las Ordenes de Compra</H1>",
					anchoLabel: 200,
					labelTotal:'Total ordenes procesadas',
					valorTotal: arrResultado[0],
					labelProcesada:'Total ordenes contabilizadas',
