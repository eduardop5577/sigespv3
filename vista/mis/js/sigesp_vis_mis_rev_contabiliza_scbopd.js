...
//-----------------------------------------------------------------------------------------------------------------------	
	...
	var	fromBusquedaRevMovOpdSCB = new Ext.form.FieldSet({ 
...
Ext.onReady(function() {
	...
	var	fromRevContabilzarMovopdSCB = new Ext.FormPanel({
		applyTo: 'formulario_RevMovopdSCB',
		width: 895,
		height: 450,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'>Reverso de Contabilización de Orden de Pago Directa</H1>",
		frame: true,
		autoScroll:true,
		items: [fromBusquedaRevMovOpdSCB,
		        gridOpdBanco
		        ]
	});
	
	fromRevContabilzarMovopdSCB.doLayout();
});
...
function irBuscar(){
	...
	var JSONObject = {
			'operacion'   : 'buscar_rev_contabilizacion_opd',
			'numdoc' : numdoc,
			'fecmov' : fecmov
		}
...
function irCancelar(){
	limpiarFormulario(fromBusquedaRevMovOpdSCB);
	gridOpdBanco.store.removeAll();
}
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_contabilizar_opd','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	total = grid.length;
	if (total>0)
	{			
		for (i=0; i < total; i++)
