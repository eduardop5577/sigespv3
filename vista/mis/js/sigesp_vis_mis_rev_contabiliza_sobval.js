...
var	fromRevContabilizaSobval = null;
...
Ext.onReady(function(){
	...
	gridSolicitud = new Ext.grid.GridPanel({
		...
		height:200,
		frame:true,
		title:"<H1 align='center'>Valuaciones Contabilizadas</H1>",
	...
	fromRevContabilizaSobval = new Ext.form.FieldSet({
	...
	plRevSobval = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Reverso de Contabilizaci&#243;n de Valuaciones</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',
		items: [fromRevContabilizaSobval,gridSolicitud]
	});	
	fromRevContabilizaSobval.doLayout();
	llenarComboTipoDocumento();
});

function irCancelar()
{
	limpiarFormulario(fromRevContabilizaSobval);
...
function irProcesar()
{
	...
	cadenajson = "{'operacion':'rev_contabilizar_sobval','codsis':'"+sistema+"','nomven':'"+vista+"', 'arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Reversar las Valuaciones',
					anchoLabel: 200,
					labelTotal:'Total valuaciones procesados',
					valorTotal: arrResultado[0],
					labelProcesada:'Total valuaciones reversadas',
