...
Ext.onReady(function(){
	...
	var	p1fromSFCDED = new Ext.FormPanel({
		applyTo: 'formulario',
		width: 935,
		height: 400,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'> Contabilización de Deducciones</H1>",
		frame: true,
		autoScroll:true,
		items: [fromSFCDED,
		        gridDeduccion
		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comDetalleModificacion = new com.sigesp.vista.comDetalleComprobante({
					...
					paramPresupuesto: 'ObjSon='+Ext.util.JSON.encode({'operacion': 'comprobante_detalle_spided',
					...
					anchoGridCO :680,
					altoGridCO :100,
					paramContable: 'ObjSon='+Ext.util.JSON.encode({'operacion': 'comprobante_detalle_scgded',
																   'comprobante':registro.get('comprobante'),
																   'procede':registro.get('procede'),
																   'fecha':registro.get('fecha')})
				});
	...
	var Xpos = ((screen.width/2)-(425));
  	fromSFCDED = new Ext.form.FieldSet({  
		    title:'Datos de Deducción',
...
function irBuscar(){
	...
	var JSONObject = {
		'operacion'  : 'buscar_contabilizar_ded',
		...
		success: function ( resultado, request){
			...
			if(objetoMovsfc!=''){
				if(objetoMovsfc.raiz == null || objetoMovsfc.raiz ==''){
					...
					gridDeduccion.store.loadData(objetoMovsfc);
				}
			}
		},
...
function irProcesar(){
	var cadenaJson = "{'operacion': 'contabilizar_ded', 'cuentaCobrar':[";				
	var arrCuentaCobrar = gridDeduccion.getSelectionModel().getSelections();
		...
		Ext.Ajax.request({
			...
			success: function (resultado, request) {
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizaci&#243;n de Deducciones',
					anchoLabel: 200,
