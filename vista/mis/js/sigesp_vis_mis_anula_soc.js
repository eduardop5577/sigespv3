...
//-----------------------------------------------------------------------------------------------------------------------	
	...
	var cmOrdenCompra = new Ext.grid.ColumnModel([
        ...
        {header: "<CENTER>Concepto</CENTER>", width: 60, sortable: true, dataIndex: 'obscom'},
		{header: "<CENTER>Concepto de Anulación</CENTER>", width: 60, sortable: true, dataIndex: 'conanusoc',editor : new Ext.form.TextArea({allowBlank : false,autoCreate: {tag: 'textarea', type: 'text', onkeypress: "return keyRestrict(event,'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.;,!@%&/\()¿?¡-+*[]{}');"}})}
	]);
	//creando datastore y columnmodel para la grid de reintegros
				
	//creando grid para los reintegros
	var gridOrdenCompra = new Ext.grid.EditorGridPanel({
	 		width:860,
	 		height:250,
			frame:true,
			title:'',
		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comDetalleOrdencompra = new com.sigesp.vista.comDetalleComprobante({
					tituloVentana: 'Anulación de Orden de Compra/Servicio',
					anchoVentana: 600,
					altoVentana: 500,
					anchoFormulario: 580,
					altoFormulario:150,
...
//creando store para la tipo compra (anulación)
	var tipcompra = 	[
                    	['-- Bienes/Servicios --','-'],
                    	['Bienes','B'],
                    	['Servicios','S']
...
//-------------------------------------------------------------------------------------------------------------------------	
//Creando el campo catalogo para llamar al proveedor (anulación)
...
//-------------------------------------------------------------------------------------------------------------------------	
	...
	var	fromBusquedaAnulaSOC = new Ext.form.FieldSet({ 
...
Ext.onReady(function() {
	...
	var	fromAnularSOC = new Ext.FormPanel({
		applyTo: 'formulario_anulaSOC',
		width: 920,
		height: 500,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:40px;',
		title: "<H1 align='center'>Anulación de Orden de Compra/Servicio</H1>",
		frame: true,
		autoScroll:true,
		items: [fromBusquedaAnulaSOC,
		        gridOrdenCompra
		        ]
	});
	fromAnularSOC.doLayout();
});
...
function irCancelar(){
	limpiarFormulario(fromBusquedaAnulaSOC);
	gridOrdenCompra.store.removeAll();
}
...
function irProcesar(){
	valido=true;
	fecha = new String(Ext.get('fechaanula').getValue());
	...
	cadenajson = "{'operacion':'anular','codsis':'"+sistema+"','nomven':'"+vista+"','fecha':'"+fecha+"','arrDetalle':[";
	if (total>0)
	{			
		for (i=0; i<total && valido==true; i++)
		{
			if (i==0) 
			{
				if(arrOrdenCompra[i].get('conanusoc')!=''){
					cadenajson += "{'numordcom':'"+arrOrdenCompra[i].get('numordcom')+"'," +
							       "'estcondat':'"+arrOrdenCompra[i].get('estcondat')+"'," +
							       "'conanusoc':'"+arrOrdenCompra[i].get('conanusoc')+"'}";
				}
				else{
					Ext.MessageBox.show({
						title:'Mensaje',
						msg:'Debe llenar el concepto de anulaci&#243;n de la Orden '+arrOrdenCompra[i].get('numordcom')+' !!!',
						buttons: Ext.Msg.OK,
			...
			Ext.Ajax.request({
				...
				{ 
					...
					var comResultado = new com.sigesp.vista.comResultadoIntegrador({
						tituloVentana: "<H1 align='center'>Resultado Anulaci&#243;n de las Ordenes de Compra</H1>",
						anchoLabel: 200,
						labelTotal:'Total ordenes procesadas',
						valorTotal: arrResultado[0],
						labelProcesada:'Total ordenes anuladas',
