		...
		'rowcontextmenu': {
			fn: function(grid, numFila, evento){
				...
				var comMovBancario = new com.sigesp.vista.comDetalleComprobante({
					tituloVentana: 'Reverso de Contabilizaci&#243;n de Banco',
					anchoVentana: 600,
					altoVentana: 500,
					anchoFormulario: 580,
					altoFormulario:150,
...
//creando store para el tipo de documentos (reverso contabilización)
	var tipoperacion = 	[
                    	['-- Seleccione --','-'],
                    	['Nota Débito','ND'],
						['Nota Crédito','NC'],
	...
	//fin creando objeto tipo documentos (reverso contabilización)

//-------------------------------------------------------------------------------------------------------------------------	
	//creando funcion que construye formulario principal para reverso de la contabilización de banco
	var	fromBusqueda_RevSCB = new Ext.form.FieldSet({
...
Ext.onReady(function() {
	...
	var	fromReversoContabilzarSCB = new Ext.FormPanel({
		applyTo: 'formulario_Rev_SCB',
		width: 920,
		height: 500,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:35px;',
		title: "<H1 align='center'>Reverso de Contabilización de Banco</H1>",
		frame: true,
		autoScroll:true,
		items: [fromBusqueda_RevSCB,
		        gridMovBanco
		        ]
	});
	
	fromReversoContabilzarSCB.doLayout();
});
...
function irBuscar(){
	...
	else
	{
		...
		var JSONObject = {
				'operacion'   : 'buscar_por_rev_contabilizacion_movbco',
				'numdoc' : numdoc,
				'fecmov' : fecmov,
				'codope' : codope,
				'numcarord' : numcarord
...
function irCancelar(){
	limpiarFormulario(fromBusqueda_RevSCB);
	gridMovBanco.store.removeAll();
}
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_contabilizacion_movbco','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	total = grid.length;
	if (total>0)
	{			
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado de Movimientos Bancarios Reversados',
					anchoLabel: 200,
