...
//	    {header: "<CENTER>Detalle</CENTER>", width: 30, sortable: true, dataIndex: 'detasi',editor : new Ext.form.TextField({allowBlank : true})}
	]);

	//creando datastore y columnmodel para la grid de asignaciones
	gridSolicitud = new Ext.grid.EditorGridPanel({
		width:570,
		height:230,
		frame:true,
		title:"<H1 align='center'>Asignaciones Anuladas</H1>",
	...
	fromRevAnulasobasi = new Ext.form.FieldSet({ 
	...
	plRevAnulaSobasi = new Ext.FormPanel({
		applyTo: 'formulario',
		width:613,
		height: 500,
		title: "<H1 align='center'>Reverso de la anulacion de Asignacion</H1>",
		frame:true,
		autoScroll:false,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',

		items: [fromRevAnulasobasi,gridSolicitud]
	});	
	fromRevAnulasobasi.doLayout();
});
function irCancelar(){
	limpiarFormulario(fromRevAnulasobasi);
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_anular_sobasi','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	total = grid.length;
	if(total>0)
	{
		...
		Ext.Ajax.request({
			...
			{ 
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Reverso de la Anulaci&#243;n de Asignaciones',
					anchoLabel: 200,
					labelTotal:'Total asignaciones procesados',
					valorTotal: arrResultado[0],
					labelProcesada:'Total asignaciones reversadas',
