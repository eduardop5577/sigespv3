...
var fromContabilizaSafdep=null;
...
Ext.onReady(function(){
	...
	var p1ContabilizaSafdep= new Ext.FormPanel({
		applyTo: 'formulario',
		width: 910,
		height: 450,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'>Contabilizar Depreciacion de Activos Fijos</H1>",
		frame: true,
		autoScroll:true,
		items: [fromContabilizaSafdep,
		        gridSolicitud
		        ]
	});
	
	p1ContabilizaSafdep.doLayout();
});
//-------------------------------------------------------------------------------------------------------------------------	
	 var arregloMes = [
					 ['-- Seleccione --','00'], 
...
//-------------------------------------------------------------------------------------------------------------------------	
	...
	gridSolicitud = new Ext.grid.EditorGridPanel({
			width:850,
	 		height:250,
			frame:true,
			title:"<H1 align='center'>Depreciaciones por Contabilizar</H1>",
...
//-----------------------------------------------------------------------------------------------------------------------	
      		fromContabilizaSafdep = new Ext.form.FieldSet({ 
...
function irBuscar()
{
	...
	else
	{
		...
		var JSONObject = {
				'operacion'   : 'buscar_por_contabilizar_depsaf',
				'mes' : mes,
				'anio' : anio,
				'estatus' : estatus
			}
...
function irCancelar()
{
	limpiarFormulario(fromContabilizaSafdep);
	gridSolicitud.store.removeAll();
}
...
function irProcesar()
{
	...
	cadenajson = "{'operacion':'contabilizar_safdep','codsis':'"+sistema+"','nomven':'"+vista+"','arrDetalle':[";
	total = grid.length;
	if (total>0)
	{			
		for (i=0; i<total; i++)
		...
		Ext.Ajax.request({
			...
			success: function (resultado, request) {
				...
				var comResultado = new com.sigesp.vista.comResultadoIntegrador({
					tituloVentana: 'Resultado Contabilizacion de Depreciaci&#243;n de Activos',
					anchoLabel: 200,
					labelTotal:'Total depreciaciones procesadas',
					valorTotal: arrResultado[0],
					labelProcesada:'Total depreciaciones contabilizadas',
