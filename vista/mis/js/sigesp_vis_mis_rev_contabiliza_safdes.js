...
//-----------------------------------------------------------------------------------------------------------------------	
	...
	var cmSolicitud = new Ext.grid.ColumnModel([
		new Ext.grid.CheckboxSelectionModel(),
		  {header: "<CENTER>N° Documento</CENTER>", width: 30, sortable: true, align: 'center',dataIndex: 'cmpmov'},
	...
	gridSolicitud = new Ext.grid.EditorGridPanel({
			width:870,
	 		height:250,
			frame:true,
			title:"<H1 align='center'>Desincorporaciones por Reversar</H1>",
...
//-------------------------------------------------------------------------------------------------------------------------	
	...
	var	fromRevDesincorp_Saf = new Ext.form.FieldSet({ 
...
Ext.onReady(function() {
	...
	var	p1RevDesincorp_Saf = new Ext.FormPanel({
		applyTo: 'formulario',
		width: 920,
		height: 600,
		style:'position:absolute;margin-left:'+Xpos+'px;margin-top:45px;',
		title: "<H1 align='center'>Reverso de contabilización de Desincorporaciones de Activos</H1>",
		frame: true,
		autoScroll:true,
		items: [fromRevDesincorp_Saf,
		        gridSolicitud
		        ]
	});
	
	p1RevDesincorp_Saf.doLayout();
});
...
function irBuscar(numdoc,fecmov,codope,numcarord)
{
	...
	var JSONObject = {
			'operacion'   : 'buscar_por_rev_contabilizar_dessaf',
			'numcmp' : numcmp,
			'feccmp' : feccmp,
			'estatus' : estatus
	}
...
function irCancelar(){
	limpiarFormulario(fromRevDesincorp_Saf);
...
function irProcesar(){
	...
	cadenajson = "{'operacion':'rev_contabilizar_safdes','codsis':'"+sistema+"','nomven':'"+vista+"','fechaconta':'"+feccont.format(Date.patterns.bdfecha)+"','arrDetalle':[";
	total = grid.length;
	if (total>0)
	{			
		for (i=0; i<total; i++)
		{
			if (i==0) 
			{
				cadenajson += "{'cmpmov':'"+grid[i].get('cmpmov')+"','numcmp':'"+grid[i].get('numcmp')+"'," +
