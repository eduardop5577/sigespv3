...
* al proceso de asignar usuarios a un Centro de Costo. 
...
var cambiar = false;
var panel      = '';
var pantalla   = 'usuarioscentrocosto';
...
(
	function()
	{
		...
		var agregar = new Ext.Action(
		{
        	...
        	tooltip: 'Agregar un usuario al Centro de Costo'
		});
		...
		var quitar = new Ext.Action(
		{
			...
			handler: irQuitar,
			iconCls: 'bmenuquitar',
        	tooltip: 'Quitar usuario del Centro de Costo'
		});
        ...
        labelWidth: 75,
     	title: 'Proceso de Asignar Usuarios a Centro de Costo',
		...
		items:[{
			    xtype:'fieldset',
				title:'Datos del centrocosto',
				id:'fsformcentrocosto',
    			autoHeight:true,
				autoWidth:true,
				cls :'fondo',	
			    items:[{			   
						xtype:'textfield',
						fieldLabel:'Código',
						name:'Código del Centro de Costo',
						id:'txtcodcencos',
						disabled: true,
						width:80
					},{
					...
					xtype:'panel',
					width:500,
					title:'Usuarios para el Centro de Costo',
					tbar: [agregar,quitar],
					contentEl:'grid-usuarioscentrocosto'
			}]
		});
...
***********************************************************************************/
	...
	{	
		...
		gridUsu.render('grid-usuarioscentrocosto');
	}
...
***********************************************************************************/		
	...
	{		 
		panel.getComponent('fsformcentrocosto').getComponent('txtcodcencos').setValue('');
		panel.getComponent('fsformcentrocosto').getComponent('txtdenominacion').setValue('');
...
***********************************************************************************/
	function irGuardar()
	{
		...
		if (Ext.getCmp('txtcodcencos').getValue()=='')
		{
			Ext.Msg.alert('Mensaje','Debe seleccionar un Centro de Costo');
		}
		else
		{
			obtenerMensaje('procesar','','Guardando Datos');
			var cadenaJson = "{'oper': 'actualizar','codsis':'CFG','seleccionado':'centrocosto','sistema': sistema,'vista': vista,'codtippersss': '"+Ext.getCmp('txtcodcencos').getValue()+"','dentippersss': ''";				
			...
			Ext.Ajax.request({
			url : '../../controlador/sss/sigesp_ctr_sss_usuariospermisos.php',
			params : parametros,
			method: 'POST',
...
/***********************************************************************************
*  @Función que llama al catalogo para mostrar los datos del Centro de Costos
* @parametros: 
* @retorno: 
* @fecha de creación: 13/12/2011. 
* @autor: Ing. Yesenia Moreno.  
...
***********************************************************************************/
	function irBuscar()
	{
		var arreglotxt     = new Array('txtcodcencos','txtdenominacion');		
		var arreglovalores = new Array('codcencos','denominacion');			
		objCatAlmacen = new catalogoCentroCostos();
		objCatAlmacen.mostrarCatalogoCentroCosto(arreglotxt, arreglovalores);
	}
...
***********************************************************************************/
	function irEliminar()
	{
		...
		function Result(btn)
		{
			if(btn=='yes')
			{ 
				if (validarObjetos('txtcodcencos','60','novacio')=='0')
				{					
				...
				else
				{
					...
					var objdata ={
						'oper': 'eliminar', 
						'codsis':'CFG',
						'seleccionado':'centrocosto',
						'sistema': sistema,
						'vista': vista,
						'codtippersss': Ext.getCmp('txtcodcencos').getValue()						
					};	
					objdata=JSON.stringify(objdata);
					parametros = 'objdata='+objdata;
					Ext.Ajax.request({
