...
* al proceso de asignar usuarios a un almacen. 
...
(
	function()
	{
		...
		var agregar = new Ext.Action(
		{
        	...
        	tooltip: 'Agregar un usuario al Almacen'
		});
		...
		var quitar = new Ext.Action(
		{
			...
			handler: irQuitar,
			iconCls: 'bmenuquitar',
        	tooltip: 'Quitar usuario del almacen'
		});
        ...
        labelWidth: 75,
     	title: 'Proceso de Asignar Usuarios a Almacen',
		...
		items:[{
			    ...
			    items:[{			   
					...
					xtype:'panel',
					width:500,
					title:'Usuarios para el Almacen',
					tbar: [agregar,quitar],
					contentEl:'grid-usuariosalmacen'
			}]
		});
...
***********************************************************************************/
	...
	{	
		...
		gridUsu.render('grid-usuariosalmacen');
	}
...
***********************************************************************************/		
	...
	{		 
		panel.getComponent('fsformalmacen').getComponent('txtcodalm').setValue('');
		panel.getComponent('fsformalmacen').getComponent('txtnomfisalm').setValue('');
...
***********************************************************************************/
	function irGuardar()
	{
		...
		if (Ext.getCmp('txtcodalm').getValue()=='')
		{
			Ext.Msg.alert('Mensaje','Debe seleccionar un Almacen');
		}
		else
		{
			obtenerMensaje('procesar','','Guardando Datos');
			var cadenaJson = "{'oper': 'actualizar','codsis':'SIV','seleccionado':'almacen','sistema': sistema,'vista': vista,'codtippersss': '"+Ext.getCmp('txtcodalm').getValue()+"','dentippersss': '"+Ext.getCmp('txtnomfisalm').getValue()+"'";				
...
/***********************************************************************************
*  @Función que llama al catalogo para mostrar los datos del Almacen
* @parametros: 
* @retorno: 
* @fecha de creación: 02/12/2011. 
* @autor: Ing. Yesenia Moreno.  
...
***********************************************************************************/
	function irBuscar()
	{
		var arreglotxt     = new Array('txtcodalm','txtnomfisalm');		
		var arreglovalores = new Array('codalm','nomfisalm');			
		objCatAlmacen = new catalogoAlmacen();
		objCatAlmacen.mostrarCatalogoAlmacen(arreglotxt, arreglovalores);
	}
...
***********************************************************************************/
	function irEliminar()
	{
		...
		function Result(btn)
		{
			if(btn=='yes')
			{ 
				if (validarObjetos('txtcodalm','60','novacio')=='0')
				{					
				...
				else
				{
					...
					var objdata ={
						...
						'sistema': sistema,
						'vista': vista,
						'codtippersss': Ext.getCmp('txtcodalm').getValue()						
					};	
					objdata=JSON.stringify(objdata);
					parametros = 'objdata='+objdata;
					Ext.Ajax.request({
