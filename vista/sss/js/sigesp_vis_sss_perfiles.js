...
*********************************************************************************/
	function obtenerPermisos()
	{
		...
		Ext.Ajax.request({
			...
			{ 
				//Ext.Msg.hide();
				Ext.MessageBox.alert('Error', 'Error al procesar la información en Obtener Permisos'); 
			}					
		});		
	}
...
*******************************************************************************/		
	function irGuardar()
	{
		...
		if (validarObjetos('cmbsistema','','novacio')!='0')
		{
			if (validarObjetos('cmbusuario&cmbgrupo','','novaciodos')!=false)			
			{
				obtenerMensaje('procesar','','Guardando Datos');
				if (validarObjetos('txtfuncionalidad','','novacio')!='0')
			  	{
					if (cambiar==false)
					{							
						evento ='incluir';
					...
					if (continuar)
					{
						...
						{ 
							Ext.Msg.hide();
							Ext.MessageBox.alert('Error', 'Error al procesar la Información al Guardar'); 
						}					
						});
					}
				}
...
******************************************************************************/	
	function Result(btn)
	{
		...
		{ 
			...
			{ 
				Ext.Msg.hide();
				Ext.MessageBox.alert('Error', 'Error al procesar la información al Aceptar'); 
			}					
			});		
		}
	}
				...
				{				
					...
					Ext.Ajax.request({
						...
						{ 
							datos = resultado.responseText;				
							var datajson = eval('(' + datos + ')');							
							if ((datajson.raiz!=null) && (datajson.raiz.length>0) && (datajson.raiz[0].valido==true))
							{
						...
						failure: function (result,request) 
						{							
							Ext.MessageBox.alert('Error', 'Error al procesar la información al Buscar'); 
						}					
					});
				}
			}
		...
		function result(btn)
		{
			...
			{ 
				...
				else
				{
					...
					{ 
						Ext.Msg.hide();
						Ext.MessageBox.alert('Error', 'Error al procesar la información al Eliminar'); 
					} 
					});
				}
			}
