...
*****************************************************************************/
	...
	{
		NominaCat    = gridNomina.getSelectionModel().getSelections();
		cantNomina   = gridNom.store.getCount()-1;
		arrAuxNomina = gridNom.store.getRange(0,cantNomina);
		existe=false;
		totalgrid=(NominaCat.length-1);
		totalcatalogo=(arrAuxNomina.length-1);
		for (i=1; i<=totalgrid; i++)
		{
	  		auxReg1 = NominaCat[i].get('codnom');
	  		for (j=0; j<=totalcatalogo; j++)
	  		{
				if (arrAuxNomina[j].get('codnom')==auxReg1)
				{
					existe=true;
					return true;
				}
