...
var formatoExcel = '';
var cencos = '';
var gridEstructuraPresupuestaria ='';
			...
			items: [{
					...
					items: [{
							...
							items:[{
									...
									items: [
											{
											boxLabel: 'Trimestres', name: 'intervalp',inputValue: '3',checked:false, hidden:true,
											listeners:{		
...
//----------------------------------------------------------------------------------------------------------------------------------
	...
	var dsestructura = new Ext.data.Store({
		reader: new Ext.data.JsonReader({root: 'raiz',id: "id"},estructura)
	});

	gridEstructuraPresupuestaria = new Ext.grid.GridPanel({
			width:705,
			height:300,
			frame:true,
			autoScroll:true,	
			...
			cm: new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel({}),
				{header: "Estructura Presupuestaria", width: 120, sortable: true, dataIndex: 'estructura'},
				{header: "Denominaci&#243;n", width: 120, setEditable: true, sortable: true, dataIndex: 'denominacion'}
			]),
...
//Creacion del formulario pagos
	...
	width: 750,
	height: 500,
	title: "<H1 align='center'>Estado de Resultado por Estructura</H1>",
	frame: true,
	autoScroll: true,
	style: 'position:absolute;margin-left:'+Xpos+'px;margin-top:15px;',
	items: [	
        	fieldsetPeriodos,
           	gridEstructuraPresupuestaria,
...
function irBuscarFormato()
{
	var myJSONObject =
	{
		...
		'sistema'	  : 'SCG',
		'seccion'     : 'REPORTE',
		'variable'    : 'ESTADO_RESULTADO_ESTRUCTURA',
		'valor'		  : 'sigesp_scg_rpp_estado_resultado_estpre.php',
		'tipo'		  : 'C'
	};	
	var ObjSon=Ext.util.JSON.encode(myJSONObject);
	var parametros ='ObjSon='+ObjSon;
	Ext.Ajax.request(
	{
		...
		failure: function (result, request)
                { 
			Ext.MessageBox.alert('Error', 'error al accesar al sistema 1.'); 
		}
	})
	...
	var myJSONObject =
	{
		...
		'sistema'	  : 'SCG',
		'seccion'     : 'REPORTE',
		'variable'    : 'ESTADO_RESULTADO_ESTRUCTURA_EXCEL',
		'valor'		  : 'sigesp_scg_rpp_estado_resultado_estpre_excel.php',
		'tipo'		  : 'C'
	};	
	var ObjSon=Ext.util.JSON.encode(myJSONObject);
	var parametros ='ObjSon='+ObjSon;
	Ext.Ajax.request(
	{
		...
		failure: function (result, request){ 
			Ext.MessageBox.alert('Error', 'error al accesar al sistema 2.'); 
		}
	})
}
...
{
    var myJSONObject ="{'operacion':'buscarEstructuras'}";		
    ...
    Ext.Ajax.request({
		...
		{ 
			...
			if(objetodata != '')
			{
				...
				{
					gridEstructuraPresupuestaria.getStore().loadData(objetodata);
				}
			}
	    },
	    failure: function ( result, request)
...
function irImprimir()
{
    var valido = true; 
        ...
        var detalle = gridEstructuraPresupuestaria.getSelectionModel();
        var cadjson = "{'estructuras':[";
        var total = detalle.selections.length;

        for ( var i = 0; i < total; i++)
	{
                    if(i==0)
                    {
                            cadjson=cadjson+"{'codestpro1':'"+detalle.selections.items[i].get('codestpro1')+"','codestpro2':'"+detalle.selections.items[i].get('codestpro2')+"','codestpro3':'"+detalle.selections.items[i].get('codestpro3')+"','estcla':'"+detalle.selections.items[i].get('estcla')+"'}";
                    }
                    else
                    {
                            cadjson=cadjson+",{'codestpro1':'"+detalle.selections.items[i].get('codestpro1')+"','codestpro2':'"+detalle.selections.items[i].get('codestpro2')+"','codestpro3':'"+detalle.selections.items[i].get('codestpro3')+"','estcla':'"+detalle.selections.items[i].get('estcla')+"'}";
			...
			if(hidbot==1)
			{
				...
				else
				{
					pagina="reportes/"+formato1+"?cmbmesdes="+cmbmesdes
							...
							+"&hidbot="+hidbot+"&precierre="+li_precierre+"&costodesde="+ls_costodesde+"&costohasta="+ls_costohasta+"&jasonest="+cadjson;
			...
			if(hidbot==2)
			{
				...
				else
				{
					pagina="reportes/"+formato1+"?txtfecdes="+txtfecdes
							...
							+"&precierre="+li_precierre+"&costodesde="+ls_costodesde+"&costohasta="+ls_costohasta+"&jasonest="+cadjson;
			...
			if(hidbot==1)
			{
				...
				else
				{
					pagina="reportes/"+formatoExcel+"?cmbmesdes="+cmbmesdes
						+"&cmbmeshas="+cmbmeshas+"&cmbagnodes="+cmbagnodes+"&cmbagnohas="+cmbagnohas+"&cmbnivel="+cmbnivel
						+"&hidbot="+hidbot+"&precierre="+li_precierre
						+"&costodesde="+ls_costodesde+"&costohasta="+ls_costohasta+"&jasonest="+cadjson;
						window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");
				}
			}
			if(hidbot==2)
