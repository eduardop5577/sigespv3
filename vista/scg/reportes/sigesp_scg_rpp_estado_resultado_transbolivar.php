...
function uf_insert_seguridad($as_titulo)
{
	...
	$lb_valido=$io_fun_scg->uf_load_seguridad_reporte("SCG","sigesp_vis_scg_r_estado_resultado.html",$ls_descripcion);
	return $lb_valido;
}
//-----------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------
function uf_print_cabecera_costos($io_pdf)
{
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//       Function: uf_print_cabecera
	//		   Access: private
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	global $io_pdf;
	$la_data=array(array('name'=>'<b>COSTOS</b> '));
	$la_columna=array('name'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'showLines'=>1, // Mostrar Líneas
						 'fontSize' => 7, // Tamaño de Letras
...
//--------------------------------------------------------------------------------------------------------------------------------
function uf_print_pie_cabecera_costo($ld_total_costo,$io_pdf)
{
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//       Function : uf_print_pie_cabecera
	//		    Acess : private
	...
	global $ls_bolivares;
	global $io_pdf;
	$la_data=array(array('total'=>'<b>Total Costo '.$ls_bolivares.'</b>','saldomay'=>$ld_total_costo));
	$la_columna=array('total'=>'','saldomay'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 7, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
...
//--------------------------------------------------------------------------------------------------------------------------------
function uf_print_pie_cabecera_utilper($ld_total_utilper,$io_pdf)
{
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//       Function : uf_print_pie_cabecera
	//		    Acess : private
	...
	global $ls_bolivares;
	global $io_pdf;
	$la_data=array(array('total'=>'<b>Utilidad o Perdida Bruta en Operaciones '.$ls_bolivares.'</b>','saldomay'=>$ld_total_utilper));
	$la_columna=array('total'=>'','saldomay'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 7, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
...
function uf_print_pie_cabecera($ld_total,$io_pdf)
{
	...
	$la_data=array(array('total'=>'<b>Utilidad o Perdida Bruta en Operaciones antes del ISLR '.$ls_bolivares.'</b>','saldomay'=>$ld_total));
	$la_columna=array('total'=>'','saldomay'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
...
if($lb_valido)
{
	...
	$lb_valido_cos=$io_report->uf_scg_reporte_estado_de_resultado_costos($ldt_fecdes, $ldt_fechas, $li_nivel);
}
...
else// Imprimimos el reporte
{
	...
	if($lb_valido_cos)
	{
		$li_tot=$io_report->dts_costos->getRowCount("sc_cuenta");
		for($li_i=1;$li_i<=$li_tot;$li_i++)
		{
			$io_pdf->transaction('start'); // Iniciamos la transacción
			$thisPageNum=$io_pdf->ezPageCount;
			$ls_sc_cuenta=trim($io_report->dts_costos->data["sc_cuenta"][$li_i]);
			$li_totfil=0;
			$as_cuenta="";
			for($li=$li_total;$li>1;$li--)
			{
			...
			$li_fila=$ia_niveles_scg[1]+1;
			$as_cuenta=substr($ls_sc_cuenta,0,$li_fila)."-".$as_cuenta;
			$ls_status=$io_report->dts_costos->data["status"][$li_i];
			$ls_denominacion=$io_report->dts_costos->data["denominacion"][$li_i];
			$ld_saldo=$io_report->dts_costos->data["saldo"][$li_i];
			$ld_total_costo=$io_report->dts_costos->data["total_costo"][$li_i];
			$ls_nivel=$io_report->dts_costos->data["nivel"][$li_i];
			if($ls_nivel>3)
			{
				$ld_saldo=abs($ld_saldo);
				$ld_saldomay=number_format($ld_saldo,2,",",".");
			...
			$la_data_cos[$li_i]=array('cuenta'=>$as_cuenta,'denominacion'=>$ls_denominacion,'saldomay'=>$ld_saldomay,'saldomen'=>$ld_saldomen,'saldo'=>$ld_saldo);
		}//for
		uf_print_cabecera_costos($io_pdf);
		uf_print_detalle_ingreso($la_data_cos,$io_pdf); // Imprimimos el detalle
		if($lb_valido_ing){
		...
		$ld_total_costo=abs($ld_total_costo);
		$ld_total_utilper = $ld_total_ingresos-$ld_total_costo;
		$ld_total_costo=number_format($ld_total_costo,2,",",".");
		uf_print_pie_cabecera_costo($ld_total_costo,$io_pdf); // Imprimimos pie de la cabecera
		uf_print_pie_cabecera_utilper(number_format($ld_total_utilper,2,",","."),$io_pdf); // Imprimimos pie de la cabecera
	}
	...
	if($lb_valido_egr)
	{
		...
		$ld_total=trim($ld_total_utilper)-($ld_total_egresos);
