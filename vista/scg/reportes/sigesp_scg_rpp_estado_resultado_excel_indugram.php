...
else // Imprimimos el reporte
{
	...
	if($lb_valido_cos)
	{
		$lo_hoja->write($li_row, 0, "COSTOS",$lo_titulo);
		$li_row=$li_row+1;
		$lo_hoja->write($li_row, 0, "Cuenta",$lo_titulo);
		$lo_hoja->write($li_row, 1, "Denominación",$lo_titulo);
		$lo_hoja->write($li_row, 2, "Saldo",$lo_titulo);
		...
		$li_row=$li_row+1;
		$lo_hoja->write($li_row, 2, "Total Costos ".$ls_bolivares,$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
		$lo_hoja->write($li_row, 4, $ld_total_costo,$lo_dataright);
		$li_row=$li_row+1;
		$lo_hoja->write($li_row, 2, "Utilidad o Perdida Bruta en Operaciones ".$ls_bolivares,$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
		$lo_hoja->write($li_row, 4, $ld_total_utilper,$lo_dataright);
		$li_row=$li_row+1;
	}
	if($lb_valido_egr)
	{
	...
	$ld_total=$ld_total_utilper-$ld_total_egresos;
	if($ld_total<0)
	{
		$ls_cadena="DESAHORRO";
	}
