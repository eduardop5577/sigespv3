...
***********************************************************************************/
	...
	function uf_print_detalle_cueproacu($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('cuenta'=>array('justification'=>'left','width'=>105), // Justificación y ancho de la columna
						 			   'denominacion'=>array('justification'=>'left','width'=>295), // Justificación y ancho de la columna
									   'saldo_cueproacu'=>array('justification'=>'right','width'=>75), // Justificación y ancho de la columna
	...
	function uf_print_cuentas_acreedoras($la_data_acreedoras,$io_pdf)
	{
		...
		//	    Arguments: la_data // arreglo de información
		//	   			   io_pdf // Objeto PDF
		//    Description: función que imprime el detalle de las Cuentas acreedoras
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	 ...
	 $ls_tiporeporte="0";
	 $ls_bolivares="";
	 require_once("sigesp_scg_class_bal_general_hatillo.php");
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
	   ...
	   //----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
		...
		$ls_pasivo_t=$_SESSION["la_empresa"]["pasivo_t"];
	...
	if($lb_valido)
	{
		...
		else// Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
				...
				if($ls_pasivo."00"==substr($ls_sc_cuenta,0,4))
				{
					$ld_total=$ld_total+$ld_saldo;
				}
				if($ls_pasivo_t."00"==substr($ls_sc_cuenta,0,4))
				{
					$ld_total=$ld_total+$ld_saldo;
				}
				if($ls_capital."00"==substr($ls_sc_cuenta,0,4))
				{
					$ld_total=$ld_total+$ld_saldo;
				}
				
				if($ls_resultado."00"==substr($ls_sc_cuenta,0,4))
				{
