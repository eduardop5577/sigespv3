...
function uf_print_detalle_ingreso($la_data,$io_pdf)
{
	...
	$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('cuenta'=>array('justification'=>'center','width'=>120), // Justificación y ancho de la columna
						 			   ...
						 			   'saldomen'=>array('justification'=>'right','width'=>90), // Justificación y ancho de la columna
...
function uf_print_pie_cabecera_ingreso($ld_total_ingresos,$io_pdf)
{
	...
	$la_data=array(array('total'=>'<b>Total Ingreso '.$ls_abrmon.'</b>','saldomay'=>$ld_total_ingresos));
...
function uf_print_pie_cabecera_egreso($ld_total_egresos,$io_pdf)
{
	...
	$la_data=array(array('total'=>'<b>Total Egreso '.$ls_abrmon.'</b>','saldomay'=>$ld_total_egresos));
	$la_columna=array('total'=>'','saldomay'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 7, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
...
function uf_print_pie_cabecera($ld_total,$io_pdf)
{
	...
	$la_data=array(array('total'=>'<b>Total ('.$ls_cadena.') '.$ls_abrmon.'</b>','saldomay'=>$ld_total));
	$la_columna=array('total'=>'','saldomay'=>'');
	$la_config=array('showHeadings'=>0, // Mostrar encabezados
...
if($ls_hidbot==1)
{
	...
	else if($_SESSION["ls_gestor"]=='oci8po')
	{
		$fecdes=$ls_cmbagnodes."-".$ls_cmbmesdes."-01";
		$ldt_fecdes=$ls_cmbagnodes."-".$ls_cmbmesdes."-01";
	}
	...
	if($lb_valido_ing)
	{
		...
		for($li_i=1;$li_i<=$li_tot;$li_i++)
		{
			...
			$ld_total_ingresos=$io_report->dts_reporte->data["total_ingresos"][$li_i];
				...
				$ld_total_ingresos=($ld_total_ingresos/$ls_tascam1);
	...
	if($lb_valido_egr)
	{
		...
		for($li_i=1;$li_i<=$li_tot;$li_i++)
		{
			...
			$ld_total_egresos=$io_report->dts_egresos->data["total_egresos"][$li_i];
				...
				$ld_saldo=($ld_saldo/$ls_tascam1);
				$ld_total_egresos=($ld_total_egresos/$ls_tascam1);
