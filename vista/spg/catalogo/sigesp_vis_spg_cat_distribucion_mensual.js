...
function CatalogoDistribucionMensual(cuenta,denominacion,montoAsig,registro,flagAjuste){
...
//									this.setValue(this.getValue());
	...
	var ventana = new Ext.Window({
		title: "<H1 align='center'>Asignaci&#243;n Manual</H1>",
        width:680,
        height:500,
        modal: false,
        closable:false,
	...
	function ajuste(registro)
	{
		...
		Ext.getCmp('enero').setValue(registro.get('enero'));
		Ext.getCmp('febrero').setValue(registro.get('febrero'));
		Ext.getCmp('marzo').setValue(registro.get('marzo'));
		Ext.getCmp('abril').setValue(registro.get('abril'));
		Ext.getCmp('mayo').setValue(registro.get('mayo'));
		Ext.getCmp('junio').setValue(registro.get('junio'));
		Ext.getCmp('julio').setValue(registro.get('julio'));
		Ext.getCmp('agosto').setValue(registro.get('agosto'));
		Ext.getCmp('septiembre').setValue(registro.get('septiembre'));
		Ext.getCmp('octubre').setValue(registro.get('octubre'));
		Ext.getCmp('noviembre').setValue(registro.get('noviembre'));
	...
	function validarMonto(objeto,valor)
	{
		...
		if(monto_aux>monto){
			...
			if(monto<total){
				Ext.MessageBox.show({
					title:'Mensaje',
					msg:'El acumulado de la distribuci&#243;n debe ser igual al Monto Asignado de la Cuenta !!! '+monto+'  '+total,
					buttons: Ext.Msg.OK,
					icon: Ext.MessageBox.INFO
				});
				objeto.setValue(formatoNumericoMostrar(0,2,'.',',','','','-',''));
