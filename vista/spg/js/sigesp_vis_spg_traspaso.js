...
var fromComprobanteTraspaso = null; //varibale para almacenar la instacia de objeto de formulario 
...
var estvalspg =trim(empresa["estvalspg"]);
var ctaspgced ='';
var ctaspgrec ='';

if (estvalspg == '1')
{
	ctaspgced =trim(empresa["ctaspgced"]);
	ctaspgrec =trim(empresa["ctaspgrec"]);
}
...
Ext.onReady(function()
{
                ...
                listeners: {'select': function()
                                {
                                    if (Actualizar == null)
                                    {
                                        ...
                                        Ext.getCmp('totalotracue').setValue('');
	...
	gridDetPresupuestario = new Ext.grid.EditorGridPanel({
 		...
 		height:200,
		frame:true,
		title:"<H1 align='center'>Partidas Cedentes</H1>",
		...
		autoScroll:true,
 		...
 		ds: dsDetPresupuestario,
	  	...
	  	tbar:[{
		        text:'Partidas Cedentes',
		        tooltip:'Agregar',
		        iconCls:'agregar',
		        id: 'btagrebie',
		        handler: function(){
					if(Ext.getCmp('comprobante').getValue()=='' || Ext.getCmp('descripcion').getValue()=='' || Ext.getCmp('coduac').getValue()=='')
					{
						Ext.Msg.show({
							title:'Mensaje',
	...
	gridDetCuentas = new Ext.grid.EditorGridPanel({
 		...
 		height:200,
		frame:true,
		title:"<H1 align='center'>Partidas Receptoras</H1>",
		sm:new Ext.grid.CheckboxSelectionModel({singleSelect:false}),
		...
		tbar:[{
			...
			},
			{
				...
				handler: function(){
					...
					acumularTotalGasto(gridDetCuentas,'totalotracue');
				}
			}]
	});
	...
	fieldset = new Ext.form.FieldSet({
		...
		height: 60,
		title: '',
		style: 'position:absolute;left:445px;top:410px',
		cls :'fondo',
		items: [{
				layout:"column",
				border:false,
				items: [{
						...
						items: [{
								xtype:"textfield",
								fieldLabel: 'Total Cuentas Cedentes',
								labelSeparator:'',
								readOnly:true,
								id:'totalcuerec',
								width:150,
	...
	fieldsetdos = new Ext.form.FieldSet({
		...
		height: 60,
		title: '',
		style: 'position:absolute;left:445px;top:685px',
		cls :'fondo',
		items: [{
				layout:"column",
				border:false,
				items: [{
						...
						items: [{
								xtype:"textfield",
								fieldLabel: 'Total Cuentas Receptoras',
								labelSeparator:'',
								readOnly:true,
								id:'totalotracue',
								width:150,
	...
	fromComprobanteTraspaso = new Ext.FormPanel({
		title: "<H1 align='center'>Comprobante de Traspaso</H1>",
		applyTo: 'formComprobanteTraspaso',
		width: 845,
		height: 550,
		...
		Ext.Ajax.request(
		{
			...
			{ 
				...
				if(Actualizar)
				{
					...
					pagina = "reportes/"+formato+"?comprobante="+comprobante+"&procede="+procede+"&fecha="+fecha+"&juscomp="+descripcion;
	...
	function irGuardar()
	{
		if(Ext.getCmp('procede').getValue()!='SPGTRA')
		{
			Ext.Msg.show({
				title:'Mensaje',
		...
		else
		{
	        ...
	        cadjson = "{'operacion':'guardar','codsis':'"+sistema+"','nomven':'"+vista+"','evento':'"+evento+"',"+getJsonFormulario(fromComprobanteTraspaso);
	        var montouno = parseFloat(ue_formato_operaciones(Ext.getCmp('totalcuerec').getValue()));
	        var montodos = parseFloat(ue_formato_operaciones(Ext.getCmp('totalotracue').getValue()));
	        if (montouno!=montodos)
			{
	        	Ext.Msg.show({
					title:'Mensaje',
					msg: 'El comprobante esta descuadrado el total cedente debe ser igual al total receptoras',
					buttons: Ext.Msg.OK,
	...
	function irNuevo()
	{
		limpiarFormulario(fromComprobanteTraspaso);
		llenarCmbPrefijos();
		gridDetPresupuestario.store.removeAll();
		gridDetCuentas.store.removeAll();
		Ext.getCmp('totalcuerec').setValue('');
		...
		else
		{
			...
			else
			{
				...
				function respuesta(btn)
				{
					...
					{
						obtenerMensaje('procesar','','Procesando Informaci&#243;n');
						var cadjson = "{'operacion':'eliminar','codsis':'"+sistema+"','nomven':'"+vista+"',"+getJsonFormulario(fromComprobanteTraspaso);
						cadjson += "}";
				        if(valido)
						{
				        	try
	...
	function CatalogoComprobante()
	{
	    ...
	    var ventanaEstructura = new Ext.Window({
	    	...
	    	modal: true,
	    	frame:true,
	    	title:"<H1 align='center'>Cat&#225;logo de Comprobante de Traspasos</H1>",
	    ...
	    {
	    	setDataFrom(fromComprobanteTraspaso,registro);
					...
					if (objDatos != ""){
						...
						for(var j = 0; j < datos.length; j++)
						{
							...
							else
							{
								...
								monto = parseFloat(ue_formato_operaciones(monto));
								montodos += monto;
								gridDetCuentas.store.insert(0,detpreInt);
							}
						}
						Ext.getCmp('totalcuerec').setValue(formatoNumericoMostrar(montouno,2,'.',',','','','-',''));
						Ext.getCmp('totalotracue').setValue(formatoNumericoMostrar(montodos,2,'.',',','','','-',''));
					}
				},
  				failure: function ( result, request)
				{ 
			...
			tbar: [{
				...
				handler: function(){
					...
					else
					{
						...
						if(gridDetCuentas.getStore().getCount()==0)
						{
							gridDetCuentas.store.insert(0,detgasInt);
						}
						else
						{
							var entro=false;
							var montoAux = 0;
							var montoReceptor = 0;
							gridDetCuentas.store.each(function (reDetGas)
							{
								if(reDetGas.get('spg_cuenta')==arrCodigos[7] && reDetGas.get('codestpro1')==arrCodigos[0] &&
										reDetGas.get('codestpro2')==arrCodigos[1] && reDetGas.get('codestpro3')==arrCodigos[2] &&
										reDetGas.get('codestpro4')==arrCodigos[3] && reDetGas.get('codestpro5')==arrCodigos[4] &&
								...
								montoAux = reDetGas.get('monto');
								montoAux = parseFloat(ue_formato_operaciones(montoAux));
								montoReceptor += montoAux; 
							})
							montoReceptor += parseFloat(ue_formato_operaciones(Ext.getCmp('catmongasto').getValue()));
							montoReceptor = montoReceptor.toFixed(2);
							var totalCedente = parseFloat(ue_formato_operaciones(Ext.getCmp('totalcuerec').getValue()));
							if(montoReceptor > totalCedente)
							{
								Ext.Msg.show({
									 title:'Mensaje',
									 msg: 'El total de las cuentas receptoras no puede ser mayor al total cedente',
									 buttons: Ext.Msg.OK,
									 icon: Ext.MessageBox.INFO
								 });
								entro=true;
