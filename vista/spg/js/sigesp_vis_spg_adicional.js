...
var fromComprobanteAdicional = null; //varibale para almacenar la instacia de objeto de formulario 
...
Ext.onReady(function() {
	...
	gridDetPresupuestario = new Ext.grid.EditorGridPanel({
 		...
 		height:200,
		frame:true,
		title:"<H1 align='center'>Detalles de Gasto</H1>",
		...
		autoScroll:true,
 		...
 		ds: dsDetPresupuestario,
	  	...
	  	tbar:[{
	        text:'Agregar detalles de Gasto',
	        tooltip:'Agregar',
	        iconCls:'agregar',
	        id: 'btagrebie',
	        handler: function(){
	...
	fieldset = new Ext.form.FieldSet({
		...
		items: [{
				...
				items: [{
						...
						items: [{
								xtype:"textfield",
								fieldLabel: 'Total Cuentas de Cr&#233;dito/Ingreso Adicional',
								labelSeparator:'',
								readOnly:true,
								id:'totalcuerec',
								width:150,
	...
	fromComprobanteAdicional = new Ext.FormPanel({
		title: "<H1 align='center'>Comprobante de Cr&#233;dito &#243; Ingreso Adicional</H1>",
		applyTo: 'formComprobanteAdicional',
		width: 845,
		height: 550,
	...
	function irImprimir()
	{
		var myJSONObject =
		{
			...
			'sistema'	  : 'SPG',
			'seccion'     : 'REPORTE',
			'variable'    : 'MOD_PRE_CREDITOADICIONAL',
	...
	function irGuardar()
	{
		if(Ext.getCmp('procede').getValue()!='SPGCRA')
		{
			Ext.Msg.show({
				title:'Mensaje',
		...
		else
		{
	        ...
	        cadjson = "{'operacion':'guardar','codsis':'"+sistema+"','nomven':'"+vista+"','evento':'"+evento+"',"+getJsonFormulario(fromComprobanteAdicional);
	        ...
	        if(valido)
			{
				...
				else
				{
					Ext.Msg.show({
						title:'Mensaje',
						msg:'El Comprobante no tiene Detalles no se puede procesar!!!',
						buttons: Ext.Msg.OK,
						icon: Ext.MessageBox.INFO
					});
				}
	...
	function irNuevo()
	{
		limpiarFormulario(fromComprobanteAdicional);
		llenarCmbPrefijos();
		gridDetPresupuestario.store.removeAll();
		Ext.getCmp('totalcuerec').setValue();
	}
		...
		if(Ext.getCmp('procede').getValue()!='SPGCRA'){
			...
			if (Ext.getCmp('estapro').getValue() == '1') {
				...
				function respuesta(btn){
					if(btn=='yes'){
						obtenerMensaje('procesar','','Procesando Informaci&#243;n');
						var cadjson = "{'operacion':'eliminar','codsis':'"+sistema+"','nomven':'"+vista+"',"+getJsonFormulario(fromComprobanteAdicional);
						cadjson += "}";
				        if(valido){
				        	try{
				        		var objjson = Ext.util.JSON.decode(cadjson);
	...
	function CatalogoComprobante()
	{
	    ...
	    var ventanaEstructura = new Ext.Window({
	    	...
	    	modal: true,
	    	frame:true,
	    	title:"<H1 align='center'>Cat&#225;logo de Comprobante de Cr&#233;ditos/Ingreso Adicional</H1>",
	    ...
	    {
	    	setDataFrom(fromComprobanteAdicional,registro);
