...
***********************************************************************************/
	...
	function uf_print_cabecera_detalle($io_encabezado,$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->ezSetY(590);
		$la_data=array(array('cuenta'=>'<b>Cuenta</b>','denominacion'=>'<b>Denominación</b>','asignado'=>'<b>Actualizado</b>',
		                     'acumulado'=>'<b>Ejecutado Acumulado</b>','ejecutado'=>'<b>Ejecutado del Mes</b>',
	...
	function uf_print_detalle($la_data,$io_pdf)
	{
		...
		$la_columnas=array('cuenta'=>'<b>Cuenta</b>',
						   ...
						   'ejecutado'=>'<b>Ejecutado del Mes</b>',
						   'disponibilidad'=>'<b>Disponibilidad</b>');
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		...
		for($z=1;$z<=$li_tot;$z++)
		{
			...
			if (!empty($ls_programatica))
		    {
				...
				$ld_monto_acumulado=number_format($ld_monto_acumulado,2,",",".");
				 ...
				 if ($ls_status=="S")
				 {
                                    $la_data[$z]=array('cuenta'=>'<b>'.$ls_spg_cuenta.'</b>','denominacion'=>'<b>'.$ls_denominacion.'</b>',
                                                       'asignado'=>'<b>'.$ld_asignado.'</b>','acumulado'=>'<b>'.$ld_monto_acumulado.'</b>',
                                                       'ejecutado'=>'<b>'.$ld_monto_ejecutado.'</b>','disponibilidad'=>'<b>'.$ld_disponible.'</b>');
                                 }
                                 else 
                                 {
				 ...
				 if($ls_tipoformato==1)
				 {
						...
						$ld_monto_acumulado=number_format($ld_monto_acumulado,2,",",".");
                                                ...
                                                if ($ls_status=="S")
                                                {
                                                    ...
                                                    $ld_monto_acumulado='<b>'.$ld_monto_acumulado.'</b>';
                                                    $ld_monto_ejecutado='<b>'.$ld_monto_ejecutado.'</b>';
