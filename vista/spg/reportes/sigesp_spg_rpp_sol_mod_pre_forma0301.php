...
//--------------------------------------------------------------------------------------------------------------------------------
function uf_print_encabezado_pagina($as_titulo,$as_procede,$ad_fecha,$as_fuentefin="",$as_unidadadministradora="",$ab_gastoc,$io_pdf)
{
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$tm=306-($li_tm/2);
	$io_pdf->rectangle(20,675,570,85);
	...
	$io_pdf->addText(25,680,7,"UNIDAD ADMINISTRADORA: ".$as_unidadadministradora);
	$io_pdf->addText($tm,730,10,$as_titulo);
	$io_pdf->rectangle(20,610,570,60);
	$io_pdf->addText(30,645,7,"INSUBSISTENCIA");$io_pdf->rectangle(95,643,10,10);
	$io_pdf->addText(130,645,7,"REDUCCIÓN");$io_pdf->rectangle(180,643,10,10);
	$io_pdf->addText(220,660,7,"<b>RECURSOS ADICIONALES</b>");
	$io_pdf->addText(223,645,7,"CRÉDITO ADICIONAL");$io_pdf->rectangle(300,643,10,10);
	$io_pdf->addText(238,623,7,"RECTIFICACIÓN");$io_pdf->rectangle(300,620,10,10);
	$io_pdf->addText(470,660,7,"<b>TRASPASO</b>");
	$io_pdf->addText(450,645,7,"GASTOS CORRIENTES");$io_pdf->rectangle(540,643,10,10);
	$io_pdf->addText(460,623,7,"GASTOS DE CAPITAL");$io_pdf->rectangle(540,620,10,10);
	//Impresión de las X para el Marcado de Operacion.

	switch ($as_procede)
        {
		case 'SPGINS':
			$io_pdf->addText(97.5,646.5,7,"<b>X</b>");//Insubsistencia
			break;
		case 'SPGCRA':
			$io_pdf->addText(302.5,646.5,7,"<b>X</b>");//Crédito Adicional.
			break;
		case 'SPGREC':
			$io_pdf->addText(302.5,645.5,7,"<b>X</b>");//Rectificacion.
			break;
		case 'SPGTRA':
			if($ab_gastoc)
                        {
				$io_pdf->addText(542.5,645.5,7,"<b>X</b>");//Traspaso.
			}
			else
                        {
				$io_pdf->addText(542.5,646.5,7,"<b>X</b>");//Traspaso.
			}
			break;
	}
	$io_pdf->addText(30,600,8,"<b>IMPUTACIÓN PRESUPUESTARIA</b>"); // Agregar el título

	$io_pdf->Rectangle(19,100,570,10);

	$io_pdf->Rectangle(19,40,570,60);
	$io_pdf->line(100,40,100,100);
	$io_pdf->line(185,40,185,100);
	$io_pdf->line(270,40,270,100);
        $io_pdf->line(350,40,350,100);
	$io_pdf->line(430,40,430,110);
	$io_pdf->line(510,40,510,100);

	$io_pdf->addText(200,102,8,"<b>INSTITUCION</b>"); // Agregar el título
	$io_pdf->addText(432,102,8,"<b>OFICINA NACIONAL DE PRESUPUESTO</b>"); // Agregar el título


	$io_pdf->addText(20,47,5,"DIRECTOR DE ADMINISTRACION"); // Agregar el título
	$io_pdf->addText(40,42,5,"    Y SERVICIOS"); // Agregar el título
	$io_pdf->addText(102,47,5,"RESPONSABLE DEL PROYECTO/"); // Agregar el título        
	$io_pdf->addText(110,42,5,"  ACCION CENTRALIZADA"); // Agregar el título
	$io_pdf->addText(198,42,5,"DIRECTOR DE FINANZAS"); // Agregar el título
	$io_pdf->addText(274,47,5,"DIRECTOR DE PLANIFICACION"); // Agregar el título
	$io_pdf->addText(288,42,5,"DE PRESUPUESTO"); // Agregar el título
        
	$io_pdf->addText(368,42,5,"MAXIMA AUTORIDAD"); // Agregar el título
	$io_pdf->addText(458,42,5,"DIRECTOR"); // Agregar el título
	$io_pdf->addText(525,42,5,"DIRECTOR GENERAL"); // Agregar el título
...
//--------------------------------------------------------------------------------------------------------------------------------
function uf_print_detalle($la_data,$ls_titulo1,$ls_titulo2,$ld_monto,$io_pdf,$posicion)
{
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
$ls_unidadadministradora =$arrResultado['ls_unidadadministradora'];
$lb_gasto_capital = $io_report->uf_validar_gasto_capital($_SESSION["la_empresa"]["codemp"],$ls_procede,$ls_comprobante,$ld_fecha);
uf_print_encabezado_pagina($ls_titulo,$ls_procede,$ld_fecha,$ls_fuentefin,$ls_unidadadministradora,$lb_gasto_capital,$io_pdf); // Imprimimos el encabezado de la página
...
else
{
	...
	$la_data_detalle_dis =$arrResultado['la_data'];
        $posicion=-1;
        if (count($la_data_detalle_dis)>0)
        {
            uf_print_detalle($la_data_detalle_dis,"Partidas Cedentes","TOTAL PARTIDAS CEDENTES",$ld_tot_dis,$io_pdf,$posicion);
            $posicion=-15;
        }
	...
	$la_data_detalle_aum =$arrResultado['la_data'];
        if (count($la_data_detalle_aum)>0)
        {
            uf_print_detalle($la_data_detalle_aum,'Partidas Receptoras',"TOTAL PARTIDAS RECEPTORAS",$ld_tot_aum,$io_pdf,$posicion);
        }
}
