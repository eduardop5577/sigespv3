...
***********************************************************************************/
	...
	function uf_print_pie_cabecera($ad_totalcomprometer,$ad_totalcausado,$ad_totalpagado,$io_pdf,$as_titulo)
	{
		...
		$la_data[]=array('cuenta'=>' ','comprobante'=>'','fecha'=>'<b>'.$as_titulo.'</b> ','comprometido'=>number_format($ad_totalcomprometer,2,",","."),
		                 'causado'=>number_format($ad_totalcausado,2,",","."),'pagado'=>number_format($ad_totalpagado,2,",","."));
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
     ...
     while(!$data_compromisos->EOF)
	 {
     	...
     	$monto_anulado = $io_report->uf_buscar_anulado_compromiso($procede, $documento, $spg_cuenta, $codestpro1, $estcla, $codestpro2, $codestpro3, $codestpro4, $codestpro5);
        ...
        $monto_comprometido =($monto_comprometido + $monto_anulado + $monto_reverso);
	 ...
	 else // Imprimimos el reporte
	 {
		...
		foreach ($arrdatareporte as $causadoparcial)
		{
			...
			if($li_control+2>$li_causadoparcial){
				uf_print_detalle($la_data,$io_pdf); // Imprimimos el detalle
				$ld_sub_total_comprometeraux=number_format($ld_sub_total_comprometer,2,",",".");
				$ld_sub_total_causadoaux=number_format($ld_sub_total_causado,2,",",".");
