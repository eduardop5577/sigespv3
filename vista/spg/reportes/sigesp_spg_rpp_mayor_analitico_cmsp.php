  ...
  // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	...
	if (!empty($ls_codfuefindes) && !empty($ls_codfuefinhas))
	{
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$io_tcpdf->SetHeaderData($_SESSION["ls_logo"], 60, date("d/m/Y"), date("h:i a").'-'.$_SESSION["ls_database"],$_SESSION["ls_height"]);
			...
			{  
				...
				// var_dump($ld_totspg_por_pagar);
			      ...
			      if(($ls_spg_cuenta!=$ls_spg_cuenta_next)&& ($li_tmp!=($li_totrow_det-1)))
			      {
					  ...
					  $la_data[$li_tmp]=array($ls_fecha,$ls_comprobante,$ls_documento,$ls_procede,
					 ...
					 $io_tcpdf->uf_print_detalle_campoCMSP($la_auxdata,$y,0,$ls_mostrar); // Imprimimos los saldos anteriores							 
					 $io_tcpdf->uf_print_detalle_campoCMSP($la_data,$y,1,$ls_mostrar); // Imprimimos el detalle
				...
				if($li_tmp==($li_totrow_det-1))
				{
					...
					$la_data_subtot2[1]=array('SUB TOTAL CUENTA '.$ls_spg_cuenta,
					 ...
					 $io_tcpdf->uf_print_titulos_campoext ($ls_mostrar);
					 $io_tcpdf->uf_print_detalle_campoCMSP($la_auxdata,$y,0,$ls_mostrar); // Imprimimos los saldos anteriores	
					 $io_tcpdf->uf_print_detalle_campoCMSP($la_data,$y,1,$ls_mostrar); // Impr,imimos el detalle 
					 $io_tcpdf->uf_print_total($la_data_subtot2,'0',$y,$ls_mostrar);
					 $la_data_subtot[1]=array("TOTAL CUENTA ".$ls_spg_cuenta,number_format($ld_totspg_asignado,2,",","."),
					  						  number_format($ld_totspg_aumento ,2,",","."),
											  number_format($ld_totspg_disminucion,2,",","."),
