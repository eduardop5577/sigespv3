...
***********************************************************************************/
	...
	{	
		...
		$ls_sql="SELECT cxp_sol_dc.*,".
				...
				"  FROM cxp_sol_dc,rpc_proveedor,rpc_beneficiario ".$ls_filtrofrom.
				" WHERE cxp_sol_dc.codemp='".$ls_codemp."'".
				"   AND cxp_sol_dc.fecope BETWEEN '".$ad_fecregdes."' AND '".$ad_fecreghas."' ".
		        $ls_aux_sql." ".$ls_filtroest.
		...
		if($rs_data!=false)
		{
			while($row=$io_aprobacion->io_sql->fetch_row($rs_data))
			{
				...
				if($li_rowspg>=1)
				{
					$arrResultado=$io_cxp->uf_verificar_cierre_spg("../../",$ls_estciespg);
				...
				if($li_rowscg>=1)
				{
					$arrResultado=$io_cxp->uf_verificar_cierre_scg("../../",$ls_estciescg);
					$lb_valido=$arrResultado["lb_valido"];
				...
				if ($ls_nivapro==1)
				{
					if(($ls_codniv!="")&&($li_monnivhas!=0)&&($li_monto_comp <= $li_monnivhas))
				    {
						if($lb_imprimir)
						{
							$li_fila=$li_fila + 1;
