...
/******************************************************************************************************************/		
	function uf_get_provbene($as_mes,$as_agno,$as_probendesde,$as_probenhasta,$as_tipo,$as_tiporet,$aa_sujret)
	{
		...
		if($rs_result===false)
		{
			$this->io_msg->message("CLASE->Generar Comprobate MÉTODO->uf_get_provbene ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;	
		}
		else
		{
...
/******************************************************************************************************************/
								  ...
								  $as_origen,$aa_seguridad,$li_i=1)
	{
		...
		{	
			...
			{
				$this->io_msg->message("CLASE->Generar Comprobate MÉTODO->uf_crear_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sqlaux->message));
				$lb_valido=false;
			}
		}
		else
...
function uf_validar_periodo_fiscal($as_codret,$as_periodofiscal)
{
		...
		if($rs_result===false)
		{
			$this->io_msg->message("CLASE->Generar Comprobate MÉTODO->uf_validar_periodo_fiscal ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
			return false;			
		}
		else
		{
