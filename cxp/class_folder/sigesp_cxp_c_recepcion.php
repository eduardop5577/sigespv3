...
/*		$ls_sql="SELECT cxp_rd_spg.numdoccom, cxp_rd_spg.codestpro, cxp_rd_spg.spg_cuenta, cxp_rd_spg.monto, spg_cuentas.sc_cuenta, ".
		...
		$ls_sql="SELECT cxp_rd_spg.numdoccom, cxp_rd_spg.codestpro, cxp_rd_spg.spg_cuenta, cxp_rd_spg.monto AS monto, spg_cuentas.sc_cuenta, ".
				...
				"      (SELECT MAX(cxp_rd_spg.codfuefin) FROM cxp_rd_spg".
		...
		if ($_SESSION["la_empresa"]["confiva"]=='P')
		   {
			 $ls_sql= "SELECT cxp_rd_cargos.codcar, cxp_rd_cargos.numdoccom, cxp_rd_cargos.monobjret, cxp_rd_cargos.monret, ".
					  ...
					  "		cxp_rd_cargos.procede_doc, trim(spg_cuentas.sc_cuenta) as sc_cuenta, cxp_rd_cargos.estcla, cxp_rd_cargos.codfuefin ".
					  "  FROM cxp_rd_cargos, spg_cuentas ".
					  " WHERE cxp_rd_cargos.codemp = '".$this->ls_codemp."'".
					  "	AND trim(cxp_rd_cargos.numrecdoc) = '".trim($as_numrecdoc)."'".
					  "	AND cxp_rd_cargos.codtipdoc = '".$as_codtipdoc."'".
		...
		elseif($_SESSION["la_empresa"]["confiva"]=='C')
		   {
		     $ls_sql = "SELECT cxp_rd_cargos.codcar, cxp_rd_cargos.numdoccom, cxp_rd_cargos.monobjret, cxp_rd_cargos.monret, cxp_rd_cargos.codestpro1, 
							   ...
							   cxp_rd_cargos.porcar, cxp_rd_cargos.formula, cxp_rd_cargos.procede_doc, cxp_rd_cargos.estcla, trim(scg_cuentas.sc_cuenta) as sc_cuenta , cxp_rd_cargos.codfuefin
						  FROM cxp_rd_cargos, spg_ep5, scg_cuentas
						  WHERE cxp_rd_cargos.codemp = '".$this->ls_codemp."' 
							AND trim(cxp_rd_cargos.numrecdoc) = '".trim($as_numrecdoc)."' 
							AND cxp_rd_cargos.codtipdoc = '".$as_codtipdoc."' 
	...
	function uf_select_recepcion($as_numrecdoc,$as_tipodestino,$as_codprovben,$as_codtipdoc)
	{
		...
		{
			$cadena="	AND codtipdoc='".$as_codtipdoc."' ";
		}
	...
	function uf_load_estatus($as_numrecdoc,$as_tipodestino,$as_codprovben,$as_codtipdoc,$as_estprodoc)
	{
		...
		$arrResultado["estprodoc"]=$as_estprodoc;
		return $arrResultado;
	...
	function uf_insert_recepcion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$as_codcla,$as_dencondoc,$ad_fecemidoc,$ad_fecregdoc,
								 ...
								 $as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$as_codproalt,
								 $as_tipdoctesnac,$as_numexprel,$as_estretasu,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$arrResultado=$this->io_keygen->uf_verificar_numero_generado2("CXP","cxp_rd","codrecdoc","CXPRCD",15,"","","",$as_codrecdoc,$_SESSION["la_logusr"]);
		$as_codrecdoc=$arrResultado['as_numero'];
		$lb_valido=true;
		if($as_tipdoctesnac=="")
			$as_tipdoctesnac='0';
		$ls_sql="INSERT INTO cxp_rd (codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, codcla, dencondoc, fecemidoc, fecregdoc, ".
				...
				" codestpro4, codestpro5, estcla, estact, numordpagmin, codtipfon,repcajchi,codproalt,codusureg,tipdoctesnac,".
				" numexprel,estretasu)  VALUES ('".$this->ls_codemp."','".trim($as_numrecdoc)."','".$as_codtipdoc."', ".
				...
				"'".$as_codtipfon."','".$as_repcajchi."','".$as_codproalt."','".$_SESSION["la_logusr"]."',".
				"'".$as_tipdoctesnac."','".$as_numexprel."','".$as_estretasu."')";
...
//			$li_moncue=str_replace(",",".",$li_moncue);
			...
			$li_disponible=0;
			$ls_numcomanu="";
			$arrResultado=$this->uf_load_monto_comprobantes_cuenta($ls_nrocomp,$ls_procede,$as_tipodes,$as_codpro,$as_cedbene,
											   					$ad_fechareg,$ls_programatica,$ls_estcla,$ls_cuenta,$li_monto_compromiso);
			$lb_valido=$arrResultado["valido"];
			$li_monto_compromiso=$arrResultado["monto"];
			unset($arrResultado);
			if($lb_valido)
			{	
				$li_monto_ajuste="";										   			
				$arrResultado=$this->uf_load_monto_ajustes_cuenta($ls_nrocomp,$ls_procede,$as_tipodes,$as_codpro,$as_cedbene,
			...
			{
				$li_monto_causado="";										   			
				$arrResultado=$this->uf_load_monto_causados_cuenta($ls_nrocomp,$ls_procede,$as_tipodes,$as_codpro,$as_cedbene,
			...
			{	
				$ls_numcomanu="";										   			
				$arrResultado=$this->uf_load_comprobantes_anulados($ls_nrocomp,$as_tipodes,$as_codpro,$as_cedbene,$ad_fechareg,
																$ls_numcomanu);
				$lb_valido=$arrResultado["valido"];
				$ls_numcomanu=$arrResultado["valor"];
				unset($arrResultado);
			}
			if(($lb_valido) &&($li_monto_causado>0))
			{
				$li_monto_anulado="";										   			
				$arrResultado=$this->uf_load_monto_anulados_cuenta($ls_nrocomp,$ls_procede,$as_tipodes,$as_codpro,$as_cedbene,
			...
			{
				$li_monto_recepcion="";										   			
				$arrResultado=$this->uf_load_monto_recepciones_cuenta($ls_nrocomp,$ls_procede,$ls_programatica,$ls_estcla,$ls_cuenta,
																   $li_monto_recepcion);
				$lb_valido=$arrResultado["valido"];
			...
			{
				$li_monto_ordenpago="";										   			
				$arrResultado=$this->uf_load_monto_ordenespago_directa_cuenta($ls_nrocomp,$ls_procede,$ls_programatica,$ls_estcla,$ls_cuenta,
																		   $li_monto_ordenpago);
				$lb_valido=$arrResultado["valido"];
...
//			if($lb_valido)
//			{
//				$arrResultado=$this->uf_load_monto_cargos_cuenta($ls_nrocomp,$ls_procede,$ls_programatica,$ls_estcla,$ls_cuenta,$li_monto_cargo);
//				$lb_valido=$arrResultado["valido"];
//				$li_monto_cargo=$arrResultado["monto"];
//				unset($arrResultado);
//			}
			if($lb_valido)
			{
...
//				$li_comprometido=$li_monto_compromiso+(($li_monto_ajuste)-$li_monto_causado+$li_monto_anulado-$li_monto_recepcion-$li_monto_cargo);
	...
	function uf_insert_cuentasscg($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$ai_totrowscg,$aa_seguridad)
	{
		...
		{
			$as_tipodocanti="";
			$arrResultado=$this->uf_select_tipodocumento($as_codtipdoc,$as_tipodocanti);
			$lb_valido=$arrResultado["valido"];
			$as_tipodocanti=$arrResultado["valor"];
			if($lb_valido)
			{
				if($as_tipodocanti=="1")
				{
	...
	function uf_insert_cargos($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_seguridad)
	{
		...
		if(array_key_exists("cargos",$_SESSION))
		{
			...
			for($li_fila=1;$li_fila<=$li_totrow;$li_fila++)
			{
				...
				$ls_codfuefin = $this->io_ds_cargos->getValue('codfuefin',$li_fila);
				if($li_baseimp>0)
				{
					$ls_sql="INSERT INTO cxp_rd_cargos (codemp,numrecdoc,codtipdoc,cod_pro,ced_bene,codcar,procede_doc,numdoccom,".
							"monobjret,monret,codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,porcar,formula,estcla,codfuefin) ".
							...
							"'".substr($ls_codpro,100,25)."','".$ls_cuenta."',".$li_porcar.",'".$ls_formula."','".$ls_estcla."','".$ls_codfuefin."')";
					$li_row=$this->io_sql->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
						$this->io_mensajes->message("CLASE->Recepción_de_Documentos MÉTODO->uf_insert_cargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
					}
					else
					{
						/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_update_recepcion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$as_codcla,$as_dencondoc,$ad_fecemidoc,$ad_fecregdoc,
								 ...
								 $as_repcajchi,$as_codproalt,$as_tipdoctesnac,$as_numexprel,$as_estretasu,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	    $ls_sql="UPDATE cxp_rd ".
				...
				"		tipdoctesnac='".$as_tipdoctesnac."',  ".
				"		estretasu='".$as_estretasu."',  ".
				"		numexprel='".$as_numexprel."'  ".
				" WHERE codemp='".$this->ls_codemp."' ".
				"	AND trim(numrecdoc) = '".trim($as_numrecdoc)."' ".
				"	AND codtipdoc='".$as_codtipdoc."' ".
	...
	function uf_guardar($as_existe,$as_numrecdoc,$as_tipodestino,$as_codprovben,$as_codtipdoc,$ad_fecregdoc,$ad_fecvendoc,
						...
						$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$as_codproalt,$as_numexprel,$as_estretasu,$aa_seguridad)
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
		//		   Access: public (sigesp_cxp_p_recepcion.php)
		...
		$ls_tipdoctesnac=substr($as_codtipdoc,12,1);
		...
		if(($as_numordpagmin!="")&&($as_codtipfon!="")&&($as_numordpagmin!="-")&&($as_codtipfon!="----"))
		{
			...
			$arrResultado=$this->uf_load_ordenministerio($as_numordpagmin,$as_codtipfon,$li_monordpagmin,$li_porrepfon);
			$lb_valido=$arrResultado["valido"];
			$li_monordpagmin=$arrResultado["valor"];
			$li_porrepfon=$arrResultado["valor1"];
			unset($arrResultado);
		...
		switch ($as_existe)
		{
			case "FALSE":
				if(!($lb_encontrado))
				{
					...
					$lb_valido=$this->uf_insert_recepcion($as_numrecdoc,$as_codtipdoc,$ls_cedbene,$ls_codpro,$as_codcla,$as_dencondoc,
														  ...
														  $as_codestpro5,$as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,
														  $as_codproalt,$ls_tipdoctesnac,$as_numexprel,$as_estretasu,$aa_seguridad);
				}
				else
				{
			...
			case "TRUE":
				...
				{
					$ls_estprodoc="";
					$arrResultado=$this->uf_load_estatus($as_numrecdoc,$as_tipodestino,$as_codprovben,$as_codtipdoc,$ls_estprodoc);
					$lb_valido=$arrResultado["valido"];
					$ls_estprodoc=$arrResultado["estprodoc"];
					if($ls_estprodoc!="R")
					{
					...
					$lb_valido=$this->uf_update_recepcion($as_numrecdoc,$as_codtipdoc,$ls_cedbene,$ls_codpro,$as_codcla,$as_dencondoc,
														  ...
														  $as_numordpagmin,$as_codtipfon,$as_repcajchi,$as_codproalt,
														  $ls_tipdoctesnac,$as_numexprel,$as_estretasu,$aa_seguridad);
				}
				else
				{
	...
	function uf_select_solicitudes_pago($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene)
	{
		...
		$ls_sql="SELECT codemp ".
				"  FROM cxp_dt_solicitudes ".$ls_filtrofrom.
				...
				"   AND cxp_dt_solicitudes.ced_bene = '".$as_cedbene."' ".$ls_filtroest;
		$rs_data=$this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$lb_valido=false; 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_comprobantes_positivos($as_tipodestino,$as_codpro,$as_cedbene,$as_fechahasta,$as_numdoccom="")
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($as_numdoccom!="")
		{
			$ls_cadena=" AND sigesp_cmp.comprobante LIKE '%".$as_numdoccom."%'";
		}
		$ls_estcomobr=$_SESSION["la_empresa"]["estcomobr"];
		if($ls_estcomobr=="0")
		{
			$ls_cadena=$ls_cadena."AND sigesp_cmp.procede <> 'SOBCON'";
		}
		$ls_sql="SELECT DISTINCT sigesp_cmp.procede, sigesp_cmp.comprobante, sigesp_cmp.fecha, sigesp_cmp.descripcion, sigesp_cmp.total,SUM(spg_dt_cmp.monto) AS monto".
				"  FROM sigesp_cmp ".
				...
				$ls_cadena.
				" GROUP BY sigesp_cmp.procede, sigesp_cmp.comprobante, sigesp_cmp.fecha, sigesp_cmp.descripcion, sigesp_cmp.total".
				" ORDER BY sigesp_cmp.comprobante ASC";//print $ls_sql;
	...
	function uf_load_comprobantes_anulados($as_comprobante,$as_tipodestino,$as_codpro,$as_cedbene,$as_fechahasta,$as_numcomanu)
	{
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["valor"]=$as_numcomanu;	
		return $arrResultado;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_monto_causado_anterior($as_comprobante,$as_procede,$as_spgcuenta,$as_codestpro,$as_estcla,$as_codfuefin,$ai_monto)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT SUM(CASE WHEN cxp_rd_spg.monto is null THEN 0 ELSE cxp_rd_spg.monto END) AS monto ".
				...
				"   AND cxp_rd_spg.estcla='".$as_estcla."' ". 
				"   AND cxp_rd_spg.codfuefin='".$as_codfuefin."' ". 
				"   AND cxp_rd_spg.codemp=cxp_rd.codemp ".
	...
	function uf_load_compromiso_sep_donacion($as_numsol)
	{
		...
		$ls_sql="SELECT sep_cuentagasto.numsol AS comprobante, sep_cuentagasto.codestpro1, sep_cuentagasto.codestpro2, ".
				...
				"		sep_cuentagasto.spg_cuenta, sep_cuentagasto.monto, spg_cuentas.sc_cuenta, sep_solicitud.codfuefin, sep_cuentagasto.estcla, ".
				"		'0' AS cargo, ".
				"       '' AS tipbieordcom, '' AS estint, '' AS cuentaint".
				"  FROM sep_cuentagasto, spg_cuentas,sep_solicitud ".
				...
				"		sep_solicitudcargos.spg_cuenta, -(sep_solicitudcargos.monto) monto, spg_cuentas.sc_cuenta, sep_solicitud.codfuefin, sep_solicitudcargos.estcla, ".
				"		'0' AS cargo, ".
				"       '' AS tipbieordcom, '' AS estint, '' AS cuentaint".
				"  FROM sep_solicitudcargos, spg_cuentas, sep_solicitud ".
	...
	function uf_load_cargos_compromiso_sepsinds($as_numsol)
	{
		...
		$ls_sql="SELECT sep_solicitudcargos.numsol AS comprobante, sep_solicitudcargos.codestpro1, sep_solicitudcargos.codestpro2, ".
				"		sep_solicitudcargos.codestpro3, sep_solicitudcargos.codestpro4, sep_solicitudcargos.codestpro5, ".
				"		sep_solicitudcargos.spg_cuenta, sep_solicitudcargos.monto, spg_cuentas.sc_cuenta, sep_solicitudcargos.codfuefin, sep_solicitudcargos.estcla, ".
				"		'1' AS cargo, ".
				"       '' AS tipbieordcom, '' AS estint, '' AS cuentaint".
	...
	function uf_load_cargos_compromiso_sep($as_numsol,$ai_total_cargos)
	{
		...
		$ls_sql="SELECT sep_solicitudcargos.codcar, sep_solicitudcargos.numsol, sep_solicitudcargos.codestpro1, ".
				...
				"		sep_solicitudcargos.formula, sep_solicitudcargos.monobjret, sep_solicitudcargos.monto,sep_solicitud.codfuefin, ".
				"		sigesp_cargos.porcar, sep_solicitudcargos.estcla ".
				"  FROM sep_solicitudcargos, sigesp_cargos,sep_solicitud ".
				" WHERE sep_solicitudcargos.codemp='".$this->ls_codemp."' ".
				"   AND sep_solicitudcargos.numsol='".$as_numsol."' ".
		...
		else
		{
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$arrResultado=$this->uf_load_baseimp_causado_anterior($ls_codcar,$ls_nrocomp,$ls_procede,$row["codestpro1"],
															     $row["codestpro2"],$row["codestpro3"],$row["codestpro4"],
																 $row["codestpro5"],$ls_estcla,$ls_cuenta,$li_baseimp_anterior);
				$lb_valido=$arrResultado["valido"];
				$li_baseimp_anterior=$arrResultado["monto"];
				unset($arrResultado);

				$arrResultado=$this->uf_load_cargo_causado_anterior($ls_codcar,$ls_nrocomp,$ls_procede,$row["codestpro1"],
															     $row["codestpro2"],$row["codestpro3"],$row["codestpro4"],
																 $row["codestpro5"],$ls_estcla,$ls_cuenta,$li_monto_anterior);
				$lb_valido=$arrResultado["valido"];
				$li_monto_anterior=$arrResultado["monto"];
				unset($arrResultado);
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["monto"]=$ai_total_cargos;	
		return $arrResultado;
	}// end function uf_load_cargos_compromiso_sep
	//-----------------------------------------------------------------------------------------------------------------------------------
		...
		if($ls_sql!="")
		{
			...
			else
			{
				if(!$rs_data->EOF)
				{
					...
					$as_formula=$rs_data->fields["formula"];
				}
				$this->io_sql->free_result($rs_data);
			}		
		}
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["valor"]=$as_sccuenta;	
		$arrResultado["formula"]=$as_formula;	
		return $arrResultado;
	}// end function uf_load_cargos_compromisocontable
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_retenciones($as_codconf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_retenciones
		//		   Access: public (sigesp_cxp_c_catalogo_ajax.php)
		//	    Arguments: as_numero  // Número de comprobante
		//				   as_procede  // procede del documento
		//				   ao_ds_cargos  // Datastored de Cargos
		...
		$as_sccuenta="";
		$as_formula="";
		$ls_sql= "SELECT cxp_dt_confded.codded, sigesp_deducciones.formula, sigesp_deducciones.iva ".
				 "  FROM cxp_dt_confded, sigesp_deducciones".
				 " WHERE cxp_dt_confded.codemp='".$this->ls_codemp."'".
				 "   AND cxp_dt_confded.codconf='".$as_codconf."'".
				 "   AND cxp_dt_confded.codemp=sigesp_deducciones.codemp".
				 "   AND cxp_dt_confded.codded=sigesp_deducciones.codded";
		if($ls_sql!="")
		{
			$rs_data=$this->io_sql->select($ls_sql);
			if ($rs_data===false)
			{
				$lb_valido=false; 
				$this->io_mensajes->message("CLASE->Recepción MÉTODO->uf_load_retenciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
			else
			{
				$li_i=0;
				...
				{
					$li_i++;
					$as_codded=$rs_data->fields["codded"];
					$as_formula=$rs_data->fields["formula"];
					$as_iva=$rs_data->fields["iva"];
					$arrDecuccion[$li_i]["codded"]=$as_codded;
					$arrDecuccion[$li_i]["formula"]=$as_formula;
					$arrDecuccion[$li_i]["iva"]=$as_iva;
				    $rs_data->MoveNext();
				}
				$this->io_sql->free_result($rs_data);
			}		
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["arrDecuccion"]=$arrDecuccion;	
		return $arrResultado;
	}// end function uf_load_cargos_compromisocontable
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_load_cargoscontable_compromiso_sep($as_numsol,$ai_total_cargos)
	{
		...
		else
		{
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$li_monto_anterior=0;
				$li_baseimp_anterior=0;
				$arrResultado=$this->uf_load_baseimp_causado_anterior($ls_codcar,$ls_nrocomp,$ls_procede,"-------------------------",
															     "-------------------------","-------------------------","-------------------------",
				...
				$arrResultado=$this->uf_load_cargo_causado_anterior($ls_codcar,$ls_nrocomp,$ls_procede,"-------------------------",
															     "-------------------------","-------------------------","-------------------------",
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["valor"]=$ai_total_cargos;	
		return $arrResultado;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_compromiso_scv($as_codcom)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_compromiso_scv
		//		   Access: public (sigesp_cxp_c_catalogo_ajax.php)
		//	    Arguments: as_codcon  // Número de Contrato
		//	      Returns: lb_valido True si se ejecuto el select
		//	  Description: Función que se encarga de buscar las cuentas presupuestarias asociadas a un contrato
		...
		$lb_valido=true;
	//	$ls_codcon = substr($as_codcon,3,12);
		$ls_sql="SELECT codcom AS comprobante, scv_dt_spg.codestpro1, scv_dt_spg.codestpro2, ".
				"		scv_dt_spg.codestpro3, scv_dt_spg.codestpro4, scv_dt_spg.codestpro5, ".
				"		scv_dt_spg.spg_cuenta, scv_dt_spg.monto, spg_cuentas.sc_cuenta, scv_dt_spg.codfuefin, scv_dt_spg.estcla, ".
				"		'0' AS cargo, ".
				"           '' AS tipbieordcom, '' AS estint, '' AS cuentaint ".
                "  FROM scv_dt_spg, spg_cuentas ".
                " WHERE scv_dt_spg.codemp='".$this->ls_codemp."' ".
				"   AND trim(scv_dt_spg.codcom) = '".$as_codcom."' ".
				"   AND scv_dt_spg.codemp = spg_cuentas.codemp ".
				"   AND scv_dt_spg.spg_cuenta = spg_cuentas.spg_cuenta ".
				"	AND scv_dt_spg.codestpro1 = spg_cuentas.codestpro1 ".
				"	AND scv_dt_spg.codestpro2 = spg_cuentas.codestpro2 ".
				"	AND scv_dt_spg.codestpro3 = spg_cuentas.codestpro3 ".
				"	AND scv_dt_spg.codestpro4 = spg_cuentas.codestpro4 ".
				"	AND scv_dt_spg.codestpro5 = spg_cuentas.codestpro5 ".
				"	AND scv_dt_spg.estcla = spg_cuentas.estcla ";
		$rs_data=$this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$lb_valido=false; 
	...
	function uf_load_compromiso_soc_donacion($as_numordcom,$as_estcondat)
	{
		...
		$ls_sql="SELECT soc_cuentagasto.numordcom AS comprobante, soc_cuentagasto.codestpro1, soc_cuentagasto.codestpro2, ".
				...
				"		soc_cuentagasto.spg_cuenta, soc_cuentagasto.monto, spg_cuentas.sc_cuenta, soc_cuentagasto.codfuefin, soc_cuentagasto.estcla, ".
				...
				"		soc_solicitudcargos.spg_cuenta, -(soc_solicitudcargos.monto) AS monto, spg_cuentas.sc_cuenta, soc_solicitudcargos.codfuefin, soc_solicitudcargos.estcla, ".
				"		'0' AS cargo, soc_ordencompra.tipbieordcom  ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_compromiso_spg($as_numordcom,$as_estcondat)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT spg_dt_cmp.comprobante, spg_dt_cmp.codestpro1, spg_dt_cmp.codestpro2, ".
				"		spg_dt_cmp.codestpro3, spg_dt_cmp.codestpro4, spg_dt_cmp.codestpro5,spg_ep1.estint,spg_ep1.sc_cuenta AS cuentaint,".
				"		spg_dt_cmp.spg_cuenta, spg_dt_cmp.monto, spg_cuentas.sc_cuenta, spg_dt_cmp.codfuefin, spg_dt_cmp.estcla, ".
				"		0 AS cargo, '' AS tipbieordcom  ".
				"  FROM spg_dt_cmp, spg_cuentas,spg_ep1 ".
				" WHERE spg_dt_cmp.codemp = '".$this->ls_codemp."' ".
				"   AND spg_dt_cmp.comprobante = '".$as_numordcom."' ".
				"   AND spg_dt_cmp.procede = 'SPGCMP' ".
				...
				"   AND spg_dt_cmp.codemp=spg_ep1.codemp".
				"   AND spg_dt_cmp.codestpro1=spg_ep1.codestpro1".
				"   AND spg_dt_cmp.estcla=spg_ep1.estcla";
		$rs_data=$this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$lb_valido=false; 
			$this->io_mensajes->message("CLASE->Recepción MÉTODO->uf_load_compromiso_spg ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
		...
		return $lb_valido;
	}// end function uf_load_compromiso_spg
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_load_cargos_compromiso_soc($as_numordcom,$as_estcondat,$ai_total_cargos)
	{
		...
		$ls_sql="SELECT soc_solicitudcargos.codcar, soc_solicitudcargos.numordcom, soc_solicitudcargos.codestpro1, ".
				...
				"		soc_solicitudcargos.formula, soc_solicitudcargos.monobjret, soc_solicitudcargos.monret, soc_solicitudcargos.codfuefin, ".
	...
	function uf_load_compromiso_sno($as_comprobante)
	{
		...
		$ls_sql="SELECT sigesp_cmp.comprobante, spg_dt_cmp.codestpro1, spg_dt_cmp.codestpro2, ".
				...
				"		spg_dt_cmp.spg_cuenta, spg_dt_cmp.monto, spg_cuentas.sc_cuenta, '0' AS cargo, spg_dt_cmp.codfuefin, spg_dt_cmp.estcla, ".
				"       '' AS tipbieordcom, '' AS estint, '' AS cuentaint".
	...
	function uf_load_cargos_compromiso($as_numero,$as_procede,$ao_ds_cargos)
	{
			...
			switch($as_procede)
			{
				case "SOCCOC": // Orden de Compra de Bienes
					$ls_sql="SELECT soc_solicitudcargos.codcar, (soc_solicitudcargos.numordcom) AS compromiso, soc_solicitudcargos.codestpro1, ".
							...
							"		soc_solicitudcargos.estcla, sigesp_cargos.dencar, sigesp_cargos.porcar, soc_solicitudcargos.codfuefin ".
							"  FROM soc_solicitudcargos, sigesp_cargos ".
							" WHERE soc_solicitudcargos.codemp='".$this->ls_codemp."' ".
				...
				case "SEPSPC": // Solicitud de Ejecución Presupuestaria
					$ls_sql="SELECT sep_solicitudcargos.codcar, (sep_solicitudcargos.numsol) AS compromiso, sep_solicitudcargos.codestpro1, ".
							...
							"		sep_solicitudcargos.estcla, sigesp_cargos.dencar, sigesp_cargos.porcar,sep_solicitud.codfuefin ".
							"  FROM sep_solicitudcargos, sigesp_cargos,sep_solicitud ".
							" WHERE sep_solicitudcargos.codemp='".$this->ls_codemp."' ".
			...
			switch ($as_procede)
			{
				case "SOCCOC": // Orden de Compra de Bienes
					$ls_sql="SELECT soc_dta_cargos.codcar, soc_dta_cargos.numordcom AS compromiso , '-------------------------' AS codestpro1, ".
							...
							" 		sigesp_cargos.spg_cuenta, soc_dta_cargos.monbasimp AS monobjret, soc_dta_cargos.monimp AS monret,".
							"       sigesp_cargos.formula, sigesp_cargos.dencar, sigesp_cargos.porcar,'--' AS codfuefin ".
							"  FROM soc_dta_cargos, sigesp_cargos ".
							" WHERE soc_dta_cargos.codemp='".$this->ls_codemp."' ".
				...
				case "SOCCOS": // Orden de Compra de Servicios
					$ls_sql="SELECT soc_dts_cargos.codcar, soc_dts_cargos.numordcom AS compromiso, '-------------------------' AS codestpro1, ".
							"		'-------------------------' AS codestpro2, '-------------------------' AS codestpro3, ".
							"       '-------------------------' AS codestpro4, '-------------------------' AS codestpro5, 'A' AS estcla, ".
							" 		sigesp_cargos.spg_cuenta, soc_dts_cargos.monbasimp AS monobjret, soc_dts_cargos.monimp AS monret,".
							"      sigesp_cargos.formula, sigesp_cargos.dencar, sigesp_cargos.porcar ,'--' AS codfuefin ".
							"  FROM soc_dts_cargos, sigesp_cargos ".
							" WHERE soc_dts_cargos.codemp='".$this->ls_codemp."' ".
				...
				case "SEPSPC": // Solicitud de Ejecución Presupuestaria
					$ls_sql="SELECT sep_dtc_cargos.codcar, sep_dtc_cargos.numsol AS compromiso, '-------------------------' AS codestpro1, ".
							"		'-------------------------' AS codestpro2, '-------------------------' AS codestpro3, ".
							"       '-------------------------' AS codestpro4, '-------------------------' AS codestpro5, 'A' AS estcla, ".
							"       sigesp_cargos.spg_cuenta, sep_dtc_cargos.monbasimp AS monobjret, sep_dtc_cargos.monimp AS monret,".
							"       sigesp_cargos.formula, sigesp_cargos.dencar, sigesp_cargos.porcar,'--' AS codfuefin  ".
							"  FROM sep_dtc_cargos, sigesp_cargos ".
							" WHERE sep_dtc_cargos.codemp='".$this->ls_codemp."' ".
		...
		if($ls_sql!="")
		{
			...
			else
			{
				while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
				{
					...
					$li_monto_anterior=0;
					$arrResultado=$this->uf_load_monto_causado_anterior($ls_nrocomp,$as_procede,$ls_cuenta,$ls_codpro,$ls_estcla,$ls_codfuefin,$li_monto_anterior);
					$lb_valido=$arrResultado["valido"];
					$li_monto_anterior=$arrResultado["monto"];
					unset($arrResultado);
					$li_monimp=$row["monret"]-$li_monto_anterior;
					if($lb_valido)
					{
						...
						$ao_ds_cargos->insertRow("sc_cuenta",$ls_scgcuenta);	
					}		
				}
				$this->io_sql->free_result($rs_data);
			}		
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["valor"]=$ao_ds_cargos;	
		return $arrResultado;
	}// end function uf_load_cargos
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_select_tipodocumento($as_codtipdoc,$as_tipodocanti)
	{
		...
		$arrResultado["valido"]=$lb_valido;
		$arrResultado["valor"]=$as_tipodocanti;
		return $arrResultado;
	}// end function uf_load_tipodocumento
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_select_amortizaciones($as_codpro,$as_cedbene)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql="SELECT cxp_rd_amortizacion.numrecdoc,cxp_rd_amortizacion.codtipdoc,monsal,montotamo,cuenta,codamo,cxp_rd.dencondoc ".
				"  FROM cxp_rd_amortizacion,cxp_rd ".
		  		" WHERE cxp_rd_amortizacion.codemp='".$this->ls_codemp."'".
				"   AND cxp_rd_amortizacion.cod_pro = '".$as_codpro."' ".
				"   AND trim(cxp_rd_amortizacion.ced_bene) = '".trim($as_cedbene)."'".
				"   AND cxp_rd_amortizacion.codemp=cxp_rd.codemp".
				"   AND cxp_rd_amortizacion.numrecdoc=cxp_rd.numrecdoc".
				"   AND cxp_rd_amortizacion.codtipdoc=cxp_rd.codtipdoc".
				"   AND cxp_rd_amortizacion.cod_pro=cxp_rd.cod_pro".
				"   AND cxp_rd_amortizacion.ced_bene=cxp_rd.ced_bene".
				" ORDER BY codamo";
	...
	function uf_select_saldoamortizacion($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene,$as_codamo)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_amortizaciones
		//		   Access: public 
	...
	function uf_update_amortizacion($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene,$aa_seguridad)
	{
		...
		$li_totamortizacion=count((array)$la_amortizacion["recdocant"]);
		$lb_valido=true;
		for($li_i=1;$li_i<=$li_totamortizacion;$li_i++)
		{
			...
			$li_monsal=$this->uf_select_saldoamortizacion($ls_numrecdoc,$ls_codtipdoc,$as_codpro,$as_cedbene,$ls_codamo);
			$li_newsal=($li_monsal-$li_monto);
			if($li_newsal>=0)
			{
			...
			{
				$this->io_mensajes->message("El saldo del Anticipo es Menor a 0. Favor verifique los datos");
				return false;
			}
		}
		return $lb_valido;
	...
	function uf_reverso_anticipos($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_seguridad)
	{
		...
		$aa_codamo="";
		$aa_monto="";
		$arrResultado=$this->uf_select_amortizacion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_codamo,$aa_monto);
		$lb_valido=$arrResultado["valido"];
		$aa_codamo=$arrResultado["valor"];
		$aa_monto=$arrResultado["valor1"];
		if(($lb_valido)&&(is_array($aa_codamo)))
		{
			$li_total=count((array)$aa_codamo);
	...
	function uf_select_amortizacion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_codamo,$aa_monto)
	{
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["valor"]=$aa_codamo;	
		$arrResultado["valor1"]=$aa_monto;	
		return $arrResultado;
	...
	function uf_validar_bienes($as_comprobante,$as_tipbieordcom)
	{
		...
		$arrResultado["valido"]=$lb_valido;
		$arrResultado["valor"]=$as_tipbieordcom;	
		return $arrResultado;
	...
	function uf_load_ordenministerio($as_numordpagmin,$as_codtipfon,$ai_monordpagmin,$ad_porrepfon)
   	{
		...
		$arrResultado["valido"]=$lb_valido;
		$arrResultado["valor"]=$ai_monordpagmin;	
		$arrResultado["valor1"]=$ad_porrepfon;	
		return $arrResultado;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_monto_cierredisminuciones($as_comprobante,$as_procede,$as_spgcuenta,$as_codestpro,$as_estcla,$as_codfuefin)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_monto_causado_anterior
		//		   Access: public (sigesp_cxp_c_recepcion_ajax.php)
		...
		$ai_monto=0;
		$lb_valido=true; 
		$ls_sql="SELECT SUM(CASE WHEN spg_dt_cmp.monto is null THEN 0 ELSE spg_dt_cmp.monto END) AS monto ".
				"  FROM spg_dt_cmp ".
				" WHERE spg_dt_cmp.codemp='".$this->ls_codemp."' ".
				"   AND spg_dt_cmp.procede like '%CIE' ".
				"   AND spg_dt_cmp.monto < 0 ".
				"   AND spg_dt_cmp.operacion = 'CS' ".
				"   AND spg_dt_cmp.procede_doc='".$as_procede."' ".
				"   AND spg_dt_cmp.documento='".$as_comprobante."' ".
				"   AND spg_dt_cmp.spg_cuenta='".$as_spgcuenta."' ".
				"   AND spg_dt_cmp.codestpro1='".substr($as_codestpro,0,25)."' ". 
				"   AND spg_dt_cmp.codestpro2='".substr($as_codestpro,25,25)."' ". 
				"   AND spg_dt_cmp.codestpro3='".substr($as_codestpro,50,25)."' ". 
				"   AND spg_dt_cmp.codestpro4='".substr($as_codestpro,75,25)."' ". 
				"   AND spg_dt_cmp.codestpro5='".substr($as_codestpro,100,25)."' ". 
				"   AND spg_dt_cmp.estcla='".$as_estcla."' ". 
				"   AND spg_dt_cmp.codfuefin='".$as_codfuefin."' ";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$lb_valido=false; 
		...
		$arrResultado["valido"]=$lb_valido;	
		$arrResultado["monto"]=abs($ai_monto);	
		return $arrResultado;
	}// end function uf_load_monto_causado_anterior
	//-----------------------------------------------------------------------------------------------------------------------------------
