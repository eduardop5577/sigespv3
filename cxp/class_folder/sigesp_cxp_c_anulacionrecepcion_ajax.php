...
***********************************************************************************/
	...
	{	
		...
		$rs_datasol=$io_aprobacion->uf_load_recepciones($as_numsol,$ad_fecregdes,$ad_fecreghas,$as_tipproben,$as_proben,$as_tipooperacion,$ls_repcon);
		$li_fila=0;
		if($rs_datasol!=false)
		{
			while($row=$io_aprobacion->io_sql->fetch_row($rs_datasol))
			{
				...
				if($li_rowspg>=1)
				{
					$arrResultado=$io_funciones_cxp->uf_verificar_cierre_spg("../../",$ls_estciespg);
				...
				if($li_rowscg>=1)
				{
					$arrResultado=$io_funciones_cxp->uf_verificar_cierre_scg("../../",$ls_estciescg);
					$lb_valido=$arrResultado["lb_valido"];
