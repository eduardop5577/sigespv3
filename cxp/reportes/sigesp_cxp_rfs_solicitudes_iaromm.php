...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,$as_denominacion,$as_nomusu,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addText(15,690,8,"TIPO DE ORDEN DE PAGO: ".$as_denominacion); // Agregar el título
		$io_pdf->addText(400,690,8,"SUJETO A RENDICION DE CTA. SI ____  NO ____"); // Agregar el título
		// cuadro inferior
        $io_pdf->Rectangle(15,140,570,70);
		$io_pdf->line(15,153,585,153);		
		$io_pdf->line(15,197,585,197);		
		$io_pdf->line(125,140,125,210);		
		$io_pdf->line(240,140,240,210);		
		$io_pdf->line(355,140,355,210);		
		$io_pdf->line(470,140,470,210);		
		$io_pdf->addText(40,202,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(40,180,7,$as_nomusu); // Agregar el título
		$io_pdf->addText(42,143,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(150,202,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(152,143,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(265,202,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(273,143,7,"CONTABILIDAD"); // Agregar el título
		$io_pdf->addText(370,202,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(360,143,7,"ADMINISTRACION Y FINANZAS"); // Agregar el título
		$io_pdf->addText(505,202,7,"APROBADO"); // Agregar el título
		$io_pdf->addText(500,143,7,"PRESIDENCIA"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(7,"RECIBE CONFORME");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,123,7,"RECIBE CONFORME"); // Agregar el título
        $io_pdf->Rectangle(15,60,570,70);
		$io_pdf->line(15,130,585,130);		
		$io_pdf->line(15,120,585,120);		
		$io_pdf->line(15,70,585,70);		
		$io_pdf->line(158,60,158,120);		
		$io_pdf->line(299,60,299,120);		
		$io_pdf->line(442,60,442,120);		
		$io_pdf->addText(80,62,7,"NOMBRE"); // Agregar el título
		$io_pdf->addText(195,62,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(355,62,7,"CEDULA"); // Agregar el título
	...
	function uf_print_cabecera($as_numsol,$as_codigo,$as_nombre,$as_denfuefin,$ad_fecemisol,$as_consol,$as_obssol,
							   $ai_monsol,$as_monto,$as_rifpro,$as_procede,$as_compromiso,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data[2]=array('titulo'=>" C.I. / R.I.F.: ".$as_rifpro);
		$la_data[3]=array('titulo'=>"                ".$as_nombre);
		...
		$la_data[1]=array('titulo'=>'<b> Concepto:          </b>'.$as_consol.' Procedencia: '.$as_procede.' Compromiso: '.$as_compromiso);
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_fuente_fin($as_denfuefin,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_cabecera
		//		   Access: private 
		//	    Arguments: as_denfuefin // Denominacion de la fuente de financiamiento
		//	    		   io_pdf // Instancia de objeto pdf
		//    Description: función que imprime la fuente de financiamiento 
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creación: 17/05/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_pdf;
		
		$la_data[1]=array('titulo'=>'<b>FUENTE DE FINANCIAMIENTO: </b>'.$as_denfuefin);
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->ezSetCmMargins(3.6,8,3,3); // Configuración de los margenes en centímetros
			$io_pdf->ezStartPageNumbers(570,47,8,'','',1); // Insertar el número de página
			$li_totrow=$io_report->DS->getRowCount("numsol");
			$li_monretiva=0;
			$ls_nomusu=$io_report->uf_sep_select_usuario($_SESSION["la_logusr"]);
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if(($ls_rifpro=="")&&($ls_cedbene!="---------"))
				{
					$ls_rifpro=$ls_cedbene;
				}
				if($ls_estretiva=="B")
				{
...
//////////////////////////  GRID RECEPCIONES DE DOCUMENTOS		//////////////////////////////////////
				...
				if($lb_valido)
				{
					...
					for($li_s=1;$li_s<=$li_totrowdet;$li_s++)
					{
						...
						$ls_dentipdoc=$io_report->ds_detalle_rec->data["dentipdoc"][$li_s];
						$ls_procede_docspg=$io_report->ds_detalle_rec->data["procede_docspg"][$li_s];
						$ls_numdoccomspg=$io_report->ds_detalle_rec->data["numdoccomspg"][$li_s];
						...
						if ($ls_dentipdocant!=$ls_dentipdoc)
						{
							$ls_denominacion=$ls_denominacion." - ".$ls_dentipdoc;
							$ls_dentipdocant=$ls_dentipdoc;
						}						
						$ls_procede=$ls_procede." - ".$ls_procede_docspg;
						$ls_compromiso=$ls_compromiso." - ".$ls_numdoccomspg;
					}
					...
					uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecemisol,$ls_estprosol,$ls_denominacion,$ls_nomusu,$io_pdf);
					uf_print_cabecera($ls_numsol,$ls_codigo,$ls_nombre,$ls_denfuefin,$ld_fecemisol,$ls_consol,$ls_obssol,
									  $li_monsol,$ls_monto,$ls_rifpro,$ls_procede,$ls_compromiso,$io_pdf);
...
//////////////////////////         GRID DETALLE CONTABLE	    //////////////////////////////////////
					uf_print_fuente_fin($ls_denfuefin,$io_pdf);
				}
			}
		}
