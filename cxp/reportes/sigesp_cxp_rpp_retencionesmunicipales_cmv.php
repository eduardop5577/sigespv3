...
***********************************************************************************/
							   ...
							   $as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$io_pdf)
	{
		...
		unset($la_config);								 
		$io_pdf->ezSetY(490);
		$ia_ano=substr($as_perfiscal,0,4);
		$ia_mes=substr($as_perfiscal,4,2);
		$la_data[1]=array('name'=>'<b>PERÍODO FISCAL</b>');
		$la_data[2]=array('name'=>$ia_ano." / ".$ia_mes);				
		$la_columna=array('name'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Lieas
	...
	function uf_print_detalle($la_data,$ai_totbasimp,$ai_totmonimp,$as_rifagenteret,$io_pdf)
	{
		...
		$la_data[5]=array('firma1'=>'Firma y Sello del Agente de Retencion','firma2'=>'Firma y Sello del Proveedor');
		$la_data[6]=array('firma1'=>'RIF: G-20004957-0','firma2'=>'');
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			$io_pdf->ezSetCmMargins(7,2.5,3,3);
			$lb_valido=true;
