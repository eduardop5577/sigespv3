...
***********************************************************************************/
	...
	function uf_print_encabezadopagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/banner_espromedbio.jpg',30,700,570,50); // Agregar Logo
       ...
       // $io_pdf->Rectangle(150,540,800,40);	
		$io_pdf->addText(40,685,12,"<b>OFICINA DE ADMINISTRACION Y SERVICIOS ESPROMED BIO</b>"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_agenteret,$as_rifagenteret,$as_perfiscal,$as_codsujret,$as_nomsujret,$as_rif,$as_diragenteret,
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$ls_numfac,$li_coniva,$li_ivaret,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private 
		...
		$ls_mes=$io_fecha->uf_load_nombre_mes(substr($ad_fecrep,3,2));
//		$io_pdf->addText(450,665,8,"No. PEGA_".$as_numcon); 			
		$io_pdf->addText(380,665,10,"No. PEGA_".$as_perfiscal); 		
		$io_pdf->addText(370,650,10,"<b>Caracas, ".substr($ad_fecrep,0,2)." de ".$ls_mes." de ".substr($ad_fecrep,6,4)."</b>");			
		$io_pdf->addText(60,615,10,utf8_decode("Señores.")); 		
		$io_pdf->addText(60,600,10,"<b>".$as_nomsujret."</b>"); 		
		$io_pdf->addText(60,585,10,"Presente."); 			
		$io_pdf->addText(100,320,10,"Sin otro particular al que hacer referencia, se despide de usted."); 			
		$io_pdf->addText(280,270,10,"Atentamente,"); 			
		$io_pdf->addText(220,220,10,"<b> MARIA GIULIA VIZZARRI  VERRATTI</b>"); 			
		$io_pdf->addText(210,205,10,utf8_decode("<b>Gerente (E) de Administración y Servicios</b>")); 			
		$io_pdf->addText(160,190,10,utf8_decode("Según Providencia administrativa Nª 003/2014 de fecha 15/09/2014")); 			
		$io_pdf->addText(130,40,8,utf8_decode("Ciudad Universitaria UCV, Los Chaguaramos. Caracas - República Bolivariana de Venezuela Cod. 1041")); 			
		$io_pdf->addText(230,30,8,utf8_decode("Teléfono: (0058-0212) 219.1798 - Ext. 1710")); 			
		$io_pdf->ezSety(570);
		$ls_parr1=utf8_decode('     El   presente   tiene   como   finalidad   hacer   de  su   conocimiento   lo  concerniente   a  la   retención      de RESPONSABILIDAD   SOCIAL  establecido  según    el    artículo    No.   34   del    Reglamento  de  la  ley  de Contrataciones Públicas la cual establece:');
		$ls_parr2=utf8_decode('<b>  "El   Compromiso   de   Responsabilidad   Social  será  requerido  en todas las  ofertas presentadas en   las    modalidades   de  selección    de   Contratistas    previstas  en   la   Ley  de  Contrataciones   Públicas, así  como; en  los  procedimientos  excluidos  de  la aplicación  de éstas, cuyo monto total, incluidos los tributos, superen las dos mil quinientas unidades tributarias (2.500 U.T)."</b>');
		$ls_parr3=utf8_decode('     En  este  sentido  debido  a  que  la(s)  factura(s)  identificada(s)  con  el(los) número  '.trim($ls_numfac) .'  por un monto total  de  Bs.  '.$li_coniva.'. Y  dando  cumplimiento a  lo establecido  en  el art.  34  está  sujeto a retención, y por política de <b>'.$as_agenteret.'</b> y de acuerdo a lo establecido  en el  artículo 35  de dicho  reglamento se  estableció un 3%  de retención por  este concepto, el cual corresponde a Bs. '.$li_ivaret.'.');
		$la_data[1]=array('titulo'=>$ls_parr1);
		$la_data[2]=array('titulo'=>'         ');
		$la_data[3]=array('titulo'=>$ls_parr2);
		$la_data[4]=array('titulo'=>'         ');
		$la_data[5]=array('titulo'=>$ls_parr3);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>500,
						 'cols'=>array('titulo'=>array('justification'=>'justify','width'=>500))); // Ancho M�imo de la tabla
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					$lb_valido=$io_report->uf_retencionesaporte_detalle2($ls_numcom);
					if($lb_valido)
					{
						$li_totalconiva = 0;
						$li_totalsiniva = 0;
						...
						uf_print_cabecera($ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_codsujret,$ls_nomsujret,$ls_rif,
										  $ls_diragenteret,$ls_numcon,$ls_fecrep,$li_estcmpret,$ls_numfac,$li_coniva,$li_ivaret,$io_pdf);
