...
***********************************************************************************/
	...
	function uf_print_encabezadopagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->line(400,60,600,60);
		$io_pdf->addText(410,50,8,"FIRMA Y SELLO DEL AGENTE DE RETENCION"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_agenteret,$as_rifagenteret,$as_perfiscal,$as_codsujret,$as_nomsujret,$as_rif,$as_diragenteret,
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$as_telpro,$as_dirpro,$as_email,$as_emailagenteret,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private 
		...
		$la_data[3]=array('name'=>'<b>CORREO DEL AGENTE DE RETENCION</b>  ');
		$la_data[4]=array('name'=>$as_emailagenteret);				
		$la_columna=array('name'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,327,370,30);	
		$io_pdf->addText(405,345,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,330,9,$as_rif); // Agregar el titulo									 
		//---------------------------------------------------------------------------------------------------
		$io_pdf->ezSetDy(-4);
		$la_data[1]=array('name'=>'<b>DIRECCION FISCAL DEL SUJETO RETENIDO</b>  ');
		$la_data[2]=array('name'=>$as_dirpro);				
		$la_data[3]=array('name'=>'<b>CORREO DEL SUJETO RETENIDO</b>  '.$as_email.'   <b>TELEFONO DEL SUJETO RETENIDO</b>   '.$as_telpro);
		$la_columna=array('name'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
	...
	$ls_emailagenteret=$_SESSION["la_empresa"]["email"];
	//--------------------------------------------------------------------------------------------------------------------------------		
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					uf_print_cabecera($ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_codsujret,$ls_nomsujret,$ls_rif,
					                  $ls_diragenteret,$ls_numcon,$ls_fecrep,$li_estcmpret,$ls_telpro,$ls_dirpro,$ls_email,$ls_emailagenteret,$io_pdf);
