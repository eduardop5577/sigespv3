...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,$io_pdf)
	{
		...
		$tm=296-($li_tm/2);
		$io_pdf->addText(535,750,9,$as_numsol); // Agregar el título
		$io_pdf->addText(500,695,9,"Fecha  ".$ad_fecregsol); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
							   ...
							   $ai_monsol,$as_monto,$ls_rifpro,$io_pdf)
	{
		...
		$la_data[1]=array('titulo'=>'<b> Beneficiario</b>    '.$as_nombre."     <b> R.I.F.:</b> ".$ls_rifpro);
	...
	function uf_print_detalle_spg($aa_data,$ai_totpre,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('titulo'=>array('justification'=>'right','width'=>490), // Justificación y ancho de la columna
						 			   'totpre'=>array('justification'=>'right','width'=>80))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_datatot,$la_columnas,'',$la_config);
	}// end function uf_print_detalle
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_scg($aa_data,$ai_totdeb,$ai_tothab,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('sc_cuenta'=>array('justification'=>'center','width'=>120), // Justificación y ancho de la columna
	...
	function uf_print_observacion($as_obssol,$io_pdf)
	{
		...
		$io_pdf->ezSetDy(-2);
		$la_data[1]=array('titulo'=>'<b> Observacion:          </b>'.$as_obssol);
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
