...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprodoc,$as_nomusu,$as_nomusuapr,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addText(230,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(220,63,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->addText(185,75,7,$as_nomusuapr); // Agregar el título
		$io_pdf->addText(375,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(360,63,7,"ADMINISTRACIÓN Y FINANZAS"); // Agregar el título
		$io_pdf->addText(490,122,7,"CONTRALORIA INTERNA"); // Agregar el título
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$ls_codusuapr=$io_report->DS->data["usuaprord"][$li_i];
				$ls_nomusureg=$io_report->uf_sep_select_usuario($ls_codusureg);
				$ls_nomusuapr=$io_report->uf_sep_select_usuario($ls_codusuapr);
				
				if($ls_estretiva=="B")
				{
					$li_monretiva=$io_report->uf_select_det_deducciones($ls_numrecdoc,$ls_codpro,$ls_cedben,$ls_codtipdoc);
				}
				uf_print_encabezado_pagina($ls_titulo,$ls_numrecdoc,$ld_fecrecdoc,$ls_estprodoc,$ls_nomusureg,$ls_nomusuapr,$io_pdf);
				if($ls_codpro!="----------")
				{
					$ls_codigo=$ls_codpro;
