					...
					while(!$io_report->rs_solicitudes->EOF)
					{
						...
						if($lb_valido)
						{
							...
							while(!$io_report->rs_pagactuales->EOF)
							{
								...
								$li_montop= $io_report->rs_pagactuales->fields['monto']; //Monto de la Solicitudes de Pago actuales.
								$ld_fecmov= $io_report->rs_pagactuales->fields['fecmov'];
								$ls_estmov= $io_report->rs_pagactuales->fields['estmov'];
								if($ls_estretiva=="B")
								{
									$li_monretiva=$io_report->uf_select_det_deducciones_solpag($ls_numsol);
									$li_montop=$li_montop+$li_monretiva;
								}
								if ($ls_estmov=='O' || $ls_estmov=='C')
								{
									$li_salsol= $li_salsol-$li_montop;
									$li_totaldebe=$li_totaldebe+$li_montop;
									$li_tolalsaldodebe=$li_tolalsaldodebe+$li_montop;
									$ld_debe=number_format($li_montop,2,',','.');
									$ld_haber="0,00";
									$ls_anulado ="";
								}
								else 
								{
									$li_salsol= $li_salsol+$li_montop;
									$li_totalhaber=$li_totalhaber+$li_montop;
									$li_tolalsaldohaber=$li_tolalsaldohaber+$li_montop;
									$ld_debe="0,00";
									$ld_haber=number_format($li_montop,2,',','.');
									$ls_anulado =" Anulado";
								}
//						$ls_montoff=$ls_montoff+$li_monto;
//						print $ls_montoff."<br>";
								...
								$ls_monto= number_format($li_montop,2,',','.');
...
//			print $ls_codigo."  PREVIO->  ".$li_monsolpre."<br>";
			...
			$li_monpagpre=0;
			$arrResultado= $io_report->uf_select_pagosanteriores($ls_tipproben,$ls_codpro,$ls_cedbene,$ld_fecemides,$ld_fecemihas,
														   $li_monpagpre);
			$lb_valido=$arrResultado["lb_valido"];
			$limonpagpre=$arrResultado["ad_pagosprevios"];
			unset($arrResultado);
