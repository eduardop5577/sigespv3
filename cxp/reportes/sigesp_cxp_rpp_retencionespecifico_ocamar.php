...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_fecdes,$as_fechas,$io_pdf)
	{
		...
		$io_pdf->addText(900,550,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(906,543,7,date("h:i a")); // Agregar la Hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_codded,$as_dended,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('retencion'=>array('justification'=>'left','width'=>60),
						               'codigo'=>array('justification'=>'left','width'=>50), // Justificación y ancho de la columna
						 			   'denominacion'=>array('justification'=>'left','width'=>690))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);			
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('cedula_rif'=>'C.I. / R.I.F.','beneficiario'=>'<b>Proveedor/Beneficiario</b>','solicitud'=>'<b>Solicitud de Pago</b>','fecha'=>'<b>Fecha</b>','numcom'=>'<b>Comprobante','monto'=>'<b>Monto Objeto Retencion','porded'=>'<b>Alicuota de Retencion (%) </b>','retencion'=>'<b>Monto Retenido</b>'));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>530, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('cedula_rif'=>array('justification'=>'left','width'=>100),
						               'beneficiario'=>array('justification'=>'left','width'=>190),
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	 function uf_print_totales($ai_filas,$ai_total,$ai_totbase,$io_pdf)
	 {
	    ...
	    $la_data[1]=array('name'=>'_________________________________________________________________________________________________________________________________________________');
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('cedula_rif'=>'','cantidad'=>'<b>Total de Retenciones :</b>','filas'=>$ai_filas,'totales'=>'<b>Total Montos </b>','base'=>$ai_totbase,'vacio'=>'','monto'=>$ai_total);
		$la_data[2]=array('cedula_rif'=>'','cantidad'=>'','filas'=>'','totales'=>'','base'=>'','vacio'=>'','monto'=>'');
		$la_data[3]=array('cedula_rif'=>'','cantidad'=>'','filas'=>'','totales'=>'','base'=>'','vacio'=>'','monto'=>'');
	    $la_columna=array('cedula_rif'=>'','cantidad'=>'','filas'=>'','totales'=>'','base'=>'','vacio'=>'','monto'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'titleFontSize' =>8,  // Tamaño de Letras de los títulos
						 'showLines'=>0, // Mostrar Líneas
						 ...
						 'maxWidth'=>530, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('cedula_rif'=>array('justification'=>'right','width'=>100),
						               'cantidad'=>array('justification'=>'right','width'=>90),
	...
	else
	{
		...
		for($li_j=1;$li_j<=$li_rowcargos;$li_j++)
		{
			...
			for ($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$ld_fecemidoc= $io_funciones->uf_convertirfecmostrar($io_report->ds_detalle->data["fecemidoc"][$li_i]);
...
//				{print "ENTRO";
				//	$ls_numcom = iif(empty($ls_numcomiva),iif(empty($ls_numcommun),iif(empty($ls_numcomapo),'',$ls_numcomapo),$ls_numcommun),$ls_numcomiva);						
					$la_data[$li_proveedor]=array('cedula_rif'=>$ls_cedrif,'beneficiario'=>$ls_nombre,'solicitud'=>$ls_numsol,'fecha'=>$ld_fecemisol,'numcom'=>$ls_numcom,'monto'=>'',
										  'porded'=>'','retencion'=>'');
					$li_proveedor++;
					...
					while ((!$io_report->rs_detalle->EOF)&&($lb_valido))
					{
						...
						$ls_numsol="" ;
						$ld_fecemisol= $io_funciones->uf_convertirfecmostrar($io_report->rs_detalle->fields["fecemidoc"]);
						$ls_numcom="";
						...
						{
							$li_porded_det=number_format($li_porded_det,2,',','.');
						}					
						$la_data[$li_proveedor]=array('cedula_rif'=>'','beneficiario'=>$ls_nombre,'solicitud'=>$ls_numsol,'fecha'=>$ld_fecemisol,'numcom'=>$ls_numcom,'monto'=>$li_monsol_det,
											  'porded'=>$li_porded_det,'retencion'=>$li_monret_det);
						$li_proveedor++;
						$io_report->rs_detalle->MoveNext();
					}
					$la_data[$li_proveedor]=array('cedula_rif'=>'','beneficiario'=>'','solicitud'=>'','fecha'=>'','numcom'=>'','monto'=>'____________________','porded'=>'____________________','retencion'=>'____________________');
					$li_proveedor++;
					$la_data[$li_proveedor]=array('cedula_rif'=>'','beneficiario'=>'								<b>SUB TOTAL</b>','solicitud'=>'','fecha'=>'','numcom'=>'','monto'=>$li_monsol,
										  'porded'=>$li_porded,'retencion'=>$li_monret);
					$li_proveedor++;
					$la_data[$li_proveedor]=array('cedula_rif'=>'','beneficiario'=>'','solicitud'=>'','fecha'=>'','numcom'=>'','monto'=>'','porded'=>'','retencion'=>'');
					$li_proveedor++;
//				}
			}
			if($li_proveedor>0)
		...
		$li_total=count((array)$la_manuales);
