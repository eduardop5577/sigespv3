...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprodoc,$ls_nomusureg,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->line(104,60,104,130);
		$io_pdf->line(198,60,198,130);		
		$io_pdf->line(291,60,291,130);		
		$io_pdf->line(393,60,393,130);		
		$io_pdf->line(495,60,495,130);	
		$io_pdf->addText(30,122,7,"ELABORADO POR:"); // Agregar el título	
        $io_pdf->addText(19,63,7,$ls_nomusureg); // Agregar el título	
		$io_pdf->addText(112,122,7,"UNIDAD SOLICITANTE"); // Agregar el título
		$io_pdf->addText(207,122,7,"DIRECTOR GENERAL"); // Agregar el título
		$io_pdf->addText(224,63,7,"APROBADO"); // Agregar el título
		$io_pdf->addText(297,122,7,"PLANIF. Y PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(323,63,7,"APROBADO"); // Agregar el título
		$io_pdf->addText(410,122,7,"ADMIN. Y FINANZAS"); // Agregar el título
		$io_pdf->addText(425,63,7,"APROBADO"); // Agregar el título
		$io_pdf->addText(518,122,7,"PRESIDENTE"); // Agregar el título
		$io_pdf->addText(518,63,7,"AUTORIZADO"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				uf_print_encabezado_pagina($ls_titulo,$ls_numrecdoc,$ld_fecrecdoc,$ls_estprodoc,$ls_nomusureg,$io_pdf);
				if($ls_codpro!="----------")
				{
					$ls_codigo=$ls_codpro;
