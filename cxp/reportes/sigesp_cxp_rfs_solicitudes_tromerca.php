...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,$as_nomusu,$io_pdf)
	{
        ...
        $io_pdf->Rectangle(15,655,570,60);*/
		$io_pdf->addJpegFromFile('../../shared/imagebank/cintillo_ver_tromerca.jpg',15,735,570,40);
		$io_pdf->addJpegFromFile('../../shared/imagebank/barra_inf_ver_tromerca.jpg',15,15,570,30);
		//$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],30,710,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		/*if ($as_estprosol=="A")
		{
			$io_pdf->addText(470,720,10,"<b>ANULADO</b>"); // Agregar la Fecha
		}
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,685,11,$as_titulo); // Agregar el título
		$io_pdf->addText(485,695,9,"No. ".$as_numsol); // Agregar el título
		$io_pdf->addText(485,665,9,"Fecha ".$ad_fecregsol); // Agregar el título
		$io_pdf->addText(540,727,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(546,721,6,date("h:i a")); // Agregar la Hora*/
		// cuadro inferior
		...
		$io_pdf->addText(20,80,7,$as_nomusu); // Agregar el título
		$io_pdf->addText(42,63,7,"FIRMA Y SELLO"); // Agregar el título
		$io_pdf->line(130,60,130,130);	//VERTICAL	
		$io_pdf->addText(167,122,7,"REVISADO POR"); // Agregar el título
		$io_pdf->addText(169,80,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(169,63,7,"FIRMA Y SELLO"); // Agregar el título
		$io_pdf->line(260,60,260,130);	//VERTICAL		
		$io_pdf->addText(302,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(300,80,7,"ADMINISTRACIÓN"); // Agregar el título
		$io_pdf->addText(305,63,7,"FIRMA Y SELLO"); // Agregar el título
		$io_pdf->line(410,60,410,130);	//VERTICAL		
		$io_pdf->addText(468,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(465,80,7,"GERENCIA GENERAL"); // Agregar el título
		$io_pdf->addText(472,63,7,"FIRMA Y SELLO"); // Agregar el título
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_firmas($as_nomusu,$io_pdf)
	{
		global $io_pdf;
		...
		$io_pdf->addText(472,63,7,"FIRMA Y SELLO"); // Agregar el título
		$io_pdf->line(15,73,585,73); //HORIZONTAL	
	}//uf_print_firmas($as_nomusu,$io_pdf)
	//-----------------------------------------------------------------------------------------------------------------------------------
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if($cabecera==true){
					...
					$io_pdf->addText($tm,685,11,$ls_titulo); // Agregar el título
					$io_pdf->addText(485,695,9,"No. ".$ls_numsol); // Agregar el título
					$io_pdf->addText(485,665,9,"Fecha ".$ld_fecemisol); // Agregar el título
					$io_pdf->addText(540,727,7,date("d/m/Y")); // Agregar la Fecha
					$io_pdf->addText(546,721,6,date("h:i a")); // Agregar la Hora	
				}
				$cabecera=false;
...
//////////////////////////         GRID DETALLE CONTABLE	    //////////////////////////////////////
		...
		uf_print_firmas($ls_nomusu,$io_pdf);
