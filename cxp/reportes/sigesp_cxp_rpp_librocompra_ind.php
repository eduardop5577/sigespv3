...
***********************************************************************************/
	...
	function uf_print_cabecera($io_pdf)
	{
		...
		$la_data=array(array('name20'=>'<b>Nro Cheque</b>','name1'=>'<b>Nro Oper</b>',
							 ...
							 'name51'=>'<b>Tipo Prov.</b>','name6'=>'<b>Nro Planilla Importacin  (C-80 o c-81)</b>','name7'=>'<b>Solicitud de Pago</b>','name8'=>'<b>Nro de Factura</b>','name9'=>'<b>Nro de Control</b>','name10'=>'<b>Nro Nota Dbito</b>','name11'=>'<b>Nro Nota Crdito</b>','name12'=>'<b>Tipo de Transacc.</b>','name13'=>'<b>Nro de Factura Afectada</b>','name14'=>'<b>Total de Compra Incluyendo IVA</b>','name15'=>'<b>Compra sin Derecho a Crdito IVA</b>','name16'=>'<b>Base Imponible</b>','name17'=>'<b>%</b>','name18'=>'<b>Impuesto Iva</b>','name19'=>'<b>IVA Retenido (Vendedor)</b>'));
		...
		$la_config=array('showHeadings'=>0,     // Mostrar encabezados
						 ...
						 'cols'=>array('name20'=>array('justification'=>'center','width'=>50),
									   ...
									   'name51'=>array('justification'=>'center','width'=>20), // Justificacin y ancho de la columna Nro de Comprobante.
						 			   ...
						 			   'name7'=>array('justification'=>'center','width'=>50), // Justificacin y ancho de la columna Nro Expediente de Importacin.
	...
	if($lb_valido)
	{
		...
		while($row=$io_report->io_sql->fetch_row($rs_resultado))	
		{
			...
			$ls_numsol=$io_report->uf_select_data($io_sql,"SELECT distinct cxp_dt_solicitudes.numsol AS numsol".
														  "  FROM cxp_dt_solicitudes".
														  " WHERE cxp_dt_solicitudes.codemp='".$ls_codemp."'".
														  "   AND cxp_dt_solicitudes.numrecdoc='".$ls_numrecdoc."'".
														  "   AND cxp_dt_solicitudes.cod_pro='".$ls_codpro."'".
														  "   AND cxp_dt_solicitudes.ced_bene='".$ls_cedben."'","numsol");
	if($ls_tiporeporte==1)
	{
	...
	else
	{
			...
			$la_cmpret=$io_report->uf_select_rowdata($io_sql,"SELECT cxp_rd_deducciones.numrecdoc as numrecdoc, cxp_rd_deducciones.monobjret as monobjret, SUM(cxp_rd_deducciones.monret) as monret, cxp_rd_deducciones.porded as porded, scb_dt_cmp_ret.codret as codret, ".
                                                             "       scb_dt_cmp_ret.numcom as numcom, scb_dt_cmp_ret.iva_ret as iva_ret, scb_dt_cmp_ret.tiptrans as tiptrans,cxp_dt_solicitudes.numsol ".
			...
			$la_data[$li]=array('name20'=>$ls_cheque,'name1'=>$li,
							 ...
							 'name4'=>$ls_nombre,'name5'=>$ls_cmpret,'name51'=>$ls_tipproben,'name6'=>'', 'name7'=>$ls_numsol,
