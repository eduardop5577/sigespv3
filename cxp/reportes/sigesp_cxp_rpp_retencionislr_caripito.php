...
***********************************************************************************/
	...
	function uf_print_encabezado($as_agente,$as_nombre,$as_rif,$as_nit,$as_direccion,$io_pdf)
	{
		...
		$la_data[1]=array('name'=>'<b><i>Agente de Retención:</i></b>'."  ".$as_agente."                      <b><i>R.I.F.:</i></b> ".$ls_rifagente);
	...
	function uf_print_detalle($as_numsol,$as_concepto,$as_fechapago,$ad_monto,$ad_monret,$ad_porcentaje,$as_numcon,$as_codded,$as_desserded,$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('fecha'=>'<b>Fecha de la Factura</b>','codded'=>'<b>Codigo de Retencion</b>','monto'=>'<b>Monto Objeto de Retención</b>','desserded'=>'<b>Descripcion del Servicio</b>',
						  'porcentaje'=>'<b>% Aplicado</b>','retenido'=>'<b>Total Impuesto Retenido</b>');	
		$la_columna=array('fecha'=>'','codded'=>'','monto'=>'','desserded'=>'','porcentaje'=>'','retenido'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
					     'fontSize' => 10, // Tamaño de Letras
					     ...
					     'colGap'=>1,
					     'width'=>500,
					     'cols'=>array('fecha'=>array('justification'=>'center','width'=>65),
									   'codded'=>array('justification'=>'center','width'=>70),
									   'monto'=>array('justification'=>'center','width'=>90),
									   'desserded'=>array('justification'=>'center','width'=>150),
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('fecha'=>$as_fechapago,'codded'=>$as_codded,'monto'=>$ad_monto,'desserded'=>$as_desserded,'porcentaje'=>$ad_porcentaje,'retenido'=>$ad_monret);	
		$la_columna=array('fecha'=>'','codded'=>'','monto'=>'','desserded'=>'','porcentaje'=>'','retenido'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
					      'fontSize' => 10, // Tamaño de Letras
					      'showLines'=>2, // Mostrar Líneas
					     ...
					     'cols'=>array('fecha'=>array('justification'=>'center','width'=>65),
									   ...
									   'desserded'=>array('justification'=>'left','width'=>150),
									   'porcentaje'=>array('justification'=>'center','width'=>80),
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					$total=$io_report->DS->getRowCount("numcom");
					$ls_numcomant="";
					for($i=1;($i<=$total);$i++)
					{
						$li_pos=$li_pos+1;
						...
						if($ls_numcom!=$ls_numcomant)
						{
							...
							$io_encabezado=$io_pdf->openObject(); // Creamos el objeto cabecera
				...
				else
				{
					...
					if($lb_valido)
					{
						$total=$io_report->DS_ISLR->getRowCount("numcom");
						$ls_numcomant="";
						for($i=1;($i<=$total);$i++)
						{
							$li_pos=$li_pos+1;
							$ls_codigo= $io_report->DS_ISLR->data["codsujret"][$li_pos];
							$ls_nombre= $io_report->DS_ISLR->data["nomsujret"][$li_pos];
							$ls_rif= $io_report->DS_ISLR->data["rif"][$li_pos];
							$ls_nit= $io_report->DS_ISLR->data["nit"][$li_pos];
							$ls_dirsujret= $io_report->DS_ISLR->data["dirsujret"][$li_pos];
							$ls_numcom= $io_report->DS_ISLR->data["numcom"][$li_pos];
							$ls_perfiscal= $io_report->DS_ISLR->data["perfiscal"][$li_pos];
							$ls_fecrep  = $io_funciones->uf_convertirfecmostrar($io_report->DS_ISLR->data["fecfac"][$li_pos]);
							if($ls_numcom!=$ls_numcomant)
							{
								if($i>1)
								{
