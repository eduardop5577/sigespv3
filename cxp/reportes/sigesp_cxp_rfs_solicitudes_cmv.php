...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_titulo2,$as_titulo3,$as_numsol,$ad_fecregsol,$as_estprosol,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->line(120,700,120,770);
        $io_pdf->Rectangle(15,700,570,70);
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],30,710,70,50); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/cmsp2.jpg',500,710,60,50); // Agregar Logo
		if ($as_estprosol=="A")
		{
		...
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,755,11,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo2);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,740,11,$as_titulo2); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo3);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,725,11,$as_titulo3); // Agregar el título
		$as_numorden="<b>Nro.: ".$as_numsol."</b>";
		$li_tm=$io_pdf->getTextWidth(11,$as_numorden);
		$tm=296-($li_tm/2)+10;
		$io_pdf->addText($tm,710,9,"<b>Nro. ".$as_numsol."</b>"); // Agregar el título
		$io_pdf->addText(20,690,8,"<b>Clase OP:  </b>"."PAGOS RELACIONADOS GASTOS GENERALES - DIA A DIA"); // Agregar el título
		$io_pdf->addText(485,690,8,"<b>Fecha OP:  </b>".$ad_fecregsol); // Agregar el título
		//$io_pdf->addText(540,770,7,date("d/m/Y")); // Agregar la Fecha
		//$io_pdf->addText(546,764,6,date("h:i a")); // Agregar la Hora
		// cuadro inferior
        $io_pdf->Rectangle(15,60,570,40);  
		$io_pdf->Rectangle(15,100,570,15);
		$io_pdf->Rectangle(15,120,570,60);
		$io_pdf->Rectangle(15,180,570,15);
		$io_pdf->line(15,165,585,165);			
		$io_pdf->line(150,120,150,180);		
		$io_pdf->line(295,120,295,180);		
		$io_pdf->line(435,120,435,180);	
		$li_tm=$io_pdf->getTextWidth(8,"<b>Firmas Autorizadas</b>");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,185,8,"<b>Firmas Autorizadas</b>"); // Agregar el título		
		$io_pdf->addText(65,170,7,"Elaborado"); // Agregar el título
		$io_pdf->addText(40,125,7,"LCDA. NAIROVIS JORDAN"); // Agregar el título
		$io_pdf->addText(210,170,7,"Revisado"); // Agregar el título
		$io_pdf->addText(170,135,7,"LCDA. MARIANGEL CARRERO"); // Agregar el título
		$io_pdf->addText(180,125,7,"Administrador Coordinador"); // Agregar el título
		$io_pdf->addText(345,170,7,"Confirmado"); // Agregar el título
		$io_pdf->addText(325,135,7,"LCDO. TONY ARISMENDI"); // Agregar el título
		$io_pdf->addText(310,125,7,"Director de Administración y Servicio"); // Agregar el título
		$io_pdf->addText(495,170,7,"Aprobado"); // Agregar el título
		$io_pdf->addText(455,135,7,"LCDO. MANUEL SOTELO TORRES"); // Agregar el título
		$io_pdf->addText(440,125,7,"Contralor Municipal (I) del Municipio Valencia"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(8,"<b>Beneficiario</b>");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,105,8,"<b>Beneficiario</b>"); // Agregar el título 
		$io_pdf->addText(25,75,7,"RECIBI CONFORME:          _________________________          _________________________          _________________________          _________________________"); // Agregar el título
		$io_pdf->addText(25,65,7,"                                                        Nombre y Apellido                            Cédula de Identidad                                         Firma                                                Fecha         ");
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_numsol,$as_codigo,$as_nombre,$as_denfuefin,$ad_fecemisol,$as_consol,$as_obssol,
							   $ai_monsol,$as_monto,$as_dirproben,$as_rifpro,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezSetDy(-3);
		$la_data[1]=array('titulo'=>'<b>Orden de Pago a Favor de:</b>');
		...
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('rif'=>'Nro. RIF - CI','nombene'=>'Nombre del Benficiario','monto'=>'Monto de la Orden');
		$la_columnas=array('rif'=>'','nombene'=>'','monto'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('rif'=>array('justification'=>'left','width'=>150),
						               'nombene'=>array('justification'=>'left','width'=>270),
									   'monto'=>array('justification'=>'left','width'=>150))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('rif'=>$as_rifpro,'nombene'=>$as_nombre,'monto'=>$ai_monsol);
		$la_columnas=array('rif'=>'','nombene'=>'','monto'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 7, // Tamaño de Letras
		...
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('name'=>'Dirección del Beneficiario');
		$la_data[2]=array('name'=>$as_dirproben);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('name'=>array('justification'=>'left','width'=>570))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('titulo'=>'<b>Concepto</b>');
		//$la_data[2]=array('titulo'=>"                            ".$as_nombre);
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
	...
	$ls_titulo="<b>REPÚBLICA BOLIVARIANA DE VENEZUELA</b>";
	$ls_titulo2="<b>CONTRALORIA MUNICIPAL DE VALENCIA</b>";
	$ls_titulo3="<b>ORDEN DE PAGO</b>";
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->ezSetCmMargins(3.6,7.1,3,3); // Configuración de los margenes en centímetros
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				uf_print_encabezado_pagina($ls_titulo,$ls_titulo2,$ls_titulo3,$ls_numsol,$ld_fecemisol,$ls_estprosol,$io_pdf);
				uf_print_cabecera($ls_numsol,$ls_codigo,$ls_nombre,$ls_denfuefin,$ld_fecemisol,$ls_consol,$ls_obssol,$li_monsol,$ls_monto,$ls_dirproben,$ls_rifpro,$io_pdf);
...
//////////////////////////   GRID DETALLE PRESUPUESTARIO		//////////////////////////////////////
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowspg;$li_s++)
							{
								...
								$li_len5=0;
								$ls_titulo="";
								$arrResultado=$io_fun_cxp->uf_loadmodalidad($li_len1,$li_len2,$li_len3,$li_len4,$li_len5,$ls_titulo);
