...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addText(30,535,12,"<b>COMPROBANTE DE RETENCION del I.S.L.R</b>"); // Agregar la Fecha
		$io_pdf->addtext(695,558,10,"Fecha de Entrega");
		$io_pdf->addText(710,548,9,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addtext(695,536,9,"Página");
		$io_pdf->ezStartPageNumbers(750,536,9,'','',1); // Insertar el número de página
		$io_pdf->addText(33,515,9,"En cumplimiento a lo dispuesto en el Artículo 24 del Reglamento Parcial de la Ley de Impuesto sobre la Renta en Materia");
		$io_pdf->addText(33,505,9,"de Retenciones, Decreto 1.808, publicado en la Gaceta Oficial N° 36.203 del 12/05/1997");
		$io_pdf->line(35,530,767,530); //LINEA HORIZONTAL
		$io_pdf->addtext(90,85,10,"Glaidymar M. Cabrera A.");
		$io_pdf->addtext(50,75,10,"<b> JEFE DE LA DIVISION DE PAGOS </b>");
		$io_pdf->addtext(40,65,10,"Elaborado en medios electrónicos por el FONDEN, S.A. ");
		$io_pdf->restoreState();
		$io_pdf->closeObject();
							   ...
							   $as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$ls_nit,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>310,
						 'cols'=>array('name'=>array('justification'=>'left','width'=>370))); // Ancho Minimo de la tabla
        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);								 
...
//		$io_pdf->ezSetY(365);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('fecfac'=>array('justification'=>'center','width'=>80), // Justificacion y ancho de la columna
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('fecfac'=>array('justification'=>'center','width'=>80), // Justificacion y ancho de la columna
						 			   'numfac'=>array('justification'=>'center','width'=>100), // Justificacion y ancho de la columna
									   'numref'=>array('justification'=>'center','width'=>100),
						 			   'montotdoc'=>array('justification'=>'right','width'=>100),
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>200,
						 'cols'=>array('name1'=>array('justification'=>'right','width'=>100), // Justificacion y ancho de la columna
				...
				if($lb_valido)
				{
					...
					for($li_i=1;($li_i<=$li_total);$li_i++)
					{
						...
						$la_data[$li_i]=array('codtipdoc'=>$ls_codtipdoc,'dentipdoc'=>$ls_dentipdoc, 'numfac'=>$ls_numdoc,'dencondoc'=>$ls_dencondoc,
											  'fecfac'=>$ld_fecemidoc,'numref'=>$ls_numref,'montotdoc'=>$li_montotdoc,'dended'=>$ls_dended,'basimp'=>$li_monobjret,
