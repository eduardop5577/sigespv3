...
***********************************************************************************/
	...
	function uf_print_detalle_recepcion($li_totrows,$lo_libro,$lo_hoja,$la_data,$totcmp_con_iva,$totimp,$iva_ret,$li_fila)
	{
		...
		$lo_hoja->write($li_fila, 8, 'DESCRIPCION DEL PAGO',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		$lo_hoja->write($li_fila, 9, 'RIF',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		$lo_hoja->write($li_fila, 10, 'BASE IMPONIBLE',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		$lo_hoja->write($li_fila, 11, 'MONTO DEL IMPUESTO',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		$lo_hoja->write($li_fila, 12, 'IVA RETENIDO',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		$lo_hoja->write($li_fila, 13, 'MUNICIPIO',$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'center','size'=>'9')));
		for($i=1;$i<=$li_totrows;$i++)
		{
			...
			$lo_hoja->write($li_fila, 8, " ".$la_data[$i]['consol'],$lo_libro->addformat(array('bold'=>0,'font'=>'Verdana','align'=>'left','size'=>'8')));
			$lo_hoja->write($li_fila, 9, " ".$la_data[$i]['rif'],$lo_libro->addformat(array('bold'=>0,'font'=>'Verdana','align'=>'left','size'=>'8')));
			$lo_hoja->write($li_fila, 10, $la_data[$i]['totcmp_con_iva'],$lo_dataright);
			$lo_hoja->write($li_fila, 11, $la_data[$i]['totimp'],$lo_dataright);
			$lo_hoja->write($li_fila, 12, $la_data[$i]['iva_ret'],$lo_dataright);
			$lo_hoja->write($li_fila, 13, "SUCRE ",$lo_libro->addformat(array('bold'=>0,'font'=>'Verdana','align'=>'left','size'=>'8')));
		}
		$li_fila++;
		$lo_hoja->write($li_fila, 9, " TOTALES....",$lo_libro->addformat(array('bold'=>0,'font'=>'Verdana','align'=>'left','size'=>'8')));
		$lo_hoja->write($li_fila, 10, $totcmp_con_iva,$lo_dataright);
		$lo_hoja->write($li_fila, 11, $totimp,$lo_dataright);
		$lo_hoja->write($li_fila, 12, $iva_ret,$lo_dataright);
...
//				$li_iva_ret= number_format($li_iva_ret,2,'.','');
				...
				$la_data[$li_i]=array('numsop'=>$ls_numsop,'numdocpag'=>$ls_numdocpag,'vacio1'=>"",'numcom'=>$ls_numcom,'nomban'=>$ls_nomban,
									  'fecmov'=>$ld_fecmov,'vacio2'=>"",'nomsujret'=>$ls_nomsujret,'consol'=>$ls_consol,'rif'=>$ls_rif,
									  'totcmp_con_iva'=>$li_basimp,'totimp'=>$li_totimp,'iva_ret'=>$li_iva_ret,'vacio3'=>"");
			}
