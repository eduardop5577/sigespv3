...
***********************************************************************************/
	...
	function uf_print_cabecera($as_agenteret,$as_rifagenteret,$as_perfiscal,$as_codsujret,$as_nomsujret,$as_rif,$as_diragenteret,
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$as_telemp,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private 
		...
		unset($la_columna);
		unset($la_config);	
		$io_pdf->addText(50,520,9,"<b>(Ley IVA - Art.11: </b>"); // Agregar el titulo
        $io_pdf->addText(120,520,8,"<b> Seran Responsables del pago del impuesto en calidad de agentes de </b>"); // Agregar el titulo	
		$io_pdf->addText(50,500,8,"<b>retencion de los compradores o adquirientes de determinados bienes muebles y los receptores</b>"); // Agregar el titulo	
		$io_pdf->addText(50,480,8,"<b>de ciertos servicios, A quienes la Administracion Tributaria designe como tal.)</b>"); // Agregar el titulo		
		...
		$io_pdf->Rectangle(845,395,100,28);	
		$io_pdf->addText(850,412,9,"<b>TELEFONO</b>"); // Agregar el titulo	
		$io_pdf->addText(850,399,9,$as_telemp); // Agregar el titulo				
		...
		// cuadro inferior
        $io_pdf->Rectangle(50,60,890,70);	
		$io_pdf->line(330,60,330,130);	
		$io_pdf->line(660,60,660,130);	
		$io_pdf->addText(60,118,9,"APROBADO POR:"); // Agregar el título	
		$io_pdf->addText(340,118,9,"FIRMA Y SELLO DE LA UNIDAD"); // Agregar el título	
		$io_pdf->addText(450,65,9,"EGDAGLIMAR DIAZ"); // Agregar el título
		$io_pdf->addText(670,118,9,"RECIBIDO POR:"); // Agregar el título		
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
	...
	if($lb_valido)
	{
		...
		{ 
			...
			$io_pdf->ezSetCmMargins(3.5,5,3,3);
			$lb_valido=true;
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					for($li_i=1;$li_i<=$li_total;$li_i++)
					{
						...
						$ls_telemp=$io_report->DS->data["telemp"][$li_i];
					}											
					uf_print_cabecera($ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_codsujret,$ls_nomsujret,$ls_rif,
					                  $ls_diragenteret,$ls_numcon,$ls_fecrep,$li_estcmpret,$ls_telemp,$io_pdf);
