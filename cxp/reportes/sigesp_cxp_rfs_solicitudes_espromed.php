...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,$io_pdf)
	{
		...
		$io_pdf->line(480,685,480,750);
		$io_pdf->line(480,717,586,717);
        $io_pdf->Rectangle(16,685,570,65);

		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],20,687,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
//		$io_pdf->addJpegFromFile('../../shared/imagebank/logo.jpg',25,740,550,50); // Agregar Logo
		if ($as_estprosol=="A")
		{
			$io_pdf->addText(470,765,10,"<b>ANULADO</b>"); // Agregar la Fecha
		}
		$io_pdf->addText(220,725,11,$as_titulo); // Agregar el título
		$io_pdf->addText(136,712,6,$ls_nomemp); // Agregar el título
                $io_pdf->addText(140,705,6,"CALLE UNIVERSIDAD CENTRAL DE VENEZUELA EDIF ESPROMED BIO PISO 1,2,3" ); // Agregar el título
               $io_pdf->addText(127,698,6,"OF PRESIDENCIA URB LAS ACACIAS CARACAS DISTRITO CAPITAL ZONA POSTAL 1040"); // Agregar el título
                $io_pdf->addText(150,690,6,"Telefonos: 0212-219-17-73  Coord. Compras y Servicios RIF: G-20011254-9"); // Agregar el título
                $io_pdf->addText(20,400,6,""); // Agregar el título
		$io_pdf->addText(485,722,9,"No. ".$as_numsol); // Agregar el título
		$io_pdf->addText(485,690,9,"Fecha ".$ad_fecregsol); // Agregar el título
		// cuadro inferior
		//$io_pdf->setColor(0,0,0);
		$io_pdf->Rectangle(15,40,572,80);
		$io_pdf->line(15,107,587,107);		
		$io_pdf->line(15,94,587,94);		
		$io_pdf->addText(50,110,7,"ELABORADO"); 
		$io_pdf->addText(35,96,7,"FIRMA Y SELLO");
		$io_pdf->line(135,40,135,120);		
		$io_pdf->addText(137,110,7,"COORDINADOR DE TESORERIA");
		$io_pdf->addText(165,96,7,"FIRMA Y SELLO");
		$io_pdf->line(260,40,260,120);		
		$io_pdf->addText(270,110,7,"GTE GENERAL DE ADMINISTRACION Y SERVICIOS");
		$io_pdf->addText(315,96,7,"FIRMA Y SELLO");
		$io_pdf->line(450,40,450,120);		
		$io_pdf->addText(495,110,7,"PRESIDENCIA");
		$io_pdf->addText(495,96,7,"FIRMA Y SELLO");
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_detalle_recepcion($la_data,$ai_totsubtot,$ai_tottot,$ai_totcar,$ai_totded,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('compromiso'=>array('justification'=>'center','width'=>142), // Justificación y ancho de la columna
	...
	function uf_print_detalle_spg($aa_data,$ai_totpre,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('titulo'=>array('justification'=>'center','width'=>572))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_datatit,$la_columnas,'',$la_config);
		unset($la_datatit);
		unset($la_columnas);
		unset($la_config);
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>152), // Justificación y ancho de la columna
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('titulo'=>array('justification'=>'right','width'=>482), // Justificación y ancho de la columna
	...
	function uf_print_detalle_scg($aa_data,$ai_totdeb,$ai_tothab,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('sc_cuenta'=>array('justification'=>'center','width'=>142), // Justificación y ancho de la columna
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('titulo'=>array('justification'=>'right','width'=>392), // Justificación y ancho de la columna
	...
	function uf_print_observacion($as_obssol,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('titulo'=>array('justification'=>'left','width'=>572))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->ezSetCmMargins(4,4.5,3,3); // Configuración de los margenes en centímetros
