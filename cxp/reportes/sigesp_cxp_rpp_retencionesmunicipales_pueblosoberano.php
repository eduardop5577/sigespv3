...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$io_pdf)
	{
		...
		$tm=396-($li_tm/2);
		$io_pdf->addText(30,515,12,$as_titulo); // Agregar el título
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
		$ls_titulo="<b>COMPROBANTE DE RETENCION DE IMPUESTO MUNICIPAL </b>";
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					uf_print_cabecera($ls_numcon,$ls_fecrep,$ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_licagenteret,
									  $ls_diragenteret,$ls_nomsujret,$ls_rif,$ls_numlic,$li_estcmpret,$ls_nit,$io_pdf);
					$lb_valido=$io_report->uf_retencionesmunicipales_detalles($ls_numcom);
					$arrResultado=$io_report->uf_retencionesmunicipales_detfact($ls_numcom);
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$la_data[$li_i]=array('codtipdoc'=>$ls_codtipdoc,'dentipdoc'=>$ls_dentipdoc, 'numfac'=>$ls_numfac,'dencondoc'=>$ls_dencondoc,
												  'fecfac'=>$ld_fecfac,'numref'=>$ls_numref,'montotdoc'=>$li_montotdoc);
							$la_data2[$li_i]=array('dended'=>$ls_dended,'basimp'=>$li_baseimp, 'porimp'=>$li_porimp,'iva_ret'=>$li_totimp);
						  }																		 																						  
  						  $li_totalbaseimp= number_format($li_totalbaseimp,2,",","."); 
  						  $li_totalmontoimp= number_format($li_totalmontoimp,2,",","."); 
						  uf_print_detalle($la_data,$la_data2,$li_totalbaseimp,$li_totalmontoimp,$ls_rifagenteret,$io_pdf); 						 						  
						  unset($la_data);							 
					}
				}
