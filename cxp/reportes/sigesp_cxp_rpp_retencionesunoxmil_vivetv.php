...
***********************************************************************************/
							   ...
							   $as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$io_pdf)
	{
		...
		$io_pdf->Rectangle(400,417,15,10);
		$io_pdf->addText(420,418,9,"<b> Persona Natural </b>");
		$io_pdf->Rectangle(530,417,15,10);
		$io_pdf->addText(550,418,9,"<b> Persona Juridica </b>");
		if (substr($as_rif,0,1)=='J')
		{
			$io_pdf->addText(535,418,9,"<b>X</b>");
		}
		else
		{
			$io_pdf->addText(405,418,9,"<b>X</b>");
		}
		...
		$la_data[1]=array('agen_ret'=>'    '.$as_agenteret);				
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'colGap'=>1,					 
						 'maxWidth'=>690,
						 'cols'=>array('agen_ret'=>array('justification'=>'left','width'=>690)));
        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		$io_pdf->addText(635,466,9,date("d/m/Y")); // Agregar la Fecha	
		$la_data[1]=array('agen_ret'=>$as_rifagenteret,
		                  'ubic'=>'ARTICULO 11 PROVIDENCIA SAT-DC N° 001 SOBRE ORDENES DE PAGO A CONTRATISTAS',
