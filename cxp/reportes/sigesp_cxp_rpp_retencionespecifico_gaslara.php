		...
		for($li_j=1;$li_j<=$li_rowcargos;$li_j++)
		{
			...
			for ($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$ls_nombre= $io_report->ds_detalle->data["nombre"][$li_i];
                                $ls_cedula_rif= $io_report->ds_detalle->data["cedula_rif"][$li_i];
...
//				{print "ENTRO";
				//	$ls_numcom = iif(empty($ls_numcomiva),iif(empty($ls_numcommun),iif(empty($ls_numcomapo),'',$ls_numcomapo),$ls_numcommun),$ls_numcomiva);						
					$la_data[$li_proveedor]=array('beneficiario'=>$ls_cedula_rif." ".$ls_nombre,'solicitud'=>$ls_numsol,'fecha'=>$ld_fecemisol,'numcom'=>$ls_numcom,'monto'=>'',
										  'porded'=>'','retencion'=>'');
					$li_proveedor++;
