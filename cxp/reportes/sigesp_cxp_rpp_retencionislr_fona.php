			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					for($li_i=1;($li_i<=$li_total);$li_i++)
					{
						...
						if(($li_i>1)&&($ls_numdoc!=$io_report->DS->data["numdoc"][$li_i-1]))
							$li_montotal=$li_montotal+$li_montotdoc;
						else
							$li_montotal=$li_montotdoc;
						$li_totobjret=$li_totobjret+$li_monobjret;
						$li_totivaret=$li_totivaret+$li_retenido;
					...
					$li_montotal  = number_format($li_montotal,2,',','.');  
					$li_totobjret  = number_format($li_totobjret,2,',','.');    
					$li_totivaret   = number_format($li_totivaret,2,',','.');  
					$la_datatot[1]=array('numfac'=>"<b>TOTALES BS F.</b>",'totalconiva'=>$li_montotal,'compsinderiva'=>$li_totdersiniva,
									  'baseimp'=>$li_totobjret,'porimp'=>"",'ivaret'=>$li_totivaret);														
