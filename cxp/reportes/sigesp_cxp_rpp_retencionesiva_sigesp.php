...
***********************************************************************************/
	...
	function uf_print_encabezadopagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],47,485,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->addText(910,540,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(916,535,7,date("h:i a")); // Agregar la Hora
		$io_pdf->setStrokeColor(0,0,0);
        $io_pdf->Rectangle(150,485,800,40);	
		$io_pdf->addText(240,500,13,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->addText(220,585,15,"SISTEMAS INTEGRADOS DE GESTION PARA ENTES DEL SECTOR PUBLICO, C.A."); // Agregar el t�ulo				
		$io_pdf->addText(255,565,9,"Ley IVA - Art. 11 Seran responsables del pago del Impuesto en calidad de agente de retencion, los compradores o adquirientes");
		$io_pdf->addText(255,555,9," de determinados bienes muebles y los receptores de ciertos servicios, a quienes la Administracion Tributaria designe como tal. ");
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(845,428,100,28);	
		$io_pdf->addText(850,445,9,"<b>FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,430,9,$ad_fecrep); // Agregar el titulo								 
		$io_pdf->Rectangle(845,384,100,28);	
		$io_pdf->addText(850,403,9,"<b>PERIODO FISCAL</b>"); // Agregar el titulo	
		$io_pdf->addText(850,388,9,$as_perfiscal); // Agregar el titulo									 
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(400,384,370,30);	
		$io_pdf->addText(405,403,9,"<b>REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,388,9,$as_rifagenteret); // Agregar el t�ulo									 
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,300,370,30);	
		$io_pdf->addText(405,320,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,305,9,$as_rif); // Agregar el titulo									 
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			$io_pdf->ezSetCmMargins(5.5,3,3,3);
			$lb_valido=true;
