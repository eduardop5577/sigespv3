...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addText(215,755,9,"<b>República Bolivariana de Venezuela</b>"); // Agregar el título
		$io_pdf->addText(223,745,9,"<b>Gobierno del Distrito Capital</b>"); // Agregar el título
		$io_pdf->addText(170,735,9,"<b>Servicio de Administración Tributaria del Distrito Capital</b>"); // Agregar el título
		
		$io_pdf->addText(180,680,9,"<b>COMPROBANTE DE RETENCION DEL IMPUESTO</b>"); // Agregar el título
							   ...
							   $as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$as_diragenteret,$as_telagenteret,$io_pdf)
	{
		...
		$as_tipper="X";
		$io_pdf->addText(380,700,8,"<b>N° CORRELATIVO</b>".$as_numcon); 
		
		$io_pdf->addText(40,650,8,"<b>Agente de Retención:        ______________________________________________________________________</b>");
...
*/
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($la_data,$ai_totbasimp,$ai_totmonimp,$ai_totmoniva,$as_rifagenteret,$as_numsop,$as_desope,$as_totconiva,
							$ad_fecfac,$ls_numfac,$ls_numref,$ls_rifagenteret,$ls_procede,$ls_fecrep,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private 
		...
		$io_pdf->addText(40,520,8,"<b>No. Factura____________________     No. de Control_____________________      Fecha de Factura: _______________</b>");
		$io_pdf->addText(100,521,8,$ls_numfac);
		$io_pdf->addText(260,521,8,$ls_numref);
		$io_pdf->addText(420,521,8,$ad_fecfac);
		$io_pdf->addText(40,500,8,"<b>Persona Natural:  _________    Persona Jurídica:  _________  Cédula de identidad o R.I.F. Nº __________________</b>");
		$io_pdf->addText(400,501,8,$ls_rifagenteret);
		$io_pdf->addText(40,480,8,"<b>Por concepto de: Prestación de servicio ______  Adquisición de bienes o suministros ______ Ejecución de Obras ______</b>");
		switch($ls_procede)
		{
			case "SOCCOC":
				$io_pdf->addText(380,591,8,"X");
			break;
			case "SOCCOS":
				$io_pdf->addText(207,591,8,"X");
			break;
		}

		$io_pdf->addText(220,401,8,$ai_totbasimp);
		$io_pdf->addText(415,401,8,$as_totconiva);
		$io_pdf->addText(150,361,8,$ai_totmoniva);
		$io_pdf->addText(350,361,8,$ls_fecrep);
		$io_pdf->ezSetDy(-230);
		...
		$io_pdf->addText(40,330,8,"<b>Llenar Solo en Casos de Pagos Efectuados Directamente en las Cuentas Receptoras de Fondos del Distrito Capital</b>");
	
		$io_pdf->addText(40,300,8,"<b>Banco: ___________________________________________________  Nº de planilla:______________________________</b>");
		...
		$io_pdf->addText(40,100,8,"<b>Agente de Retención:                                                                Firma:                                                            Sello:</b>");
		$io_pdf->addText(40,90,8,"<b>(Responsable)</b>");
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$rs_relac=$io_report->uf_select_compromisos_relacionados($ls_numsop);
							if(!$rs_relac->EOF)
							{
								$ls_procede= $rs_relac->fields["procede_doc"];
							}
						  ...
						  uf_print_detalle($la_data,$li_totalbaseimp,$li_totalmontoimp,$li_totmontoiva,$ls_rifagenteret,$ls_numsop,$ls_desope,
						  				   $ls_totconiva,$ld_fecfac,$ls_numfac,$ls_numref,$ls_rifagenteret,$ls_procede,$ls_fecrep,$io_pdf);
