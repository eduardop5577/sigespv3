...
***********************************************************************************/
	...
	function uf_print_encabezadopagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],47,510,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		...
		$io_pdf->addText(190,530,9,"Ley IVA - Art. 11 Ser�n responsables del pago del Impuesto en calidad de agente de retenci�n, los");
		$io_pdf->addText(190,520,9,"compradores o adquirientes de determinados bienes muebles y los receptores de ciertos servicios,");
		$io_pdf->addText(265,510,9," a quienes la Administraci�n Tributaria designe como tal");

		//$io_pdf->line(23,75,357,75); //LINEA HORIZONTAL
		$io_pdf->addtext(560,85,10,"FIRMA DEL AGENTE DE RETENCION");
		$io_pdf->Line(525,100,760,100);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(845,512,100,28);
		$io_pdf->addText(850,530,9,"<b>1. FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,520,9,$ad_fecrep); // Agregar el titulo
		$io_pdf->Rectangle(845,467,100,28);
		$io_pdf->addText(850,485,9,"<b>4. PERIODO FISCAL</b>"); // Agregar el titulo
		$io_pdf->addText(850,475,9,$as_perfiscal); // Agregar el titulo
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(400,467,370,30);
		$io_pdf->addText(405,485,9,"<b>3. REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,475,9,$as_rifagenteret); // Agregar el t�ulo
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(400,383,370,30);
		$io_pdf->addText(405,398,9,"<b>7. REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,388,9,$as_rif); // Agregar el titulo
	...
	function uf_print_detalle($la_data,$ai_totconiva,$ai_totsiniva,$ai_totbasimp,$ai_totmonimp,$ai_totivaret,$io_pdf)
	{
		...
		$la_columna=array('numope'=>'<b>Oper Nro.</b>',
						  ...
						  'tiptrans'=>'<b>Tipo de Trans</b>',
						  'numfacafec'=>'<b>No del Documento  Afectado</b>',
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('numope'=>array('justification'=>'center','width'=>60), // Justificacion y ancho de la columna
									   ...
									   'numnotdeb'=>array('justification'=>'center','width'=>50),
						 			   ...
						 			   'tiptrans'=>array('justification'=>'center','width'=>50), // Justificacion y ancho de la columna
									   'numfacafec'=>array('justification'=>'center','width'=>80),
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			$io_pdf->ezSetCmMargins(2.5,3,3,3);
			$lb_valido=true;
