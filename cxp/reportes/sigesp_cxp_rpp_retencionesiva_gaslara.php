...
***********************************************************************************/
	...
	function uf_print_encabezadopagina($as_titulo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],40,500,220,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
       ...
       // $io_pdf->Rectangle(150,540,800,40);	
		$io_pdf->addText(280,555,13,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->addText(50,492,10,'(LEY IVA-ART.11: "Seran responsables del pago del impuesto en calidad de agentes de retencion,');
		$io_pdf->addText(50,483,10,'los compradores o adquirientes de determinados bienes muebles y los receptores de ciertos servicios,');
		$io_pdf->addText(50,475,10,'a quienes la Administracion Tributaria designe como tal")'); // Agregar el titulo
                $io_pdf->addText(170,180,10,'AGENTE DE RETENCION'); // Agregar el titulo
                $io_pdf->addText(700,180,10,'BENEFICIARIO'); // Agregar el titulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(845,480,100,28);
                $io_pdf->Rectangle(150,120,180,80);
                $io_pdf->Rectangle(650,120,180,80); 	
		...
		$io_pdf->addText(405,458,9,"<b>R. I. F. DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		...
		$io_pdf->addText(405,375,9,"<b>R. I. F. DEL SUJETO RETENIDO</b>"); // Agregar el titulo
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$la_data[$li_i]=array('numope'=>$ls_numope,'fecfac'=>$ld_fecfac,'numfac'=>$ls_numfac,'numref'=>$ls_numref,
												  ...
												  'numfacafec'=>$ls_numfacafec,'totalconiva'=>$li_coniva,'compsinderiva'=>$li_siniva,
