...
***********************************************************************************/
	...
	$arrResultado=$io_fun_cxp->uf_load_seguridad("CXP","sigesp_cxp_r_relacionfacturas.php",$ls_permisos,$la_seguridad,$la_permisos);
	$ls_permisos=$arrResultado["as_permisos"];
	$la_seguridad=$arrResultado["aa_seguridad"];
	$la_permisos=$arrResultado["aa_permisos"];
	unset($arrResultado);
	$ls_reporte=$io_fun_cxp->uf_select_config("CXP","REPORTE","RELACION_FACTURAS","sigesp_cxp_rpp_relacionfacturas.php","C");
	$ls_reporteexcel=$io_fun_cxp->uf_select_config("CXP","REPORTE","RELACION_FACTURAS_EXCEL","sigesp_cxp_rpp_relacionfacturas_excel.php","C");
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
var patron2 = new Array(1,3,3,3,3)
	...
	function ue_search()
	{
		...
		fecdes=f.txtfecregdes.value;
		fechas=f.txtfecreghas.value;
		...
		ajax.send("catalogo=RELACIONFACTURAS&tipproben="+tipproben+"&codprobendes="+codprobendes+"&codprobenhas="+codprobenhas+
				  "&fecdes="+fecdes+"&fechas="+fechas);
	}
	...
	function uf_mostrar_reporte()
	{
			...
			if(li_imprimir==1)
			{
				...
				pantalla="reportes/"+formato+"?tipproben="+tipproben+"&codprobendes="+codprobendes+"&codprobenhas="+codprobenhas+"&fecregdes="+fecregdes+"&fecreghas="+fecreghas+"&ordendoc="+ordendoc+"&ordenfec="+ordenfec+"&ordencod="+ordencod+"&tiporeporte="+tiporeporte+"&comprobante="+ls_comprobantes+"";
	...
	function ue_openexcel()
	{
			...
			if(li_imprimir==1)
			{
				...
				tiporeporte=f.cmbbsf.value;
				formato=f.formato.value;
				pantalla="reportes/sigesp_cxp_rpp_relacionfacturas_excel?tipproben="+tipproben+"&codprobendes="+codprobendes+"&codprobenhas="+codprobenhas+"&fecregdes="+fecregdes+"&fecreghas="+fecreghas+"&ordendoc="+ordendoc+"&ordenfec="+ordenfec+"&ordencod="+ordencod+"&tiporeporte="+tiporeporte+"&comprobante="+ls_comprobantes+"";
				window.open(pantalla,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
			}
			else
			{alert("No tiene permiso para realizar esta operación");}
...
function ue_ayuda()
{
	...
	window.open("ayudas/sigesp_ayu_cxp_rep_relacionfacturas.pdf","Ayuda","menubar=no,toolbar=no,scrollbars=yes,width="+width+",height="+height+",resizable=yes,location=no");
}
</script>
