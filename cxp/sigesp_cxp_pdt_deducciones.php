...
function ue_aceptar()
{
	...
	for(j=1;(j<=totrow);j++)
	{
		...
		if((marcado==true)&&(parseFloat(monto)>0)&&(parseFloat(montoobj)>=parseFloat(monto)))
		{
			...
			ctaasu=eval("f.txtctaasu"+j+".value");
			...
			parametros=parametros+"&txtcodded"+li_i+"="+codded+"&txtmonobjret"+li_i+"="+monobjret+""+
					   ...
					   "&procededoc"+li_i+"="+procede+"&islr"+li_i+"="+islr+"&ctaasu"+li_i+"="+ctaasu;
		}

	}
	if(li_i==0)
	{
		alert("No selecciono ninguna Deduccion, o el monto de Retencion es invalido");
	}
	...
	if(opener.document.formulario.chkestretasu.checked==true)
		estretasu="1";
	else
		estretasu="0";
	...
	parametros=parametros+"&documento="+ls_documento+"&generarcontable="+generarcontable+"&estretasu="+estretasu;
	ls_proceso="";
