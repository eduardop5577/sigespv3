...
***********************************************************************************/
	...
	$arrResultado=$io_fun_cxp->uf_load_seguridad("CXP","sigesp_cxp_p_modcmpret.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<?php
	...
	switch($ls_operacion)
	{
		case "NEW":
		  ...
		  $ls_ano=date('Y');
	      $ls_mesaux=date('m');
		  $ls_mesanterior = str_pad(date("m")-1,2,'0',0);
		  if($ls_mesanterior=="00")
		  {
		    $ls_anoaux=$io_fun_cxp->uf_obtenervalor("txtano",date('Y'));
			if(($ls_ano-1)==$ls_anoaux)
				$ls_ano=$ls_anoaux;
			$ls_mesanterior="12";
		  }
		  $ls_mes=$io_fun_cxp->uf_obtenervalor("cmbmes",date('m'));
		  if(($ls_mes==$ls_mesaux)||($ls_mes==$ls_mesanterior))
		  {
			  $lb_valido=$io_cmpret->uf_validar_periodo_fiscal($ls_codret,$ls_ano.$ls_mes);
			  if($lb_valido)
			  {
				  $ls_numcom=$io_cmpret->uf_get_nrocomprobante($ls_codret,$ls_ano.$ls_mes,$ls_numcom);
				  $ls_parametros=uf_load_data($ls_parametros);
			  }
			  else
			  {
				  $io_msg->message("Existen comprobantes con Periodo Fiscal Superior al que intenta introducir");
				  $ls_operacion="";
			  }
		  }
		  else
		  {
 	    	  $io_msg->message("Periodo Fiscal No valido");
			  $ls_operacion="";
		  }		  
		break;
		...
		case "GUARDAR":
			...
			$io_sql->begin_transaction();
  		    $lb_flag=true;
			$ls_numcomaux=$ls_numcom;
			if($ls_existe=="FALSE")
			{
				$ls_fecha=date('Y-m-d');
				if($ls_basdatcmp!="")
				{
					$arrResultado=$io_cmpret->uf_crear_comprobante_consolida($ls_codret,$ls_numcom,$ls_fecha,$ls_ano.$ls_mes,$ls_codigo,
																		$ls_nomsujret,$ls_dirsujret,$ls_rif,"","1",$ls_logusr,"",
																		"M",$la_seguridad);
					$lb_flag=$arrResultado["lb_valido"];
					$ls_numcom=$arrResultado["as_numcom"];
					unset($arrResultado);
				}
				$arrResultado=$io_cmpret->uf_crear_comprobante($ls_codret,$ls_numcom,$ls_fecha,$ls_ano.$ls_mes,$ls_codigo,$ls_nomsujret,
														  $ls_dirsujret,$ls_rif,"","1",$ls_logusr,"","M",$la_seguridad);
				$lb_flag=$arrResultado["lb_valido"];
				$ls_numcomaux=$arrResultado["as_numcom"];
			}
			if($lb_flag)
			{
				 // $lb_flag=$io_modcmpret->uf_liberar_rd($ls_codret,$ls_probene,$ls_codigo,$li_totrowrecepciones);
				 $lb_flag=$io_modcmpret->uf_liberar_recepciones($ls_codret,$ls_numcomaux,$ls_probene,$ls_codigo);
			}
			if($lb_flag)
			{
			  $lb_flag=$io_modcmpret->uf_update_cmpret($ls_numcomaux,$ls_codret, $li_totrowrecepciones,$ls_probene,$ls_codigo, $la_seguridad);
			}
//			$lb_flag=false;
			if($lb_flag)
			{
		    	$io_msg->message("El comprobante se proceso satisfactoriamente");
				if($ls_numcomaux!=$ls_numcom)
				{
		    		$io_msg->message("Se le ha asignado un nuevo numero de Comprobante ".$ls_numcomaux);
					$ls_numcom=$ls_numcomaux;
				}
				$io_sql->commit();
			}
			else
              ...
              <tr>
                ...
                <td height="22" align="right"><div align="left">
				  ...
				  <?php if($ls_modageret=$_SESSION["la_empresa"]["estretiva"]!="B"){?>
                    ...
                    <option value="0000000006" <?php if($ls_codret=="0000000006"){ print "selected";} ?>>I.S.L.R.</option>
                  </select>
                  </label>
                </div></td>
              </tr>
		...
		else
		{
			...
			if(valido)
			{
				...
				for(row=1;row<rowrecepciones;row++)
				{
					...
					if((ls_mes!=fecsis)&&(ls_mes!=fecsisaux))
					{
...
function ue_ayuda()
{
	...
	window.open("ayudas/sigesp_ayu_cxp_modcmpret.pdf","Ayuda","menubar=no,toolbar=no,scrollbars=yes,width="+width+",height="+height+",resizable=yes,location=no");
}
</script>
