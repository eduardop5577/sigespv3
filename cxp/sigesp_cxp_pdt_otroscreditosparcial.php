...
***********************************************************************************/
	...
	$la_comprobante=explode("=",$la_comprobante);
	$li_total=count((array)$la_comprobante);
	unset($io_fun_cxp);
?>
...
<input name="comprobantes" type="hidden" id="comprobantes" value="<?php print $li_total; ?>">
...
function ue_aceptar()
{
	...
	//Suma cargos parciales
	opener.document.formulario.hidcargos.value=eval(li_cargos);
	total_parc=opener.document.formulario.hidcargos.value;
	//Suma cargos parciales
	li_total=eval(li_subtotal+"+"+total_parc);// se cambio li_cargos por total_parc
	...
	if(causadoparcial=="1")
	{
		...
		parametros=parametros+"&totalparc="+total_parc;
	}
	...
	parametros=parametros+"&deducciones="+li_deducciones+"&totgeneral="+li_totalgeneral+"&cargarcargos=1&eliminarcargo="+ls_eliminarcargo;
