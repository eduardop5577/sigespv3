...
{
	$ls_desproc ="";
	$arrResultado = $this->io_int_spg->uf_valida_procedencia($as_procedencia,$ls_desproc );
								...
								$as_codestpro5='-------------------------',$as_estcla='-')
{
	...
	if($ls_mensaje!="")
	{
		...
		if($lb_valido)
		{
			...
			$arrResultado = $this->io_int_spi->uf_valida_procedencia( $this->io_int_spi->is_procedencia , $ls_denproc);
			...
			$arrResultado = $this->uf_spi_select_movimiento( $ls_cuenta, $ls_procede_doc, $ls_documento, $ls_operacion, $lo_monto_movimiento,$lo_orden,$as_codestpro1,$as_codestpro2,$as_codestpro3,$as_codestpro4,$as_codestpro5,$as_estcla);
 ...
 return $lb_valido;
                                     ...
                                     $as_codestpro1,$as_codestpro2,$as_codestpro3,$as_codestpro4,$as_codestpro5,$as_estcla)
   {
	...
	function uf_spi_integracion_scg($as_codemp, $as_scgcuenta, $as_procede_doc, $as_documento, $as_descripcion, $adec_monto_actual)
	{
		...
		$arrResultado = $this->io_int_spg->io_int_scg->uf_scg_select_cuenta( $as_codemp, $as_scgcuenta, $ls_status, $ls_denominacion);
		$ls_status = $arrResultado['as_status'];
		$ls_denominacion = $arrResultado['as_denominacion'];
		$lb_existe = $arrResultado['lb_existe'];
		if (!$lb_existe)
		...
		$arrResultado = $this->uf_scg_select_movimiento($as_scgcuenta, $as_procede_doc, $as_documento, $ls_debhab, $ldec_monto, $li_orden);
    ...
    {											  
		...
		$arrResultado = $this->io_int_spg->io_int_scg->uf_valida_procedencia( $as_procede , $ls_desproc);
		$ls_desproc = $arrResultado['as_desproc'];
		$lb_valido = $arrResultado['lb_valido'];
		if (!($lb_valido)) 
		{ return false; }	 
...
function uf_guardar_movimientos_contable($arr_cmp,$as_cuenta,$as_procede_doc,$as_descripcion,$as_documento,$as_operacioncon,$adec_monto)
{
											    ...
											    $as_procede_doc,$as_documento,$as_operacion,$as_descripcion,$adec_monto)
	{
		...
		$lb_valido=true;
		$arrResultado = $this->io_int_scg->uf_scg_select_cuenta($as_codemp,$as_sc_cuenta,$ls_status,$ls_denominacion);
		$ls_status = $arrResultado['as_status'];
		$ls_denominacion = $arrResultado['as_denominacion'];
		$lb_existe = $arrResultado['lb_existe'];
		...
		$arrResultado = $this->io_int_scg->uf_valida_procedencia($as_procede_doc,$as_descproc);
		$ls_desproc = $arrResultado['as_desproc'];
		$lb_valido = $arrResultado['lb_valido'];
		if(!$lb_valido)
		{
		...
		$arrResultado = $this->uf_scg_select_movimiento($as_sc_cuenta,$as_procede_doc,$as_documento,$as_operacion,$adec_monto_anterior,$ai_orden);
		$adec_monto_anterior = $arrResultado['adec_monto'];
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////   
	function uf_spi_saldo_select($as_codemp,$as_cuenta,$adec_previsto,$adec_aumento,$adec_disminucion,$adec_devengado,$adec_cobrado,$adec_cobrado_ant)
	{
	    $lb_valido=true;
		...
		$arrResultado['adec_cobrado_ant']=$adec_cobrado_ant;
