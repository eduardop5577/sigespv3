...
//-----------------------------------------------------------------------------------------------------------------------------------
  function uf_spi_select_modalidad_apertura($as_estmodape)
  {
	...
	$arrResultado['as_estmodape']=$as_estmodape;
	$arrResultado['lb_valido']=$lb_valido;
	return $arrResultado;		
                                 ...
                                 $as_cuenta,$ad_previsto,$ai_distribuir,$estprog,$aa_seguridad)
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ls_estpreing=='1')
		{
			$arrResultado = $this->int_spi->uf_spi_select_cuenta_estructuras($this->is_codemp,$as_cuenta,$ls_status,$ls_denominacion,$ls_sc_cuenta,
		...
		{
		    $arrResultado = $this->int_spi->uf_spi_select_cuenta($this->is_codemp,$as_cuenta,$ls_status,$ls_denominacion,$ls_sc_cuenta);
		   	$ls_status=$arrResultado['as_status'];
			$ls_denominacion=$arrResultado['as_denominacion'];
			$ls_sc_cuenta=$arrResultado['as_scgcuenta'];
			$lb_existe = $arrResultado['lb_existe'];
		...
		$arrResultado = $this->int_spi->uf_spi_select_movimiento($as_cuenta,$this->is_procedencia,$this->is_comprobante,"PRE",$ldec_monto,$li_orden,
		                                             $ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,
													 $ls_codestpro5,$ls_estcla);
		$ldec_monto = $arrResultado['ldec_monto'];
		$li_orden = $arrResultado['li_orden'];
		$lb_existe = $arrResultado['lb_existe'];
		...
		{								
		  if ($ad_previsto <> 0) 
		  {  
		    $arrResultado = $this->uf_spi_buscar_estructuras($this->is_codemp,$estprog,$as_cuenta,$lb_vali);
			$lb_vali = $arrResultado['lb_vali'];
			...
			if($lb_valido)
			{
				...
				{	
				   ...
				   if ($ls_estpreing=='1')// si maneja estructura con las cuentas de ingreso
			       {
				  	$aa_montos_acum = $this->uf_spi_buscar_meses_cuentas($this->is_codemp,$as_cuenta,$aa_montos_acum);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{  
	    ...
	    $aa_hijos=$this->uf_obtener_hijos($as_codemp,$ls_cta_sin_ceros);
  	    $li_total_hijos=count((array)$aa_hijos);
		for($li_i=1;$li_i<=$li_total_hijos;$li_i++)
		{ 
			$ls_cuenta_spi=$aa_hijos[$li_i];
			//Obtengo los montos de los hijos de la cuenta actual y los acumulo.		
			$aa_montos = $this->uf_spi_obtener_montos_cuenta($as_codemp,$ls_cuenta_spi,$aa_montos);
...
//-----------------------------------------------------------------------------------------------------------------------------------------
                                                        ...
                                                        $ls_codestpro5,$ls_estcla,$rs_data)
  {
	...
	$ls_sql = "	SELECT spi_cuentas.spi_cuenta,spi_cuentas.denominacion,spi_cuentas.status, spi_cuentas_estructuras.previsto,spi_cuentas.distribuir,spi_cuentas.enero,spi_cuentas.febrero, spi_cuentas.marzo, spi_cuentas.abril,spi_cuentas.mayo,      spi_cuentas.junio,spi_cuentas.julio,spi_cuentas.agosto,spi_cuentas.septiembre,spi_cuentas.octubre,spi_cuentas.noviembre,spi_cuentas.diciembre
				FROM
				    spi_cuentas
				    INNER JOIN spi_cuentas_estructuras ON spi_cuentas.codemp = spi_cuentas_estructuras.codemp
				    AND spi_cuentas_estructuras.spi_cuenta = spi_cuentas.spi_cuenta
				    AND spi_cuentas_estructuras.codestpro1 = '$ls_codestpro1'
				    AND spi_cuentas_estructuras.codestpro2 = '$ls_codestpro2'
				    AND spi_cuentas_estructuras.codestpro3 = '$ls_codestpro3'
				    ...
				    AND spi_cuentas_estructuras.codestpro5 = '$ls_codestpro5'
				    AND spi_cuentas_estructuras.estcla = '$ls_estcla'
				    INNER JOIN spg_ep5 ON spi_cuentas_estructuras.codemp = spg_ep5.codemp
				    AND spi_cuentas_estructuras.codestpro1 = spg_ep5.codestpro1
				    AND spi_cuentas_estructuras.codestpro2 = spg_ep5.codestpro2
				    AND spi_cuentas_estructuras.codestpro3 = spg_ep5.codestpro3
				    AND spi_cuentas_estructuras.codestpro4 = spg_ep5.codestpro4
