...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
        $arrResultado=$io_report->uf_select_registro_concurso($ls_codcon, $rs_data);
		...
		else  // Imprimimos el reporte
		{
		   ...
		   while ($row=$io_report->io_sql->fetch_row($rs_data))
			...
			$lp_p++;	
			 ...
			 unset($la_datacon);
			 $rs_data2='';
			 $arrResultado=$io_report->uf_select_participantes_concurso($ls_codcon, $ls_orden, $rs_data2);
