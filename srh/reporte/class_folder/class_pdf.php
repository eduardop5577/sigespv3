...
***********************************************************************************/
 ...
 include ('../../base/librerias/php/ezpdf/class.ezpdf.php');
...
class class_pdf extends Cezpdf
{
  ...
  public function __construct($pagina='LETTER',$orientacion='portrait')
  {
  ...
  function convertir_coordenadas_mm_px($coord_x,$coord_y,$ancho=0)
  {
	...
	$arrResultado['coord_x']=$coord_x;
	$arrResultado['coord_y']=$coord_y;
	return $arrResultado;		
  }
  ...
  {
	for($i=0;$i<count((array)$arreglo);$i++)
	{
  ...
  function contar_lineas($cadena,$ancho_col,$tamano_texto)
  {
	...
	$lineas      = 1;
	$ancho_col = $this->convertir_valor_mm_px($ancho_col);
    $ancho_col = $ancho_col - 10;
	$ancho_disponible = $ancho_col;
	for ($n=0; $n<count((array)$la_palabras); $n++)
	{
	  if ($this->getTextWidth($tamano_texto,$la_palabras[$n]) <= $ancho_disponible)
	  {
		...
		if ($n < (count((array)$la_palabras)-1))
		{
	  ...
	  else
	  {
		...
		if (count((array)$la_semipalabras) > 1)
		{
		  for ($m=0; $m<count((array)$la_semipalabras); $m++)
		  {
		    if ($m < (count((array)$la_semipalabras)-1))
			{
  ...
  function alinear_columnas($tamano_texto,$ancho1,$ancho2,$cadena1,$cadena2)
  {
    $la_cadena1 = explode("\n",$cadena1);
    $la_cadena2 = explode("\n",$cadena2);
    $li_filas = count((array)$la_cadena1);
	...
	$arrResultado['cadena1']=$cadena1;
	$arrResultado['cadena2']=$cadena2;
	return $arrResultado;		
  }
  ...
  function obtener_lineas_por_fila($tamano_texto,$la_anchos_col,$la_datos)
  {
    ...
    if ($filas > 0)
    {
      ...
      {
        $max = 1;
        for($j=0; $j<count((array)$la_indices); $j++)
        {
          $indice = $la_indices[$j];
          if (array_search($indice,$la_indices_claves) !== false)
          {
		    ...
		    $lineas = 0;
		    for($x=0; $x<count((array)$la_cadena); $x++)
		    {
  ...
  function get_alto_disponible()
  {
	...
	$li_alto_disponible = $this->convertir_valor_px_mm($li_alto_disponible);
	$li_margen_inferior = $this->margen_inferior;
	$li_margen_inferior = $this->convertir_valor_px_mm($li_margen_inferior);
  ...
  function get_alto_usado()
  {
	...
	$li_alto_area_trabajo = $this->convertir_valor_px_mm($li_alto_area_trabajo);    
  ...
  function get_ancho_area_trabajo()
  {
	...
	$li_ancho_area_trabajo = $this->convertir_valor_px_mm($li_ancho_area_trabajo);
  ...
  {
    $arrResultado = $this->convertir_coordenadas_mm_px($coord_x,$coord_y);
	$coord_x = $arrResultado['coord_x'];
	$coord_y = $arrResultado['coord_y'];

    $ancho = $this->convertir_valor_mm_px($ancho);
    $alto = $this->convertir_valor_mm_px($alto);
    $color = $this->convertir_colores_rgb($color);
  ...
  {    
    $arrResultado = $this->convertir_coordenadas_mm_px($coord_x1,$coord_y1);
	$coord_x1 = $arrResultado['coord_x'];
	$coord_y1 = $arrResultado['coord_y'];

    $arrResultado = $this->convertir_coordenadas_mm_px($coord_x2,$coord_y2);
	$coord_x2 = $arrResultado['coord_x'];
	$coord_y2 = $arrResultado['coord_y'];
  ...
  function add_imagen($archivo,$coord_x,$coord_y,$ancho_imagen)
  {
    ...
    $arrResultado = $this->convertir_coordenadas_mm_px($coord_x,$coord_y,$ancho_imagen);    
	$coord_x = $arrResultado['coord_x'];
	$coord_y = $arrResultado['coord_y'];
  ...
  { 
    $coord_x_aux = $coord_x; 
    $arrResultado = $this->convertir_coordenadas_mm_px($coord_x_aux,$coord_y,$this->getTextWidth($tamano,$texto));  
	$coord_x_aux = $arrResultado['coord_x'];
	$coord_y = $arrResultado['coord_y'];

    $lineas = explode("\n",$texto);
    for ($i=0; $i<count((array)$lineas); $i++)
    {
      $coord_x_aux = $coord_x;
      $arrResultado = $this->convertir_coordenadas_mm_px($coord_x_aux,$coord_y2,$this->getTextWidth($tamano,$lineas[$i]));
	  $coord_x_aux = $arrResultado['coord_x'];
	  $coord_y2 = $arrResultado['coord_y'];
  ...
  function add_tabla($coord_x,$datos,$opciones)
  {
    ...
    if (count((array)$datos,COUNT_RECURSIVE) == count((array)$datos))
    {
      // es unidimensional y lo convertimos en bidimensional
      $la_datos = array();
	  for($i=1;$i<=count((array)$datos);$i++)
	  {
	...
	$color_fondo = $this->convertir_colores_rgb($color_fondo);
	...
	$color_texto = $this->convertir_colores_rgb($color_texto);
	...
	if (array_key_exists("anchos_col",$opciones))
	{
	  ...
	  for ($i=0; $i<count((array)$anchos_col); $i++)
	  {
	    $anchos_col[$i] = $this->convertir_valor_mm_px($anchos_col[$i]);
	  }
	};
