...
function ue_guardar_trabajos()
{
 ...
 { lb_valido= true;}    
    ...
    {	
      ...
      la_mensajes=new Array ("el Codigo del Trabajo","el nombre de la Empresa","el último cargo ocupado"," el último sueldo","la fecha de ingreso",
	  ...
	  if (lb_valido)
	  {
		...
		else
		{
			...
			{
			  alert ('La fecha de ingreso a la institucion '+$F('txtfecingper')+' debe ser mayor a la fecha de ingreso al trabajo anterior.');
...
function calcular_anos_servicio() {
	var years = 0;
	var months = 0;
	f=document.form1;
	if(f.txtfecingadmpub.value !=''){
		
		var dia = f.txtfecingadmpub.value.substr(0,2);
		var mes = f.txtfecingadmpub.value.substr(3,2);
		var ano = f.txtfecingadmpub.value.substr(6,4);
		fecingadmpub = new Date(ano+'-'+mes+'-'+dia);	
		fecactual       = new Date();
		var difference = (fecactual - fecingadmpub);
	    var years = Math.floor(difference / (1000 * 60 * 60 * 24 * 365));
	    difference -= years * (1000 * 60 * 60 * 24 * 365);
	    var months = Math.floor(difference / (1000 * 60 * 60 * 24 * 30.4375));
	
	    f.txtanoservpreper.value = years;
	    f.txtmesserprev.value = months;
	    
	}
	else {
		alert ('Indique la fecha de ingreso a la administración pública');
	}
}
