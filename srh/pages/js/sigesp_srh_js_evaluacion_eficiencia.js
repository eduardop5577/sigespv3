...
function ue_guardar()
{
  ...
  var la_mensajes=new Array ("el n�mero de evaluaci�n", "el tipo de evaluacion", "la fecha de la evaluaci�n", "el c�digo del personal", "el c�digo del evaluador", "la fecha inicial del per�odo de evaluaci�n", "la fecha final el per�odo de evaluaci�n", "el comentario del supervisor", "los aspectos a mejorar","las acciones");
 ...
 if (($F('txtcodper') == $F('txtcodeva'))  && ($F('txtcodper')!=""))
  ...
  if (lb_valido) {
	...
	codaspant="";
	checkcodasp=true;
	while ((x <= total)&&(lb_correcto))
	{
		codasp = eval ("document.form1.txtcodasp"+x+".value");
		if (codasp!=codaspant)
		{
			m=m+1;
		...
		{
			if (eval("document.form1.rdselec"+m) instanceof NodeList)
			{
				x=x+1;
				if(eval("document.form1.rdselec"+m+"["+h+"].checked"))
				{
					checkcodasp=true;
				}
				else
			...
			{
				x=x+1;
				if(eval("document.form1.rdselec"+m+".checked"))
				{
					checkcodasp=true;
				}
				else
...
{
	 alert ('Debe Completar todos los Items de Evaluaci�n.');
	 lb_correcto=false;
}
  if(lb_correcto)
	  ...
	  //Arreglo 
		...
		while (h!=10000)
		{
			while (h <= g)
			{
				...
				{
					g=g+1;
					if (eval("document.form1.txtcodite"+g+".value")!="")
					{
						h=h+1;	
					}
					else
...
function ue_eliminar()
{
  ...
  var la_mensajes=new Array ("el n�mero de evaluaci�n. Seleccione una Evaluaci�n de Eficiencia del Catalago");
  lb_valido = valida_datos_llenos(la_objetos,la_mensajes);
...
function consultar_items () {
	 ...
	 if  ( ($('txtcodeval').value=="") ) 		
	 {
		 alert("Debe llenar el  Tipo de Evaluaci�n");	
...
function consultar_rango_actuacion ()
 {
	if  ( ($('txttotal').value=="") || ($('txtcodeval').value=="")) 		
	 {
		 alert("El total y el tipo de evaluaci�n deben ser llenados.");
