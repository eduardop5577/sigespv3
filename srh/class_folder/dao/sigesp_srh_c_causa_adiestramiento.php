...
function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo código de un causa de adiestramiento
		//    Description: Funcion que genera un código de un causa de adiestramiento
	...
	function uf_srh_select_causa_adiestramiento($as_codcauadi)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Causa Adiestramiento MÉTODO->uf_srh_select_causa_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_srh_insert_causa_adiestramiento($as_codcauadi,$as_dencauadi,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Causa Adiestramiento MÉTODO->uf_srh_insert_causa_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó la causa de adiestramiento ".$as_codcauadi;
	...
	function uf_srh_update_causa_adiestramiento($as_codcauadi,$as_dencauadi,$aa_seguridad) 
	{
		...
		//         areaess: public 
		//      Argumento: $as_codcauadi        // código de causa de adiestramiento
	    //                 $as_dencauadi       // denominación de causa de adiestramiento
	    //                 $aa_seguridad   // arreglo de registro de seguridad
		//	      Returns: Retorna un Booleano
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Causa Adiestramiento MÉTODO->uf_srh_update_causa_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modificó la causa de adiestramiento ".$as_codcauadi;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_select_causa_adiestramiento_necesidad ($as_codcauadi)
	{
		...
		//		   Access: private
 		//	    Arguments: as_codcauadi // código de la causa de adiestramiento
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->Causa Adiestramiento  MÉTODO->uf_select_causa_adiestramiento_necesidad  ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
	...
	function uf_srh_delete_causa_adiestramiento($as_codcauadi,$aa_seguridad)
	{
		...
		//         areaess: public 
		//      Argumento: $as_codcauadi       // código de la causa de adiestramiento
	    //                 $aa_seguridad   // arreglo de registro de seguridad
		//	      Returns: Retorna un Booleano
		...
		else
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->Causa Adiestramiento MÉTODO->uf_srh_delete_causa_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			}
			else
			{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Eliminó la causa de adiestramiento ".$as_codcauadi;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_causa_adiestramiento($as_codcauadi,$as_dencauadi)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Causa Adiestramiento MÉTODO->uf_srh_buscar_causa_adiestramiento( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
