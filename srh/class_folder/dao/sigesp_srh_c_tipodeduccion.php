...
function uf_srh_calcular_monto_deduccionnew ($as_codper, $as_codtipded, $as_sexper)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->personal MÉTODO->uf_srh_calcular_monto_deduccionnew ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ls_valor=0;
...
function uf_srh_calcular_monto_deduccionnew_cod ($as_codper, $as_codtipded, $as_sexper)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->personal MÉTODO->uf_srh_calcular_monto_deduccionnew_cod ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ls_valor=0;
...
function uf_srh_calcular_monto_deduccionnewfam ($as_codper, $as_codtipded, $as_sexper, $as_cedfam)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->personal MÉTODO->uf_srh_calcular_monto_deduccionnewfam ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
...
function uf_srh_calcular_monto_deduccionnew_codfam ($as_codper, $as_codtipded, $as_sexper, $as_cedfam)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->personal MÉTODO->uf_srh_calcular_monto_deduccionnew_codfam ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
...
function uf_srh_guardar_configuracion_deduccion($ao_deduccion, $aa_seguridad)
  { 
	...
	$lb_guardo = true;
	$li_det = 0;
	while (($li_det < count((array)$ao_deduccion->deduccion)) &&
	       ($lb_guardo))
	{
...
function uf_srh_eliminar_dt_configuracion_deduccion($as_codtipded, $aa_seguridad)
  {
    ...
    function uf_srh_buscar_deduccion($as_codper,$as_codtipded,$as_tipo,$as_sueldo,$as_edad,$as_sexo,$as_valor) 
	{
		...
		//     Creado Por: Maria Beatriz Unda		
		// Fecha Creación: 29/05/2008							Fecha Última Modificación : 08/03/2010 
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
