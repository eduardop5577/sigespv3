  	...
  	if ($as_operacion == "modificar")
	{
	 $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "UPDATE srh_requisitos_minimos SET ".
	            ...
	            "WHERE codper= '$ao_requisitos->codper' AND codcon = '$ao_requisitos->codcon'  AND fecha = '".$as_fecha."' AND codemp='".$this->ls_codemp."'" ;
...
function guardarDetalles_Evaluacion ($ao_requisitos, $aa_seguridad)
  {
	...
	$as_fecha=$this->io_funcion->uf_convertirdatetobd($ao_requisitos->fecha);
	$lb_guardo = $this-> uf_srh_eliminar_dt_requisitos_minimos($ao_requisitos->codper,$ao_requisitos->codcon,$as_fecha, $aa_seguridad);
	  
	//Ahora guardamos
	$li_req = 0;
	while (($li_req < count((array)$ao_requisitos->req_min)) &&($lb_guardo))
	{
	  $lb_guardo = $this-> uf_srh_guardar_dt_requisitos_minimos($ao_requisitos->req_min[$li_req], $ao_requisitos->codcon,$aa_seguridad);
	  $li_req++;
	}
...
function uf_srh_eliminarrequisitos_minimos($as_codper,  $codcon, $as_fecha, $aa_seguridad)
  {
     ...
     $this-> uf_srh_eliminar_dt_requisitos_minimos($as_codper, $codcon, $as_fecha, $aa_seguridad);
     $this->io_sql->begin_transaction();	
...
function uf_srh_buscar_requisitos_minimos($as_codper,$as_fecha1, $as_fecha2)
	{
		...
		$ls_sql= " SELECT srh_requisitos_minimos.*,srh_concurso.descon,  ".
				 ...
				 " LEFT JOIN srh_concursante ON (srh_concursante.codper = srh_requisitos_minimos.codper AND srh_concursante.codcon = srh_requisitos_minimos.codcon) ".
...
function uf_srh_guardar_dt_requisitos_minimos($ao_requisitos, $codcon,$aa_seguridad)
  { 
	 ...
	 $ao_requisitos->fecha=$this->io_funcion->uf_convertirdatetobd($ao_requisitos->fecha);
	  $ls_sql = "INSERT INTO srh_dt_requisitos_minimos (codper,fecha,codite, puntos,codemp,codcon) ".	  
	            " VALUES ('$ao_requisitos->codper','$ao_requisitos->fecha','$ao_requisitos->codite',$ao_requisitos->puntos,'".$this->ls_codemp."','".$codcon."')";
...
function uf_srh_eliminar_dt_requisitos_minimos($as_codper,$codcon, $as_fecha, $aa_seguridad)
  {
