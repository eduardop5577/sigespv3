...
function uf_buscar_evaluacion_existente ($ls_personal,$ld_fecini,$ld_fecfin)
{
	...
	if($rs_data===false)
	{
		$this->io_msg->message("CLASE->evaluacion_desempeno MÉTODO->uf_buscar_evaluacion_existente( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
...
function uf_guardarCompetencias_evaluacion_desempeno ($ao_evaluacion, $aa_seguridad)
...

  {
	...
	$lb_guardo = true;
	$li_obj = 0;
	while (($li_obj < count((array)$ao_evaluacion->competencia)) &&
	       ($lb_guardo))
	{
...
function uf_guardarODIS_evaluacion_desempeno ($ao_evaluacion, $aa_seguridad)
  {
	...
	$lb_guardo = true;
	$li_obj = 0;
	while (($li_obj < count((array)$ao_evaluacion->odi)) &&
	       ($lb_guardo))
	{
