...
<html>
<head>
<title>Definici&oacute;n de Tarifas por Cargo</title>
...
require_once("class_folder/sigesp_scv_c_tarifacargos.php");
$io_region= new sigesp_scv_c_tarifacargos($conn);
...
$ls_exterior=   $io_fun_viaticos->uf_obtenervalor("chkexterior","");
...
{
	case "NUEVO":
		$ls_codtar= $io_keygen->uf_generar_numero_nuevo("SCV","scv_tarifacargos","codtar","",4,"","","");
		$ls_dentar= "";
		$ls_checked="";
		$ls_codmon=$io_region->uf_load_codigo_moneda_nacional();
		$ls_denmon=$io_region->uf_load_nombre_moneda_nacional();
		$ls_estatus="NUEVO";
		$li_lastrow= 0;
		$li_i=1;
		$object[$li_i][1]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='';><input type=text name=txtcodcar".$li_i."  id=txtcodcar".$li_i."  value=''  class=sin-borde  readonly  style=text-align:center  size=10>";
//		$object[$li_i][2]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value=''>";
        ...
        $object[$li_i][3]="<input type=text name=txtmontarcar".$li_i."  id=txtmontarcar".$li_i."  value=''  class=sin-borde    onKeyPress=return(ue_formatonumero(this,'.',',',event));  style=text-align:right    size=30>";
		...
		for ($li_i=1;$li_i<=$li_totrows;$li_i++)
		{
			if (array_key_exists("txtcodcar".$li_i,$_POST))
			{
				...
				$ls_codnom = $_POST["txtcodnom".$li_i];
 				...
 				$ls_montarcar = $_POST["txtmontarcar".$li_i];
                                
				$object[$li_i][1]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='".$ls_codnom."';><input type=text      name=txtcodcar".$li_i."    id=txtcodcar".$li_i."  value='".$ls_codcar."'     class=sin-borde  readonly  style=text-align:center  size=10>";
				//$object[$li_i][2]="<input type=hidden      name=txtcodnom".$li_i."    id=txtcodnom".$li_i."  value='".$ls_codnom."'     class=sin-borde  readonly  style=text-align:center  size=10>";
                                ...
                                $object[$li_i][3]="<input type=text   name=txtmontarcar".$li_i." id=txtmontarcar".$li_i."  value='".$ls_montarcar."'  class=sin-borde    onKeyPress=return(ue_formatonumero(this,'.',',',event));  style=text-align:right    size=30>";
				$object[$li_i][4]="<a href=javascript:uf_delete(".$li_i.");><img src=../shared/imagebank/tools20/eliminar.gif alt=Aceptar width=15 height=15 border=0></a>";
			}
			else
			{
				$object[$li_i][1]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='';><input type=text name=txtcodcar".$li_i."  id=txtcodcar".$li_i."  value=''  class=sin-borde  readonly  style=text-align:center  size=10>";
//				$object[$li_i][2]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value=''  class=sin-borde  readonly  style=text-align:center  size=10>";
                                $object[$li_i][2]="<input type=text name=txtdescar".$li_i."  id=txtdescar".$li_i."  value=''  class=sin-borde  readonly  style=text-align:left  size=30>";
                                $object[$li_i][3]="<input type=text name=txtmontarcar".$li_i."  id=txtmontarcar".$li_i."  value=''  class=sin-borde    onKeyPress=return(ue_formatonumero(this,'.',',',event));  style=text-align:right    size=30>";
				$object[$li_i][4]="<a href=javascript:uf_delete(".$li_i.");><img src=../shared/imagebank/tools20/eliminar.gif alt=Aceptar width=15 height=15 border=0></a>";
			} 
	...
	case "GUARDAR":
		...
		for ($li_i=1;$li_i<=$li_lastrow;$li_i++)
		{
			...
			$lr_grid["montarcar"][$li_i]= $ls_montarcar;                          
			$li_total++;
		}
		...
		{           
			...
			{ 
				$lb_valido = $io_region->uf_update_tarifa($ls_codemp,$ls_codtar,$ls_dentar,$ls_tipvia,$lr_grid,$li_total,$ls_exterior,$ls_codmon,$la_seguridad);
				if ($lb_valido)
				{
		...
		{  
			$arrResultado = $io_region->uf_insert_tarifa($ls_codemp,$ls_codtar,$ls_dentar,$ls_tipvia,$lr_grid,$li_total,$ls_exterior,$ls_codmon,$la_seguridad);
...
//		$object[$li_i][2]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value=''  class=sin-borde  readonly  style=text-align:center  size=10>";
	...
	case "CARGAR":
                ...
                $lb_valido = $io_region->uf_load_dt_tarifacargo($ls_codemp,$ls_codtar,$ls_codpais);
		...
		if ($lb_valido)
		{
			...
			{	
				...
				$ls_montarcar = $io_region->ds_dtregion->getValue('montarcar',$li_i);

                                
                                $ls_montarcar= number_format($ls_montarcar,2,',','.');
				$object[$li_i][1]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='".$ls_codnom."';><input type=text name=txtcodcar".$li_i."  id=txtcodcar".$li_i."  value='".$ls_codcar."'  class=sin-borde  readonly  style=text-align:center  size=10>";
//				$object[$li_i][2]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='".$ls_codnom."'  class=sin-borde  readonly  style=text-align:center  size=10>";
				$object[$li_i][2]="<input type=text name=txtdescar".$li_i."  id=txtdescar".$li_i."  value='".$ls_descar."'  class=sin-borde  readonly  style=text-align:left  size=30>";
	...
	case "DELETEROW":
		...
		{ 
			...
			{  		
				...
				$object[$li_temp][1]="<input type=hidden name=txtcodnom".$li_i."  id=txtcodnom".$li_i."  value='".$ls_codnom."';><input type=text name=txtcodcar".$li_temp."  id=txtcodcar".$li_temp."  value='".$ls_codcar."'  class=sin-borde  readonly  style=text-align:center  size=10>";
//				$object[$li_temp][2]="<input type=hidden name=txtcodnom".$li_temp."  id=txtcodnom".$li_temp."  value='".$ls_codnom."'  class=sin-borde  readonly  style=text-align:center  size=10>";
                                ...
                                $object[$li_temp][3]="<input type=text name=txtmontarcar".$li_temp."  id=txtmontarcar".$li_temp."  value='".$ls_montarcar."'  class=sin-borde    onKeyPress=return(ue_formatonumero(this,'.',',',event));  style=text-align:right  size=30>";
...
if(($ls_estatus=="NUEVO")&&($ls_operacion!="PINTAR"))
{
	$ls_selec0="selected";
	$ls_selec1="";
	$ls_selec2="";
...
<table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
      <tr class="titulo-celdanew"> 
        <td height="22" colspan="2" class="titulo-ventana">Definici&oacute;n de Tarifas por Cargo</td>
      </tr>
      <tr>
        <td height="22" >&nbsp;</td>
      ...
      <tr> 
        ...
        <td height="22"><p>
          <input name="txtdentar" id="txtdentar" value="<?php print $ls_dentar ?>" type="text" size="60" maxlength="254"  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+',.-()');">
        </p>        </td>
      </tr>
      <tr>
        <td height="22" align="right">Tipo de Viatico </td>
        <td height="22"><select name="cmbtipvia" id="cmbtipvia" onChange="javascript: ue_tipoviatico();">
          <option value="0" <?php print $ls_selec0; ?>>--Ninguna--</option>
          <option value="1" <?php print $ls_selec1; ?>>Viaticos de Instalacion</option>
          <option value="2" <?php print $ls_selec2; ?>>Orden de Transporte</option>
          <option value="3" <?php print $ls_selec3; ?>>Permanencia</option>
        </select></td>
      </tr>
      <tr>
        <td height="22" align="right"><div align="right">Exterior</div></td>
        <td height="22"><input name="chkexterior" type="checkbox" class="sin-borde" id="chkexterior" value="1" onChange="javascript:uf_verificar();" <?php print $ls_checked; ?>></td>
      </tr>
      <tr>
...
function uf_cat_cargos()
{
	f= document.form1;
	f.operacion.value= "";			
	pagina= "sigesp_scv_cat_cargo.php";
...
function uf_verificar()
{
	...
	{
		f.txtcodmon.value="001";
		f.txtdenmon.value="Bolivar";
	}
	else{
		f.txtcodmon.value="";
		f.txtdenmon.value="";
...
function ue_nuevo()
{
	...
	{	
		...
		f.action="sigesp_scv_d_tarifacargos.php";
		f.submit();
	}
	else
	{
     ...
     with (document.form1)
          {	
	        if (campo_requerido(txtcodtar,"El Código de la Tarifa debe estar lleno")==false)
		       ...
		       { 
		         if (campo_requerido(txtdentar,"La Denominación de la Tarifa debe estar llena")==false)
			        {
			          txtdentar.focus();
			        }
		         else
			        {
					   ...
					   li_numdet = f.hidlastrow.value;//Obtenemos el Número de Detalles 
						for (i=1;i<=li_numdet;i++)
						{
							montarcar = eval("f.txtmontarcar"+i+".value");
							if(montarcar=="")
							{
								alert("Debe indicar el monto del cargo");
								valido=false;
								break;
							}
						}
...
function ue_buscar()
{
	...
	{
		f.operacion.value="";			
		pagina="sigesp_scv_cat_tarifacargos.php";
