...
<html>
<head>
<title>Cat&aacute;logo de Categorias de Viaticos por Cargos</title>
...
$ls_sql="SELECT scv_catcargos.*,".
		"       (SELECT dencat FROM scv_categorias ".
		"         WHERE scv_catcargos.codcat=scv_categorias.codcat) AS dencat,".
		"       (SELECT denmon FROM sigesp_moneda ".
		"         WHERE scv_catcargos.codmon=sigesp_moneda.codmon) AS denmon".
		"  FROM scv_catcargos".
		" ORDER BY codcatcar ASC ";
$rs_data= $io_sql->select($ls_sql);
$data= $rs_data;
?>
<table width="500" border="0" align="center" cellpadding="1" cellspacing="1">
  <tr>
   <td width="496" colspan="2" class="titulo-celda">Cat&aacute;logo de Categorias de Viaticos por Cargo </td>
  </tr>
</table>
  <br>
...
if ($row=$io_sql->fetch_row($rs_data))
   {
     ...
     $totrow= $io_dsclasi->getRowCount("codcatcar");
	 ...
	 for ($z=1;$z<=$totrow;$z++)
         {
			...
			$ls_codigo       = $data["codcatcar"][$z];
			$ls_codcat = $data["codcat"][$z];
			$ls_dencat = $data["dencat"][$z];
			$ls_tipvia = $data["tipvia"][$z];
			$ls_exterior = $data["foraneo"][$z];
			$ls_codmon = $data["codmon"][$z];
			$ls_denmon = $data["denmon"][$z];
			print "<td><a href=\"javascript: aceptar('$ls_codigo','$ls_codcat','$ls_dencat','$ls_tipvia','$ls_exterior','$ls_codmon','$ls_denmon');\">".$ls_codigo."</a></td>";
...
<script >
function aceptar(codigo,codcat,dencat,tipvia,exterior,codmon,denmon)
{
	f= opener.document.form1;
	f.txtcodtar.value= codigo;
	f.txtcodcat.value= codcat;
	f.txtdencat.value= dencat;
