...
/*		$ls_sql="SELECT MAX(sno_nomina.desnom) as desnom,MAX(sno_nomina.codnom) as codnom,(CASE sno_nomina.racnom WHEN '1' THEN sno_asignacioncargo.denasicar ELSE sno_cargo.descar END) AS cargo,".
				...
				" ORDER BY sno_personalnomina.codper,codclavia";print $ls_sql;
*/
$ls_sql="SELECT MAX(sno_nomina.desnom) as desnom,MAX(sno_nomina.codnom) as codnom,(CASE sno_nomina.racnom WHEN '1' THEN sno_asignacioncargo.denasicar ELSE sno_cargo.descar END) AS cargo, ".
        "       codclavia,sno_personalnomina.codper,(SELECT nomper FROM sno_personal WHERE sno_personal.codper=sno_personalnomina.codper) as nomper,".
        "       (SELECT apeper FROM sno_personal WHERE sno_personal.codper=sno_personalnomina.codper) as apeper,".
        "       (SELECT cedper FROM sno_personal WHERE sno_personal.codper=sno_personalnomina.codper) as cedper ".
		...
		"      	   ON sno_personal.estper ='1' ".
		"         AND rpc_beneficiario.codemp=sno_personal.codemp ".
		"         AND rpc_beneficiario.ced_bene=sno_personal.cedper)  ".  
		"    ON sno_personalnomina.staper ='1' ".
		...
		while(!$rs_per->EOF)
		{
				$ls_codper=$rs_per->fields["codper"];
				$ls_cedper=$rs_per->fields["cedper"];
				$ls_nomper=$rs_per->fields["nomper"];
				$ls_apeper=$rs_per->fields["apeper"];
				$ls_cargo=$rs_per->fields["cargo"];
				$ls_codclavia=$rs_per->fields["codclavia"];
				$ls_codnom=$rs_per->fields["codnom"];
				$ls_desnom=$rs_per->fields["desnom"];
...
function ue_search()
{
	...
	if((codper!="")||(cedper!="")||(nomper!="")||(apeper!=""))
	{
	...
	{
		alert("Favor indicar un parametro para la busqueda");
	}
}
</script>
</html>
