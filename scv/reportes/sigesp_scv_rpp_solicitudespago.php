...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_cabecera($as_cedper,$as_nomper,$as_desuniadm,$as_codcueban,$as_tipcuebanper,$as_desded,$as_destipper,
							  $as_codclavia,$ad_fecsalvia,$ad_fecregvia,$ai_numdiavia,$as_denmis,$ai_acompanante,$as_cargo,$ld_fecsolvia,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			else // Imprimimos el reporte
			{
				...
				while($row=$io_report->io_sql->fetch_row($rs_data))
				{
					...
					uf_print_cabecera($ls_cedper,$ls_nomper,$ls_desuniadm,$ls_codcueban,$ls_tipcuebanper,$ls_desded,$ls_destipper,
									  $ls_codclavia,$ld_fecsalvia,$ld_fecregvia,$li_numdiavia,$ls_denmis,$li_acompanante,$ls_cargo,$ld_fecsolvia,$io_pdf); // Imprimimos la cabecera del registro
					...
					if ($li_tipvia=='-')
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
							{
								...
								$ls_codmon= $io_report->ds_detalle->data["codmon"][$li_s];
								if($ls_codmon!="001")
								{
									$li_monto=($li_monto*$li_tascam1);
								}
