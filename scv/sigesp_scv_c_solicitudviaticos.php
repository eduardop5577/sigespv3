...
*/			}
	...
	function uf_scv_select_dt_misiones($as_codemp,$as_codsolvia,$as_codmis)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_select_dt_misiones ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_scv_insert_dt_misiones($as_codemp,$as_codsolvia,$as_codmis,$ai_canmisdes,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_insert_dt_misiones ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó la Asignación ". $as_codmis ."  asociado a la Solicitud de Viaticos ".$as_codsolvia." de la Empresa ".$as_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_scv_delete_dt_misiones($as_codemp,$as_codsolvia,$as_codmis,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->solicitudviaticos MÉTODO->uf_scv_delete_dt_misiones ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="DELETE";
			$ls_descripcion= "Eliminó la mision ".$as_codmis." de la Solicitud de Viaticos ".$as_codsolvia." Asociado a la Empresa ".$as_codemp;
			$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////			
	...
	function uf_scv_load_dt_mision($as_codemp,$as_codsolvia,$ai_totrows,$ao_object)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_load_dt_mision ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_scv_load_mision($as_codemp) 
	{
		...
		//	       Arguments:  $as_codemp     //  codigo de la Empresa.
		//	         Returns:  $lb_valido.
		//	     Description:  Función que se encarga de buecar la mision destino con el campo=>estdesviaper igual a '1'
	...
	function uf_scv_select_permisoadministrador($as_codemp,$as_codusu)
	{
		...
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creación: 09/11/2020 								Fecha Última Modificación :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_select_permisoadministrador ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
