	...
	function uf_print_estructura1()
   	{
		...
		$li_len5=0;
		$ls_titulo="";
		$arrResultado=$io_funciones_scv->uf_loadmodalidad($li_len1,$li_len2,$li_len3,$li_len4,$li_len5,$ls_titulo);
	...
	function uf_print_cuentasspg()
   	{
		...
		else
		{
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_programatica="";
				$ls_programatica=$io_funciones_scv->uf_formatoprogramatica($ls_codestpro,$ls_programatica);
				if($ls_status=="C")
				{
			...
			if($lb_valido)
			{
				...
				if($li_totrow>0)
				{
					...
					for($li_i=1;($li_i<=$li_totrow)&&($lb_valido);$li_i++)
					{
						...
						$li_disponible=0;
						$ls_numcomanu="";
						$arrResultado=$io_recepcion->uf_load_monto_ajustes($ls_comprobante,$ls_procede,$ls_tipodes,$ls_codprov,$ls_cedbene,$li_monto_ajuste);
						$lb_valido=$arrResultado["valido"];	
						$li_monto_ajuste=$arrResultado["monto"];	
						if($lb_valido)
						{
							$arrResultado=$io_recepcion->uf_load_monto_causados($ls_comprobante,$ls_procede,$ls_tipodes,$ls_codprov,$ls_cedbene,$li_monto_causado);
							$lb_valido=$arrResultado["valido"];	
							$li_monto_causado=$arrResultado["monto"];	
						}
						if($lb_valido)
						{
							$arrResultado=$io_recepcion->uf_load_comprobantes_anulados($ls_comprobante,$ls_tipodes,$ls_codprov,$ls_cedbene,$ld_fechareg,$ls_numcomanu);
							$lb_valido=$arrResultado["valido"];	
							$ls_numcomanu=$arrResultado["valor"];	
						}
						if($lb_valido)
						{
							$arrResultado=$io_recepcion->uf_load_monto_anulados($ls_numcomanu,$ls_procede,$ls_tipodes,$ls_codprov,$ls_cedbene,$li_monto_anulado);
							$lb_valido=$arrResultado["valido"];	
						...
						/*if($lb_valido)
						{
							$arrResultado=$io_recepcion->uf_load_monto_cargos($ls_comprobante,$ls_procede,$li_monto_cargo);
							$lb_valido=$arrResultado["valido"];	
...
//							$li_disponible=($li_total+$li_monto_ajuste)-$li_monto_causado+$li_monto_anulado-$li_monto_recepcion-$li_monto_cargo;
 					...
 					//print" DISPONIBLE->".$li_disponible." TOTAL->".$li_total." AJUSTE->".$li_monto_ajuste." CAUSADO->".$li_monto_causado." Anulado->".$li_monto_anulado." Recepcion->".$li_monto_recepcion."<br><br>";
							if($li_disponible>0)
							{
								$arrResultado=$io_recepcion->uf_load_acumulado_solicitudes($ls_numdoc,$ls_codtipdoc,$ls_codprov,$ls_cedbene,$li_monto_solicitud);
								$lb_valido=$arrResultado["valido"];	
