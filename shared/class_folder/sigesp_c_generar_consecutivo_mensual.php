...
class sigesp_c_generar_consecutivo_mensual
 {
	...
	function uf_generar_numero_nuevo($as_codsis,$as_tabla,$as_campo,$as_procede,$ai_loncam,$as_camini,$as_filtro,$as_valor,$as_mes,$as_ano)
	{
		...
		if($ls_nroact!="")
		{
			if($ls_prefijo!="0000")
			{
				$li_nrolen=$ai_loncam-9;
				$ls_numpre=substr($ls_nroact,0,9);
				$ls_nro=substr($ls_nroact,9,$li_nrolen);
			}
			else
			{
				$ls_nro=$ls_nroact;
		...
		{
			$ls_nvonro= $this->io_funciones->uf_cerosizquierda($li_nvonro,$ai_loncam-9);
			$ls_nvonro= $ls_prefijo.'M'.$as_mes.$ls_nvonro;
		}
		else
		{
