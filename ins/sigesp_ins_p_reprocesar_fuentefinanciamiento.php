...
***********************************************************************************/
	...
	$la_seguridad = Array();
	$la_permisos = Array();
	$arrResultado = $oi_fun_instala->uf_load_seguridad("INS","sigesp_ins_p_reprocesar_fuentefinanciamiento.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<html>
<head>
<title >Reprocesar Fuente de Financimiento</title>
	...
	if($ls_operacion=="EJECUTAR")
	{
		switch($ls_sistema)
		{
			case "SEP": // Sistema de Solicitud de Ejecución Presupuestaria
				$lb_valido=$io_reprocesar->uf_reprocesar_fuentefinanciamiento_comprobante_sep($la_seguridad);
				break;
			case "SOC": // Sistema de Compras
				$lb_valido=$io_reprocesar->uf_reprocesar_fuentefinanciamiento_comprobante_soc($la_seguridad);
				break;
			case "CXP": // Sistema de Cuentas por Pagar
				$lb_valido=$io_reprocesar->uf_reprocesar_fuentefinanciamiento_comprobante_cxp($la_seguridad);
				break;
			case "SCB": // Sistema de Caja y Banco
				$lb_valido=$io_reprocesar->uf_reprocesar_fuentefinanciamiento_comprobante_scb($la_seguridad);
				break;
		}
		if($lb_valido)
		{
    ...
    <td width="571" height="221" valign="top">
        ...
        <table width="360" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
          <tr class="titulo-ventana">
            <td colspan="4"><p>Reprocesar Fuente de Financimiento </p>            </td>
          </tr>
          ...
          <tr class="formato-blanco">
            ...
            <td colspan="3"><div align="left">
              ...
              <select name="cmbsistema" id="cmbsistema">
                ...
                <option value="SOC">Orden de Compra</option>
...
function  uf_ejecutar()
{
	...
	if(li_ejecutar==1)
	{
		if(f.cmbsistema.value!="")
		{
			...
			f.action="sigesp_ins_p_reprocesar_fuentefinanciamiento.php";
			f.submit();
		}
		else
		{
