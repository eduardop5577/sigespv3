...
$ls_logusr=$_SESSION["la_logusr"];
	...
	$la_seguridad = Array();
	$la_permisos = Array();
	$arrResultado = $oi_fun_instala->uf_load_seguridad("INS","sigesp_ins_p_integracionsigefirrhh.php",$ls_permisos,$la_seguridad,$la_permisos);
...
require_once("class_folder/sigesp_ins_c_integracionsigefirrhh.php");
$io_integracion=new sigesp_ins_c_integracionsigefirrhh();

$ls_gestor_int=$io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","GESTOR_INT","POSTGRES","C");
$ls_puerto_int=$io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","PUERTO_INT","5432","C");
$ls_servidor_int=$io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","SERVIDOR_INT","127.0.0.1","C");
$ls_basedatos_int=$io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","BASE_DATOS_INT","db_sigefirrhh","C");
$ls_login_int=$io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","LOGIN_INT","xxxxxxx","C");
$ls_password_int=trim($io_integracion->uf_select_config("INS","INTEGRACION-SIGEFIRRHH","PASSWORD_INT","xxxxxx","C"));
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
"http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>Integraci&oacute;n con SIGEFIRRHH</title>
...
<body>
<?php 
	$ls_titletable="Información de integración";
	$li_widthtable=500;
	$ls_nametable="grid";
	$lo_title[1]="";
	$lo_title[2]="Nómina";
	$lo_title[3]="Periodo";
	$lo_title[4]="comprobante";
	$lo_title[5]="Descripcion";
	$lo_title[6]="Detalles";
	...
	switch ($ls_operacion) 
	{
		case "BUSCAR":
			...
			$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","GESTOR_INT",$ls_gestor_int,"C");
			if ($lb_valido)
			{
				$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","PUERTO_INT",$ls_puerto_int,"C");
			}
			if ($lb_valido)
			{
				$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","SERVIDOR_INT",$ls_servidor_int,"C");
			}
			if ($lb_valido)
			{
				$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","BASE_DATOS_INT",$ls_basedatos_int,"C");
			}
			if ($lb_valido)
			{
				$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","LOGIN_INT",$ls_login_int,"C");
			}
			if ($lb_valido)
			{
				$lb_valido=$io_integracion->uf_insert_config("INS","INTEGRACION-SIGEFIRRHH","PASSWORD_INT",$ls_password_int,"C");
			}
			if ($lb_valido)
			{
			...
			if($lb_valido)
			{
				for($li_i=1;(($li_i<$li_totrows)&&$lb_valido);$li_i++)
				{
					if(array_key_exists("chksel".$li_i,$_POST))
					{
						...
						$ls_tipoconcepto=$_POST["txttipoconcepto".$li_i];
						$lb_valido=$io_integracion->uf_procesar_integracion($ls_gestor_int,$ls_puerto_int,$ls_servidor_int,$ls_basedatos_int,$ls_login_int,
														 				    $ls_password_int,$ls_codnom,$ls_codperi,$ls_codcom,$ls_tipoconcepto,$la_seguridad);
					}
				}
			}
			if($lb_valido)
...
<label></label>
  <table width="200" border="0" align="center">
    <tr>
      ...
      <tr>
         <td height="22" colspan="3" class="titulo-celdanew">Integraci&oacute;n con SIGEFIRRHH</td>
      </tr>
         ...
         <td height="13" colspan="3">&nbsp;</td>
          ...
          <tr>
            <td height="22" colspan="3" class="titulo-celdanew"><div align="right"></div>
                <div align="center">Informaci&oacute;n de Conexion SIGEFIRRHH </div></td>
          </tr>
          <tr>
...
function ue_nuevo()
{
   location.href='sigesp_ins_p_integracionsigefirrhh.php' 
}
...
{	
	...
	f.action="sigesp_ins_p_integracionsigefirrhh.php";
	f.submit();
}

function uf_verdetalle(codcom,tipoconcepto,gestor_int,puerto_int,servidor_int,basedatos_int,login_int,password_int)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_sigefirrhh.php?codcom="+codcom+"&tipoconcepto="+tipoconcepto+"&gestor_int="+gestor_int+"&puerto_int="+puerto_int+"&servidor_int="+servidor_int+"&basedatos_int="+basedatos_int+"&login_int="+login_int+"&password_int="+password_int+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
