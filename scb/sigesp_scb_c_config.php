...
class sigesp_scb_c_config
{
	...
	public function __construct($aa_security,$path='../')
	{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_buscar_solicitud_carta_orden($ls_codban,$ls_cuenta_banco,$ls_numcarord,$ls_codope)
	{
	    /*----------------------------------------------------------------
		Function: uf_select_cartaorden
		Descripción: funcion que retorna true si existe la carta orden
		...
		$ls_sql="SELECT numsol ".
				"FROM cxp_sol_banco ".
				"WHERE codemp='".$ls_codemp."' ".
				"AND codban='".$ls_codemp."' ".
				"AND ctaban='".$ls_cuenta_banco."' ".
				"AND numdoc='".$ls_numcarord."' ".
