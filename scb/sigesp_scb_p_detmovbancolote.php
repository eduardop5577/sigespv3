...
$la_seguridad=Array();
$la_permisos=Array();
$arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_p_movbanco.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<script >
	function uf_valida_cuadre()
	{
		...
		if((ldec_diferencia!=0)&&((ls_operacion=="")||(ls_operacion=="GUARDAR")||(ls_operacion=='NUEVO')))
		{
			...
			f.action="sigesp_scb_p_detmovbancolote.php";
			f.submit();
		}
	}
</script>
...
<?php
	...
	$ls_codoficom=$io_fun_banco->uf_obtenervalor_get("codoficom","");
	$ls_codforpag=$io_fun_banco->uf_obtenervalor_get("codforpag","");
	$ls_denoficom=$in_classmovbco->uf_select_oficina($ls_codoficom);
	$ls_denforpag=$in_classmovbco->uf_select_formapago($ls_codforpag);
	if( array_key_exists("operacion",$_POST))
	{
		$ls_operacion= $_POST["operacion"];
		$ls_mov_operacion=$_POST["cmboperacion"];
		if(($ls_mov_operacion=="DEP")||($ls_mov_operacion=="Dep"))
		{$ls_mov_operacion="DP";}
		...
		$ls_codoficom=$_POST["txtcodoficom"];
	...
	{
		//codban="+codban+"&denban="+denban+"&cuenta="+cuenta+"&dencuenta="+dencuenta+"&operacion="+operacion+"&documento="+documento+"&fecha="+fecha
		$ls_operacion= "NUEVO" ;
		$ls_estdoc="N";
		$li_estimpche=0;
		$ls_connocontabi=0;
	...
	function uf_cargar_dt()
	{
		...
		$ldec_monspi="";
		$arrResultado="";
		$arrResultado=$in_classmovbco->uf_cargar_dt($ls_docmov,$ls_codban,$ls_cuenta_banco,$ls_mov_operacion,$ls_estmov,$objectScg,$li_row,$ldec_mondeb,$ldec_monhab,$objectSpg,$li_rows_spg,$ldec_monspg,$objectSpi,$li_rows_spi,$ldec_monspi);
	...
	function uf_nuevo()
	{
		...
		global $ls_mov_operacion;
		global $ls_chevau;
		global $ls_estdoc,$li_estpreing,$in_classmovbco;
		$ls_estdoc="N";
		global $ls_mov_operacion;
		$ls_mov_operacion=$_GET["operacion"];
		if(($ls_mov_operacion=="DEP")||($ls_mov_operacion=="Dep"))
		{$ls_mov_operacion="DP";}
	    global $ls_opepre;
		$ls_opepre=0;		
		...
		$ls_estmov="N";
		global $ls_cuenta_banco;
		$ls_cuenta_banco=$_GET["cuenta"];
		global $ls_dencuenta_banco;
		$ls_dencuenta_banco=$_GET["dencuenta"];
		global $ls_provbene;
		$ls_provbene="----------";
		global $ls_desproben;
		...
		$ldec_montomov=$_GET["monto"];
		$ldec_montomov=str_replace(".","",$ldec_montomov);
		$ldec_montomov=str_replace(",",".",$ldec_montomov);
		global $ldec_montoislr;
		$ldec_montoislr=$_GET["islr"];
		$ldec_montoislr=str_replace(".","",$ldec_montoislr);
		$ldec_montoislr=str_replace(",",".",$ldec_montoislr);
		...
		$ls_cuenta_scg=$_GET["sccuenta"];
		global $ldec_disponible;
		$ldec_disponible=$_GET["disponible"];
		global $lb_nocontab;
		$lb_nocontab="";
		...
		$ls_denfuefin=$_GET["denfuefin"];
		global $ls_codcon;
		$l_codcon="";
		global $ls_numordpagmin,$ls_codtipfon,$ls_dentipfon,$ld_monmaxmov;
		$ls_numordpagmin = $ls_codtipfon = $ls_dentipfon = "";
		$ld_monmaxmov = 0;
		$ls_codoficom=$_GET["codoficom"];
		$ls_codforpag=$_GET["codforpag"];
		$ls_numdoc=trim($_GET["documento"]);
		$ls_denoficom=$in_classmovbco->uf_select_oficina($ls_codoficom);
		global $ls_codoficom,$ls_denoficom;
		$ls_denforpag=$in_classmovbco->uf_select_formapago($ls_codforpag);
		global $ls_codforpag,$ls_denforpag;
		...
		$ls_desmov=$ls_concepto.$ls_denforpag." (".$ls_codforpag.") No.".$ls_numdoc;
		$arrResultado="";
		$arrResultado=$in_classmovbco->uf_cargar_detalle($ls_numdoc,$ls_codban,$ls_cuenta_banco,$ls_mov_operacion,$ls_codforpag,$ls_cuenta_scg,$ldec_montomov,$ldec_montoislr);
		$ls_chevau=$in_classmovbco->uf_numero_voucher($_SESSION["la_empresa"]["codemp"],$ls_codban,$ls_cuenta_banco,$ls_docmov);
		$objectScg=$arrResultado['objectScg'];
		$li_row=$arrResultado['li_row_scg'];
		$ldec_mondeb=$arrResultado['ldec_mondeb'];
...
//		   }
	...
	if($ls_operacion=="PRINT_CARTAORDEN")
	{
		...
		uf_cargar_dt();	
		$ls_codigo="001";
		$ls_solicitud_carta=$in_classconfig->uf_buscar_solicitud_carta_orden($ls_codban,$ls_cuenta_banco,$ls_numcarord,'ND');
		$ls_pagina = "reportes/".$ls_report."?codigo=$ls_codigo&codban=$ls_codban&ctaban=$ls_cuenta_banco&numdoc=$ls_numcarord&chevau=&codope=ND&tipproben=$ls_tipo&solicitud=$ls_solicitud_carta";
		?>
<script >						
	...
	{			
		...
		$arrResultado="";
		$lb_valido="";
		$arrResultado=$in_classmovbco->uf_guardar_automatico($ls_codban,$ls_cuenta_banco,$ls_docmov,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_desproben,$ldec_montomov,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,$li_estint,"$ls_opepre",$ls_estbpd,'SCBMOV',$ls_estreglib,$ls_estdoc,$ls_tipo,$ls_codfuefin,$ls_numordpagmin,$ls_codtipfon,$li_estcob,$ls_numcontint,$li_esttranoreglib,$ls_codoficom,$ls_codforpag);
		$lb_valido=$arrResultado["lb_valido"];
		$ls_numcontint=$arrResultado["as_numcontint"];
		if($lb_valido)
		{
			$lb_valido = $in_classmovbco->uf_actualizar_estatus_ch2($ls_codban,$ls_cuenta_banco,$ls_docmov,$ls_numchequera);
			if (!$lb_valido)
			{
...
?>
  <form name="form1" method="post" action="" id="sigesp_scb_p_detmovbancolote.php">
<?php
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
?>
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr>
      <td height="22" colspan="6"><div align="center"><a href="javascript: ue_guardar();"></a><a href="javascript: ue_cerrar();"><img src="../shared/imagebank/tools20/salir.gif" alt="Cerrar Ventana" title="Cerrar Ventana" width="20" height="20" border="0"></a></div></td>
    </tr>
    ...
    <tr>
      ...
      <td><div align="left">
          <select name="cmboperacion" id="select" onChange="javascript:uf_impedir_cambio();" style="width:120px">
      ...
      <td><input name="hidoperacion" type="hidden" id="hidoperacion" value="<?php print $ls_mov_operacion; ?>"></td>
      <?php
	    if ($ls_mov_operacion!='RE')
		   {
	  ?>
	  <td><div align="right">
	      <input name="txtnumordpagmin" type="hidden" id="txtnumordpagmin" onKeyPress="return keyRestrict(event,'0123456789'); " value="<?php echo $ls_numordpagmin; ?>" size="20" maxlength="15" style="text-align:center" onKeyUp="javascript:uf_chequear();" <?php print $ls_readOnly_doc; if ($ls_mov_operacion=='CH' || $ls_mov_operacion=='ND'){ ?> readonly <?php } ?>>
	      <?php if ($ls_mov_operacion=='CH' || $ls_mov_operacion=='ND'){ ?>
	      &nbsp;<a href="javascript:uf_catalogo_ordenes();"></a>      
	      <?php }?>Oficina
	  </div></td>
	  <td colspan="2"><input name="txtdenoficom" type="text" id="txtdenoficom" value="<?php print $ls_denoficom; ?>" size="45">
      <input name="txtcodoficom" type="hidden" id="txtcodoficom" value="<?php print $ls_codoficom; ?>"></td>
	  <?php 
	       }
	  ?> 
    </tr>
	...
	<tr>
      ...
      <td style="text-align:left"><label>
        <input name="txtcodtipfon" type="hidden" id="txtcodtipfon" value="<?php echo $ls_codtipfon; ?>" size="9" maxlength="4" style="text-align:center" readonly>
        
        <input name="txtdentipfon" type="hidden" class="sin-borde" id="txtdentipfon" value="<?php echo $ls_dentipfon; ?>" size="110" maxlength="254" readonly>
      </label></td>
      <td style="text-align:left">&nbsp;</td>
      <td style="text-align:left"><div align="right">Tipo Operacion </div></td>
      <td colspan="2" style="text-align:left"><input name="txtdenforpag" type="text" id="txtdenforpag" value="<?php print $ls_denforpag; ?>" size="45">
      <input name="txtcodforpag" type="hidden" id="txtcodforpag" value="<?php print $ls_codforpag; ?>"></td>
    </tr>
    <?php
	   }
	if ($ls_mov_operacion!='RE')
    ...
    <tr>
      <td width="118" height="22" style="text-align:right">Banco</td>
      <td colspan="3" style="text-align:left"><input name="txtcodban" type="text" id="txtcodban"  style="text-align:center" value="<?php print $ls_codban;?>" size="10" readonly>
        ...
        <input name="txtdenban" type="text" id="txtdenban" value="<?php print $ls_denban?>" size="51" class="sin-borde" readonly>      </td><td width="90" style="text-align:right">Fecha</td>
      <td width="169" style="text-align:left"><input name="txtfecha" type="text" id="txtfecha"  style="text-align:center" value="<?php print $ld_fecha;?>" size="12" maxlength="10" readonly></td>
    </tr>
...
-->	<tr>
    ...
    <tr>
      ...
      <td colspan="3" style="text-align:left"><input name="txtdocumento" type="text" id="txtdocumento" style="text-align:center" onBlur="rellenar_cad(this.value,15,'doc')" value="<?php print $ls_docmov;?>" size="24" maxlength="15" <?php print $ls_readOnly_doc; ?>>
         <?php
         if ($_SESSION["la_empresa"]["confi_ch"]==0 && $ls_mov_operacion=='CH')
		    {
		?>
      ...
      <td width="127" style="text-align:left"><input name="txtcuenta_scg" type="text" id="txtcuenta_scg" style="text-align:center" value="<?php print $ls_cuenta_scg;?>" size="24" readonly></td>
    </tr>
    <tr>
    ...
    <tr>
      <td height="22" style="text-align:right"></td>
      <td colspan="2" style="text-align:left"><table width="241" border="0" cellpadding="0" cellspacing="0" class="formato-blanco"  style="visibility:hidden">
        <tr>
      ...
      <td colspan="3"  style="visibility:hidden"><input name="txtprovbene" type="text" id="txtprovbene" style="text-align:center" value="<?php print $ls_provbene?>" size="24" readonly>
...
<input name="txtmonto" type="text" id="txtmonto" style="text-align:right" onBlur="javascript:uf_montoobjret(this);javascript:uf_verificar_monto(this);" onKeyPress="return(currencyFormat(this,'.',',',event));" value="<?php print number_format($ldec_montomov,2,",",".");?>" size="24" maxlength="22">	  </div></td>
      <td width="72"><div align="right">M.O.R.</div></td>
      <td width="131">
        <div align="left">
    ...
    <tr>
      ...
      <td colspan="2"><input name="txtftefinanciamiento" type="text" id="txtftefinanciamiento" style="text-align:center" value="<?php print $ls_codfuefin;?>" size="3" maxlength="2" readonly>
      ...
      <td>&nbsp;</td>
      <td align="right"><input name="chkimpvou" type="checkbox" class="sin-borde" id="chkimpvou" onClick="javascript:uf_imprimircartavoucher();" style="width:15px; height:15px" value="1"></td>
      <td><span style="text-align:left">Imprimir voucher</span></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td height="13">&nbsp;</td>
       ...
       <?php
			...
			?>
   <tr>
      <td height="22" colspan="6">&nbsp;&nbsp;<a href="javascript: uf_agregar_dtpre();"></a> </td>
    </tr>
    <tr>
      <td height="22" colspan="6"><div align="center"><?php $io_grid->makegrid($li_rows_spg,$title,$objectSpg,770,'Detalles Presupuestarios',$grid1);?>
         <input name="totspi" type="hidden" id="totspi" value="1">
        <input name="lastspi" type="hidden" id="lastspi" value="1">
       <input name="totpre"  type="hidden" id="totpre"  value="<?php print $li_rows_spg; ?>">
        ...
        <input name="txtcuentaspi1"  type="hidden" id="totpre"  value="">
    ...
    <tr>
      <td height="22" colspan="6">&nbsp;&nbsp; </td>
       ...
       <?php
			...
			if(($ls_mov_operacion=="NC")||($ls_mov_operacion=="DP"))
			{
			?>
    <tr>
      <td height="22" colspan="6">&nbsp;&nbsp;</td>
    ...
    <tr>
      <td height="22" colspan="6"><div align="center">
        ...
        <input name="totpre"  type="hidden" id="totpre"  value="1">
        <input name="txtcuenta1"  type="hidden" id="txtcuenta1"  value="">
        <input name="lastspg" type="hidden" id="lastspg" value="1">
        <input name="delete_spg" type="hidden" id="delete_spg">
		<input name="delete_spi" type="hidden" id="delete_spi">
      </div></td>
    </tr>
	   ...
	   if ((ls_status!='C')&&(ls_status!='O')&&(ls_status!='A'))
	      {
						   ...
						   if (lb_valido)
						      {
							      if(f.txtdiferencia.value=='0,00') {
							    	...
							    	alert("El movimiento contable esta descuadrado debe corregirlo para continuar");	
								  }
							  }
						 }
			        }
			...
			{
				alert("No puede realizar esta operacion el movimiento ya fue Procesado");
			}
		  }
	   else
	      {
...
function ue_imprimir(ls_reporte)
{
	...
	ls_codban=f.txtcodban.value;
	ls_ctaban=f.txtcuenta.value;
	if((f.chkimpco.checked==false) && (f.chkimpvou.checked==false))
	{	
	...
	{
		if(f.chkimpco.checked) {
			f.operacion.value="PRINT_CARTAORDEN";
			f.submit();
		}
		else if(f.chkimpvou.checked) {
			ls_pagina="reportes/"+ls_reporte+"?codban="+ls_codban+"&ctaban="+ls_ctaban+"&numdoc="+ls_numdoc+"&chevau="+ls_chevau+"&codope="+ls_codope;
			window.open(ls_pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
		}
	}
...
function ue_buscar()
{
	...
	f.operacion.value="NUEVO";
	f.submit();
	window.open("sigesp_cat_mov_bancario.php?opener=sigesp_scb_p_detmovbancolote.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width="+x+",height="+y+",left=0,top=0,location=no,resizable=yes");
}
		    ...
		    if ((ls_codban!=""))
		       {
				 ...
				 {
					 if(operacion == 'CH'){
						pagina="sigesp_cat_ctabanco2.php?codigo="+ls_codban+"&hidnomban="+ls_nomban+"&codcon="+valor;	
					 }
					 else {
				 ...
				 {
				 	alert("El Movimiento ya fue Procesado !!!");   
				 }
			   }
		    else
		       {
									 ...
									 if (lb_valido)
										{
										  ...
										  ls_pagina = "sigesp_w_regdt_contable.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+"&tipo="+ls_tipo+
													  ...
													  "&tip_mov= &opener=sigesp_scb_p_movbanco.php&estdoc="+ls_estdoc+"&codfuefin="+ls_codfuefin+"&openneraux="+openneraux+
							  ...
							  if (lb_valido)
								 {
								   ...
								   openneraux="1";
								   ls_pagina = "sigesp_w_regdt_presupuesto.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+"&openneraux="+openneraux+
		   ...
		   if ((ls_status!="C")&&(ls_status!='O')&&(ls_status!='A'))
			  {
							  ...
							  if (lb_valido)
								 {
									...
									ls_pagina = "sigesp_w_regdt_deducciones.php?txtdocumento="+ls_documento+"&txtprocede=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+
												...
												"&tip_mov= &opener=sigesp_scb_p_detmovbancolote.php&estdoc="+ls_estdoc+"&codfuefin="+ls_codfuefin+"&codtipfon="+ls_codtipfon+"&numordpagmin="+ls_numordpagmin+"&estcob="+li_estcob;
				...
				{
					alert("El Movimiento esta Procesado no puede modificarlo !!!");
				}
			  }	
		   else
			  {
...
{   
  ...
  {
  		alert("El movimiento posee detalles contables, no puede tildar esta opción");
		f.nocontabili.checked=false;
  }
	
}

function  uf_imprimircarta()
{   
  if (f.txtnumcarord.value!="")
  {
		f.chkimpco.checked=true;
		
  }
  else
  {
  		alert("Este estatus es solo para Carta Orden!");
		f.chkimpco.checked=false;
		
  }
}

function uf_imprimircartavoucher()
{   
	  if (f.txtnumcarord.value!="")
	  {
			f.chkimpvou.checked=true;
	  }
	  else
	  {
	  		alert("Este estatus es solo para Carta Orden!");
			f.chkimpvou.checked=false;
	  }
}
...
function cat_cheque()
{
	ls_codban=f.txtcodban.value;
  	...
  	if ((ls_codban!="")&&(ls_ctaban!="")) {
		...
		alert('Debe selecionar el banco y la cuenta!')
  	}
  	  	
}

function uf_impedir_cambio()
{
	operacion=f.hidoperacion.value;
	f.cmboperacion.value=operacion;
