...
$la_seguridad=array();
$la_permisos=array();
$arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_p_progpago.php",$ls_permisos,$la_seguridad,$la_permisos);
...
if ($ls_operacion=="NUEVO")
   {
	 ...
	 $object=array();
	 $arrResultado=array();
     $arrResultado=uf_clear_grid($li_totsolpag,$object);
	 $li_totsolpag=$arrResultado["li_totsolpag"];
...
if (($ls_operacion=="CAMBIO_TIPO")||($ls_operacion=="GUARDAR")||($ls_operacion=="CAMBIO_OPERA"))
   {
	...
	$ldec_total_prog=str_replace(".","",$ldec_total_prog);
	$ldec_total_prog=str_replace(",",".",$ldec_total_prog);
	$ldec_total_prog=intval($ldec_total_prog);
		  ...
		  while(!$rs_solpag->EOF)
			   {
				 ...
				 $ld_montotnot=0;
				 $ld_montotnot=$io_propag->uf_load_notas_asociadas($ls_codemp,$ls_numsol,$ld_montotnot);
					  ...
					  if (!empty($ls_numordpagmin) && !empty($ls_codtipfon) && $ls_codtipfon!='----' && $ls_numordpagmin!='-')
					     {
						   ...
						   $ls_dencta="";
						   $arrResultado="";
						   $arrResultado  = $io_propag->uf_load_datos_orden_pago($ls_numordpagmin,$ls_codtipfon,$ls_cuenta,$ls_nomban,$ls_dencta);
						   
						   $ls_banco=$arrResultado["ls_codban"];
						   $ls_cuenta=$arrResultado["ls_ctaban"];
						   $ls_nomban=$arrResultado["ls_nomban"];
						   $ls_dencta=$arrResultado["ls_denctaban"];
						   
						   $ld_totmondis=0;
						   $arrResultado=array();
						   $arrResultado=$io_ctaban->uf_verificar_saldo($ls_banco,$ls_cuenta,$ld_totmondis);
					 	   $ld_totmondis=$arrResultado['ldec_saldo'];
						 }
					  else
					     {
