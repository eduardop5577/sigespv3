	...
	function uf_comprobante_ret($ls_mes,$ls_agno,$ls_probendesde,$ls_probenhasta,$ls_tipo)
	{
		...
		if($lb_valido)
		{
			...
			for($li_i=1;$li_i<=$li_total;$li_i++)
			{
				...
				$ls_mensaje="";
				$arrResultado="";
				$arrResultado=$this->uf_crear_comprobante($ld_fecdesde,$ld_fechasta, $ls_tipo, $ls_codproben, $ls_mensaje);
		...
		for($li_x=1;$li_x<=$li_total;$li_x++)
		{
			...
			{				
				if(!($this->lb_ivacxp)||($this->ls_retid=='0000000003'))
				{
					...
					$arr_resultado="";
					$lb_valido="";
					$arr_resultado=$this->uf_rep_cur_iva_retenido($ls_numdoc, $ldec_ivaretenido,$ldec_porc,$ldec_mor);
					$lb_valido=	$arr_resultado["lb_valido"];			
					$ldec_ivaretenido=	$arr_resultado["ldec_ivaretenido"];			
					if(!$lb_valido)
					{ 
						$lb_valido= false;
					}
				...
				else
				{
					...
					$arr_resultado="";
					$lb_valido="";
					$arr_resultado=$this->uf_rep_cur_iva_retenido($ls_numsol, $ldec_ivaretenido, $ldec_porc,$ldec_mor);
					$lb_valido=	$arr_resultado["lb_valido"];			
					$ldec_ivaretenido=	$arr_resultado["ldec_ivaretenido"];			
					if(!$lb_valido)
					{
