...
if ($ls_operacion=="GUARDAR")
   {
	  ...
	  $ldec_monto				  = str_replace(",",".",$ldec_monto);
          $ldec_montobs=$ldec_monto;
            if($ldec_tascam>1)
            {
                $ldec_montobs=($ldec_monto*$ldec_tascam);
                $ldec_montobs= number_format($ldec_montobs, 2, ".", "");
            }
	  if ($lb_valido_fuente)
	  {
		  if ($lb_valido)
			 {
			   $ls_operacioncon="H";
			   if ($li_cobrapaga==0)
					$lb_valido  = $in_classmovbanco->uf_procesar_dt_contable($arr_movbco,$ls_cuenta,$ls_procedencia,$ls_mov_descripcion,$ls_mov_document,$ls_operacioncon,$ldec_montobs,$ldec_objret,false,'00000');
			   ...
			   $ld_mondeb       = $_POST["monto"];
                           ...
                           if($ldec_tascam>1)
                           {
                               $ld_mondeb=($ld_mondeb*$ldec_tascam);
                               $ld_mondeb= number_format($ld_mondeb, 2, ".", "");
                           }
			   if ($lb_valido)
				  {
