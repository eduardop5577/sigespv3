...
function uf_load_datos_beneficiario($as_codemp,$as_cedbene,$as_nombene,$as_apebene,$ab_valido,$ls_procede)
{
  ...
  if ($rs_data===false)
	...
	$arrResultado['ab_valido']=$ab_valido;
	$arrResultado['rs_datos']=$rs_datos;
  return $arrResultado;
}	
	...
	{		
		...
		for($li_i=1;$li_i<=$li_totalRows;$li_i++)				
		{
			if(array_key_exists("chk".$li_i,$_POST))
			{
			    ...
			    $lb_valido="";
			    $arrResultado="";
				$arrResultado=uf_load_datos_beneficiario($ls_codemp,$ls_cedbene,$ls_nombene,$ls_apebene,$lb_valido,$ls_procede);	
			    $rs_datosbene=$arrResultado["rs_datos"];
			    $lb_valido=$arrResultado["ab_valido"];
				...
				{			
					if($ls_ctaban!="")
					{
						...
						if($li_aux==1)
						{
							...
							$ls_numcarord=$in_class_carta->uf_procesar_movbanco($ls_codban,$ls_cuenta_banco,$ls_numcarord,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,$ls_codpro,$ls_codbene,$ls_nombene,$ldec_montomov,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,0,0,'T','SCBCOR','',$ls_tipo,$ls_numcarord);
						}	
