		...
		{          
			...
			if($totrow>0)
			{
				for($z=1;$z<=$totrow;$z++)
				{
					...
					$object[$z][4]="<input type=text name=txtmonobjret".$z." value='".number_format($ldec_objret,2,',','.')."' id=txtmonobjret".$z." class=sin-borde style=text-align:right  size=10 maxlength=20 onBlur=javascript:uf_calcular_monret($z); onKeyPress=return(currencyFormat(this,'.',',',event));>";
			...
			else
			{
					...
					$object[$z][4]="<input type=text name=txtmonobjret".$z." value='".number_format(0,2,',','.')."' id=txtmonobjret".$z." class=sin-borde style=text-align:right  size=10 maxlength=20 onBlur=javascript:uf_calcular_monret($z); onKeyPress=return(currencyFormat(this,'.',',',event));>";
...
elseif($ls_operacion=="EVALUAR")
{
	...
	for($z=1;$z<=$totrow;$z++)
	{
		...
		if(array_key_exists("chk".$z,$_POST))
		{
				$lb_bool=true;
				$ldec_montoret=$io_evaluate->uf_evaluar($ls_formula,$ldec_objret,$lb_bool);
				$ldec_montoret=round($ldec_montoret,2);
				if($li_status_islr==1)
				{
					$ldec_montoret=$ldec_montoret-$ldec_monded;
				}
