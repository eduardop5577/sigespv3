...
$la_seguridad=Array();
$la_permisos=Array();
$arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_p_procesar_no_contabilizables.php",$ls_permisos,$la_seguridad,$la_permisos);
...
switch ($ls_operacion){
  case 'NUEVO':
	...
	$object=Array();
	$li_totrows="";
    $arrResultado= uf_nuevo($object,$li_totrows);
	$object=$arrResultado["object"];
	$li_totrows=$arrResultado["li_totrows"];
  break; 
  case 'BUSCAR':
		$arrResultado=Array();
		$object=Array();
		$li_totrows="";
    $arrResultado=$io_noncon->uf_cargar_documentos($ls_codemp,$ls_numdoc,$ls_codban,$ls_ctaban,$ls_codope,$ld_fecmov,$object,$li_totrows,$ls_orden,$ls_accion);
		$object=$arrResultado["object"];
		$li_totrows=$arrResultado["li_totrows"];
  break;
  case 'PROCESAR':
