...
$la_seguridad=Array();
$la_permisos=Array();
    $arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_r_retencionesislr.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<script type="text/javascript"  src="../cxp/js/funcion_cxp.js"></script>
...
<?php
	...
	require_once("../cxp/class_folder/sigesp_cxp_c_cmp_islr.php");
	...
	if(li_imprimir==1)
	{
		...
		if(basdatcmp!="")
		{
			...
			if(ls_comprobantes!="")
			{
				...
				f.action="sigesp_scb_r_retencionesislr.php";
				f.submit();
			}
			else
			{
...
function ue_print()
{
	...
	if(li_imprimir==1)
	{
		...
		if(ls_comprobantes!="")
		{
			...
			pagina="../cxp/reportes/"+formato+"?comprobantes="+ls_comprobantes+"&procedencias="+ls_procedencias+"&tiporeporte="+ls_tiporeporte;
...
function ue_search()
{
	...
	ajax.send("catalogo=RETENCIONESISLR&tipproben="+tipproben+"&codprobendes="+codprobendes+"&codprobenhas="+codprobenhas+
			  "&fecdes="+fecdes+"&fechas="+fechas+"&basdatcmp="+basdatcmp);
}
