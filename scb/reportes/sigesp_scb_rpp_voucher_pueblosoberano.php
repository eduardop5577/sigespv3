...
***********************************************************************************/
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  if ((file_exists($as_archivo)) && (filesize($as_archivo)>0))
		 ...
		 {
		   print "<script>";
		   print "alert('No existe!!! Ocurrio un error, por favor cargue de nuevo el cheque (Las medidas seran inicializadas por fallo de lectura y escritura)');";
		   print "close();";
		   print "</script>";
		 }
	...
	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ls_monto,$ls_cedben,$io_pdf)
	{
		/*---------------------------------------------------------------------------------------
		...
		-----------------------------------------------------------------------------------------*/
		//Voucher  
		//$io_pdf->add_texto(124,78,8,substr($ls_chevau,-8));//99.5
		//banco  
		//$io_pdf->add_texto(25,88.5,8,substr($ls_nomban,0,30));//99.5
		//Numero Cuenta (solo mostrara los ultimos 20 digitos del numero)
		$io_pdf->add_texto(73,78,8,$ls_ctaban);//99.5	
		//Numero de cheque(solo mostrara los ultimos 8 digitos del numero)
		$io_pdf->add_texto(124,78,8,substr($ls_numdoc,-8));//99.5		
		//Monto del Cheque
		$io_pdf->add_texto(150,78,8,$ls_monto);//99.5		
		//Nombre del Beneficiario
		$io_pdf->add_texto(10,96,8,$ls_nomproben);//99.5		
		//Cedula del Beneficiario
		$io_pdf->add_texto(145,96,8,$ls_cedben);//99.5		
		$io_pdf->add_texto(30,150,8,"AVISO: VER ORDEN DE PAGO");//99.5		
		//Orden de Pago (solo mostrara los ultimos 6 digitos del numero)
		//$io_pdf->add_texto(15,183,7,"<b>Solicitud de Pago:  ".$ls_solicitudes."</b>");//99.5
		//Concepto
		...
		$la_opciones = array("color_texto" => array(0,0,0),
							   ...
							   "margen_horizontal"=>1,
							   "margen_vertical"=>0.5);
		$io_pdf->add_tabla(8,$la_data,$la_opciones);
		return $io_pdf;
	}// 
	function uf_print_detalle($la_title,$la_data,$li_totdeb,$li_tothab,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$la_datacompleta[$li_i]["scg_cuenta"]="<b>".$la_data[($li_i)]["scg_cuenta"]."</b>";
			$la_datacompleta[$li_i]["spg_cuenta"]="<b>".$la_data[($li_i)]["spg_cuenta"]."</b>";
			$la_datacompleta[$li_i]["debe"]="<b>".$la_data[($li_i)]["debe"]."</b>";
			$la_datacompleta[$li_i]["haber"]="<b>".$la_data[($li_i)]["haber"]."</b>";

		}
		$li_pos=133;
		$io_pdf->convertir_valor_mm_px($li_pos);		
		$io_pdf->ezSetY($li_pos);
//		$la_anchos_col = array(45,65,32,30);
		$la_anchos_col = array(45,75,27,25);
		$la_justificaciones = array("center","left","right","right");
		$la_opciones = array("color_texto" => array(0,0,0),
							   "anchos_col"  => $la_anchos_col,
							   "tamano_texto"=> 7	,
							   "lineas"=>0,
							   ...
							   "margen_horizontal"=>1,
							   "margen_vertical"=>1);
		$io_pdf->add_tabla(5,$la_datacompleta,$la_opciones);

		$io_pdf->add_texto(138,183,7,"<b>".number_format($li_totdeb,2,',','.')."</b>");//99.5	
		$io_pdf->add_texto(168,183,7,"<b>".number_format($li_tothab,2,',','.')."</b>");//99.5	
		return $io_pdf;
	}//
	
	function uf_print_retencion($data,$io_pdf) {
		$li_pos=90;
		$io_pdf->convertir_valor_mm_px($li_pos);
		$io_pdf->ezSetY($li_pos);
		$la_columna=array('sc_cuenta'=>'','denominacion'=>'','monret'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>550,
						 'cols'=>array('sc_cuenta'=>array('justification'=>'center','width'=>100),
						 		       'denominacion'=>array('justification'=>'left','width'=>280),
						 			   'monret'=>array('justification'=>'left','width'=>170))); // Ancho M�ximo de la tabla
		$io_pdf->ezTable($data,$la_columna,'',$la_config);;
		return $io_pdf;
	}
...
if ($ls_tipimp=='lote')
   {
	 ...
	 if ($li_total>0)
		{	
		  ...
		  if(trim($ls_codban)=="019")//Banco Banesco.
			 {
		  ...
		  elseif(trim($ls_codban)=="029")//Banco Industrial.
			 {
		  ...
		  elseif(trim($ls_codban)=="015")//Banco BOD.
			   {
				 $ls_archivo   = "cheque_configurable/medidas_bod.txt";
				 $ls_contenido = "144.00-0.00-24.00-20.00-26.00-30.00-27.00-33.00-22.00-38.00-79.00-38.00-120.00-60.00-130.00-65.00-10.00-91.00-8.00-100.00-12.00-117.00";
				 $li_medidas   = 22;
			   }
		  elseif(trim($ls_codban)=="005")//Banco Bicentenario.
			  {
				 $ls_archivo   = "cheque_configurable/medidas_bicentenario.txt";
				 $ls_contenido = "144.00-0.00-24.00-20.00-26.00-30.00-27.00-33.00-22.00-38.00-79.00-38.00-120.00-60.00-130.00-65.00";
				 $li_medidas   = 16;
			  }
		  elseif(trim($ls_codban)=="008")//Banco de Vzla.
			   {
		  ...
		  elseif (trim($ls_codban)=="017")//Banco Provincial.
			 {
				...
				for ($li_i=1;$li_i<=$li_totrow;$li_i++)
				    {
					  ...
					  $ls_cedben	  = $ds_voucher->data["cedben"][$li_i];
					 ...
					 $io_pdf= uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$ls_fecha,$ls_agnio,$io_pdf,$ls_fecha_corta); // Imprimimos el encabezado de la página
					  $io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,number_format($ldec_total,2,",","."),$ls_cedben,$io_pdf); // Imprimimos la cabecera del registro
								 ...
								 if (array_key_exists("spg_cuenta",$ds_dt_spg->data))
								    {
							          ...
							          $la_data[$li_s]=array('spg_cuenta'=>$ls_denominacion,
									                        'estpro'=>$ls_estpro,
						   ...
						   for ($li_s=1;$li_s<=$li_totrow_spg;$li_s++)
							   {
							     ...
							     $la_data[$li_s]=array('spg_cuenta'=>$ls_denominacion,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab);
							   }
					     }
					 //$io_pdf= uf_print_detalle(array('title'=>'Detalle Presupuestario Pago','title2'=>'Detalle Contable Pago'),$la_data,0,0,$io_pdf);
				    }
			    $io_pdf=uf_print_firmas($io_pdf);	
				if ($i<$li_total)
				   {			
	 ...
	 for ($li_i=1;$li_i<=$li_totrow;$li_i++)
		 {
		   ...
		   if ($li_totrow_spg > 0) {
				...
				for ($li_s=1;$li_s<=$li_totrow_spg;$li_s++) {
					...
					$ls_estpro       = $ds_dt_spg->getValue("estpro",$li_s);	  
					$ldec_monto_spg  = number_format($ds_dt_spg->getValue("monto",$li_s),2,",",".");
					$ls_denominacion = $ds_dt_spg->getValue("denominacion",$li_s);	  
					
					$la_data[$li_d]=array('spg_cuenta'=>$ls_denominacion,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_estpro.", ".$ls_cuentaspg,'debe'=>$ldec_monto_spg,'haber'=>"0,00");
					$li_d++;
				}
			...
			if ($li_totrow_scg > 0){
				for ($li_s=1;$li_s<=$li_totrow_scg;$li_s++) {
					$ls_scg_cuenta   = trim($ds_dt_scg->data["scg_cuenta"][$li_s]);
					$ls_debhab 	     = $ds_dt_scg->data["debhab"][$li_s];
					$ls_denominacion = $ds_dt_scg->data["denominacion"][$li_s];
					$ldec_monto	     = $ds_dt_scg->data["monto"][$li_s];
					...
					if ($ls_debhab=='D') {
						$ldec_mondeb = number_format($ldec_monto,2,",",".");
						$ldec_monhab = "";
						$li_totdeb=$li_totdeb+$ldec_monto;
					}
					else {
						$ldec_monhab = number_format($ldec_monto,2,",",".");
						$ldec_mondeb = "";
						$li_tothab=$li_tothab+$ldec_monto;
					}
							   
					$la_data[$li_d]=array('spg_cuenta'=>$ls_denominacion,'estpro'=>'','monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab);
					$li_d++;
				}
			}
			
			if (!empty($la_data)) {
				//$io_pdf=uf_print_detalle(array('title'=>'<b>Detalle Presupuestario Pago</b>','title2'=>'<b>Detalle Contable Pago</b>'),$la_data,$li_totdeb,$li_tothab,$io_pdf);
			}
			
			$io_pdf=uf_print_retencion($class_report->uf_select_dt_deducciones($ls_solicitudes),$io_pdf);
