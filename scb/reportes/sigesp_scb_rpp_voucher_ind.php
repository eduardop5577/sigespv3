...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_codban,$ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$io_pdf)
	{
		...
		$ls_prov_cortado=wordwrap($ls_nomproben,70,"?");
		$la_arr_prov=array();
		$la_arr_prov=explode("?",$ls_prov_cortado);
		if(array_key_exists(0,$la_arr_prov))
			$io_pdf->add_texto($valores[2],$valores[3],9,"<b>$la_arr_prov[0]</b>");
		if(array_key_exists(1,$la_arr_prov))
			$io_pdf->add_texto($valores[2],$valores[3]+3,9,"<b>$la_arr_prov[1]</b>");
	...
	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf)
	{
		...
		$io_pdf->addText(320,466,9,'<b>'.$ls_nomban.' - '.$ls_ctaban.'</b>');
		$io_pdf->addText(70,466,9,'<b>'.substr($ls_chevau,-10).'</b>');
		$io_pdf->addText(434,505,9,'<b>'.$ls_numdoc.'</b>');
		//print $ls_numdoc."<br>";
	    ...
	    $io_pdf->addText(15,50,9,"<b>".strtoupper($ls_nomusu)."</b>");
		$io_pdf->addText(160,50,9,"<b>TESORERIA</b>");
		$io_pdf->addText(300,50,9,"<b>ADMIN. Y FINANZAS</b>");
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre líneas
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'xPos'=>325, // Orientación de la tabla
						 'width'=>580, // Ancho de la tabla
						 'maxWidth'=>580,
						 'rowGap'=>5,
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('estpro'=>array('justification'=>'right','width'=>150),
									   ...
									   'monto_spg'=>array('justification'=>'right','width'=>80),
						 			   'scg_cuenta'=>array('justification'=>'right','width'=>80), // Justificación y ancho de la columna
				...
				for ($li_i=1;$li_i<=$li_totrow;$li_i++)
				    {
					 ...
					 $io_pdf= uf_print_encabezado_pagina($ls_codban,number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
				 ...
				 for ($li_s=1;$li_s<=$li_totrow_det;$li_s++)
					 {
					   ...
					   $ls_estpro=str_replace("    ","-",$ls_estpro);
					   $la_data[$li_s]=array('spg_cuenta'=>$ls_cuentaspg,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab);
					 }
			   }
			if ($li_totrow_spg>$li_totrow_det)
