...
***********************************************************************************/
	...
	function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->line(272,90,272,148);
		$io_pdf->line(455,90,455,148);		
		$io_pdf->line(191,43,191,75);
		$io_pdf->line(310.5,43,310.5,75);
		$io_pdf->line(410,43,410,75);	
		
		
		$io_pdf->addText(17,177.6,9,'<b>Funcionario:</b>');
		$io_pdf->addText(17,157.6,9,'<b>Lugar de envio:</b>');
		$io_pdf->addText(16,137.6,9,'<b>Elaborado por:</b>');
		$io_pdf->addText(132,137.6,9,'<b>Revisado por:</b>');
		$io_pdf->addText(273,137.6,9,'<b>Jefe de despacho:</b>');		
		$io_pdf->addText(457,137.6,9,'<b>Dirección general:</b>');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$ls_numdoc		= $ds_voucher->data["numdoc"][$li_i];
			$ls_codban		= $ds_voucher->data["codban"][$li_i];
			$ls_codbanben   = $ds_voucher->data["codbanben"][$li_i];
			$ls_nomban		= $class_report->uf_select_data($io_sql,"SELECT nomban FROM scb_banco WHERE codban ='".$ls_codbanben."' AND codemp='".$ls_codemp."'","nomban");
			$ls_ctabanben	= $ds_voucher->data["codctabanben"][$li_i];
			$ls_ctaban		= $ds_voucher->data["ctaban"][$li_i];
			$ls_chevau		= $ds_voucher->data["chevau"][$li_i];
			$ld_fecmov	  	= $io_funciones->uf_convertirfecmostrar($ds_voucher->data["fecmov"][$li_i]);
			$ls_nomproben 	= $ds_voucher->data["nomproben"][$li_i];
			...
			$ls_monto= $numalet->letra();
			$io_pdf=uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
			$io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctabanben,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf); // Imprimimos la cabecera del registro
