...
***********************************************************************************/
	...
	function uf_print_detalle($la_title,$la_data,$la_data_spg,$io_pdf)
	{
		...
		$li_total_data=count((array)$la_data);
		$li_total_data++;
		...
		$li_total_data=count((array)$la_dataaux);
		$li_pos=130;
		$io_pdf->convertir_valor_mm_px($li_pos);
		$io_pdf->ezSetY($li_pos);
		$li_pos_x=98;
	...
	if ($ls_tipimp=='lote')
	{
	   ...
	   //Descompongo la cadena de operaciones en un arreglo tomando como separación el '-'
		...
		{	
			...
			$ldec_totdeb=0;
			$ldec_tothab=0;			
			$io_pdf->selectFont('../../base/librerias/php/ezpdf/fonts/Helvetica-Bold.afm');
			...
			for ($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
				 ...
				 $io_pdf= uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),strtoupper($ls_nomproben),$ls_monto,$ls_fecha,$io_pdf,$y_pos); // Imprimimos el encabezado de la página
				  $io_pdf= uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,'  ',$ls_conmov,$ld_fecmov,$io_pdf); // Imprimimos la cabecera del registro
				  ...
				  if (empty($la_data_spg))
				   ...
				   $io_pdf=uf_print_detalle(array('title'=>'','title2'=>''),$la_data,$la_data_spg,$io_pdf); // Imprimimos el detalle		
				   $io_pdf=uf_print_autorizacion(number_format($ldec_totdeb,2,",","."),number_format($ldec_tothab,2,",","."),$io_pdf);//Muestra los totales de debe y haber	
				} // Fin del for 2
