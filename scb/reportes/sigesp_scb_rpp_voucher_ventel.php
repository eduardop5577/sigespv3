	 ...
	 for ($li_i=1;$li_i<=$li_totrow;$li_i++)
		 {
		   ...
		   $ls_monto= $numalet->letra();
		   $io_pdf=uf_print_encabezado_pagina($ls_codban,number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
		   $io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,number_format($ldec_total,2,",","."),$ls_dencta,$ls_nomban,$io_pdf); // Imprimimos la cabecera del registro
