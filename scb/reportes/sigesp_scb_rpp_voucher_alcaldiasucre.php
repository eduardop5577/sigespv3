...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$ls_numdoc,$ls_nomban,$ls_ctaban,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $valores;
		//Imprimo el numero del Cheque
		//$io_pdf->add_texto($valores[0]-65,$valores[1],10,$ls_numdoc);
		//Imprimo el monto
		...
		$ls_monto_cortado=wordwrap($ls_monto,55,"?");
		$la_arreglo=array();
...
if(array_key_exists(0,$la_arreglo))
		...
		$ls_mes=substr($ls_fecha,12,-5);
		$ls_dia=substr($ls_fecha,8,-8);
		if ($ls_mes==01)
		{$ls_mesletra="Enero";}
		if ($ls_mes==02)
		{$ls_mesletra="Febrero";}
		...
		$ls_fecha_letra=$ls_lugar." ".$ls_dia." de ".$ls_mesletra." ".$ls_anio;
		$ls_fecha_corta=substr($ls_fecha,0,(strlen($ls_fecha)-5));	
		//Fecha del Cheque
		$io_pdf->add_texto($valores[8],$valores[9],10,"<b>$ls_fecha_letra</b>");
		//$io_pdf->add_texto($valores[10],$valores[11],10,"<b>$ls_anio</b>");	
		$io_pdf->add_texto($valores[12],$valores[13],14,"<b> **NO ENDOSABLE**</b>");
		$io_pdf->add_texto($valores[14],$valores[15],10,"<b>CADUCA A LOS ".$_SESSION["la_empresa"]["diacadche"]." DIAS</b>");
		// $io_pdf->add_texto($valores[2],$valores[1],9,'CTA. No. : '.$ls_ctaban); //Numero de cuenta 
	        $io_pdf->addText(80,520,11,$ls_ctaban);
                $io_pdf->addText(230,520,11,'Cheque No. : '.$ls_numdoc);
		$io_pdf->addText(80,500,11,'Banco: '.$ls_nomban);
	return $io_pdf;

	}// end function uf_print_encabezadopagina.
	//function uf_print_cabecera($ls_numdoc,$ls_valor,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf)
	function uf_print_cabecera($ls_ctaban,$ls_conmov,$ldec_total,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////		//////////////////////////////////////////////////////////////////////////////////
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				...
				'width'=>380, // Ancho de la tabla
				'maxWidth'=>380,
				'cols'=>array('cuenta'=>array('justification'=>'center','width'=>100))); // Ancho M�ximo de la tabla
	...
	function uf_print_cabecera($ls_ctaban,$ls_conmov,$ldec_total,$io_pdf)
	{
		...
		$io_pdf->convertir_valor_mm_px($li_pos);
		$io_pdf->ezSetY($li_pos);
		$la_data=array(array('cuenta'=>' ','ordenes'=>' ','valor'=>'  '),
				array('cuenta'=>$ls_ctaban,'ordenes'=>$ls_conmov,'valor'=>$ldec_total));
		$la_columna=array('cuenta'=>'','ordenes'=>'','valor'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				'showLines'=>0, // Mostrar L�neas
				'shaded'=>0, // Sombra entre l�neas
				'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
				...
				'maxWidth'=>580,
				'cols'=>array('cuenta'=>array('justification'=>'left','width'=>300),'ordenes'=>array('justification'=>'left','width'=>300),
				'cuenta'=>array('justification'=>'rigth','width'=>100),'valor'=>array('justification'=>'rigth','width'=>160))); // Ancho M�ximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	}// end function uf_print_cabecera.
...
*/
	...
	function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->addText(384,207.6,9,'<b>Administraci�n</b>');		
		$io_pdf->addText(508,207.6,9,'<b>Presidencia</b>');
		$io_pdf->addText(258,148.85,10,'<b>Recib� Conforme</b>');
		
		$io_pdf->addText(16,133.27,10,'<b>Nombre:</b>');		
		$io_pdf->addText(196,133.27,10,'<b>C�dula de Identidad:</b>');		
		...
		{	
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$io_pdf=uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$ls_numdoc,$ls_nomban,$ls_ctaban,$io_pdf); // Imprimimos el encabezado de la p�gina
					
					//uf_print_cabecera($ls_numdoc,$ldec_monto,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf); // Imprimimos la cabecera del registro
					$io_pdf=uf_print_cabecera($ls_ctaban,$ls_conmov,$ldec_total,$io_pdf);
