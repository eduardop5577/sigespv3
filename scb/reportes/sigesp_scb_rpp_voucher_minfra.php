			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				if ($ls_codban=='001')//Banco Industrial de Venezuela.
			   {
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$io_pdf =uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$ls_fecha,$ls_agnio,$io_pdf,$ls_fecha_corta); // Imprimimos el encabezado de la p�ina
					...
					else
					{
						...
						{				
							...
							if($lb_print || $li_i==$x)
							{
								$io_pdf =uf_print_detalle(array('title'=>'Detalle Presupuestario Pago','title2'=>'Detalle Contable Pago'),$la_aux,$io_pdf); // Imprimimos el detalle 								
								$la_aux=array();
								if($li_i<$x)
									$io_pdf->ezNewPage();
							}
