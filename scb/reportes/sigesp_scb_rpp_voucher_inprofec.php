...
***********************************************************************************/
	...
	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf,$ls_fecha_corta)
	{
		/*---------------------------------------------------------------------------------------
		...
		-----------------------------------------------------------------------------------------*/
		//Voucher  
		$io_pdf->add_texto(150,88,8,substr($ls_chevau,-8));//99.5
		//fecha (Fecha del cheque vaucher)
		$io_pdf->add_texto(150,96,8,"<b>".$ls_fecha_corta."</b>");

		//banco  
		$io_pdf->add_texto(25,88.5,8,substr($ls_nomban,0,50));//99.5
		//Numero Cuenta (solo mostrara los ultimos 20 digitos del numero)
		$io_pdf->add_texto(25,92.5,8,$ls_ctaban);//99.5	
		//Numero de cheque(solo mostrara los ultimos 8 digitos del numero)
		$io_pdf->add_texto(25,96.5,8,substr($ls_numdoc,-8));//99.5		
		//Orden de Pago (solo mostrara los ultimos 6 digitos del numero)
		$io_pdf->add_texto(25,120,8,"<b>Solicitud de Pago:  ".$ls_solicitudes."</b>");//99.5
		//Concepto
		...
		$la_opciones = array("color_texto" => array(0,0,0),
							   ...
							   "margen_horizontal"=>1,
							   "margen_vertical"=>0.5);
		$io_pdf->add_tabla(20,$la_data,$la_opciones);
		return $io_pdf;
	}// 
				...
				for ($li_i=1;$li_i<=$li_totrow;$li_i++)
				    {
					 ...
					 $io_pdf= uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$ls_fecha,$ls_agnio,$io_pdf,$ls_fecha_corta); // Imprimimos el encabezado de la página
					  $io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf,$ls_fecha_corta); // Imprimimos la cabecera del registro
