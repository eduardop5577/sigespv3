...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina2($io_pdf,$count)
	{
		/*----------------------------------------------------------------------------------------
		...
		$io_pdf->addText(320,725,11,"<b>ANEXO</b>"); // Agregar el t?ulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->setStrokeColor(0,0,0);		
		$io_pdf->Rectangle(11,13,580,105);
		$io_pdf->line(11,70,590,70);
		$io_pdf->line(11,54.6,590,54.6);		
		$io_pdf->line(160,70,160,118);
		$io_pdf->line(300,70,300,118);
		$io_pdf->line(460,70,460,118);		
		$io_pdf->line(191,13,191,54);
		$io_pdf->line(310.5,13,310.5,54);
		$io_pdf->line(411,13,411,54);	
		
		$io_pdf->addText(16,109,9,'<b>Elaborado por:</b>');
		$io_pdf->addText(165,109,9,'<b>Contabilizado por:</b>');
		$io_pdf->addText(305,109,9,'<b>Administración</b>');		
		$io_pdf->addText(465,109,9,'<b>Presidencia</b>');
		$io_pdf->addText(258,58.85,10,'<b>Recibí Conforme</b>');
		
		$io_pdf->addText(16,43.27,9,'<b>Nombre:</b>');		
		$io_pdf->addText(196,43.27,9,'<b>Cédula de Identidad:</b>');		
		$io_pdf->addText(316,43.27,9,'<b>Fecha:</b>');
		$io_pdf->addText(416,43.27,9,'<b>Firma:</b>');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
		retun $io_pdf;
...
//		$io_pdf->selectFont('../../base/librerias/php/ezpdf/fonts/Helvetica.afm'); // Seleccionamos el tipo de letra
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$io_pdf=uf_print_detalle(array('title'=>'Detalle Presupuestario Pago','title2'=>'Detalle Contable Pago'),$la_data,$io_pdf);
			if (count((array)$la_data)>6)
			{
				$io_pdf=uf_print_encabezado_pagina2($io_pdf,$count);
			}
		}
		//$io_pdf->ezStopPageNumbers(1,1);
		$io_pdf->ezStream();
