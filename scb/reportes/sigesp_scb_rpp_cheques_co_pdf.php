...
//			$io_pdf->add_texto(10,60,9,"<b>TODAS LAS FECHAS</b>");		
	...
	function uf_print_detalle($la_dataimprimir,$io_pdf)
	{
		...
		$io_pdf->add_tabla(7,$la_data,$la_opciones);
		//Datos
		$li_totrow=count((array)$la_dataimprimir);
		$la_dataaux=array();
	...
	if ($ls_conchk==1)
	{
		...
		for($li_p=0;$li_p<$li_total;$li_p++)
		{
			...
			$lb_valido=true;
			$rs_data            = $io_report->uf_cargar_documentos_ch_co($ls_codope,$ld_fecdesde,$ld_fechasta,$ls_codban,$ls_ctaban,$ls_codconcep,$ls_estatus,$ls_orden,$ls_numdocmov,$ls_conchk);
			if($lb_valido)
			{
