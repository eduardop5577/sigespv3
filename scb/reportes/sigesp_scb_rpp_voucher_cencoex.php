...
***********************************************************************************/
	...
	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ldec_monto,$io_pdf)
	{
		...
		$y=470; 
		$wrap=$io_pdf->addTextWrap(15,$y,325,9,"$ls_conmov",full);//aki inicializamos la variable ke contendrá el texto
		...
		while($wrap!="") //condicional ke se cumple hasta ke ya no hay mas palabras a imprimir en el explorador
		{
			...
			$wrap=$io_pdf->addTextWrap(15,$y,325,9,"$wrap",full); // imprime y almacena lo ke kedó del mensaje 
		}

		//monto a pagar
		$io_pdf->addText(390,360,13,number_format($ldec_monto,2));
		//nuemro del documento
		$io_pdf->addText(5,325,13,$ls_numdoc);
		//nombre del banco
		$io_pdf->addText(110,360,13,$ls_nomban);
		//numero de cuenta
		$io_pdf->addText(100,340,13,$ls_ctaban);
		//modifica el codigo del voucher
		$io_pdf->addText(300,520,13,$ls_chevau);
		//modifica nombre proveedor
		//$io_pdf->addText(350,340,9,$ls_nomproben);
		$io_pdf->addText(1,240,13,$_SESSION["la_nomusu"]." ".$_SESSION["la_apeusu"]);
