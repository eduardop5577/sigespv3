...
function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$io_pdf)
	...
	function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->setStrokeColor(0,0,0);		
		$io_pdf->Rectangle(11,43,580,80);
		$io_pdf->line(11,100,590,100);		
		$io_pdf->line(200,40,200,123);
		$io_pdf->line(400,40,400,123);
		$io_pdf->addText(20,110.6,9,'<b>Realizado y Elaborado por:</b>');
		$io_pdf->addText(230,110.6,9,'<b>Direccion de Administracion:</b>');		
		$io_pdf->addText(480,110.6,10,'<b>Presindencia:</b>');		
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
		return $io_pdf;
...
for($li_i=1;$li_i<=$li_totrow;$li_i++)
{
	...
	if($ls_numcarord!=""){
		$io_pdf=uf_print_cabecera($ls_numcarord,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,$ls_operacion,$io_pdf); // Imprimimos la cabecera del registro
	}
	else{
		$io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,$ls_operacion,$io_pdf); // Imprimimos la cabecera del registro
	}
