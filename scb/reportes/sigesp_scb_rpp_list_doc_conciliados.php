...
***********************************************************************************/
	...
	if($ls_estatus!="")
	{
		$arr_estatus=explode("-",$ls_estatus);
		$li_totestatus=count((array)$arr_estatus);
	}	
	else
	{
		$li_totestatus=0;
	...
	if($li_total>0)
	{
		...
		$io_pdf=uf_print_encabezado_pagina($ls_titulo,$ls_titulo_estatus,$ls_fecdesde,$ls_fechasta,$ls_denban,$ls_ctaban,$ls_dencta,$io_pdf); // Imprimimos el encabezado de la página
		...
		for($i=1;$i<=$li_total;$i++)
		{
			...
			$io_pdf=uf_print_detalle($la_data,$la_columna,$la_config,$io_pdf);
			unset($la_data);
			...
			$io_pdf=uf_print_detalle($la_data_con,$la_columna_con,$la_config_con,$io_pdf);
		}
