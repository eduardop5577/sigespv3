...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$io_pdf,$x)
	{
		...
		//$io_pdf->add_texto($valores[4],$valores[5],10,"<b>$ls_monto</b>");
		//Cortando el monto en caso de que sea muy largo		
		$ls_monto_cortado=wordwrap($ls_monto,83,-3);
		...
		$io_pdf->add_texto($valores[14],$valores[13],12,"Caduca a los 90 Días");
		//no endosable
		//$io_pdf->add_texto($valores[12],$valores[13],12,"NO ENDOSABLE");
		return $io_pdf;
	...
	function uf_print_autorizacion($io_pdf)
	{
	    ...
	    $ls_preparado  = 'TSU. LEIDY BLANCO';
		$ls_revisado   = 'LIC. PABLO JESUS BARRENO R.';
		$ls_autorizado = 'LIC. ELVIS ENRIQUE UFRE M.';
		...
		{	
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$io_pdf=uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf,$y_pos); // Imprimimos el encabezado de la página
					$io_pdf=uf_print_cabecera(array('title'=>'<b>Registro Presupuestario Pago</b>','title2'=>'<b>Registro Contable Pago</b>'),$ls_numdoc,$ls_nomban,$ls_ctaban,$ls_denctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,number_format($ldec_total,2,",","."),$ls_monto,$io_pdf); // Imprimimos la cabecera del registro
					...
					$io_pdf=uf_print_detalle($la_data,$io_pdf,$x_pos); // Imprimimos el detalle 	
				}
