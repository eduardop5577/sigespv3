...
$numalet->setSufijo("***");
	...
	for($li_i=1;$li_i<=$li_totrow;$li_i++)
	{
		...
		if (!empty($ls_solicitudes))
		   {
			 ...
			 $li_totrowsol   = count((array)$la_numsolpag);
		   }
		...
		if ($li_totrowsol>0)
		   {
		     ...
		     $ld_montotiva="";
		     $ls_tipretislr="";
		     $arrResultado = $class_report->uf_load_retenciones_cxp($ls_codemp,$ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ls_estmov,$ls_tipretislr,"IVA");
			  $ld_montotiva=$arrResultado["ld_montotret"];
			  $ls_tipretislr=$arrResultado["as_tipretislr"];
			 if ($ld_montotiva>0)
			    {
		     ...
		     $ld_montotislr="";
		     $ls_tipretislr="";
			 $arrResultado = $class_report->uf_load_retenciones_cxp($ls_codemp,$ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ls_estmov,$ls_tipretislr,"ISLR");
		     $ld_montotislr=$arrResultado["ld_montotret"];
		     $ls_tipretislr=$arrResultado["as_tipretislr"];
			 if ($ld_montotislr>0)
				{
		     ...
		     if ($ldec_monret>0)
			    {
		          ...
		          $arrResultado = $class_report->uf_load_retenciones_scb($ls_codemp,$ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ls_estmov,$ls_tipretislr,"IVA");
				  $ld_montotiva=$arrResultado["ld_montotret"];
				  $ls_tipretislr=$arrResultado["as_tipretislr"];
				  if ($ld_montotiva>0)
					 {
				  ...
				  if ($ld_montotiva<$ldec_monret)//Realizamos esta verificacion para ver si hace falta buscar retenciones de islr.
				     {
					   ...
					   $arrResultado = $class_report->uf_load_retenciones_scb($ls_codemp,$ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ls_estmov,$ls_tipretislr,"ISLR");
					   $ld_montotislr=$arrResultado["ld_montotret"];
					   $ls_tipretislr=$arrResultado["as_tipretislr"];
					   if ($ld_montotislr>0)
					      {
		...
		$io_pdf=uf_print_cabecera($la_datsolpag,$ls_conmov,$io_pdf); // Imprimimos la cabecera del registro
		...
		if ($li_totrow_spg>$li_totrow_det)
		   {
		     ...
		     $li_y = 0;
			 $li_totrows = count((array)$la_data);
		...
		$io_pdf=uf_print_retenciones($la_dataret,$io_pdf);
	}
	$io_pdf->ezStream();
