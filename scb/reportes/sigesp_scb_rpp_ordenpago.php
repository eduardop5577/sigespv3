...
***********************************************************************************/
	...
	$ls_codestpro=str_pad($ls_codestpro,25,"0",'LEFT');
...
//--------------------------Fila 3-------------------------------------//
 ...
 $la_opciones = array("color_texto" => array(0,0,0),
  ...
  {
		$li_row=$li_i+1;
		$la_data[$li_i]["ae"]=substr($ds_dt_spg->getValue("estpro",$li_row),21,6);
...
{ 
 ...
 $io_pdf->add_tabla(26,$la_data,$la_opciones);
  ...
  while($li_i<count((array)$la_lineasfila))
  {	  
	  $la_aux=array();
	  $li_j=0;
	  $li_altotabla=0;
	  $li_disponible=$io_pdf->get_alto_disponible()-30;
	  while(($li_altotabla<$li_disponible) && $li_i<count((array)$la_lineasfila))
	  {
	 ...
	 //break;
	 if($li_i+1<count((array)$la_lineasfila))
	 {
		  //-------------Marco de la Pagina y encabezado----------------------//
		 $io_pdf->ezNewPage();
		 $la_opciones = array("color_fondo" => array(0,0,0),//para que me pinte de nuevo las lineas color negro
