...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_numdoc,$as_fecha,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],30,310,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->Rectangle(135,320,450,40);
		$io_pdf->line(380,340,585,340);		
		$io_pdf->line(380,360,380,320);		
		$io_pdf->addText(390,342,11,"<b>Nro.:</b>     ".$as_numdoc); // Nmero de Orden de compra
		$io_pdf->addText(220,335,13,"<b>CARTA ORDEN</b>"); // Agregar el t?ulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
		//Fecha actual
		$io_pdf->addText(390,323,10,"<b>".$as_fecha."</b>"); // Agregar el t?ulo
	...
	function uf_print_detalle($la_data,$io_pdf,$x_pos,$as_tipproben)
	{
		...
		if ($as_tipproben=='B')
		   {
			 $la_config=array('showHeadings'=>1, // Mostrar encabezados
							  ...
							  'cols'=>array('cedbene'=>array('justification'=>'center','width'=>60),// Justificación y ancho de la columna
										    ...
										    'ctaban'=>array('justification'=>'center','width'=>110), // Justificación y ancho de la columna
		...
		$io_pdf->ezText('                     ',8);//Inserto una linea en blanco
...
//	$ls_codigo=$_GET["codigo"];	
	$ls_codigo=$class_report->uf_buscar_seleccionado();
	...
	if(!array_key_exists("opener",$_GET))
	{
		...
		$li_cont=count((array)$la_cartaorden["monto"]);
		$li_totales=0;
		for ($i=1;$i<=$li_cont;$i++)
		{
		...
		if($la_cartaorden["tipo_destino"][1]=='P')
		{
			...
			$la_campo["bancoreceptor"]=$arrResultado["as_nomban"];
			$la_campo["cuentaproveedor"]=$arrResultado["as_ctaban"];
		}
		else
		{
			...
			$la_campo["nombre"] = $la_cartaorden["nomproben"][1];
			$arrResultado="";
			$arrResultado=$class_report->uf_select_banco_cuenta("rpc_beneficiario",$ls_criterio,$la_campo["bancoemisor"],$la_campo["cuentabancariaemisor"],$la_campo["rif"]);
			$la_campo["bancoemisor"]=$arrResultado["as_nomban"];
			$la_campo["cuentabancariaemisor"]=$arrResultado["as_ctaban"];
			$la_campo["rif"]=$arrResultado["as_rif"];
			$la_campo["bancoreceptor"]=$arrResultado["as_nomban"];
	...
	uf_print_texto($la_data["encabezado"][1],$io_pdf,320);
	...
	uf_print_detalle($la_data_dt,$io_pdf,320,$ls_tipproben); // Imprimimos el detalle 	
