...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$io_pdf)
	{
		...
		global $valores;
		//Imprimo el monto
		$io_pdf->add_texto($valores[0],$valores[1],12,"***".$ldec_monto."***");
		//Beneficiario del Cheque
		
		$salto = 0;
        $tamano_fuente=9;
		$n = strlen($ls_nomproben);
		$cx = 70;
        if($n>70){
			for($i=70;$i>0;$i--){						  
				if($ls_nomproben[$i]==' '){$cx = $i; $ls_nomproben[$i]='';break;}
			}			
			$linea1 = substr($ls_nomproben,0,$cx);
			$linea2=substr($ls_nomproben,$cx);
                        $ls_nomproben=$linea1."\n".$linea2;
			$salto = 3;
		}
		if($salto>0){$tamano_fuente=8;}else{$tamano_fuente=10;}
		$io_pdf->add_texto($valores[2],($valores[3]-$salto),$tamano_fuente,"<b>$ls_nomproben</b>");
		...
		if(array_key_exists(0,$la_arreglo))
			$io_pdf->add_texto($valores[4],$valores[5],10,"$la_arreglo[0]");
		if(array_key_exists(1,$la_arreglo))
			$io_pdf->add_texto($valores[6],$valores[7],10,"$la_arreglo[1]");
	...
	function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->line(157,60,157,130);		
		$io_pdf->line(300,60,300,130);		
		$io_pdf->line(442,60,442,130);		
		$io_pdf->addText(55,122,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(57,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(200,122,7,"REVISADO POR"); // Agregar el título
		$io_pdf->addText(200,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(330,122,7,"APROBADO (GERENCIA)"); // Agregar el título
		$io_pdf->addText(340,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(480,122,7,"MAXIMA AUTORIDAD"); // Agregar el título
		$io_pdf->addText(485,63,7,"FIRMA / SELLO"); // Agregar el título

/*		$io_pdf->Rectangle(11,43,580,105);
		$io_pdf->line(11,90,590,90);
		$io_pdf->line(11,74.6,590,74.6);		
		$io_pdf->line(128,90,128,148);
		$io_pdf->line(290,90,290,148);
		//$io_pdf->line(359,90,359,148);
		$io_pdf->line(450,90,450,148);		
		$io_pdf->line(191,43,191,75);
		$io_pdf->line(315,43,315,75);
		$io_pdf->line(415,43,415,75);	
		...
		$io_pdf->addText(298,137.6,9,'<b>Autorizado por:</b>');		
		$io_pdf->addText(460,137.6,9,'<b>Aprobado por:</b>');
		...
		$io_pdf->addText(320,63.27,10,'<b>Fecha:</b>');
		$io_pdf->addText(420,63.27,10,'<b>Firma:</b>');*/
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	/*function uf_print_autorizacion($io_pdf)
	{
		...
		$io_pdf->addText(132,137.6,9,'<b>Revisado por:</b>');
		//$io_pdf->addText(273,137.6,9,'<b>Presupuesto</b>');		
		$io_pdf->addText(314,137.6,9,'<b>Autorizado por:</b>');		
		$io_pdf->addText(485,137.6,9,'<b>Aprobado por:</b>');
		$io_pdf->addText(258,78.85,10,'<b>Recibí Conforme</b>');
...
if(!$_GET['codban']){
	     echo "<script language=JavaScript>";
		 echo "alert('Debe seleccionar un banco');"	;	
		 echo "close();";
		 echo "</script>";	
		 exit();
}
...
switch($_GET['codban']){
		...
		case '001':
			...
			$ls_contenido="138.00-8.00-35.00-24.00-35.00-31.00-32.00-34.00-35.00-44.00-86.00-44.00-250.00-58.00-250.00-63.00";
			$li_medidas=22;
			break;
	...
	if ($ls_tipimp=='lote')
	{
	   ...
	   //Descompongo la cadena de operaciones en un arreglo tomando como separación el '-'
		...
		{	
			...
			$io_pdf->selectFont('../../base/librerias/php/ezpdf/fonts/Courier.afm');//Seleccionamos el tipo de letra			
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$beneftxt = $ds_voucher->data["nomproben"][$li_i];
					$benefalt = $ds_voucher->data["benefalt"][$li_i];	
					if($benefalt){$beneftxt .= "\n".'<b>BENEF. ALT:</b>'.$benefalt;}
					$ls_nomproben = ($benefalt)?$benefalt:$ls_nomproben;
					
					$io_pdf=uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
					
					$io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$beneftxt,$ls_solicitudes,$ls_conmov,$io_pdf); // Imprimimos la cabecera del registro
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		if(!$li_totrow){
			 echo "<script language=JavaScript>";
			 if($ls_numdoc){echo "alert('No se encontro el Cheque. Revise si fue anulado o el número es correcto.');"	;}
			 else{echo "alert('No se encontro el Cheque');"	;}	
			 echo "close();";
			 echo "</script>";	
			 exit();
		}
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$ls_numdocx = $ls_numdoc;
			$ls_nombanx = $ls_nomban;
			$ls_ctabanx = $ls_ctaban;
			$ls_chevaux = $ls_chevau;
			$ls_nomprobenx = $ls_nomproben;
			$ls_solicitudesx = $ls_solicitudes;
			$ls_conmovx = $ls_conmov;
			...
			$ldec_totalx = $ldec_total;
			$ls_nomprobenx = $ls_nomproben;
			$ls_montox = $ls_monto;
			$ld_fecmovx = $ld_fecmov;
		...
		$io_pdf=uf_print_autorizacion($io_pdf);
		$io_pdf=uf_print_encabezado_pagina(number_format($ldec_totalx,2,",","."),$ls_nomproben,$ls_montox,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmovx,$io_pdf);
		$io_pdf=uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$beneftxt,$ls_solicitudes,$ls_conmov,$io_pdf);
		$io_pdf=uf_print_detalle(array('title'=>'Detalle Presupuestario Pago','title2'=>'Detalle Contable Pago'),$la_data,$io_pdf);
