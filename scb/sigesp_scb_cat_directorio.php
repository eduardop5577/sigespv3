...
***********************************************************************************/
   ...
   function uf_print($as_ruta)
   {
		...
		print "</tr>";
		if(		$handle = opendir($as_ruta))
		{		
			while ($file = readdir($handle))
			{
				 if(($file != '.') && ($file != '..'))
				 {
					if(($file!="accord_ticket.xls")&&($file!="accord_tarjeta.xls")&&($file!="valeven_ticket.xls"))
					{
						print "<tr>";
						print "<td width='360' height='22'><div align='left'>".$file."</div></td>";
						print "<td width='70' height='22'><div align='center'><a href='sigesp_scb_cat_descarga.php?enlace=".$as_ruta."&file=".$file."'><img src='../shared/imagebank/tools20/download.gif' width='20' height='20' border='0'></a></div></td>";
...
function ue_delete(ruta,file)
{
	...
	{
		f=document.form1;
		f.action="sigesp_scb_cat_directorio.php?ruta="+ruta+"&file="+file+"";
		f.submit();
	}
}
</script>
