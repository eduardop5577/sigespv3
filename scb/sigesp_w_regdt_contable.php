...
else
{
	...
	$ls_opener   ="sigesp_scb_p_movbanco.php";
...
if ($ls_operacion=="GUARDAR")
   {
	  ...
	  if ($ls_anticipo=='1')
	  ...
	  else
		 {
			...
			$lb_valido="";
			$ls_numconint="";
		   $arrResultado=$in_classmovbanco->uf_guardar_automatico($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ld_fecha,$ls_mov_descripcion,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_nomproben,$ldec_monto_mov,$ldec_objret, $ldec_retenido,$ls_chevau,$ls_estmov,$li_estint,$li_cobrapaga,$ls_estbpd,$ls_mov_procede,' ',$ls_estdoc,$ls_tipo,$ls_codfuefin,$ls_numordpagmin,$ls_codtipfon,$li_estcob,$ls_numconint,$ls_reglibro,$ls_codmon,$ldec_tascam,$ldec_montot);
	  ...
	  if ($lb_valido)
		 {
		   $lb_valido       = $in_classmovbanco->uf_procesar_dt_contable($arr_movbco,$ls_cuenta_scg,$ls_procedencia,$ls_mov_descripcion,$ls_mov_document,$ls_operacioncon,$ldec_montobs,$ldec_objret,true,$ls_codded);
...
<table width="567" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <td height="22"><input name="txtcuenta" type="text" id="txtcuenta" value="<?php print $ls_cuentaplan ?>" size="20" style="text-align:center" readonly> 
  ...
  <tr>
    ...
    <td height="22"><input name="operacion" type="hidden" id="operacion">
      ...
      <input name="openneraux" type="hidden" id="openneraux" value="<?php print  $ls_openneraux;?>">
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	f.estdoc.value=fop.status_doc.value;
	f.codconmov.value=fop.ddlb_conceptos.value;
	if (f.mov_operacion.value=="CHEQUE")
	{
		f.mov_operacion.value="CH";
	}	
	//descripcion=fop.txtconcepto.value;
	f.txtdescripcion.value=fop.txtconcepto.value;
