...
class sigesp_scb_c_cmp_ret_iva
{
	...
	public function __construct($as_retid)
	{
	...
	function uf_comprobante_ret($ls_mes,$ls_agno,$ls_probendesde,$ls_probenhasta,$ls_tipo,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			for($li_i=1;$li_i<=$li_total;$li_i++)
			{
				...
				$arrResultado=$this->uf_crear_comprobante($ld_fecdesde,$ld_fechasta,$ls_tipo,
				                                       $ls_codproben,$ls_mensaje,$aa_seguridad);
				$ls_mensaje=$arrResultado["ls_mensaje"];
	...
	function uf_crear_comprobante($ld_fecdesde,$ld_fechasta, $ls_tipo, $ls_codproben,$ls_mensaje,$aa_seguridad)
	{
		...
		$lb_valido="";
		$arr_datos_proben="";
		$arrResultado=$this->uf_inf_source($ls_tipo,$ls_codproben,$arr_datos_proben);
		$lb_valido=$arrResultado["lb_valido"];
		$arr_datos_proben=$arrResultado["arr_proben"];
		if(!$lb_valido)
		{
		...
		$lb_valido="";
		$ls_nrocomp="";
		$arrResultado=$this->uf_ccr_get_nro($ls_periodofiscal,$ls_nrocomp);
		$lb_valido=$arrResultado["lb_valido"];
		...
		for($li_x=1;$li_x<=$li_total;$li_x++)
		{
					...
					{				
						if(!($this->lb_ivacxp)||($this->ls_retid=='0000000003'))
						{
							...
							$lb_valido="";
							$ldec_ivaretenido="";
							$arrResultado=$this->uf_rep_cur_iva_retenido($ls_numdoc,$ldec_ivaretenido,$ldec_porc,$ldec_mor);
							$lb_valido=$arrResultado["lb_valido"];
							$ldec_ivaretenido=$arrResultado["ldec_ivaretenido"];
							if(!$lb_valido)
							{ 
							    $lb_valido=false;
							}
						...
						else
						{
							...
							$lb_valido="";
							$ldec_ivaretenido="";
							$arrResultado=$this->uf_rep_cur_iva_retenido($ls_numsol, $ldec_ivaretenido, $ldec_porc,$ldec_mor);
							$lb_valido=$arrResultado["lb_valido"];
							$ldec_ivaretenido=$arrResultado["ldec_ivaretenido"];
							if(!$lb_valido)
							{
						...
						if($lb_continue)
						{
							...
							if(!is_null($ls_numsol)||(!empty($ls_numsol)))
							{
								...
								{								  
									$arrResultado="";
									$arrResultado = $this->uf_ccr_guarda_inf_clc($lb_firsttime, $ls_periodofiscal, $ls_nrocomp, 
									             $arr_datos_proben,$ldec_ivaretenido,$ls_numsol,$ls_Concepto,$ldec_porc, 
												 $ldec_mor,$aa_seguridad);
									$lb_firsttime=	$arrResultado["lb_firsttime"];								
									$lb_valido=	$arrResultado["lb_valido"];								
								}
								else
								{
		...
		$arrResultado["ls_mensaje"]=$ls_mensaje;
	...
	{			
		...
		$arrResultado["lb_valido"]=$lb_valido;
		$arrResultado["ldec_ivaretenido"]=$ldec_ivaretenido;
		return $arrResultado;	
	}
	...
	{	
		...
		//////////////////////////////////////////////////////////////////////////////
			...
			else
			{
				...
				$arrResultado["arr_proben"]=$arr_proben;
				$arrResultado["lb_valido"]=$lb_valido;
				return $arrResultado;				
			}
	}
	...
	{		
		...
		{	
			...
			{				
				...
				$ldec_basei="";				
				$ldec_impuesto="";
				$arrResultado=$this->uf_rep_montos($ls_numsol,$ls_codcar,$ls_numdoc,$ldec_basei,$ldec_impuesto);				
				$ldec_basei=$arrResultado["ldec_basei"];				
				$ldec_impuesto=$arrResultado["ldec_impuesto"];	
	...
	$arrResultado["lb_valido"]=$lb_valido;	
	$arrResultado["lb_firsttime"]=$lb_firsttime;	
	return $arrResultado;	
    }
	...
	{		
		...
		$arrResultado["ldec_basei"]=$ldec_basei;
		$arrResultado["ldec_impuesto"]=$ldec_impuesto;
		$arrResultado["lb_valido"]=$lb_valido;
		return $arrResultado;
	}
	...
	function uf_select_cmp_ret($as_numcom,$as_codret,$object,$li_total_rows)
	{
		...
		$arrResultado["li_total_rows"]=$li_total_rows;
		return $arrResultado;
	}		
	                                  ...
	                                  $as_numsol,$totant,$ar_datosgrid,$aa_seguridad)
	{
		...
		//////////////////////////////////////////////////////////////////////////////
		 ...
		 $arrResultado="";
		 $arr_proben="";
		 $arrResultado=$this->uf_inf_source($arr_datos_probene["tipo"],$arr_datos_probene["codigo"],$arr_proben); 	
		$lb_valido=$arrResultado["lb_valido"];
		$arr_proben=$arrResultado["arr_proben"];
