...
***********************************************************************************/
	...
	$la_seguridad=Array();
	$la_permisos=Array();
	$arrResultado=$io_fun_scb->uf_load_seguridad("SCB","sigesp_scb_p_cmp_retencion.php",$ls_permisos,$la_seguridad,$la_permisos);
	...
	switch($ls_operacion)
	{
		...
		case "PROCESAR":
			...
			elseif($ls_tipproben=='B')
			  ...
			  $li_totrocmp="";
			  $la_numcmp="";
			  $arrResultado = $io_retencion->uf_procesar_comprobante_retencion($ls_mes,$ls_agno,$ls_provbenedesde,$ls_provbenehasta,$ls_tipproben,$la_numcmp,$la_seguridad);
			  $li_totrocmp=$arrResultado["li_numcmp"];
