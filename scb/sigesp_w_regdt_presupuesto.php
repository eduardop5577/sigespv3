...
if (array_key_exists("operacion",$_POST))
   {
	 ...
	 $ls_openneraux      = $_POST["openneraux"];
         $ls_codmon      = $_POST["codmon"];
         $ldec_tascam      = $_POST["tascam"];
         $ldec_montot      = $_POST["montot"];
...
else
   {
     ...
     $ls_numordpagmin  = $_GET["numordpagmin"];
         ...
         $ldec_tascam      = $_GET["tascam"];
         $ldec_montot      = $_GET["montot"];
     
	 if (array_key_exists("openneraux",$_GET))
	 {$ls_openneraux=$_GET["openneraux"];}
	 else
	 {$ls_openneraux="";}
	 if ($ls_contintmovban==0)
	 {
...
$la_seguridad=Array();
$la_permisos=Array();
$arrResultado=$io_fun_banco->uf_load_seguridad("SCB",$ls_opener,$ls_permisos,$la_seguridad,$la_permisos);
...
if ($ls_operacion=="GUARDARPRE")
   {
	  $lb_valido_fuente=true;
	  if ($li_estmodest=='2')
	  {
		 $ls_estruc1   = str_pad($_POST["codestpro1"],25,0,0);
	     $ls_estruc2   = str_pad($_POST["codestpro2"],25,0,0);
	     $ls_estruc3   = str_pad($_POST["codestpro3"],25,0,0);
		 $ls_estruc4 	= str_pad($_POST["codestpro4"],25,0,0);
		 $ls_estruc5 	= str_pad($_POST["codestpro5"],25,0,0);
		 $lb_valido_fuente=$in_classmovbanco->uf_verificar_fuentefinanciamiento($li_estmodest,$ls_codfuefin,$ls_estruc1,$ls_estruc2,$ls_estruc3,$ls_estruc4,$ls_estruc5);
	  }
	  else
	  {
	  	 $ls_estruc1   = str_pad($_POST["codestpro1"],25,0,0);
	     $ls_estruc2   = str_pad($_POST["codestpro2"],25,0,0);
	     $ls_estruc3   = str_pad($_POST["codestpro3"],25,0,0);
		 $lb_valido_fuente=$in_classmovbanco->uf_verificar_fuentefinanciamiento($li_estmodest,$ls_codfuefin,$ls_estruc1,$ls_estruc2,$ls_estruc3,'','');
	  }
	  ...
	  $lb_valido="";
	  $ls_numconint="";
	  $arrResultado= $in_classmovbanco->uf_guardar_automatico($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ld_fecha,$ls_mov_descripcion,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_nomproben,$ldec_monto_mov,$ldec_objret,$ldec_retenido,$ls_chevau,$ls_estmov,$li_estint,$li_cobrapaga,$ls_estbpd,$ls_mov_procede,$ls_estreglib,$ls_estdoc,$ls_tipo,$ls_codfuefin,$ls_numordpagmin,$ls_codtipfon,$li_estcob,$ls_numconint,$ls_reglibro,$ls_codmon,$ldec_tascam,$ldec_montot);
	  ...
	  $arr_movbco["estmov"]=$ls_estmov;
          $ldec_montobs=$ldec_monto_mov;
            if($ldec_tascam>1)
            {
                $ldec_montobs      = $ldec_montot;
            }
	  ...
	  if ($lb_valido_fuente)
	  {
		  if ($lb_valido)
			{
			   ...
			   $lb_valido		= $in_classmovbanco->uf_procesar_dt_contable($arr_movbco,$ls_cuenta_scg,$ls_procedencia,$ls_mov_descripcion,$ls_mov_document,$ls_operacioncon,$ldec_montobs,$ldec_objret,true,'00000');
			   ...
			   $ldec_monto      = str_replace(",",".",$ldec_monto);
                           if($ldec_tascam>1)
                           {
                               $ldec_monto=($ldec_monto*$ldec_tascam);
                           }
			   if ($lb_valido)
				  {
					if ($ls_afectacion!='PG')
					...
					$lb_valido=$in_classmovbanco->uf_procesar_dt_gasto($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ls_estmov,$ls_programa,$ls_spgcuenta,$ls_mov_document,$ls_desmov,$ls_procedencia,$ldec_monto,$ls_operacion,$ls_estcla,$ls_codfuefin);
					if ($lb_valido)
					   {
						 ...
						 $ls_estdoc='C';
							  print "<script language=javascript>";
								...
								if($ls_openneraux=="1")
									print "f.action='sigesp_scb_p_detmovbancolote.php';";
								else
	   ...
	   else
	   {
			...
			?>
		    <script >
			alert("La estructura no existe para la fuente de financiamiento seleccionada!");
			close();
		    </script>	
		    <?php
	   }
...
<table width="583" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    <td height="22" colspan="2"><input name="txtcuentascg" type="hidden" id="txtcuentascg">
	  ...
	  <input name="openneraux" type="hidden" id="openneraux" value="<?php print  $ls_openneraux;?>"></td>
	  <input name="codmon" type="hidden" id="codmon" value="<?php print  $ls_codmon;?>"></td>
          <input name="tascam" type="hidden" id="tascam" value="<?php print  $ldec_tascam;?>"></td>
	  <input name="montot" type="hidden" id="montot" value="<?php print  $ldec_montot;?>"></td>
    </tr>
</table>
</form>
</body>
