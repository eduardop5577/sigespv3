...
class sigesp_scb_c_transferencias
{
	...
	//////////////////////////////////////////////////////////////////////////////
		...
		$li_total=count((array)$arr_data["numtra"]);
		...
		for($li_i=1 ; $li_i<=$li_total ; $li_i++)//for datos de origen
		{
			...
			$ls_docdestrans=$arr_data["docdestrans"][$li_i];
			$ls_tiptrans=$arr_data["tiptrans"][$li_i];
			$ls_numchequera=$arr_data["chequera"][$li_i];
			...
			if($li_i==1)	
			{
				...
				if($lb_existe)
				{
					...
					$arrResultado=$this->io_class_movbco->uf_guardar_automatico($ls_codban,$ls_cuenta_banco,$ls_numtrans,$ls_codope,$ld_fecha,$ls_conmov,'---',$ls_codpro,$ls_cedbene,$ls_nomproben,$ldec_monto,$ldec_monobjret,$ldec_monret,$ls_chevau,$ls_estmov,0,1,$ls_estbpd,$ls_procede,' ','N','-','--','','',0,$ls_numcontint,'');
					...
					{
						$lb_valido = $this->io_class_movbco->uf_actualizar_estatus_ch2($ls_codban,$ls_cuenta_banco,$ls_numtrans,$ls_numchequera);
						$lb_valido=$this->uf_cambio_estatus_transferencia_origen($ls_codban,$ls_cuenta_banco,$ls_numtrans,$ls_codope,$ls_docdestrans);
					}
			}
			if ($lb_valido)
			   {
		...
		if($lb_valido)
		{
			$li_total=count((array)$arr_datadestino["numtra"]);
			...
			for($li_i=1 ; $li_i<=$li_total ; $li_i++)//for datos destino
			{
				...
				$ls_docdestrans=$arr_datadestino["docdestrans"][$li_i];
				$ls_tiptrans=$arr_datadestino["tiptrans"][$li_i];
				$ld_feccon="1900/01/01";
				$lb_existe=$this->uf_select_documento($ls_codemp,$ls_numtrans,$ls_codope);
				if($lb_existe)
				{
				...
				{
					$lb_valido=$this->uf_cambio_estatus_transferencia_destino($ls_codban,$ls_cuenta_banco,$ls_numtrans,$ls_codope);
				}
	...
	function uf_declaracion_xml_cabecera($as_fecemidocdes,$as_fecemidochas,$as_periodo,$as_year)
	{
		...
		$ls_sql="SELECT '' AS codper, rpc_proveedor.rifpro, rpc_beneficiario.rifben,cxp_rd.numrecdoc, cxp_rd.numref,".
				...
				"   AND to_char(extract(YEAR from sno_hperiodo.fecdesper),'9999') = '".$as_year."' ".
				"   AND to_char(extract(MONTH from sno_hperiodo.fecdesper),'09') = '".$as_periodo."' ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_cambio_estatus_transferencia_origen($ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ls_numdocdestino)
	{
		////////////////////////////////////////////////////////////////////////////////////////////////
		//
		// -Funcion que inserta la cabecera del movimiento  bancario
		...
		$ls_sql=" UPDATE scb_movbco SET tiptrans='1', docdestrans='".$ls_numdocdestino."' ".
				"  WHERE codemp='".$ls_codemp."' ".
				"  AND codban='".$ls_codban."' ".
				"  AND ctaban='".$ls_ctaban."' ".
				"  AND numdoc='".$ls_numdoc."' ".
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	function uf_cambio_estatus_transferencia_destino($ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope)
	{
		////////////////////////////////////////////////////////////////////////////////////////////////
		//
		// -Funcion que inserta la cabecera del movimiento  bancario
		...
		$ls_sql=" UPDATE scb_movbco SET tiptrans='1', docdestrans='---------------' ".
				"  WHERE codemp='".$ls_codemp."' ".
				"  AND codban='".$ls_codban."' ".
				"  AND ctaban='".$ls_ctaban."' ".
				"  AND numdoc='".$ls_numdoc."' ".
