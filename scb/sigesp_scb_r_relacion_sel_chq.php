...
$la_seguridad=Array();
$la_permisos=Array();
$arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_r_relacion_sel_chq.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr> 
  	...
  	<a href="javascript:ue_imprimir('<?php print $ls_reporte ?>','<?php print $ls_reporte2 ?>');"><img src="../shared/imagebank/tools20/imprimir.gif" alt="Imprimir" title="Imprimir" width="20" height="20" border="0"></a>
  	...
  	<a href="javascript: ue_descargar();"><img src="../shared/imagebank/tools20/download.gif" title="Descargar" alt="Salir" width="20" height="20" border="0"></a>
  	<a href="sigespwindow_blank.php"><img src="../shared/imagebank/tools20/salir.gif" alt="Salir" title="Salir" width="20" height="20" border="0"></a><img src="../shared/imagebank/tools20/ayuda.gif" alt="Ayuda" title="Ayuda" width="20" height="20"></td>
  </tr>
</table>
  <?Php
...
if(array_key_exists("operacion",$_POST))
{
	...
	$ls_documentos = $_POST["documentos"];
...
else
{
   ...
   if($ls_operacion=="CARGAR_DT")
   {
		...
		$li_total=0;
		$arrResultado=array();
  		$arrResultado=uf_cargar_bancos($object_bancos,$li_total,$ls_codban,$ls_ctaban,$ld_fecdesde,$ld_fechasta);
		$object_bancos=$arrResultado["object_bancos"];
		$li_total=$arrResultado["li_row"];
   }
   else if($ls_operacion=="TXT"){
   		$ld_fecha=date("Y_m_d_H_i");
		$ls_nombrearchivo="txt/preindicacion/txtprovincial_preindicacion_".$ld_fecha.".txt";
		...
		$ls_cadena = '';
		for ($j = 0; $j < $li_totdoc; $j++) {
			$arrdatos = uf_buscar_datocheque($ls_codban, $ls_ctaban, $arr_documentos[$j], $arr_fecmov[$j]);
			
			if($arrdatos['id']!=''){
				//echo 'numche ->'.$arr_documentos[$j].' id->'.$arrdatos['id'].' nombre->'.$arrdatos['nombre'].' monto->'.$arrdatos['monto'];
				$arr_num       = explode(".",$arrdatos['monto']);
				$parte_entera  = $arr_num[0];
				$parte_decimal = substr($arr_num[1],0,2);
				$monto         = str_pad($parte_entera.$parte_decimal,15,0,0);
				
				if ($arrdatos['nacionalidad']!="NA") {
					$numdoc = str_pad($arrdatos['id'],9,0,0);
					$id_provbene = $arrdatos['nacionalidad'].$numdoc; 
				}
				else {
					$id_provbene = str_replace('-', '', $arrdatos['id']);
				}
				$numcheque = substr($arr_documentos[$j], -9);
				
				$nombre = substr($arrdatos['nombre'], 0, 70);
				$nombre = str_pad($nombre, 70);
				$ls_cadena.= $numcheque."        ".$id_provbene."  ".$nombre."   ".$monto."\r\n";
			}
		}
   ...
   $grid="grid";
	...
	//////////////////////////////////////////////////////////////////////////////
	  ...
	  $ls_sql="SELECT   codban, numdoc, codope, ctaban, fecmov, MAX(conmov) AS conmov, SUM(monto-monret) AS total, MAX(nomproben) AS nomproben
	   ...
	   $arrResultado["object_bancos"]=$object_bancos;
	...
	function uf_buscar_datocheque($codban, $ctaban, $numdoc, $fecha) 
	{
	//////////////////////////////////////////////////////////////////////////////
	//
	//	Metodo: uf_cargar_bancos
	...
	//////////////////////////////////////////////////////////////////////////////
	    $arrdata = array();
	  	...
	  	$io_sql=new class_sql($con);	
	  	$io_fun=new class_funciones();
	  	$ld_fecha=$io_fun->uf_convertirdatetobd($fecha);
		switch ($_SESSION["ls_gestor"])
		{
	  	...
	  	$ls_sql="SELECT  (CASE tipo_destino WHEN 'P' THEN 'NA'
                   							WHEN 'B' THEN (SELECT rpc_beneficiario.nacben
                                   							FROM rpc_beneficiario
				                           					WHERE rpc_beneficiario.codemp=scb_movbco.codemp
				                          					 AND rpc_beneficiario.ced_bene=scb_movbco.ced_bene) 
				           					ELSE 'NINGUNO' END ) AS nacionalidad,
						(CASE tipo_destino WHEN 'P' THEN (SELECT rpc_proveedor.rifpro
                   							...
                   							WHEN 'B' THEN (SELECT rpc_beneficiario.ced_bene
				                           					...
				                           					WHERE rpc_beneficiario.codemp=scb_movbco.codemp
				                          					 AND rpc_beneficiario.ced_bene=scb_movbco.ced_bene)
				           					ELSE 'NINGUNO' END ) AS documento_id,
				         (CASE tipo_destino WHEN 'P' THEN (SELECT rpc_proveedor.nompro 
				   	                       	...
				   	                       	WHEN 'B' THEN (SELECT ".$ls_cadena." 
				   	                                       ...
				   	                                       WHERE rpc_beneficiario.codemp=scb_movbco.codemp 
				   	                                         AND rpc_beneficiario.ced_bene=scb_movbco.ced_bene) 
				   	                       	ELSE 'NINGUNO' END ) AS nombre,monto
				 FROM scb_movbco
				 WHERE codemp='".$ls_codemp."' AND numdoc='".$numdoc."'
				 AND codban='".$codban."' AND ctaban='".$ctaban."'
				 AND fecmov='".$ld_fecha."'
				 AND estmov<>'A' AND estmov<>'O' AND codope='CH'";
	   
	   $rs_cheque=$io_sql->select($ls_sql);
	   ...
	   {
		   if($row=$io_sql->fetch_row($rs_cheque))
		   {
				$arrdata['nacionalidad'] = $row["nacionalidad"];
		   		$arrdata['id']           = $row["documento_id"];
				$arrdata['nombre']       = $row["nombre"];
				$arrdata['monto']        = $row["monto"];
			}
		   $io_sql->free_result($rs_cheque);
	   }
  ...
  <table width="524" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
	 ...
	 <tr >
 	 ...
 	 <tr >
			<td height="28" align="right">Generar TXT(Preindicaci&oacute;n - Provincial)</td>
			<td height="28" colspan="2">
			  <input type=checkbox id=chktxtpreindicacion value=1>
			</td>
 		 </tr>
    <tr>
      <td height="22" align="center">     <div align="left"><span class="Estilo1">
        <input name="operacion"   type="hidden"   id="operacion"   value="<?php print $ls_operacion;?>">
        <input name="documentos"   type="hidden"   id="documentos"   value="<?php print $ls_documentos;?>">
        <input name="fechas"   type="hidden"   id="fechas"   value="<?php print $ls_fechas;?>">
      </span></div></td>
	  ...
	  if (li_imprimir=='1')
	     {
			  ...
			  else if (ls_tiprep=="1")
			  {
					  ...
					  if(ls_documentos.length>0)
					  {
							  ...
							  if(f.chktxtpreindicacion.checked){
								  f.documentos.value = ls_documentos
								  f.fechas.value = ls_fechas;
								  f.operacion.value='TXT';
...
{
	window.open("sigesp_scb_cat_directorio.php?ruta=txt/preindicacion","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
</script>
