...
***********************************************************************************/
	...
	$la_seguridad=Array();
	$la_permisos=Array();
	$arrResultado=$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_p_concilerror.php",$ls_permisos,$la_seguridad,$la_permisos);
...
/*	$la_security[1]=$ls_empresa;
	...
	if( array_key_exists("operacion",$_POST))
	{
		...
		$ls_docmov=substr($_POST["txtdocumento"],0,15);
		...
		$ldec_montoret=str_replace(",",".",$ldec_montoret);
		$ldec_montomov=number_format($ldec_montomov,2,'.','');
		$ldec_monobjret=number_format($ldec_monobjret,2,'.','');
	...
	function uf_cargar_dt()
	{
		...
		$ldec_montoret="";
		$arrResultado="";
		$arrResultado=$in_classmovbco->uf_cargar_dt($ls_docmov,$ls_codban,$ls_cuenta_banco,$ls_mov_operacion,$ls_estmov,$objectScg,$li_row,$ldec_mondeb,$ldec_monhab,$objectSpg,
									  $li_rows_spg,$ldec_monspg,$objectSpi,$li_rows_spi,$ldec_monspi,$objectRet,$li_rows_ret,$ldec_montoret);
	...
	{			
		...
		if($ls_tip_mov=='B')
		{
			...
			else
			{
			 ...
			 $arr_errorbco["monto"] =number_format($ldec_montomov,2,'.','');
			 $arr_errorbco["monret"]=number_format($ldec_montoret,2,'.','');
		...
		if ($ls_tip_mov=='L')
		{
		   ...
		   $ls_numordpagmin="";
			...
			$ls_numordpagmin=$arrResultado["as_numcontint"];
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td width="202"><div align="left">
          <input name="txtdocumento" type="text" id="txtdocumento" style="text-align:center" onBlur="rellenar_cad(this.value,15,'doc')" value="<?php print $ls_docmov;?>" size="15">
      </div></td>
