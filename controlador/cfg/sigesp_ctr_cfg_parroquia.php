...
if (($_POST['ObjSon']) && ($sessionvalida))
{
	...
	if ($_POST['ObjSon']) 		
	{
		...
		$oservicio = new ServicioCfg('sigesp_parroquia');
		$Evento = $ArJson->oper;
		...
		switch ($Evento)
		{
			case 'incluir':
				...
				$mensaje="Inserto la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
				$tipoevento=true;
				...
				{
					echo "|0";
					$mensaje="Error al Insertar la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
					$tipoevento=false;
				}
				...
				$servicioEvento->nomfisico="sigesp_vis_cfg_parroquia.php";
			...
			case 'actualizar':
				...
				$mensaje="Actualizo la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
				$tipoevento=true;
				if (ServicioCfg::comTransaccion ())
				{
					echo "|1";
				...
				{
					echo "|0";
					$mensaje="Error al Actualizar la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
					$tipoevento=false;
				}
			...
			case 'eliminar':
				$oservicio->setCodemp ($_SESSION["la_empresa"]["codemp"]);
				$ultimo=$oservicio->verificarUltimo('codpar','sigesp_parroquia'," WHERE codpai='".$ArJson->codpai."' AND codest='".$ArJson->codest."' AND codmun='".$ArJson->codmun."'",$ArJson->codpar);
				if ($ultimo)
				{
					ServicioCfg::iniTransaccion ();
					$respuesta = $oservicio->eliminarDto ( $ArJson );
					$mensaje="Elimino la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
					$tipoevento=true;
					if (ServicioCfg::comTransaccion ())
					{
						echo "|1";
					...
					else
					{
						...
						$mensaje="Error al Eliminar la Parroquia ".$ArJson->codpar." asociada al municipio ".$ArJson->codmun.", asociado al Estado ".$ArJson->codest." del Pais ".$ArJson->codpai;
						$tipoevento=false;
					}
				}
				else
			...
			case 'nuevo' :
				$contador="";
				$oservicio->setCodemp ($_SESSION["la_empresa"]["codemp"]);
				$contador = $oservicio->buscarCodigoParroquia($ArJson->codpai,$ArJson->codest,$ArJson->codmun);
