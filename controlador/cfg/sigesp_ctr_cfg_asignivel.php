...
{
	require_once($_SERVER['DOCUMENT_ROOT'].'/'.$_SESSION['sigesp_sitioweb'].'/modelo/servicio/cfg/sigesp_srv_cfg_asignivel.php');
	...
	$objAsigNivel = new AsigNivel();		
	$arrResultado = pasarDatos($objAsigNivel,$objdata,$evento);
	$objAsigNivel = $arrResultado["objDao"];
	$evento = $arrResultado["evento"];

	$objAsigNivel->codemp = $_SESSION['la_empresa']['codemp'];	
	$objAsigNivel->codsis = $objdata->sistema;
	$objAsigNivel->nomfisico = $objdata->vista;	
	$objAsigNivel->tipproc = $objdata->codtipniv;	
	...
	switch ($evento)
	{
		case 'catalogo':
			...
			if ($accionvalida)
			{
				$datos = $objAsigNivel->leer();
				if ($objAsigNivel->valido)
				{
					if (!$datos->EOF)
					{
