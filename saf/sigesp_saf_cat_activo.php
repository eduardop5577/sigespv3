...
if($ls_operacion=="BUSCAR")
{
	if (array_key_exists("txtdesde",$_POST))
	{
		...
		if (array_key_exists("txthasta",$_POST))
		{
			...
			if($lb_fechavalida)
			{
				...
				$ls_filtro = '';
				if($ls_maract!='%%'){
					$ls_filtro = "   AND saf_activo.maract like '".$ls_maract."'";	
				}
				
				if ($ls_modact != '%%') {
					$ls_filtro .= "   AND saf_activo.modact like '".$ls_modact."'";
				}
				...
				$ls_sql="SELECT saf_activo.*, ".
						...
						"         WHERE saf_catalogo.catalogo = saf_activo.catalogo) as dencatsige, ".	
						...
						" 		(SELECT dencol FROM saf_color ".
						"         WHERE saf_color.codcol = saf_activo.codcol) as dencol, ".
						" 		(SELECT dencatsud FROM saf_sudebip ".
						"         WHERE saf_sudebip.codcatsud = saf_activo.codcatsud) as dencatsud, ".
						" 		(SELECT denforadq FROM saf_formaadquisision ".
						"         WHERE saf_formaadquisision.codforadq = saf_activo.codforadq) as denforadq, ".
						" 		(SELECT denestuso FROM saf_estadouso ".
						"         WHERE saf_estadouso.codestuso = saf_activo.codestuso) as denestuso, ".
						" 		(SELECT dentipsem FROM saf_tiposemoviente ".
						"         WHERE saf_tiposemoviente.codtipsem = saf_activo.codtipsem) as dentipsem, ".
						" 		(SELECT denprosem FROM saf_proposemoviente ".
						"         WHERE saf_proposemoviente.codprosem = saf_activo.codprosem) as denprosem, ".
						" 		(SELECT desciu FROM saf_ciudades ".
						"         WHERE saf_ciudades.codciu = saf_activo.codciu ".
						"           AND saf_ciudades.codest = saf_activo.codest ".
						"           AND saf_ciudades.codpai = saf_activo.codpai) as desciu, ".	
						...
						"   AND saf_activo.denact like '".$ls_denact."' ".$ls_filtro.	
	...
	if($li_numrows>0)
	{
		...
		while($row=$io_sql->fetch_row($rs_cta))
		{
			...
			$ls_codciu= $row["codciu"];
			$ls_desciu= $row["desciu"];
			$ls_codprosem= $row["codprosem"];
			$ls_denprosem= $row["denprosem"];
			$ls_codtipsem= $row["codtipsem"];
			$ls_dentipsem= $row["dentipsem"];
			$ls_codestuso= $row["codestuso"];
			$ls_denestuso= $row["denestuso"];
			$ls_codforadq= $row["codforadq"];
			$ls_denforadq= $row["denforadq"];
			$ls_codcatsud= $row["codcatsud"];
			$ls_dencatsud= $row["dencatsud"];
			$ls_codcol= $row["codcol"];
			$ls_dencol= $row["dencol"];
			$ls_expediente= $row["expediente"];
			print " <td><a href=\"javascript: aceptar('$ls_codact','$ls_maract','$ls_modact','$ld_fecregact','$ld_feccmpact',".
			       ...
			       "'$ls_catalogo','$ls_dencatsige','$ls_ctaspg','$ls_esttipinm','$ls_numord','$ls_codpro','$ld_fecordcom',".
				   ...
				   "'$ls_denite','$ls_denspg','$ls_clasif','$ls_codcat','$ls_dencat','$ls_expediente','$ls_colact','$ls_codprosem',".
				   "'$ls_denprosem','$ls_codtipsem','$ls_dentipsem','$ls_codestuso','$ls_denestuso','$ls_codforadq','$ls_denforadq','$ls_codcatsud',".
				   "'$ls_dencatsud','$ls_codcol','$ls_dencol','$li_i');\">".$ls_codact."</a></td>";
			print "<td><input name='txtdenact".$li_i."' type='hidden' id='txtdenact".$li_i."' value='$ls_denact'>".$row["denact"]."</td>";
			print "<td>".$row["maract"]."</td>";
			print "<td>".$row["modact"]."</td>";
			print "<td>".$ld_fecregact."<input name='txtobsact".$li_i."' type='hidden' id='txtobsact".$li_i."' value='".$ls_obsact."'></td>";
			print "</tr>";			
		}
	}
	else
...
<script language="JavaScript">   
  function aceptar(ls_codact,ls_maract,ls_modact,ld_fecregact,ld_feccmpact,li_cosact,ls_codconbie,ls_denconbie,ls_codfuefin,
				   ...
				   ls_denmun,ls_catalogo,ls_dencatsige,ls_ctaspg,ls_esttipinm,ls_numord,ls_codpro,ld_fecordcom,ls_nompro,li_monord,
				   ls_numsolpag,ld_fecemisol,ls_estdepact,ls_fotact,ls_coddestino,ls_dendestino,ls_codgru,ls_codsubgru,ls_codsec,ls_codite,
				   ls_dengru,ls_densubgru,ls_densec,ls_denite,ls_denspg, ls_clasif,ls_codcat,ls_dencat,ls_expediente,ls_colact,codprosem,
				   denprosem,codtipsem,dentipsem,codestuso,denestuso,codforadq,denforadq,codcatsud,dencatsud,codcol,dencol,li_i)
  {
	f=document.form1;
	...
	{alert("entro");
		...
		obj.value=eval("f.txtdenact"+li_i+".value");
	}
	else
	{
      ...
      //  opener.document.form1.txtobsact.value = ls_obsact; 
		opener.document.form1.txtdenact.value=eval("f.txtdenact"+li_i+".value");
        opener.document.form1.txtobsact.value=eval("f.txtobsact"+li_i+".value");
		
		opener.document.form1.txtmaract.value=ls_maract;
		opener.document.form1.txtmodact.value=ls_modact;
		//opener.document.form1.txtcolact.value=ls_colact;
		...
		//opener.document.form1.txtobsact.value=ls_obsact;
		opener.document.form1.txtcatalogo.value=ls_catalogo;
		opener.document.form1.txtcuenta.value=ls_ctaspg;
		opener.document.form1.txtdencat.value=ls_dencatsige;
		...
		opener.document.form1.txtexpediente.value=ls_expediente;		
		if (ls_clasif!="")
		{
		...
		if(codtipsem!="")
		{
			filasem=opener.document.getElementById("filasem");
		    filasem.style.display="";
		    filasem.style.display="compact";
			opener.document.form1.txtcodprosem.value=codprosem;
			opener.document.form1.txtdenprosem.value=denprosem;
			opener.document.form1.txtcodtipsem.value=codtipsem;
			opener.document.form1.txtdentipsem.value=dentipsem;
		}
		else
		{
			filasem=opener.document.getElementById("filasem");
		...
		opener.document.form1.txtcodestuso.value=codestuso;
		opener.document.form1.txtdenestuso.value=denestuso;
		opener.document.form1.txtcodforadq.value=codforadq;
		opener.document.form1.txtdenforadq.value=denforadq;
		opener.document.form1.txtcodcatsud.value=codcatsud;
		opener.document.form1.txtdencatsud.value=dencatsud;
		opener.document.form1.txtcodcol.value=codcol;
		opener.document.form1.txtdencol.value=dencol;
