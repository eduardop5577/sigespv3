...
<form name="form1" method="post" action="">
  ...
  <br>
      ...
      <tr>
        ...
        <td width="162" height="22" style="text-align:left"><input name="txtcmpmov" type="text" id="txtcmpmov"></td>
        <td width="68">&nbsp;</td>
      </tr>
      <tr>
        <td>&nbsp;</td>
...
if($ls_operacion=="BUSCAR")
{
	...
	if ($ls_tipo=='retorno')
	   {
		 $ls_sqlaux = " AND saf_prestamo.codresrec = '".$ls_codbenpre."'";
		 $ls_tabaux = " , saf_dta";
		 $ls_sqlaux = $ls_sqlaux." AND saf_dta.estactpre = '1' 
		                           AND saf_dt_prestamo.codemp=saf_dta.codemp
								   AND saf_dt_prestamo.codact=saf_dta.codact
								   AND saf_dt_prestamo.ideact=saf_dta.ideact";
	   }
	$ls_sql="SELECT distinct saf_prestamo.cmppre,saf_prestamo.coduniced, saf_prestamo.obspre,
			 ...
			 saf_prestamo.codresrec,
			  ...
			  (select $ls_cadena_personal 
			    ...
			    FROM saf_prestamo,saf_dt_prestamo $ls_tabaux
			   WHERE saf_prestamo.codemp='".$ls_codemp."'
			     AND saf_prestamo.codemp = '".$ls_codemp."' $ls_sqlint
			     AND saf_prestamo.cmppre like '".$ls_cmpres."' $ls_sqlaux
	...
	if($li_row>0)
	{
		while($row=$io_sql->fetch_row($rs_cta))
		{
			...
			if ($ls_tipo=='retorno')
			   {
			     print "<td><a href=\"javascript: uf_aceptar_comprobante('$ls_cmpres','$ld_feccmp');\">".$ls_cmpres."</a></td>";
			   }
			else
			   {
...
function uf_aceptar_comprobante(as_cmppre,as_fecpre)
{
  fop = opener.document.form1;
  fop.hidnumcmp.value = as_cmppre;
  fop.hidfeccmp.value = as_fecpre;   
  fop.operacion.value = "LOAD_ACTIVOS";
  fop.cmbtipcmp.value = "P";
