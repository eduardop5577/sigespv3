...
if($ls_operacion=="BUSCAR")
{
    ...
    $ls_cadena_personal=$con->Concat("sno_personal.nomper","' '","sno_personal.apeper");
    $ls_cadena_beneficiario=$con->Concat("rpc_beneficiario.nombene","' '","rpc_beneficiario.apebene");
	...
	$ls_sql="SELECT saf_movimiento.*,saf_causas.dencau, ".
	        ...
	        "       (SELECT densed  ".
            "        FROM   saf_sede ".
            "        WHERE  saf_sede.codemp=saf_movimiento.codemp".
			"          AND saf_sede.codsed=saf_movimiento.codsed) as densed".
			...
			"   AND saf_movimiento.numcmp like '".$ls_cmpmov."' ".
	...
	if($li_row>0)
	{
		while($row=$io_sql->fetch_row($rs_cta))
		{
			...
			$ls_codsed = $row["codsed"];
			$ls_densed = $row["densed"];
			...
			print "<td><a href=\"javascript: aceptar('$ls_cmpmov','$ls_codcau','$ls_dencau','$ld_feccmp',".
				  ...
				  "'$ls_denuniadm','$ls_ubigeo','$ls_denrespri','$ls_denresuso','$ls_tiprespri','$ls_tipresuso','$ld_fecent','$ls_numcmp','$ls_codsed','$ls_densed','$ls_estmov');\">".$ls_numcmp."</a></td>";
...
<script language="JavaScript">
	function aceptar(ls_cmpmov,ls_codcau,ls_dencau,ld_fectraact,ls_obstra,ls_estpromov,ls_coddestino,ls_codrespri,
					 ...
					 ls_tipresuso,ld_fecent,as_numcmp,ls_codsed,ls_densed,ls_estmov)
	{
		...
		if(ls_coddestino!="reporte")
		{
			...
			if(tipo=="incorporacion")
			{
				...
				if(ls_estmov=="A")
				{
					opener.document.form1.txtestatus.value="ANULADA";
				}
				else
				{
					opener.document.form1.txtestatus.value="REGISTRO";
				}
			}
		}
