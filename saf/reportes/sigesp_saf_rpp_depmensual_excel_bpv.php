...
***********************************************************************************/
	...
	function uf_print_detalle($li_total,$lo_libro,$lo_hoja,$la_data,$li_fila)
	{
		...
		for($li_j=1;$li_j<=$li_total;$li_j++)
		{
			...
			if(strlen($la_data[$li_j]['fecinc'])>10){
				$lo_hoja->write($li_fila, 3, $la_data[$li_j]['fecinc'],$lo_titulo);
			}
			else{
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_totales($li_total,$lo_libro,$lo_hoja,$la_data,$li_fila)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_detalle
		//		   Access: private 
		...
		$lo_titulo->set_size('9');
		for($li_j=1;$li_j<=$li_total;$li_j++){
			$li_fila=$la_datat[li_j]['posicion'];
			$lo_hoja->write($li_fila, 3, "Total Acumulado",$lo_titulo);
			$lo_hoja->write($li_fila, 5, $la_datat[li_j]['costo'],$lo_dataright);
			$lo_hoja->write($li_fila, 6, $la_datat[li_j]['cossal'],$lo_dataright);
			$lo_hoja->write($li_fila, 7, $la_datat[li_j]['mondep'],$lo_dataright);
			//$lo_hoja->write($li_fila, 8, $la_datat[li_j]['mesdep'],$lo_dataright);
			$lo_hoja->write($li_fila, 9, $la_datat[li_j]['depmen'],$lo_dataright);
			$lo_hoja->write($li_fila, 10, $la_datat[li_j]['depacu'],$lo_dataright);
			$lo_hoja->write($li_fila, 11, $la_datat[li_j]['pordep'],$lo_dataright);
		}
		return $li_fila;
	}// end function uf_print_detalle
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_total($la_datat,$lo_libro,$lo_hoja,$li_fila)
	{
		...
		$li_fila++;
		$li_fila++;
		$lo_hoja->write($li_fila, 3, $la_datat[1]['total'],$lo_titulo);
		$lo_hoja->write($li_fila, 5, $la_datat[1]['costo'],$lo_dataright);
		$lo_hoja->write($li_fila, 6, $la_datat[1]['cossal'],$lo_dataright);
		$lo_hoja->write($li_fila, 7, $la_datat[1]['mondep'],$lo_dataright);
		$lo_hoja->write($li_fila, 8, $la_datat[1]['mesdep'],$lo_dataright);
		$lo_hoja->write($li_fila, 9, $la_datat[1]['depmen'],$lo_dataright);
		$lo_hoja->write($li_fila, 10, $la_datat[1]['depacu'],$lo_dataright);
		$lo_hoja->write($li_fila, 11, $la_datat[1]['pordep'],$lo_dataright);
		$li_fila++;
		return $li_fila;
...
/*	$lo_dataright= &$lo_libro->addformat(array('num_format' => '#,##0.00'));
	...
	{
			$lb_valido=true;
			$rs_data=$io_report->uf_saf_load_depmensual($ls_codemp,$li_ordenact,$ld_fecdep,$ld_fecdepaux,$ls_codcatsudeban,'',$li_cuecon); // Cargar el DS con los datos de la cabecera del reporte
			/////////////////////////////////         SEGURIDAD               ////////////////////////////////////////////////////////
			...
			////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////////
			$li_fila=uf_print_encabezado_pagina($lo_libro,$lo_hoja,$ls_fecha,$ls_codcatsudeban,$li_fila); // Imprimimos el encabezado de la página
			...
			if($li_cuecon!='')
			{
				$sc_cuenta=$rs_data->fields["sc_cuenta"];
				$denominacion=$rs_data->fields["denominacion"];
				$cadena='Total '.$denominacion.' - Cuenta '.$sc_cuenta;
				while(!$rs_data->EOF){
				...
				$li_fila=uf_print_detalle($li_i,$lo_libro,$lo_hoja,$la_data,$li_fila); // Imprimimos el detalle 
				//total general
...
//				$li_totcosto_tot=$li_totcosto_tot+$li_totcosto;
				//total general
				$la_datat[1]=array('total'=>$cadena,'costo'=>$li_totcosto,'cossal'=>$li_totcossal,'mondep'=>$li_totmondep,
								   'mesdep'=>" -- ",'depmen'=>$li_totdepmen,'depacu'=>$li_totdepacu,'pordep'=>$li_totpordep);
				$li_fila=uf_print_total($la_datat,$lo_libro,$lo_hoja,$li_fila);
			}
			else{
				$sc_cuenta=$rs_data->fields["sc_cuenta"];
				$denominacion=$rs_data->fields["denominacion"];
				...
				while(!$rs_data->EOF)
				{
					if(trim($sc_cuenta)==trim($rs_data->fields["sc_cuenta"]))
					{
						$ls_codact=  $rs_data->fields["codact"];
						$ls_denact=  $rs_data->fields["denact"];
						$ls_ideact=  $rs_data->fields["ideact"];
						...
						$li_i++;
						$li_i++;
						$la_data[$li_i]=array('codact'=>'','ideact'=>'','denact'=>'','fecinc'=>$cadena,'mesdep'=>'',
									          'viduti'=>'','costo'=>$li_totcosto,'cossal'=>$li_totcossal,'depmen'=>$li_totdepmen,
									          'depacu'=>$li_totdepacu,'mondep'=>$li_totmondep,'pordep'=>$li_totpordep);
						$sc_cuenta=$rs_data->fields["sc_cuenta"];
						$li_i++;
						$li_totmondep=0;
						$li_totcosto=0;
...
//			$li_totcosto_tot=$io_fun_activos->uf_formatonumerico($li_totcosto_tot);
			...
			$li_fila=uf_print_totales_generales($la_datatt,$lo_libro,$lo_hoja,$li_fila);
			unset($la_data);			
			unset($la_data1);
			unset($la_datatt);
			//total general
	...
	else
	{
		...
		else // Imprimimos el reporte
		{
			...
			////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////
			$li_fila=uf_print_encabezado_pagina($lo_libro,$lo_hoja,$ls_fecha,$ls_codcatsudeban,$li_fila);
			...
			for($li_j=1;$li_j<=$li_totrowcat;$li_j++)
			{
				...
				{
					$li_i++;
					if($sc_cuenta==$rs_data->fields["sc_cuenta"]){
						$ls_codact=  $rs_data->fields["codact"];
						$ls_denact=  $rs_data->fields["denact"];
						$ls_ideact=  $rs_data->fields["ideact"];
						$li_mesdep=" -- ";
						...
						$li_totcossal_tot=$li_totcossal_tot+$li_totcossal;
						$li_totcosto_tot=$li_totcosto_tot+$li_totcosto;
						$la_data[$li_i]=array('codact'=>'','ideact'=>'','denact'=>'','fecinc'=>'Total depreciacion acumulada','mesdep'=>'',
											  'viduti'=>'','costo'=>$li_totcosto_tot,'cossal'=>$li_totcossal_tot,'depmen'=>$li_totdepmen_tot,
											  'depacu'=>$li_totdepacu_tot,'mondep'=>$li_totmondep_tot,'pordep'=>$li_totpordep_tot);
						$li_totmondep_tot=0;
						$li_totdepmen_tot=0;
						$li_totdepacu_tot=0;
						$li_totpordep_tot=0;
				...
				$li_conta=count((array)$la_data);
				if($la_data!="")
				{
					$li_fila=uf_print_detalle($li_conta,$lo_libro,$lo_hoja,$la_data,$li_fila); // Imprimimos el detalle 
					unset($la_data);			
					unset($la_data1);			
				}
			}
