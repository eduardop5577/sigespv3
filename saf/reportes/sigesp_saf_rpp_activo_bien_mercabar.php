...
***********************************************************************************/
	...
	function uf_print_detalle($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codgru'=>array('justification'=>'center','width'=>40), // Justificación y ancho de la columna	
									   ...
									   'denact'=>array('justification'=>'left','width'=>400), // Justificación y ancho de la columna						 			  
									   'denconbie'=>array('justification'=>'left','width'=>80), // Justificación y ancho de la columna						 			  
	...
	{
		print("<script language=JavaScript>");
		print(" alert('No 2hay nada que Reportar');"); 
		print(" close();");
		print("</script>");
	}
	else // Imprimimos el reporte
	{
		...
		$io_pdf->ezStartPageNumbers(940,50,10,'','',1); // Insertar el número de página
			$li_numpag=$io_pdf->ezPageCount; // Número de página	
				uf_print_encabezado_pagina($ls_codemp,$ls_nomemp,$rs_data->fields["denuniadm"],$ls_distrito,$ls_direccion,$rs_data->fields["denuniadm"],$ls_titulo,"",$ld_fecha,$io_pdf); // Imprimimos el encabezado de la página		
			if($lb_valido)
			{			
	    		 $li_totrow_det=$io_report->io_sql->num_rows($rs_data);		
				$la_data="";
				$i=0;
				$li_total_pag=$li_totrow_det/12;
				$li_total_pag=ceil($li_total_pag);
				$acum_vienen=0;
				$acum_van=0;
				$li_s=0;
				$acum_item=0;
				$li_d=0;
				$entro=true;
				for($li_p=1;$li_p<=$li_total_pag;$li_p++){
					$acum_item=0;
						while(!$rs_data->EOF)
						{  				
							$i++;
							...
							$ls_denact = preg_replace("[\n|\r|\n\r]", '', $ls_denact);
							$ls_denact = str_replace("  ", '', $ls_denact);
							...
							$acum_van=$acum_van+$li_costo;
							//print $li_costo."---".$ls_denact."<br>";
							...
							$la_data[$li_s]=array('codact'=>$ls_codact,'codgru'=>$ls_codgru,'codsubgru'=>$ls_codsubgru,'codsec'=>$ls_codsec,'denact'=>$ls_denact,'seract'=>$ls_seract,'denuniadm'=>$ls_denuniadm,'estact'=>$ls_estact,'maract'=>$ls_maract,'modact'=>$ls_modact,'costo1'=>$li_costo,'costo'=>$li_costo,'ideact'=>$ls_ideact,'cantidad'=>$ls_cantidad,'servicio'=>$ls_servicio,'denconbie'=>$ls_denconbie);
							//print_r($la_data);print "<br><br>";
							if($li_d==20)
							{
								if($entro){
									$io_pdf->addText(830,445,9,'VIENEN                      '.'0,00');
									$entro=false;
								}
								else{
									$la_total_vienen=number_format($acum_vienen,2,",",".");
									$io_pdf->addText(830,440,9,'VIENEN            '.$la_total_vienen);
								}
								uf_print_detalle($la_data,$io_pdf);
								$io_pdf->addText(130,70,9,'CANTIDAD         '.$i);
								$la_total_van=number_format($acum_van+$acum_vienen,2,",",".");
								$acum_vienen=$acum_vienen+$acum_van;
								$acum_van=0;
//							print "--------------------------------<br>";
								$io_pdf->addText(830,70,9,'VAN              '.$la_total_van);
								$io_pdf->ezNewPage();
								unset($la_data);
								$li_d=0;
								$li_s=0;
