...
***********************************************************************************/
	...
	$ld_fecha="";
	$ls_titulo="RELACION DEL MOVIMIENTO DE BIENES MUEBLES";
	...
	$ls_denunidep=$io_fun_activos->uf_obtenervalor_get("denunidep","");
...
//					uf_print_cabecera($ls_estemp,'',$ls_direccion,$ls_codemp,$ls_nomemp,$ls_denuniadm,'',$periodo,$io_pdf); // Imprimimos la cabecera del registro
		$li_row++;
		$lo_hoja->write($li_row,0,"1. ESTADO:",$lo_titulo);
		$lo_hoja->write($li_row,1,$ls_estemp,$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"2. MUNICIPIO:",$lo_titulo);
		$lo_hoja->write($li_row,1,"",$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"3. DIRECCION O LUGAR:",$lo_titulo);
		$lo_hoja->write($li_row,1,$ls_direccion,$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"4. DEPENDENCIA O UNIDAD PRIMARIA:",$lo_titulo);
		$lo_hoja->write($li_row,1,$ls_nomemp,$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"5. SERVICIO:",$lo_titulo);
		$lo_hoja->write($li_row,1,$ls_denuniadm,$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"6. UNIDAD DE TRABAJO O DEPENDENCIA:",$lo_titulo);
		$lo_hoja->write($li_row,1,$ls_denunidep,$lo_dataleft);
		$li_row++;
		$lo_hoja->write($li_row,0,"7. PERIODO DE LA CUENTA: (MES Y AÑO)",$lo_titulo);
		$lo_hoja->write($li_row,1,$periodo,$lo_dataleft);
		$lb_valido=$io_report->uf_saf_load_dt_relmovbienes($ls_codemp,$ls_coduniadm,$ld_desde,$ld_hasta,$ls_coddesde,
		...
		if($lb_valido)
		{
			...
			$lo_hoja->write($li_row,3,"Cod. Mov.",$lo_titulo);
			$lo_hoja->write($li_row,4,"Identificador",$lo_titulo);
			$lo_hoja->write($li_row,5,"Cantidad",$lo_titulo);
			$lo_hoja->write($li_row,6,"Descripcion",$lo_titulo);
			$lo_hoja->write($li_row,7,"Comprobante",$lo_titulo);
			$lo_hoja->write($li_row,8,"Fecha",$lo_titulo);
			$lo_hoja->write($li_row,9,"Detalle",$lo_titulo);
			$lo_hoja->write($li_row,10,"Incorporacion Bs.",$lo_titulo);
			$lo_hoja->write($li_row,11,"Desincorporacion Bs.",$lo_titulo);
...
//					$li_total=$io_fun_activos->uf_formatonumerico($li_total);
				...
				$li_row++;
				$lo_hoja->write($li_row,0," ".$ls_codgru,$lo_dataleft);
				$lo_hoja->write($li_row,1," ".$ls_codsubgru,$lo_dataleft);
				$lo_hoja->write($li_row,2," ".$ls_codsec,$lo_dataleft);
				$lo_hoja->write($li_row,3," ".$ls_codcau,$lo_dataleft);
				$lo_hoja->write($li_row,4," ".$ls_ideact,$lo_dataleft);
				$lo_hoja->write($li_row,5,$ls_cantidad,$lo_dataleft);
				$lo_hoja->write($li_row,6,$ls_denact,$lo_dataleft);
				$lo_hoja->write($li_row,7," ".$ls_cmpmov,$lo_dataleft);
				$lo_hoja->write($li_row,8,$ld_feccmp,$lo_dataleft);
				$lo_hoja->write($li_row,9,$ls_descmp,$lo_dataleft);
				$lo_hoja->write($li_row,10,$li_totinc,$lo_dataright2);
				$lo_hoja->write($li_row,11,$li_totdesinc,$lo_dataright2);
			}
		...
		$lo_hoja->write($li_row,10,$li_moninc,$lo_dataright2);
		$lo_hoja->write($li_row,11,$li_mondesinc,$lo_dataright2);
		if(($lb_valido))
		{
			$lo_libro->close();
