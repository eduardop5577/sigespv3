...
***********************************************************************************/
	...
	else // Imprimimos el reporte
	{
		...
		//					$li_total=$io_fun_activos->uf_formatonumerico($li_total);
							if ($ls_estatus == 'I')
							{
							 $acum_van_inc=$acum_van_inc+$li_total;
							...
							{
							 $acum_van_des=$acum_van_des+$li_total;
							 $li_totinc = '';
					...
					if($entro){
						$io_pdf->addText(750,405,9,'VIENEN                      '.'0,00                             '.'0,00');
						$entro=false;
					}
					else{
						$la_total_vienen_inc=number_format($acum_vienen_inc,2,",",".");
						$la_total_vienen_des=number_format($acum_vienen_des,2,",",".");
						$io_pdf->addText(750,405,9,'VIENEN                   '.$la_total_vienen_inc.'                      '.$la_total_vienen_des);
					}
					uf_print_detalle($la_data,$io_pdf); 
					$io_pdf->addText(215,70,9,'CANTIDAD         '.$acum_item);
					$la_total_van_inc=number_format($acum_van_inc+$acum_vienen_inc,2,",",".");
					$acum_vienen_inc=$acum_vienen_inc+$acum_van_inc;
					$la_total_van_des=number_format($acum_van_des+$acum_vienen_des,2,",",".");
					$acum_vienen_des=$acum_vienen_des+$acum_van_des;
					$io_pdf->addText(750,70,9,'VAN                       '.$la_total_van_inc.'                    '.$la_total_van_des);
					$io_pdf->ezNewPage();
					unset($la_data);
				}
			}
			...
			uf_print_pie_cabecera($li_moninc,$li_mondesinc,$li_montot,$io_pdf);
