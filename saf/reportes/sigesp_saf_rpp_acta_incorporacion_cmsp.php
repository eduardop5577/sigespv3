...
***********************************************************************************/
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_cabecera($ls_titulo_emp,$ls_feccmp,$ls_denunimed,$ls_dencau,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$li_tm=$io_pdf->getTextWidth(11,"ACTA DE INCORPORACION");
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,710,11,"ACTA DE INCORPORACION"); // Agregar el título
	    $la_data=array(array('name'=>' Los sucritos ______________________________________________________, mayores de edad, hacemos constar por la presente acta que con'.
									 ' fecha:   '.$ls_feccmp.   ', y de conformidad con las instrucciones recibidas de '.$ls_titulo_emp.'  se han incorporado al inventario'.
									 ' de la Unidad    '.$ls_denunimed.'    que funciona en Sarare, Municipio Simon Planas del Estado Lara, los bienes que a continuación se'.
									 ' especifican, los cuales fueron adquiridos por '.$ls_dencau));
	...
	function uf_print_pie_detalle($io_pdf)
	{
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
	    $la_data=array(array('name'=>'   Esta  acta es realizada por duplicado a un solo tenor y efecto para los fines de la incorporación '),
				       array('name'=>'   correspondiente, en la ciudad de Sarare 	a los ____dias, del mes__________del año___________. '));
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_pie_cabecera($ls_feccmp,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezSetY(350);
		$ls_dia=substr($ls_feccmp,0,2);
		$ls_mes=substr($ls_feccmp,3,2);
		$ls_annio=substr($ls_feccmp,6,4);
		$ls_mes=$io_fecha->uf_load_nombre_mes($ls_mes);
		//$io_pdf->ezSetY(250);
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
	    $la_data=array(array('name'=>'   Esta  acta es realizada por duplicado a un solo tenor y efecto para los fines de la desincorporación correspondiente,'.
									 ' en la ciudad de Sarare a los '.$ls_dia.' dias, del mes de '.$ls_mes.' del año '.$ls_annio));
		$la_columnas=array('name'=>'','name'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'showLines'=>0, // Mostrar Líneas
						 'fontSize' => 12, // Tamaño de Letras
        ...
        $io_pdf->addText(30,195,10,"JEFE O DIRECTOR DE LA UNIDAD");   
        $io_pdf->addText(30,180,10,"QUIEN RECIBE");   
        $io_pdf->addText(30,165,10,"__________________________________");   
        $io_pdf->addText(30,150,10,"<b>Nombre:</b>");   
        $io_pdf->addText(30,135,10,"<b>C:I. No.:</b>");   
        $io_pdf->addText(30,115,10,"<b>Cargo:</b>");   		

        $io_pdf->addText(370,195,10,"BIENES");   
        $io_pdf->addText(370,165,10,"__________________________________");   
        $io_pdf->addText(370,150,10,"<b>ING. YAINETH HERRERA</b>");   
        $io_pdf->addText(370,135,10,"<b>C:I. No.: 15.339.694</b>");   
        $io_pdf->addText(370,115,10,"<b>ADM. DE BIENES Y SERVICIOS JUNIOR</b>");   
		
		$io_pdf->addText(200,115,10,"TESTIGO");   
        $io_pdf->addText(200,95,10,"__________________________________");   
        $io_pdf->addText(200,80,10,"<b>LCDA. CARMEN FLORES</b>");   
	    $io_pdf->addText(200,65,10,"<b>C:I. No.: 07.390.085</b>");   
        $io_pdf->addText(200,50,10,"<b>DIRECTORA DE ADMINISTRACION Y SERVICIOS</b>");  
	...
	$arrResultado=$io_report->uf_saf_load_fecmovimiento($ls_codemp,$ls_cmpmov);
	$ls_feccmp=$arrResultado["feccmp"];
	$ls_dencau=$arrResultado["dencau"];
	$ls_feccmp=$io_funciones->uf_convertirfecmostrar($ls_feccmp);
	$ls_denunimed=$io_report->uf_saf_load_unidadejecutora($ls_codemp,$ls_coduniadm);
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido==false) // Existe algún error ó no hay registros
	{
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$io_pdf->ezSetCmMargins(6,7,3,3); // Configuración de los margenes en centímetros
		...
		$li_s=0;
		$li_totrow_det=0;
	    uf_cabecera($ls_titulo_emp,$ls_feccmp,$ls_denunimed,$ls_dencau,$io_pdf);
		if($lb_valido)
		{
			$li_aux=0;
			$i=1;
		...
		uf_print_pie_cabecera($ls_feccmp,$io_pdf);
		unset($la_data);			
		unset($la_datat);
		//unset($la_data);
