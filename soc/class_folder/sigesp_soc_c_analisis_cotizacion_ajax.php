...
***********************************************************************************/
	...
	{	
		...
		$lo_object=array();
		$li_total=count((array)$aa_cotizaciones);
	...
	{	
		...
		if($ab_catalogo)//si el cargar biene de seleccionar cotizaciones una a una
			$la_items = uf_cargar_items($la_items);			
		else
			$la_items = uf_select_items_cotizacion($ls_numcot,$ls_codpro,$ls_tipcot,$la_items);
			
		$li_total=count((array)$la_items);
		$li_row = $li_fila-1;
