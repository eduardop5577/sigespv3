...
//		$io_pdf->Rectangle(544,529,240,42);
	...
	function uf_print_proveedores($la_cotizaciones,$io_ds_detalle,$io_ds_detallepro,$la_countcot,$ds_contcol,$li_calculado,$io_pdf)
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			if($li_preuniart_1<=0)
			{
				$la_data[0]["4"]="NO COTIZO";
				$la_data[0]["5"]="NO COTIZO";
			}
			else
			{
			...
			if($li_preuniart_2<=0)
			{
				$la_data[0]["7"]="NO COTIZO";
				$la_data[0]["8"]="NO COTIZO";
			}
			else
			{
			...
			if($li_preuniart_3<=0)
			{
				$la_data[0]["10"]="NO COTIZO";
				$la_data[0]["11"]="NO COTIZO";
			}
			else
			{
	...
	function uf_print_subtotales($ds_contcol,$la_cotizaciones,$la_countcot,$io_ds_detalle,$as_numanacot,$as_tipsolcot,$aa_ganadores,$ds_linea_tot,$io_pdf)
	{
		...
		$ls_estrnc1=$ds_contcol[0]["estrnc1"];
		$ls_estrnc2=$ds_contcol[1]["estrnc2"];
		$ls_estrnc3=$ds_contcol[2]["estrnc3"];
		switch($ls_estrnc1)
		{
			case "R":
				$ls_estrnc1="REGISTRADO";	
			break;
			case "N":
				$ls_estrnc1="NO REGISTRADO";	
			break;
			case "S":
				$ls_estrnc1="SUSPENDIDO";	
			break;
			default:
				$ls_estrnc1="";
			break;
		...
		{
			case "R":
				$ls_estrnc2="REGISTRADO";	
			break;
			case "N":
				$ls_estrnc2="NO REGISTRADO";	
			break;
			case "S":
				$ls_estrnc2="SUSPENDIDO";	
			break;
			default:
				$ls_estrnc2="";
			break;
		...
		{
			case "R":
				$ls_estrnc3="REGISTRADO";	
			break;
			case "N":
				$ls_estrnc3="NO REGISTRADO";	
			break;
			case "S":
				$ls_estrnc3="SUSPENDIDO";	
			break;
			default:
				$ls_estrnc3="";
			break;
		...
		$la_anchos=array();
		$la_justificaciones=array();
		$la_data[0]["0"]="ESTATUS RNC";
		$la_data[0]["1"]=$ls_estrnc1;
		$la_data[0]["2"]=$ls_estrnc2;
		$la_data[0]["3"]=$ls_estrnc3;
		$la_anchos_col = array(69,65,65,65);
		$la_justificaciones = array("left","center","center","center","center","center");
		$la_opciones = array("color_texto"     => array(0,0,0),
							 "anchos_col"    => $la_anchos_col,
	...
	function uf_print_ganadores($as_numanacot,$as_tipsolcot,$aa_ganadores,$io_pdf)
	{
		...
		$la_data=array();
		$li_totalganadores=count((array)$aa_ganadores);
		$li_totalsubtotal=0;
		$li_totaliva=0;
		$li_totalmonto=0;
		for($li_i=0;$li_i<$li_totalganadores;$li_i++)
		{
			...
			$la_items = "";
			$li_totrow = "";
			$arrResultado = $io_class_report->uf_select_items_proveedor($ls_cotizacion,$ls_proveedor,$as_numanacot,$as_tipsolcot,$la_items,$li_totrow); 
			$la_items = $arrResultado['aa_items'];
			$li_totrow = $arrResultado['li_i'];
			$la_totales = "";
			$la_totales = $io_class_report->uf_calcular_montos($li_totrow,$la_items,$la_totales,$ls_tipo_proveedor);
	...
	//------------------------------------------------------------------------------------------------------------------------------------
    //------------------------------------------------------------------------------------------------------------------------------------
    function uf_print_unidades($as_uniejeaso,$io_pdf)
    {
		global $io_pdf;
        ...
        $la_anchos=array();
        $la_justificaciones=array();
        $la_data[0]["1"]="<b>Unidades Asociadas:</b>";
        $la_data[0]["2"]=$as_uniejeaso;        
        $la_anchos_col = array(69,195);
        $la_justificaciones = array("left","left");
        $la_opciones = array("color_texto"     => array(0,0,0),
                               "anchos_col"    => $la_anchos_col,
	...
	function uf_print_pie_pagina($io_pdf)
	{
		...
		$io_pdf->addText(280,90,8,"<b>REVISADO POR:</b>");
                
		//$io_pdf->addText(243,32,8,"<b>JEFE DE LA UNIDAD DE COMPRAS</b>");
		$io_pdf->addText(228,32,8,"<b>COODINADOR(A) DEL ÁREA DE COMPRAS</b>");
		...
		//$io_pdf->addText(405,32,8,"<b>DIRECTOR (A) DE SERVICIOS ADMINISTRATIVO</b>");
                $io_pdf->addText(450,90,8,"<b>APROBADO POR:</b>");
		$io_pdf->addText(430,32,8,"<b>DIRECTOR(A) DE ADMINISTRACIÓN</b>");

                $io_pdf->line(594,30,593,100);	//VERTICAL

                
		$io_pdf->addText(670,90,8,"<b>SELLO</b>");
    ...
    function uf_agrupa_cotizaciones($ls_countcot,$la_cotizaciones,$la_cotizaciones_ag,$ds_contcol,$io_ds_detalle)
    {
        ...
        $row_items=count((array)$io_ds_items);
        $li_totrow=count((array)$la_cotizaciones);
	...
	if($lb_valido)
	{
        ...
        $ls_codpro2 = '';
        $ls_codpro3 = '';
        $arrResultado=$io_class_report->uf_cargar_cotizaciones_v2($ls_numanacot,$la_cotizaciones,$ls_tipsolcot,$la_proveedor,$li_cotizaciones,$ls_codpro1,$ls_codpro2,$ls_codpro3);
		$la_cotizaciones = $arrResultado['aa_proveedores'];
		$la_proveedor = $arrResultado['la_proveedor'];
		$li_cotizaciones = $arrResultado['li_cotizaciones'];
		$ls_codpro1 = $arrResultado['ls_codpro1'];
		$ls_codpro2 = $arrResultado['ls_codpro2'];
		$ls_codpro3 = $arrResultado['ls_codpro3'];
		$lb_valido=$arrResultado['lb_valido'];
		$ls_uniejeaso=$io_class_report->uf_select_unidades_cotizacion($ls_numanacot);
		if($lb_valido)
		{
            ...
            if ($li_totcot==1)
            {
                ...
                $io_ds_detallepro->insertRow('estrnc_1',$la_proveedor[1]["estrnc"]);
                $io_ds_detallepro->insertRow('estrnc_2','' );
                $io_ds_detallepro->insertRow('estrnc_3','');
            } 
            ...
            if ($li_totcot==2)
            {
                ...
                $io_ds_detallepro->insertRow('estrnc_1',$la_proveedor[1]["estrnc"]);
                $io_ds_detallepro->insertRow('estrnc_2',$la_proveedor[2]["estrnc"]);
                $io_ds_detallepro->insertRow('estrnc_3','');
            }
            ...
            if ($li_totcot==3)
            {
                ...
                $io_ds_detallepro->insertRow('estrnc_1',$la_proveedor[1]["estrnc"]);
                $io_ds_detallepro->insertRow('estrnc_2',$la_proveedor[2]["estrnc"]);
                $io_ds_detallepro->insertRow('estrnc_3',$la_proveedor[3]["estrnc"]);
            }
			...
			$cCol=1;
                ...
                $estrnc1     = $io_ds_detallepro->getValue('estrnc_1',1 );
                $estrnc2     = $io_ds_detallepro->getValue('estrnc_2',1 );
                $estrnc3     = $io_ds_detallepro->getValue('estrnc_3',1 );
				...
				$ds_col1 = array("nombre1"=>$nompro1,"rif1"=>$rifpro1,"dirpro1"=>$dirpro1,"feccot1"=>$feccot1,"codpro1"=>$codpro1,"diaentcom1"=>$diaentcom1,"forpagcom1"=>$forpagcom1,"estasitec1"=>$estasitec1,"estesp1"=>$estesp1,"garanacot1"=>$garanacot1,"diavalofe1"=>$diavalofe1,"estrnc1"=>$estrnc1); //
				$ds_col2 = array("nombre2"=>$nompro2,"rif2"=>$rifpro2,"dirpro2"=>$dirpro2,"feccot2"=>$feccot2,"codpro2"=>$codpro2,"diaentcom2"=>$diaentcom2,"forpagcom2"=>$forpagcom2,"estasitec2"=>$estasitec2,"estesp2"=>$estesp2,"garanacot2"=>$garanacot2,"diavalofe2"=>$diavalofe2,"estrnc2"=>$estrnc2); //
				$ds_col3 = array("nombre3"=>$nompro3,"rif3"=>$rifpro3,"dirpro3"=>$dirpro3,"feccot3"=>$feccot3,"codpro3"=>$codpro3,"diaentcom3"=>$diaentcom3,"forpagcom3"=>$forpagcom3,"estasitec3"=>$estasitec3,"estesp3"=>$estesp3,"garanacot3"=>$garanacot3,"diavalofe3"=>$diavalofe3,"estrnc3"=>$estrnc3); //
			...
			if($lb_valido)
			{
				...
				uf_print_unidades($ls_uniejeaso,$io_pdf);
