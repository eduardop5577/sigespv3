...
***********************************************************************************/
	...
	function uf_print_analisiscualitativo($aa_cotizaciones,$aa_precios,$io_pdf)
	{
		...
		$io_pdf->add_tabla(10,$la_data,$la_opciones);	//primera fila del item, color gris	
		unset($la_data);
		$li_cot=count((array)$aa_cotizaciones);
	...
	function uf_print_analisisproveedores($aa_cotizaciones,$aa_proveedores,$io_pdf)
	{
		...
		$totalitems=count((array)$la_items);
		$li_k=0;
		$ls_item1=$la_items[0]; 
		$li_totalcalificadores=count((array)$aa_proveedores[$ls_item1]);
	...
	if($lb_valido)
	{
		...
		if($lb_valido)
		{
			...
			$la_items = "";
			$arrResultado=$io_class_report->uf_select_items($ls_numanacot,$ls_tipsolcot,$la_items);
			$la_items = $arrResultado['la_items'];
			$lb_valido=$arrResultado['lb_valido'];
			if($lb_valido)
			{
				...
				$io_pdf->ezNewPage();
				$la_precios="";
				$arrResultado=$io_class_report->uf_select_precios_cotizacion($ls_tipsolcot,$la_cotizaciones,$la_precios);//print_r($la_precios);
				$la_precios = $arrResultado['aa_items'];
				$lb_valido = $arrResultado['lb_valido'];
				if($lb_valido)
				{
				...
				$la_cualitativo = "";
				$arrResultado=$io_class_report->uf_analisis_cualitativo($la_cotizaciones,$la_cualitativo);//print_r($la_cualitativo);
				$la_cualitativo = $arrResultado['la_arre2'];
				$lb_valido = $arrResultado['lb_valido'];
				if(($lb_valido)&&(!empty($la_cualitativo)))
				{
					uf_print_analisisproveedores($la_cotizaciones,$la_cualitativo,$io_pdf);
				}
				$arrResultado=$io_class_report->uf_analisis_cualitativo_items($ls_tipsolcot,$la_cotizaciones,$la_items);//print_r($aa_items);
				$la_items = $arrResultado['aa_items'];
				$lb_valido = $arrResultado['lb_valido'];
				if($lb_valido)
				{
