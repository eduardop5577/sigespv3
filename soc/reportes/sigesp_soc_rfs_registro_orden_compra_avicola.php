...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],25,723,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		if ($as_estcom=='3')
		{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/delagro.jpg',505,723,80,60); // Agregar Logo
		
		$li_tm=$io_pdf->getTextWidth(7,"REPUBLICA  BOLIVARIANA DE VENEZUELA");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,770,7,"REPUBLICA  BOLIVARIANA DE VENEZUELA"); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(7,"MINISTERIO DEL PODER POPULAR PARA LA AGRICULTURA Y TIERRAS");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,762,7,"MINISTERIO DEL PODER POPULAR PARA LA AGRICULTURA Y TIERRAS"); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(7,"CORPORACIÓN VENEZOLANA DE ALIMENTOS");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,754,7,"CORPORACIÓN VENEZOLANA DE ALIMENTOS"); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(7,"EMPRESA MIXTA SOCIALISTA AVÍCOLA DEL ALBA, S.A");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,746,7,"EMPRESA MIXTA SOCIALISTA AVÍCOLA DEL ALBA, S.A"); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(7,"Av. Circunvalación Sur, Local Galpón Cval, Zona Industrial");
		$tm=291-($li_tm/2);
		$io_pdf->addText($tm,738,7,"Av. Circunvalación Sur, Local Galpón Cval, Zona Industrial"); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(7,"Acarigua Estado Portuguesa Teléfono: 0255-6211243/ <b>pypemsaa@gmail.com</b> ");
		$tm=291-($li_tm/2);
		$io_pdf->addText($tm,730,7,"Acarigua Estado Portuguesa Teléfono: 0255-6211243/ <b>pypemsaa@gmail.com</b> "); // Agregar el título

		$li_tm=$io_pdf->getTextWidth(11,$ls_titulo);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,705,10,$ls_titulo); // Agregar el título
		$io_pdf->addText(485,715,9,"No. ".$as_numordcom); // Agregar el título
		$io_pdf->addText(485,695,9,"Fecha ".$ad_fecordcom); // Agregar el título
...
//		$io_pdf->addText(485,710,9,"<b>Fecha </b>".$ad_fecordcom); // Agregar el título
		...
		$io_pdf->addText(18,121,6,"ELABORADO POR:"); // Agregar el título
		$io_pdf->addText(18,111,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(45,74,6,"ANALISTA"); // Agregar el título

		$io_pdf->line(105,60,105,130);	//VERTICAL
		$io_pdf->addText(108,121,6,"REVISADO POR:"); // Agregar el título
		$io_pdf->addText(108,111,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(130,74,6,"JEFE DE COMPRAS"); // Agregar el título

		$io_pdf->line(215,60,215,130);	//VERTICAL
        $io_pdf->addText(218,121,6,"CONTABILIZADO POR:"); // Agregar el título
		$io_pdf->addText(218,111,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(230,74,6,"GERENTE DE PLANIFICACION"); // Agregar el título
		$io_pdf->addText(245,68,6," Y PRESUPUESTO"); // Agregar el título

		$io_pdf->line(335,60,335,130);	//VERTICAL
		$io_pdf->addText(338,121,6,"APROBADO POR:"); // Agregar el título
		$io_pdf->addText(338,111,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(360,74,6,"GERENTE DE ADMINISTRACION"); // Agregar el título
		$io_pdf->addText(385,68,6,"Y FINANZAS"); // Agregar el título

		$io_pdf->line(475,60,475,130);	//VERTICAL
		$io_pdf->addText(478,121,6,"APROBADO POR:"); // Agregar el título
		$io_pdf->addText(478,111,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(505,74,6,"PRESIDENTA"); // Agregar el título
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			$io_pdf->ezSetCmMargins(9,6,3,3); // Configuración de los margenes en centímetros
