...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_clausulas($la_data,$io_pdf)
	{
		...
		$io_pdf->ezSetDy(-10);
		$la_datatit[1]=array('titulo'=>'<b> Cláusulas </b>');
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
			   ...
			   $arrResultado = $io_report->uf_select_clausulas($ls_numordcom,$ls_estcondat,$lb_valido);
			   $rs_datos_clausulas = $arrResultado['rs_data'];
			   $lb_valido = $arrResultado['lb_valido'];
			   if ($lb_valido)
			   {
		     	 $li_totrows = $io_sql->num_rows($rs_datos_clausulas);
				 if ($li_totrows>0)
				 {
				    $li_i = 0;
				    while($row=$io_sql->fetch_row($rs_datos_clausulas))
					{
						$li_i=$li_i+1;
						$ls_dencla=$row["dencla"];
						$la_data[$li_i]=array('titulo'=>' -  '.$ls_dencla);
					}
					uf_print_clausulas($la_data,$io_pdf);
					unset($la_data);
			      }
		       }
	     	}
