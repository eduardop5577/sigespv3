...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,$ls_telpro,$ls_numsep,$ls_fecsep,
									   $ls_numsolcot,$ls_lugentdir,$ls_lugentnomdep,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,700,12,$ls_titulo); // Agregar el título
		$io_pdf->addText(260,715,10,$ls_rifemp); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(10,$ls_nomemp);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,725,10,$ls_nomemp); // Agregar el título
		$io_pdf->addText(175,735,10,"DIRECCION GENERAL DE EMPRESAS Y SERVICIOS"); // Agregar el título
		$io_pdf->addText(215,745,10,"VICEMINISTERIO DE SERVICIOS"); // Agregar el título
		$io_pdf->addText(165,755,10,"MINISTERIO DEL PODER POPULAR PARA LA DEFENSA"); // Agregar el título
		$io_pdf->addText(195,765,10,"REPUBLICA BOLIVARIANA DE VENEZUELA"); // Agregar el título
		...
		// cuadro inferior
        $io_pdf->Rectangle(15,20,570,70); 
		$io_pdf->line(15,77,585,77);	//HORIZONTAL	
		$io_pdf->addText(40,82,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(45,23,7,"COMPRADOR"); // Agregar el título
		$io_pdf->line(130,20,130,90);	//VERTICAL	
		$io_pdf->addText(167,82,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(155,23,7,"DIRECTOR DE OCHINA"); // Agregar el título
		$io_pdf->addText(135,35,6,"VA. LUISA MARIA LOZADA FERGUSSON"); // Agregar el título
		$io_pdf->line(260,20,260,90);	//VERTICAL		
		$io_pdf->addText(295,82,7,"CONFORMADO POR"); // Agregar el título
		$io_pdf->addText(280,23,7,"DIRECTOR ADMON/FINANZAS"); // Agregar el título
		$io_pdf->addText(280,35,6,"CN. OVELIO BARRERA CORRALES"); // Agregar el título
		$io_pdf->line(410,20,410,90);	//VERTICAL		
		$io_pdf->addText(465,82,7,"REVISADO POR"); // Agregar el título
		$io_pdf->addText(440,23,7,"JEFE DIVISION ADQUISICIONES"); // Agregar el título
		$io_pdf->addText(425,35,6,"TN. (RA) DANYELS ISMAEL GONZALEZ TINEO"); // Agregar el título
		$io_pdf->line(15,33,585,33); //HORIZONTAL		
		
		$io_pdf->ezSetY(695);
		$la_data[1]=array('columna1'=>'<b>Proveedor</b>  '.$as_nombre.'<b>             Rif</b> '.$as_rifpro,
		                 'columna2'=>'<b>Direccion</b> '.$as_dirpro.'     <b>Telf </b>'.$ls_telpro);
		...
		$la_data[1]=array('columna1'=>'<b>No. SEP </b>    '.$ls_numsep.' <b> Fecha</b> '.$ls_fecsep,'columna2'=>'<b>Condiciones de Pago</b>    '.$ls_forpagcom);
		...
		$la_data[1]=array('columna1'=>'<b>Fuente Financiamiento</b>   '.$ls_fuefin,'columna2'=>'<b> Tiempo de Entrega</b>    '.$as_diaplacom.' dias');
		$la_columnas=array('columna1'=>'','columna2'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
		...
		$la_data[1]=array('columna1'=>'<b>Solicitud de Cotizacion</b>','columna2'=>'<b> Contabilidad C/O</b>','columna3'=>'<b> Contabilidad</b>','columna4'=>'<b> Presupuesto</b>');
		$la_data[2]=array('columna1'=>$ls_numsolcot,'columna2'=>'   ','columna3'=>'    ','columna4'=>'    ');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('columna1'=>array('justification'=>'left','width'=>142), // Justificación y ancho de la columna
						 			   'columna2'=>array('justification'=>'left','width'=>143), // Justificación y ancho de la columna
						 			   'columna2'=>array('justification'=>'left','width'=>143), // Justificación y ancho de la columna
						 			   'columna2'=>array('justification'=>'left','width'=>142))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
		...
		$la_data[1]=array('columna1'=>'<b>Lugar de Entrega</b>   '.$ls_lugentdir,'columna2'=>'<b> Entregar a</b>    '.$ls_lugentnomdep);
		$la_columnas=array('columna1'=>'','columna2'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
...
*/		$la_data[1]=array('columna1'=>'<b>Concepto</b>         '.$as_conordcom);
	...
	function uf_print_detalle($as_estcondat,$la_data,$io_pdf)
	{
		...
		if($as_estcondat=="B")
		{
			$ls_titulo_grid="SIRVASE DESPACHAR EL SIGUIENTE MATERIAL DE ACUERDO A LAS INSTRUCCIONES ESPECIFICADAS";
		}
		else
		{
			$ls_titulo_grid="DETALLE DE SERVICIOS";
		}
		$io_pdf->ezSetDy(-10);
		$la_datatitulo[1]=array('columna1'=>'<b> '.$ls_titulo_grid.'</b>');
		$la_columnas=array('columna1'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'maxWidth'=>570, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('renglon'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'right','width'=>50), // Justificación y ancho de la columna
...
/*		$la_data[1]=array('titulo'=>'<b> Son: '.$ls_monlet.'</b>');
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_piecondiciones($io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_piecabecera
		//		    Acess: private 
		...
		$la_data[1]=array('titulo'=>'CONDICIONES GENERALES');
		$la_columnas=array('titulo'=>'CONDICIONES GENERALES');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('titulo'=>'PRIMERA: El Proveedor Deberá pagar al Servicio Autónomo Ochina el 1% sobre el monto total de la presente contratación, por cada día de retardo en el'.
									'cumplimiento del objeto del presente instrumento: dicho pago no excederá al 15% del monto total de la presente contratación.');
		$la_data[2]=array('titulo'=>'SEGUNDA: Las facturas deberán indicar el numero del presente instrumento contractual.');
		$la_data[3]=array('titulo'=>'TERCERA: El proveedor deberá presentar independientemente del monto de la contratación:');
		$la_data[4]=array('titulo'=>'A) Declaración de No Contar Dentro de su Organización, con Personas Naturales, Socios, Miembros o Administradores de Alguna Empresa, Sociedad o Agrupación con que se encuentre inhabilitada. ');
		$la_data[5]=array('titulo'=>'B) Declaración Jurada de No Tener Obligaciones Exigibles con OCHINA (contrataciones no finiquitadas en ejecución o por ejecutar). ');
		$la_data[6]=array('titulo'=>'CUARTA: Cuando la contratación sea superior a las 2.500 UT,  deberá anexarse: ');
		$la_data[7]=array('titulo'=>' A) Los instrumentos que contemplen la garantías contractuales que le sean aplicables.');
		$la_data[8]=array('titulo'=>'B) Acta de Compromiso de Responsabilidad Social de 3% sobre el monto total de la contratación sin incluir el IVA, como muestra de la aceptación de la obligación. ');
		$la_data[9]=array('titulo'=>'Acto administrativo que se suscribe en virtud de las atribuciones contenidas en el artículos 19 del Reglamento de la Ley de Faros y Boyas del'.
									' Sistema Nacional de Ayudas a la Navegación Acuática, publicado en Gaceta Oficial de la Republica Bolivariana de Venezuela Nº 41.010 de fecha 17OCT16. ');
		$la_columnas=array('titulo'=>'CONDICIONES GENERALES');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>540, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('titulo'=>array('justification'=>'JUSTIFY','width'=>570))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
										   ...
										   $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$ls_estcom,$ls_telpro,$ls_numsep,
										   $ls_fecsep,$ls_numsolcot,$ls_lugentdir,$ls_lugentnomdep,$io_pdf);
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
				    ...
				    while($row=$io_sql->fetch_row($rs_datos))
					{
						...
						$la_data[$li_i]=array('renglon'=>$li_i,'codigo'=>$ls_codartser,'denominacion'=>$ls_denartser,'cantidad'=>$li_cantartser,
		...
		uf_print_piecabecera($ld_monsubtot,$ld_monimp,$ld_montot,$ls_monto,$io_pdf);
		 uf_print_piecondiciones($io_pdf);
	} 	  	 
