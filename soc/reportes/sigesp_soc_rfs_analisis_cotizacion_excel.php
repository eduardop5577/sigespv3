...
***********************************************************************************/
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;$li_i<=$li_total;$li_i++)
		{
			...
			$arrResultado=$io_class_report->uf_select_items_solicitud($ls_numcot,$ls_codpro,$ls_numanacot,$ls_tipsolcot,$io_ds,$li_j);
			$io_ds = $arrResultado['aa_items'];
			$li_j = $arrResultado['li_i'];
			$lb_valido=$arrResultado['lb_valido'];
			for($li_k=1;$li_k<=$li_articulos;$li_k++)
			{
				$li_line=10;
		...
		$lo_hoja->write($li_newline+8, 0, "OBSERVACIONES:",$lo_dataleft);
		...
		for($li_i=$li_newline+12;$li_i<$li_totalganadores+$li_newline+12;$li_i++)
		{
			...
			$la_items = "";
			$li_totrow = "";
			$arrResultado = $io_class_report->uf_select_items_proveedor($ls_cotizacion,$ls_proveedor,$ls_numanacot,$ls_tipsolcot,$la_items,$li_totrow); 
		...
		$li_i=$li_i+2;
		$lo_hoja->write($li_i, 0, "Unidades Asociadas",$lo_encabezado);
			$lo_hoja->write($li_i, 1, $ls_uniejeaso,$lo_dataleftcombinado1);
			$lo_hoja->write_blank($li_i, 2,                $lo_dataleftcombinado2);
			$lo_hoja->write_blank($li_i, 3,                $lo_dataleftcombinado2);
			$lo_hoja->write_blank($li_i, 4,                $lo_dataleftcombinado2);
			$lo_hoja->write_blank($li_i, 5,                $lo_dataleftcombinado2);
