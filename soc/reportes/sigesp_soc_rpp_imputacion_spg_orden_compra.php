...
***********************************************************************************/
	...
	$arrResultado = $io_report->uf_select_imputacion_spg_orden_compra($ls_numordcomdes,$ls_numordcomhas,$ls_fecordcomdes,$ls_fecordcomhas,$ls_estcondat,$ls_tipo,$lb_valido);
    ...
    $arrResultado = $io_report->uf_select_imputacion_cuentas_spg_orden_compra($ls_numordcomdes,$ls_numordcomhas,$ls_fecordcomdes,$ls_fecordcomhas,$ls_estcondat,$ls_tipo,$lb_valido);
	$rs_data_spg = $arrResultado['rs_data_spg'];
	$lb_valido = $arrResultado['lb_valido'];
			...
			$ls_spg_cuenta_buscar   = $row_spg["spg_cuenta"];			
                        ...
                        for ($li_i=1;$li_i<=$li_total_row;$li_i++)
                        {
                            ...
                            if ($ls_orden==$ls_orden_buscar)
                            {
                                ...
                                if (($ls_orden==$ls_orden_buscar)and(trim($ls_spg_cuenta)==trim($ls_spg_cuenta_buscar)))
                                {
									$li_registro++;
	...
	else // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			{
				print("<script language=JavaScript>");
				print("alert('No hay nada que reportar 3');"); 
				//print("close();");
				print("</script>");		
			}				
		}	
