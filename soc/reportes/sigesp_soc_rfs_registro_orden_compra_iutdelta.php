...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
	                                   $as_denfuefin,$as_codigo,$as_nombre,$as_conordcom,$as_rifpro,$as_diaplacom,$as_dirpro,
										$as_telpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->line(480,700,480,740); //Vertical
		$io_pdf->line(585,700,585,740); //Vertical
		$io_pdf->line(480,740,585,740); //Horizontal
		$io_pdf->line(480,720,585,720); //Horizontal
		$io_pdf->line(480,700,585,700); //Horizontal

		$io_pdf->addJpegFromFile('../../shared/imagebank/MES_Nuevo.jpg',20,710,80,60); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],400,705,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		
		$texto1="REPUBLICA BOLIVARIANA DE VENEZUELA"; 
		$texto2="MINISTERIO DEL PODER POPULAR";
		$texto3="PARA LA EDUCACION UNIVERSITARIA";
		$texto4="INSTITUTO UNIVERSITARIO DE TECNOLOGÍA";
		$texto5="DR DELFÍN MENDOZA";
		$texto6="TUCUPITA - EDO. DELTA AMACURO";
		$texto7="RIF: G-20005821-8 NIT: 0482826288";
		$y=772;
		$io_pdf->addTextWrap(110,$y,300,7,$texto1,'centre');
		$io_pdf->addTextWrap(110,$y-8,300,7,$texto2,'centre');
		$io_pdf->addTextWrap(110,$y-16,300,7,$texto3,'centre');
		$io_pdf->addTextWrap(110,$y-24,300,7,$texto4,'centre');
		$io_pdf->addTextWrap(110,$y-32,300,7,$texto5,'centre');
		$io_pdf->addTextWrap(110,$y-40,300,7,$texto6,'centre');
		$io_pdf->addTextWrap(110,$y-48,300,7,$texto7,'centre');
		...
		$io_pdf->addTextWrap(110,$y-63,300,14,$ls_titulo,'centre');
		$io_pdf->addText(485,725,9,"<b>No. </b>".$as_numordcom); // Agregar el título
		$io_pdf->addText(485,705,9,"<b>Fecha </b>".$ad_fecordcom); // Agregar el título
		$io_pdf->addText(515,750,8,"Página"); // Agregar la palabra Página
		...
		$y=155;
		//$io_pdf->Rectangle($x1,$y+15,$x2-15,$y+70); 
		$io_pdf->line($x1,$y+15,$x2,$y+15);		//HORIZONTAL
		$io_pdf->line($x1,$y,$x2,$y);			//HORIZONTAL
		$io_pdf->line($x1,$y-15,$x2,$y-15);		//HORIZONTAL
		$io_pdf->line($x1,$y-55,$x2,$y-55);		//HORIZONTAL
		$io_pdf->line($x1,$y-70,$x2,$y-70);		//HORIZONTAL
		$t=113;
		$io_pdf->line($x1,$y+15,$x1,$y-70);		//VERTICAL
		$io_pdf->line($x1+$t,$y+15,$x1+$t,$y-70);		//VERTICAL
		$io_pdf->line($x1+$t*2,$y+15,$x1+$t*2,$y-70);	//VERTICAL
		$io_pdf->line($x1+$t*3,$y+15,$x1+$t*3,$y-70);	//VERTICAL
		$io_pdf->line($x1+$t*4,$y+15,$x1+$t*4,$y-70);	//VERTICAL
		$io_pdf->line($x2,$y+15,$x2,$y-70);		//VERTICAL

		
		$io_pdf->addText($x1+5,$y+6,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText($x1+5,$y-9,7,"FECHA:___/___/_____"); // Agregar el título
		$io_pdf->addText($x1+5,$y-65,7,"COMPRAS"); // Agregar el título


		$io_pdf->addText($x1+5+$t,$y+6,7,"AUTORIZADO POR"); // Agregar el título		
		$io_pdf->addText($x1+5+$t,$y-9,7,"FECHA:___/___/_____"); // Agregar el título
		$io_pdf->addText($x1+5+$t,$y-65,7,"COORDINADOR"); // Agregar el título

		$io_pdf->addText($x1+5+$t*2,$y+6,7,"AUTORIZADO POR"); // Agregar el título		
		$io_pdf->addText($x1+5+$t*2,$y-9,7,"FECHA:___/___/_____"); // Agregar el título
		$io_pdf->addText($x1+5+$t*2,$y-65,7,"ADMINISTRADOR"); // Agregar el título

		$io_pdf->addText($x1+5+$t*3,$y+6,7,"DISP.PRESUPUESTARIA"); // Agregar el título		
		$io_pdf->addText($x1+5+$t*3,$y-9,7,"FECHA:___/___/_____"); // Agregar el título
		$io_pdf->addText($x1+5+$t*3,$y-65,7,"PRESUPUESTO"); // Agregar el título

		$io_pdf->addText($x1+5+$t*4,$y+6,7,"PROVEEDOR"); // Agregar el título		
		$io_pdf->addText($x1+5+$t*4,$y-9,7,"FECHA:___/___/_____"); // Agregar el título
		$io_pdf->addText($x1+5+$t*4,$y-65,7,"FIRMA / SELLO"); // Agregar el título


		$texto1="La aceptación por parte del proveedor de esta orden de compra/servicios obliga al cumplimiento estricto de las condiciones de entrega,"; 
		$texto2="en consecuencia, cualquier retraso o incumplimiento de dichas condiciones genera una sancion equivalente al 10% del precio  convenido,";
		$texto3="por cada día de retraso, sin perjuicio de las sanciones establecidas en las leyes vigentes aplicables en la materia.";
		$texto4="El Instituto Universitario de Tecnología Dr. Delfín Mendoza, se reserva en derecho de anular, de forma unilateral esta orden, sin  pago de";
		$texto5="indemnización alguna, de conformidad con lo dispuesto en el Reglamento que rige la materia.";
		
		$io_pdf->addTextWrap($x1,$y-80,570,9,$texto1);
		$io_pdf->addTextWrap($x1,$y-89,570,9,$texto2);
		$io_pdf->addTextWrap($x1,$y-98,570,9,$texto3);
		$io_pdf->addTextWrap($x1,$y-115,570,9,$texto4);
		$io_pdf->addTextWrap($x1,$y-124,570,9,$texto5);
		...
		$la_data[1]=array('columna1'=>'<b>Proveedor</b>  '.$as_nombre.'<b>             Rif</b> '.$as_rifpro,
		                 'columna2'=>'<b>Direccion</b> '.$as_dirpro.'<b>        Telefono</b> '.$as_telpro.'');
		...
		if($lb_valido)	
		{
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
									...
									$ls_diaplacom,$ls_dirpro,$ls_telpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$io_pdf);
