...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,$ls_nomusureg,$ld_montotaux,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// cuadro inferior
        $io_pdf->Rectangle(15,60,570,55);
		$io_pdf->line(140,130,140,60);	// lineas verticales que separan una caja de otra
		$io_pdf->line(280,130,280,60);	// lineas verticales que separan una caja de otra
		$io_pdf->line(420,130,420,60);	// lineas verticales que separan una caja de otra
		
		$io_pdf->line(300,200,300,130);	// lineas verticales que separan una caja de otra
		
		
		$io_pdf->addText(65,138,6,""); // Agregar el título
		$io_pdf->addText(65,138,6,""); // Agregar el título
		
		$io_pdf->Rectangle(15,60,570,70);
        $io_pdf->Rectangle(100,130,400,70); //RECTANGULO PARA PRESUPUESTO
		$io_pdf->line(100,185,500,185); //linea superior rectangulo de validacion presupuestaria

		if ($ld_montotaux > 1500000){
			//$io_pdf->addText(470,75,7,"<b>DIRECTOR GENERAL IND (E)</b>"); // 10-07-2015
			$io_pdf->addText(470,75,7,"<b>PRESIDENTE IND (E)</b>"); // Agregar la Fecha
		}else{
		$io_pdf->addText(455,75,7,"<b>DIRECTOR GENERAL IND </b>"); // 10-07-2015
		//	$io_pdf->addText(470,75,7,"<b>DIRECTORIO IND (E)</b>"); // Agregar la Fecha $ls_bolivares
			}
		
		//$io_pdf->line(200,60,200,130);		
		//$io_pdf->line(400,60,400,130);		
		$io_pdf->addText(50,120,7,"ELABORADO POR"); // Agregar el título del usuario que elaboro orden
		$io_pdf->addText(60,63,7,"FIRMA"); // Agregar el título para que firme el usuario
		$io_pdf->addText(170,120,7,"UNIDAD RESPONSABLE"); // Agregar el título
		$io_pdf->addText(310,120,7,"DG. ADMIN. Y FINANZAS"); // Agregar el título
		$io_pdf->addText(190,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(320,63,7,"FIRMA / SELLO"); // FIRMA Y SELLO DEL DG DE ADMIN Y FINANZAS
		$io_pdf->addText(30,74,7,$ls_nomusureg); // Agregar el título
		$io_pdf->addText(475,120,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(440,63,7,"FIRMA  AUTÓGRAFA, SELLO, FECHA"); // firma de la maxima autoridad
		$io_pdf->addText(160,135,7,"FIRMA, SELLO, FECHA"); // Agregar Presupuesto
		$io_pdf->addText(135,148,7,"D.G. PLANIFICACIÓN Y PRESUPUESTO"); // Validar Presupuesto
		$io_pdf->addText(150,187,7,"VALIDACION PRESUPUESTARIA"); // Agregar el Presupuesto
		$io_pdf->addText(380,187,7,"PROVEEDOR "); // Agregar el Presupuesto
		$io_pdf->addText(377,135,7,"FIRMA,  FECHA"); // Agregar Presupuesto
		$io_pdf->ezSetY(695); 
	...
	function uf_print_detalle($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras 9
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos 12
						 'showLines'=>1, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
						 'width'=>570, // Ancho de la tabla
						 'maxWidth'=>570, // Ancho Máximo de la tabla
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			$io_pdf->ezSetCmMargins(8,7.5,3,3); // Configuración de los margenes en centímetros 
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
										   ...
										   $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$ls_estcom,$ls_nomusureg,$ld_montotaux,$io_pdf);
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
				    ...
				    while($row=$io_sql->fetch_row($rs_datos))
					{
						...
						$ld_carartser=number_format($ld_totartser-$ld_subtotartser,2,'.','');
						if($ld_carartser==0)
						{
					...
					if($lb_valido)
					{
						 ...
						 if ($li_totrows>0)
						 {
							...
							while($row=$io_sql->fetch_row($rs_datos_cuenta))
							{
								...
								$ls_programatica = "";
								$ls_programatica = $io_fun_soc->uf_formatoprogramatica($ls_codpro,$ls_programatica);
