...
$arrResultado = $io_fun_compra->uf_load_seguridad("SOC","sigesp_soc_p_analisis_cotizacion.php",$ls_permisos,$la_seguridad,$la_permisos);
...
$ls_reporte_excel=$io_fun_compra->uf_select_config("SOC","REPORTE","ANALISIS_COTIZACION_EXCEL","sigesp_soc_rfs_analisis_cotizacion_excel.php","C");
...
$ls_valunitri=$io_analisis_cotizacion->uf_valor_unidad_tributaria();
?>
...
<p>
  <?php 
   function uf_limpiarvariables()
   {
		...
		$li_estciespi = 0;
		$li_estciescg = 0;
		$arrResultado = $io_fun_compra->uf_load_estatus_cierre($li_estciespi,$li_estciescg);
...
?>
  <table width="800" border="0" cellpadding="0" cellspacing="0" class="formato-blanco"  >
	  <tr class="titulo-ventana">
        ...
        <input name="valunitri" type="hidden" id="valunitri" value="<?php print $ls_valunitri; ?>"></td>
	  </tr>
	  ...
	  <tr>
		...
		<td height="22" colspan="6" style="text-align:left "><div align="left">
		  <input name="txtobservacion" type="text" id="txtobservacion" value="<?php print $ls_observacion ?>" size="127" maxlength="254" <?php echo $ls_disabled; ?>  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyzáéíóú ()@#!%/[]*-+_,.$¿?¡');">
		  </div></td>
	  </tr>
	  <tr>
        <td height="22" colspan="2" style="text-align:right">Recomendaci&oacute;n</td>
	    <td height="22" colspan="6" style="text-align:left "><input name="txtrecanacot" type="text" id="txtrecanacot" value="<?php print $ls_recanacot; ?>" size="127" maxlength="254" <?php echo $ls_disabled; ?>  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyzáéíóú ()@#!%/[]*-+_,.$¿?¡');"></td>
      </tr>
	  <tr>
...
if (uf_evaluate_cierre())
   {
	if(confirm("¿Está seguro de eliminar la cotización?"))
	{
		...
		//f.totalcotizaciones.value=f.totalcotizaciones.value-1;		
				  ...
				  if (j!=fila)
				     {
					   ...
					   parametros = parametros+"&txtnumsol"+li_cot+"="+numsol+"&txtnumcot"+li_cot+"="+numcot+"&txtnompro"+li_cot+"="+nompro+"&txtcodpro"+li_cot+"="+codpro+
							   "&txtfecha"+li_cot+"="+fecha+"&txtmonto"+li_cot+"="+monto+"&txtiva"+li_cot+"="+iva+"&txttipsolcot"+li_cot+"="+tipsolcot;						  
				     }
			...
			parametros=parametros+"&totalcotizaciones="+li_cot;	
  ...
  if (uf_evaluate_cierre())
     {
	   ...
	   unidadtributaria=f.valunitri.value;
	   uni5000=(parseFloat(unidadtributaria) * 5000);
	   uni2500=(parseFloat(unidadtributaria) * 2500);
	   if (((lb_catalogo=="T")&&(li_cambiar==1))||(lb_catalogo=="")&&(li_incluir==1))
	      {
			...
			else 
			{
				...
				for(li_i=1;li_i<=totalitem;li_i++)
				{
					...
					monto=eval("f.txtmonselec"+li_i+".value");
			...
			if(parseFloat(montototal)>parseFloat(uni5000))
			{
				alert("El procedimiento no se puede llevar a cabo. Debe remitirse a la Comision de Contrataciones Publicas segun Art. 73.");
			}
			else
			{
				if(parseFloat(montototal)>parseFloat(uni2500))
				{
					alert("El procedimiento debe ser sometido a consideracion de la Comision de Contrataciones Publicas segun Art. 75.");
				}
			}
		    if (lb_valido)
		       { 
...
function ue_ayuda()
{
	...
	window.open("ayudas/sigesp_ayu_soc_analisis_cotizacion.pdf","Ayuda","menubar=no,toolbar=no,scrollbars=yes,width="+width+",height="+height+",resizable=yes,location=no");
}
