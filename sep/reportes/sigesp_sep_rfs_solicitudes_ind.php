...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_nomusu,$lb_nombreusu,$li_montof,$li_codtipsol,$as_estsol,$io_pdf)
	{
		...
		$tm2=300-($li_tm2/2);
		$tm3=350-($li_tm3/2);
		$io_pdf->addText($tm,750,12,$as_titulo); // Agregar el título
		$io_pdf->addText($tm2,740,10,$ls_titulo2); // Agregar el título
		...
		$io_pdf->addText(78,122,7,"ELABORADO POR"); // Agregar el título del usuario que elaboro sep
		$io_pdf->addText(80,63,7,"FIRMA"); // Agregar el título para que firme el usuario
		$io_pdf->addText(60,75,7,$lb_nombreusu); // Agregar el nombre del usuario que elaboro sep
		$io_pdf->addText(202,122,7,"RESPONSABLE DE EJECUTAR PRESUPUESTO"); // Agregar el título
		...
		$io_pdf->addText(240,148,7,"D.G. PLANIFICACIÓN Y PRESUPUESTO"); // Validar Presupuesto
	...
	function uf_print_detalle_cargos($la_data,$li_codtipsol,$io_pdf)
	{
		...
		else
		{
			...
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'outerLineThickness'=>0.5,
							 'innerLineThickness' =>0.5,
							 'cols'=>array('codigo'=>array('justification'=>'center','width'=>215), // Justificación y ancho de la columna
										   'dencar'=>array('justification'=>'center','width'=>355))); // Justificación y ancho de la columna
			$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		}
		return $io_pdf;		
	}// end function uf_print_detalle
	...
	function uf_print_detalle_cuentas($la_data,$li_codtipsol,$io_pdf)
	{
		...
		else
		{
			...
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'outerLineThickness'=>0.5,
							 'innerLineThickness' =>0.5,
							 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>320), // Justificación y ancho de la columna
	...
	if($lb_valido)
	{
		...
		if($lb_valido)
		{
			...
			else  // Imprimimos el reporte
			{
				...
				$io_pdf->ezSetCmMargins(3.6,7,3,3); // Configuración de los margenes en centímetros
