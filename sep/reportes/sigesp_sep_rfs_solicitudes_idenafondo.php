...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina_req($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],20,702,58,56); // Agregar Logo
		if ($as_estsol=="A")
		{
			$io_pdf->addText(400,765,10,"<b>ANULADA</b>"); // Agregar la Fecha
		}
		...
		$io_pdf->addText(18,95,8,$_SESSION["la_apeusu"].", ".$_SESSION["la_nomusu"]);
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,$io_pdf)
	{
		...
		$io_pdf->addText(18,95,8,$_SESSION["la_apeusu"].", ".$_SESSION["la_nomusu"]);
		$io_pdf->addText(35,63,7,"ANALISTA (FIRMA/SELLO)"); // Agregar el título
		$io_pdf->line(150,60,150,130); //vertical
		$io_pdf->addText(200,122,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(160,63,7,"COORD PROYECTOS COMUN. Y PROTEC."); // Agregar el título
		$io_pdf->line(310,60,310,130);	//vertical
		$io_pdf->addText(340,122,7,"CONTABILIZADO POR"); // Agregar el título
		$io_pdf->addText(315,63,7,"PLANIFICACION Y PRESUPUESTO"); // Agregar el título
		$io_pdf->line(440,60,440,130);	//vertical
		$io_pdf->addText(480,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(455,63,7,"ADMINISTRADOR FONDO NNA"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	}// end function uf_print_encabezado_pagina
	...
	function uf_print_cabecera($as_numsol,$as_dentipsol,$as_denuniadm,$as_denfuefin,$as_codigo,$as_nombre,$as_consol,$io_pdf)
	{
		...
		$io_pdf->ezSetDy(-5);
		$la_data[1]=array('titulo'=>'<b> Detalle '.$as_dentipsol.'</b>');
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	function uf_print_detalle_cuentas($la_data,$ls_estmodest,$io_pdf)
	{
		...
		$la_datatit[1]=array('titulo'=>'<b> Detalle Presupuesto </b>');
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if($ls_estope=="S")
				{
					uf_print_encabezado_pagina_req($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_estsol,$io_pdf);
				}
				else
				{
...
*/						$li_monart=number_format($li_monart,2,".","");
						...
						if($ls_estope=="S")
						{
							uf_print_detalle_req($la_data,$io_pdf);
						}
						else
						{
