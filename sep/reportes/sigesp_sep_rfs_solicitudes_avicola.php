...
//        $io_pdf->Rectangle(15,700,570,60);
		...
		$io_pdf->addText(485,715,9,"No. ".$as_numsol); // Agregar el título
		$io_pdf->addText(485,695,9,"Fecha ".$ad_fecregsol); // Agregar el título
...
//		$io_pdf->addText(546,764,6,date("h:i a")); // Agregar la Hora
		...
		$io_pdf->line(585,60,585,130);		
		$io_pdf->addText(20,122,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(20,110,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(90,60,7,"ANALISTA"); // Agregar el título
		$io_pdf->addText(210,122,7,"CONTABILIZADO POR"); // Agregar el título
		$io_pdf->addText(210,110,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(280,60,7,"ANALISTA"); // Agregar el título
		$io_pdf->addText(400,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(400,110,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(410,60,7,"GERENTE DE PLANIFICACIÓN Y PRESUPUESTO"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$io_pdf = uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_estsol,$io_pdf);
...
*/						$li_monart=number_format($li_monart,2,".","");
						...
						if($ls_tipo=="B")
						{
							...
							$ls_denunimed=$arrResultado['as_denunimed'];
							$lb_valido=$arrResultado['lb_valido'];
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							$io_pdf = uf_print_detalle_cuentas($la_data,$ls_estmodest,$io_pdf);
							unset($la_data);
						}//if
					}//if
				}//if
