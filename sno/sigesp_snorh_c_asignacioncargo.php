...
class sigesp_snorh_c_asignacioncargo
{
	...
	function uf_select_grado($as_codnom,$as_codgra)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Asignación Cargo MÉTODO->uf_select_grado ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
									   ...
									   $as_codestpro3,$as_codestpro4,$as_codestpro5,$as_estcla,$as_codgraobrero,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("El grado asociado para esta asignación de cargo no existe para dicha nomina");
		}
	...
	function uf_actualizarocupados($as_codasicar,$as_codnom,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Asignación Cargo MÉTODO->uf_actualizarocupados ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualizó el nro de ocupados de  la Asignación de Cargo ".$as_codasicar." asociada a la nómina ".$as_codnom;
			$lb_valido=$this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			if($lb_valido)
			{	
				$this->io_mensajes->message("El nro de ocupados de la Asignación de Cargo fue actualizada.");
				$this->io_sql->commit();
			}
			else
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Asignación Cargo MÉTODO->uf_actualizarocupados ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));				$this->io_sql->rollback();
			}
		}
		return $lb_valido;
