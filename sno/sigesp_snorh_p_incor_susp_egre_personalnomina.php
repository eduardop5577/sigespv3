...
***********************************************************************************/
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad("SNR","sigesp_snorh_p_incor_susp_egre_personalnomina.php",$ls_permisos,$la_seguridad,$la_permisos);
		  ...
		  <table width="500" border="0" align="center" cellpadding="1" cellspacing="0" class="formato-blanco">
              ...
              <tr>
                ...
                <td colspan="4"><div align="left" >
				 <?php
				  $rs_data=0;
				  $arrResultado=$io_c_incorsuspegr->uf_llenarcombo_nomina($rs_data);
...
/***********************************************************CARGAR************************************************************************/	
	if ($ls_operacion=="CARGAR")
	{
      $rs_data=0;
      $ls_staper=$_POST["staper"];	
	  $arrResultado=$io_c_incorsuspegr->uf_load_grid_personalnomina($ls_staper,$ls_cmbnomina,$rs_data);
	  $rs_data=$arrResultado['rs_data'];
	  $lb_valido=$arrResultado['lb_valido'];
	  if($lb_valido)
	  {
