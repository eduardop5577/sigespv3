...
***********************************************************************************/
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad_nomina("SNO","sigesp_sno_d_constantes.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
...
<?php 
	...
	switch ($ls_operacion) 
	{
		...
		case "BUSCAR":
			...
			$arrResultado=$io_constante->uf_load_constante($ls_existe,$ls_codcons,$ls_nomcon,$ls_unicon,$ld_topcon,$ld_valcon,$ls_reicon,$ls_conespseg,$ls_esttopmod,$ls_perenc);
			$ls_existe=$arrResultado['as_existe'];
			$ls_codcons=$arrResultado['as_codcons'];
			$ls_nomcon=$arrResultado['as_nomcon'];
			$ls_unicon=$arrResultado['as_unicon'];
			$ld_topcon=$arrResultado['ad_topcon'];
			$ld_valcon=$arrResultado['ad_valcon'];
			$ls_reicon=$arrResultado['as_reicon'];
			$ls_conespseg=$arrResultado['as_conespseg'];
			$ls_esttopmod=$arrResultado['as_esttopmod'];
