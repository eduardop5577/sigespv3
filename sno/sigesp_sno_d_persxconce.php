...
***********************************************************************************/
   ...
   session_start();
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad_nomina("SNO","sigesp_sno_d_persxconce.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
	...
	switch ($ls_operacion) 
	{
		...
		case "GUARDAR":
			...
			if($lb_valido)
			{
				...
				$arrResultado=$io_conceptopersonal->uf_load_conceptos_x_personal($ls_codper,$li_totrows,$lo_object, $ls_aplcontodo);
				$li_totrows=$arrResultado['ai_totrows'];
				$lo_object=$arrResultado['aa_object'];
				$ls_aplcontodo=$arrResultado['ls_aplcontodo'];
