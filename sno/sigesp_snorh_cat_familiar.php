...
***********************************************************************************/
   ...
   function uf_print($as_codper, $as_cedfam, $as_nomfam, $as_apefam)
   {
		...
		print "</tr>";
		$ls_sql="SELECT codemp, codper, cedfam, nomfam, apefam, sexfam, fecnacfam, nexfam, estfam, hcfam, hcmfam, hijesp, estbonjug, cedula, estbec, nivaca ".
				"  FROM sno_familiar ".
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				print "'$ls_nexfam','$li_estfam','$li_hcfam','$li_hcmfam','$li_hijesp','$li_bonjug','$ls_cedula','$ls_estbec','$ls_nivaca');\">".$ls_cedfam."</a></td>";
...
<script >
function aceptar(cedfam,nomfam,apefam,sexfam,fecnacfam,nexfam,estfam,hcfam,hcmfam,hijesp,bonjug,cedula,ls_estbec,ls_nivaca)
{
	...
	opener.document.form1.cmbnivaca.value=ls_nivaca;		
	if(estfam==1)
	{
	...
	if(ls_estbec==1)
	{
		opener.document.form1.chkestbec.checked=true;
	}
	else
	{
		opener.document.form1.chkestbec.checked=false;
	}
	close();
}
