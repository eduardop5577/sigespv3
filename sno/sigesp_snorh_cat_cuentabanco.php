   ...
   function uf_print($as_codban, $as_tipo)
   {
	 ...
	 else
	 {
		...
		$ls_sql="SELECT scb_ctabanco.ctabanext, scb_ctabanco.dencta, scb_tipocuenta.nomtipcta, scb_ctabanco.sc_cuenta, scb_ctabanco.ctaban ".
				"  FROM scb_ctabanco, scb_tipocuenta, sss_permisos_internos  ".
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					...
					case "config": // el llamado se hace desde configuración
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarconfig('$ls_ctaban','$ls_dencta');\">".$ls_ctaban."</a></td>";
						print "<td>".$ls_nomtipcta."</td>";
						print "<td>".$ls_dencta."</td>";
						print "</tr>";			
						break;
						
					case "pagdirche": // el llamado se hace desde configuración
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarpagdirche('$ls_ctaban','$ls_dencta');\">".$ls_ctaban."</a></td>";
						print "<td>".$ls_nomtipcta."</td>";
						print "<td>".$ls_dencta."</td>";
						print "</tr>";			
						break;		
...
function aceptarconfig(ctaban,dencta)
{
	...
	opener.document.form1.txtdenominacion.readOnly=true;
	close();
}

function aceptarpagdirche(ctaban,dencta)
{
	opener.document.form1.txtcuentaperche.value=ctaban;
	opener.document.form1.txtcuentaperche.readOnly=true;
	opener.document.form1.txtdenominacionperche.value=dencta;
	opener.document.form1.txtdenominacionperche.readOnly=true;
	close();
}
