...
***********************************************************************************/
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad_nomina("SNO","sigesp_sno_p_manejoperiodo.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		global $lb_cerrar, $lb_abrir, $io_cierreperiodo, $li_contabilizadoant, $io_fun_nomina, $ls_conpernom, $ls_perresnom;
...
<script type="text/javascript"  src="../../../swap/js/stm31.js"></script>
...
<?php 
	...
	switch ($ls_operacion) 
	{
		case "CERRAR_PERIODO":
			...
			$ls_concam=$io_fun_nomina->uf_obtenervalor("chkconcam","0");
			$lb_valido=$io_cierreperiodo->uf_chequear_encargaduria();
			if ($lb_valido)
			{
		 		$lb_valido=$io_cierreperiodo->uf_procesar_cierre_periodo($ls_peractnom,$ld_fechadesde,$ld_fechahasta,$la_seguridad,$ls_concam); 
				if ($lb_valido)
				{
					uf_limpiarvariables();
				}
  ...
  <table width="650" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
	...
	<p>&nbsp;</p>
		<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
              ...
              <tr>
                <td height="22" colspan="6">Conservar cambios del periodo historico 
                  <label>
                  <input name="chkconcam" type="checkbox" class="sin-borde" value="1" onClick="javascript:ConservarCambios();">
                (S&oacute;lo para periodos abiertos)</label></td>
              </tr>
              <tr>
                <td height="22" colspan="6">
					<div id=transferir style="visibility:hidden" align="center"><img src="../shared/imagebank/cargando.gif">Cerrando Periodo... </div>                 </td>
              </tr>
            <tr>
              ...
              <td colspan="5">
				...
				<input name="perresnom" type="hidden" id="perresnom" value="<?php print $ls_perresnom; ?>">			  
			   </td>
            </tr>
          </table>
        <p>&nbsp;</p></td>
...
{
	f=document.form1;
	if(f.chkconcam.checked)
	{
		if(f.perresnom.value=="000")
		{
			f.chkconcam.checked =false;
		}
		else
		{
			alert("Al tildar esta opción se van a mantener todos los valores de la Asignación de Personal, Conceptos, Constantes, etc.");
		}
	}
}
</script>
