...
***********************************************************************************/
   ...
   //--------------------------------------------------------------
   function uf_print($as_sc_cuenta, $as_denominacion, $as_tipo, $ai_nro)
   {
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_print
		//		   Access: public
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					...
					case "LOTE":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarlote('$ls_sc_cuenta','$ai_nro');\">".$ls_sc_cuenta."</a></td>";
						print "<td>".$ls_denominacion."</td>";
						print "</tr>";			
						break;
					...
					case "PATRONALLOTE":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarpatronallote('$ls_sc_cuenta','$ai_nro');\">".$ls_sc_cuenta."</a></td>";
						print "<td>".$ls_denominacion."</td>";
						print "</tr>";			
						break;
				}
...
<?php
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		uf_print($ls_sc_cuenta, $ls_denominacion, $ls_tipo, $li_nro);
	}
	else
	{
		if (($ls_tipo=='PRESTACION')||($ls_tipo=='PRESTACIONINT'))
...
function aceptarlote(sc_cuenta,nro)
{
	eval("opener.document.form1.txtcuecon"+nro+".value='"+sc_cuenta+"';");
	close();
}

function aceptarpatronallote(sc_cuenta,nro)
{
	eval("opener.document.form1.txtcueconpatcon"+nro+".value='"+sc_cuenta+"';");
	close();
}
...
function ue_search(existe)
{
	f=document.form1;
  	...
  	f.action="sigesp_sno_cat_cuentacontable.php?tipo=<?php print $ls_tipo;?>&nro=<?php print $li_nro;?>";
  	f.submit();
}
</script>
</html>
