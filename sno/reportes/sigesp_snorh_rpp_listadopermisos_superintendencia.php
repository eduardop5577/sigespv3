...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_datos($la_data,$li_totrow,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_config);
		$io_pdf->ezSetDy(-2);
		$li_totrow=number_format($li_totrow,2,',','.');
		$la_data[1]=array('numper'=>"",'text'=>"TOTAL DE PERMISOS....",'totrow'=>$li_totrow);
		$la_columna=array('numper'=>'','text'=>'','totrow'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 10,  // Tamaño de Letras de los títulos
						 'showLines'=>0, // Mostrar Líneas
						 ...
						 'maxWidth'=>90, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('numper'=>array('justification'=>'center','width'=>215),
						 			   'text'=>array('justification'=>'center','width'=>280),
									   'totrow'=>array('justification'=>'left','width'=>210))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_permisospersonal_personal($ls_codnomdes,$ls_codnomhas,$ls_codperdes,$ls_codperhas,$ls_activo,
															...
															$ls_fec_des,$ls_fec_has,$ls_tipo_permiso,$ls_uniadmin,$ls_codestper,$ls_codmunper,$ls_codparper); // Obtenemos el detalle del reporte
	}
			...
			if($lb_valido)
			{
				...
				for($li_j=1;(($li_j<=$li_total)&&($lb_valido));$li_j++)
				{
					...
					switch($ls_tipper)
					{
						...
						case "8":
							$ls_tipper="Permiso Sindical";
							break;

						case "9":
							$ls_tipper="Compensatorio";
							break;
				...
				uf_print_datos($la_data,$li_total,$io_pdf);
				unset($la_data);
