...
***********************************************************************************/
	...
	function uf_insert_seguridad($as_titulo,$as_titulo2,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","sigesp_sno_r_gastoguarderia.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","sigesp_sno_r_hgastoguarderia.php",$ls_descripcion,$ls_codnom);
		}
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
...
$io_pdf->setStrokeColor(0,0,0);
		$io_pdf->Rectangle(50,60,500,70); 
		$io_pdf->line(50,117,550,117);	//HORIZONTAL	
		$io_pdf->addText(100,121,7,"ELABORADO POR:"); // Agregar el título		
		$io_pdf->addText(75,80,7,"Samantha Vanessa Morlets Vargas"); // Agregar el título
              $io_pdf->addText(110,71,7,"Capitan"); // Agregar el título
              $io_pdf->addText(68,63,7,"Jefe de la Oficina de Gestión Humana"); // Agregar el título			
		$io_pdf->line(210,60,210,130);	//VERTICAL	
		$io_pdf->addText(275,121,7,"REVISADO POR:"); // Agregar el título
		$io_pdf->line(390,60,390,130);	//VERTICAL
		$io_pdf->addText(258,80,7,"Jorge Alejandro Diaz Angulo"); // Agregar el título
              $io_pdf->addText(290,71,7,"Mayor"); // Agregar el título
              $io_pdf->addText(230,63,7,"Jefe de la Oficina de Servicios Administrativos"); // Agregar el título		
        $io_pdf->addText(440,121,7,"APROBADO POR:"); // Agregar el título
		$io_pdf->addText(420,80,7,"Juan De Jesus Garcia Toussaintt"); // Agregar el título	
              $io_pdf->addText(450,71,7,"Mayor General"); // Agregar el título
              $io_pdf->addText(420,63,7,"Presidente CONSTRUFANB, C.A."); // Agregar el título		
		$io_pdf->line(50,86,550,86); //HORIZONTAL
	...
	$ls_titulo2="<b>Presupuestario de Guarderias</b>";
	...
	{
		$lb_valido=$io_report->uf_guarderiaconcepto_presupuesto();
	}
	/*if($lb_valido) // Buscamos la información que afecta contabilidad por el debe
	{
		$lb_valido=$io_report->uf_guarderiaconcepto_contable();
	}*/
	...
	else // Imprimimos el reporte
	{
		...
		$li_totrow=$io_report->DS->getRowCount("spgcuenta");
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_cueprecon=$io_report->DS->data["spgcuenta"][$li_i];
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_denominacion=$io_report->DS_detalle->data["denominacion_cont"][$li_i];
