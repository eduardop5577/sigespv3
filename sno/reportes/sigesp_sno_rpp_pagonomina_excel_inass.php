...
***********************************************************************************/
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->write(4, 1, "Sexo",$lo_titulo);
		$lo_hoja->write(4, 2, "Cédula",$lo_titulo);
		$lo_hoja->write(4, 3, "Apellidos y Nombres",$lo_titulo);
		$lo_hoja->write(4, 4, "Cargo",$lo_titulo);
		$lo_hoja->write(4, 5, "Unidad Adscripción",$lo_titulo);
		$lo_hoja->write(4, 6, $ls_titulo,$lo_titulo);
		$lo_hoja->write(4, 7, "Fecha Ingreso",$lo_titulo);
		$lo_hoja->write(4, 8, "Cuenta Bancaria",$lo_titulo);
		$lo_hoja->write(4, 9, "Ubicación Física",$lo_titulo);
		$lo_hoja->write(4, 10, "Estado",$lo_titulo);
		$lo_hoja->write(4, 11, "Municipio",$lo_titulo);
		$lo_hoja->write(4, 12, "Parroquia",$lo_titulo);
		$lo_hoja->write(4, 13, "Salario",$lo_titulo);
		$lo_hoja->write(4, 14, "Fecha de Ingreso a Nómina",$lo_titulo);
		$lo_hoja->write(4, 15, "Fecha de Culminación de Contrato",$lo_titulo);
		$li_col=15;
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$lo_hoja->write($li_row, 1, $ls_sexper, $lo_datacenter);
			$lo_hoja->write($li_row, 2, $ls_cedper, $lo_datacenter);
			$lo_hoja->write($li_row, 3, $ls_apenomper, $lo_dataleft);
			$lo_hoja->write($li_row, 4, $ls_descar, $lo_dataleft);
