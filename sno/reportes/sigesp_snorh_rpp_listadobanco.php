...
***********************************************************************************/
	...
	$lb_pagotrban=$io_fun_nomina->uf_obtenervalor_get("pagotrban",false);	
	if ($ls_quincena==1)
	{
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$lb_valido=$io_report->uf_listadobanco_personal($ls_codban,$ls_suspendido,$ls_tipcueban,$ls_quincena,$ls_codnomdes,
													 		$ls_codnomhas,$ls_codperdes,$ls_codperhas,$ls_subnomdes,$ls_subnomhas,$ls_orden,$lb_pagotrban); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				$li_subtot=0;
