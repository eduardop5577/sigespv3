...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$io_pdf)
	{
		...
		$tm=400-($li_tm/2);
		$io_pdf->addText($tm,528,11,$as_titulo); // Agregar el título
	...
	function uf_print_cabecera($io_pdf)
	{
		...
		$io_pdf->ezSety(520);
                ...
                $io_pdf->filledRectangle(40,496,705,$io_pdf->getFontHeight(19));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>55), // Justificación y ancho de la columna
						 			   ...
						 			   'anoins'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   'mesins'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   'diains'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   'anoant'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   'mesant'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   ...
						 			   'dia'=>array('justification'=>'center','width'=>30))); // Justificación y ancho de la columna
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_cargo="";
                        $arrResultado=$io_report->uf_obtener_anio_trabajado_anterior($ls_codper);
			...
			else
			{
				...
				else
				{
					...
					else
					{
                        ...
                        if($lb_valido)
                        {
                            $li_diatot=$li_diaant+$li_dia;
                            $li_mestot=$li_mesant+$li_mes;
                            $li_dias=floor(($li_diaant+$li_dia)/30);
                            if($li_dias>0)
                            {
                                    $li_mestot=$li_mestot+$li_dias;
                                    $n = (($li_diaant+$li_dia)/30); 
                                    $whole = floor($n); 
                                    $fraction = $n - $whole; 

                                    $li_diatot= round($fraction*30,0);
                            }
                            
                            $li_meses=floor(($li_mestot)/12);
                            if($li_meses>0)
                            {
                                    $li_anotot=$li_ano+$li_meses+$li_anioant;
                                    $n = (($li_mestot)/12); 
                                    $whole = floor($n); 
                                    $fraction = $n - $whole;   
                                    $li_mestot= round($fraction*12,0);
                            }
                            else
                            {
                                    $li_anotot=$li_ano+$li_anioant;
                            }
                        }
			...
			$la_data[$li_i]=array('codigo'=>$ls_codper,'nombre'=>$ls_nomper,'cargo'=>$ls_cargo,'fecha'=>$ld_fecingper,
					      'anoins'=>$li_ano,'mesins'=>$li_mes,'diains'=>$li_dia,'anoant'=>$li_anioant,'mesant'=>$li_mesant,
					      'diaant'=>$li_diaant,'ano'=>$li_anotot,'mes'=>$li_mestot,'dia'=>$li_diatot);
		}
