...
***********************************************************************************/
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->write(5, 0, "Nacionalidad",$lo_titulo);
		$lo_hoja->write(5, 1, "Cédula",$lo_titulo);
		$lo_hoja->write(5, 2, "Apellidos y Nombres",$lo_titulo);
		$lo_hoja->write(5, 3, "Cargo",$lo_titulo);
		$lo_hoja->write(5, 4, "Nomina",$lo_titulo);
		$lo_hoja->write(5, 5, "Unidad Adscripción",$lo_titulo);
		$lo_hoja->write(5, 6, $ls_titulo,$lo_titulo);
		$lo_hoja->write(5, 7, "Fecha Ingreso",$lo_titulo);
		$lo_hoja->write(5, 8, "Cuenta Bancaria",$lo_titulo);
		$lo_hoja->write(5, 9, "Ubicación Física",$lo_titulo);
		$lo_hoja->write(5, 10, "Estado",$lo_titulo);
		$lo_hoja->write(5, 11, "Municipio",$lo_titulo);
		$lo_hoja->write(5, 12, "Parroquia",$lo_titulo);
		$lo_hoja->write(5, 13, "Salario",$lo_titulo);
		$lo_hoja->write(5, 14, "Fecha de Ingreso a Nómina",$lo_titulo);
		$lo_hoja->write(5, 15, "Fecha de Culminación de Contrato",$lo_titulo);
		$li_col=15;
		...
		if($lb_valido)
		{
			...
			while(!$io_report->rs_data_detalle->EOF)
			{
				...
				$ls_cueprecon=$io_report->rs_data_detalle->fields["cueprecon"];
				$li_col++;
				$lo_hoja->set_column($li_col,$li_col,20);
				$lo_hoja->write(4, $li_col, $ls_cueprecon,$lo_titulo);
				$lo_hoja->write(5, $li_col, $ls_nomcon,$lo_titulo);
				$io_report->DS_detalle2->insertRow("codconc",$ls_codconc);
				$io_report->DS_detalle2->insertRow("columna",$li_col);
                                if ($ls_incunicon == 1)
                                {
                                    $li_col++;
                                    $lo_hoja->write(5, $li_col, "UNIDAD",$lo_titulo);
                                }
		...
		if($lb_valido)
		{
			...
			while(!$io_report->rs_data_detalle->EOF)
			{
				$ls_codconc=$io_report->rs_data_detalle->fields["codconc"];
				$ls_nomcon=$io_report->rs_data_detalle->fields["nomcon"];
				$ls_cueprepatcon=$io_report->rs_data_detalle->fields["cueprepatcon"];
				$li_col++;
				$lo_hoja->set_column($li_col,$li_col,20);
				$io_report->DS_detalle2->insertRow("codconc",$ls_codconc."P2");
				$io_report->DS_detalle2->insertRow("columna",$li_col);
				$lo_hoja->write(4, $li_col, $ls_cueprepatcon,$lo_titulo);
				$lo_hoja->write(5, $li_col, $ls_nomcon,$lo_titulo);
				$io_report->rs_data_detalle->MoveNext();
			}
		}
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$ls_nomina=$io_report->rs_data->fields["nomina"];
			...
			$lo_hoja->write($li_row, 4, $ls_nomina, $lo_dataleft);
			$lo_hoja->write($li_row, 5, $ls_desuniadm, $lo_dataleft);
			$lo_hoja->write($li_row, 6, $ls_programatica, $lo_dataleft);
			$lo_hoja->write($li_row, 7, $ld_fecingper, $lo_datacenter);
			$lo_hoja->write_string($li_row, 8, $ls_codcueban, $lo_dataleft);
			$lo_hoja->write($li_row, 9, $ls_desubifis, $lo_dataleft);
			$lo_hoja->write($li_row, 10, $ls_desest, $lo_dataleft);
			$lo_hoja->write($li_row, 11, $ls_denmun, $lo_dataleft);
			$lo_hoja->write($li_row, 12, $ls_denpar, $lo_dataleft);
			$lo_hoja->write($li_row, 13, $ld_sueper, $lo_dataright);
			$lo_hoja->write($li_row, 14, $ld_fecingnom, $lo_datacenter);
			$lo_hoja->write($li_row, 15, $ld_feculcont, $lo_datacenter);
			$li_col=14;
