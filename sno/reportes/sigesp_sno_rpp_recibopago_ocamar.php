...
***********************************************************************************/
	...
	function uf_print_cabecera($as_cedper,$as_nomper,$as_descar,$as_descasicar,$io_cabecera,$io_pdf)
	{
		...
		$io_pdf->ezSetDy(-11);
		$la_data=array(array('nombre'=>'Apellidos y Nombres:', 'cedula'=>'Cédula:', 'cargo'=>'Cargo:', 'denominacion'=>'Denominación del Cargo:' ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'','denominacion'=>'');
		...
		$la_data=array(array('nombre'=>$as_nomper, 'cedula'=>$as_cedper, 'cargo'=>substr($as_descar,0,100), 'denominacion'=>substr($as_descasicar,0,100)));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'','denominacion'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						...
						case "2": // segunda quincena;
							while(!$io_report->rs_data_detalle->EOF)
							{
								...
								if(($ls_tipsal=="A") || ($ls_tipsal=="V1") || ($ls_tipsal=="V2") || ($ls_tipsal=="R")) // Buscamos las asignaciones
								{
									...
									$la_data_a[$li_asig]=array('denominacion'=>$ls_nomcon." ".$ls_cuota,'valor'=>$li_valsal);
								}
								else // Buscamos las deducciones y aportes
								{
									...
									$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->rs_data_detalle->fields["valsal"]));
									$la_data_d[$li_dedu]=array('denominacion'=>$ls_nomcon." ".$ls_cuota,'valor'=>$li_valsal);
								}
