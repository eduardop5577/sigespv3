...
***********************************************************************************/
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$ls_ctnom,$io_pdf)
	{
		...
		if($ls_ctnom=="1")
		{
			$li_tm=$io_pdf->getTextWidth(6,"Segun Art. 105 de LOTTT, el mismo no genera Insidencia Salarial Alguna.");
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,440,6,"Segun Art. 105 de LOTTT, el mismo no genera Insidencia Salarial Alguna."); // Agregar el título
		}
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_pie_cabecera2($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$ls_ctnom,$io_pdf)
	{
		...
		if($ls_ctnom=="1")
		{
			$li_tm=$io_pdf->getTextWidth(6,"Segun Art. 105 de LOTTT, el mismo no genera Insidencia Salarial Alguna.");
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,20,6,"Segun Art. 105 de LOTTT, el mismo no genera Insidencia Salarial Alguna."); // Agregar el título
		}
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_piepagina,'all');
