...
***********************************************************************************/
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_datos($la_data,$li_dialabacum,$li_meslabacum,$li_anolabacum,$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		global $io_pdf;
		$io_pdf->ezSetDy(-1);
		$la_columna=array('codtraant'=>'','emptraant'=>'','emppubtraant'=>'','ultcartraant'=>'','ultsuetraant'=>'','fecingtraant'=>'','fecrettraant'=>'','anolab'=>'','meslab'=>'','dialab'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_config);
                $io_pdf->ezSetDy(-20);
		$la_data[0]=array('observacion'=>' Antiguedad Trabajos Anteriores '.$li_dialabacum.' Dias '.$li_meslabacum.' Meses '.$li_anolabacum.' Años.');
		$la_columna=array('observacion'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 10, // Tamaño de Letras
						 'titleFontSize' => 10,  // Tamaño de Letras de los títulos
						 'showLines'=>0, // Mostrar Líneas
						 ...
						 'maxWidth'=>90, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('observacion'=>array('justification'=>'right','width'=>690))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_trabajosanteriorespersonal_personal($ls_codnomdes,$ls_codnomhas,$ls_codperdes,$ls_codperhas,
																	  ...
																	  $ls_masculino,$ls_femenino,$ls_orden,$ls_uniadmin,$ls_codestper,
																	  $ls_codmunper,$ls_codparper); // Obtenemos el detalle del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			uf_print_cabecera($ls_cedper,$ls_nomper,$ls_apeper,$ls_estper,$ls_dirper,$ls_telhabper,$ls_telmovper,$ls_coreleper,$io_cabecera,$io_pdf);
			...
			if($lb_valido)
			{
				...
				for($li_j=1;(($li_j<=$li_total)&&($lb_valido));$li_j++)
				{
                    ...
                    $li_anolabacum=number_format($li_anolabacum+$li_anolab,0,"","");
                    $li_meslabacum=number_format($li_meslabacum+$li_meslab,0,"","");
                    $li_dialabacum=number_format($li_dialabacum+$li_dialab,0,"","");
                ...
                $li_meses=floor(($li_dialabacum)/30);
                if($li_meses>0)
                {
                    $li_meslabacum=$li_meslabacum+$li_meses;
                    $li_dialabacum = number_format((($li_dialabacum)/30),2,".","");
                    $li_dialabacum = $li_dialabacum - $li_meses;
                    $li_dialabacum = ($li_dialabacum)*30;
                    $li_dialabacum = round($li_dialabacum,0) ;
                }
                $li_anios=floor(($li_meslabacum)/12);
                if($li_anios>0)
                {
                    $li_anolabacum=$li_anolabacum+$li_anios;
                    $li_meslabacum = number_format((($li_meslabacum)/12),2,".","");
                    $li_meslabacum = $li_meslabacum - $li_anios;
                    $li_meslabacum = ($li_meslabacum)*12;
                    $li_meslabacum = round($li_meslabacum,0) ;
                }
				uf_print_datos($la_data,$li_dialabacum,$li_meslabacum,$li_anolabacum,$io_pdf);
				unset($la_data);
			}
