...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,545,9,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,535,9,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,525,9,$as_desnom); // Agregar el título
		$io_pdf->addText(712,560,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(718,553,6,date("h:i a")); // Agregar la Hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
