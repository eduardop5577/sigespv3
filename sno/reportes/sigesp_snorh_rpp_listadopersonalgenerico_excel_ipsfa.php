...
***********************************************************************************/
	...
	if($lb_valido)
	{
		$arrResultado=$io_report->uf_listadogenerico_ipsfa($ls_codnomdes,$ls_codnomhas,$ls_anio,$ls_mes,$ls_codperi,$ls_codperdes,$ls_codperhas,$ls_orden,$rs_data); // Obtenemos el detalle del reporte
	...
	else // Imprimimos el reporte
	{
		...
		while ((!$rs_data->EOF)&&($lb_valido))
		{
			...
			$arrResultado=$io_report->uf_buscar_valor_concepto_personal($ls_codnom,$ls_codper,$ls_anio,$ls_mes,$ls_sueldobasico,$li_sueldobasico);
			$li_sueldobasico=$arrResultado['ad_valor'];
			$lb_valido=$arrResultado['lb_valido'];
			if($lb_valido)
			{
				$arrResultado=$io_report->uf_buscar_valor_concepto_personal($ls_codnom,$ls_codper,$ls_anio,$ls_mes,$ls_sueldobasicocargo,$li_sueldobasicocargo);
				$li_sueldobasicocargo=$arrResultado['ad_valor'];
				$lb_valido=$arrResultado['lb_valido'];
			}
			if($lb_valido)
			{
				$arrResultado=$io_report->uf_buscar_valor_concepto_personal($ls_codnom,$ls_codper,$ls_anio,$ls_mes,$ls_difsueldobasico,$li_difsueldobasico);
				$li_difsueldobasico=$arrResultado['ad_valor'];
				$lb_valido=$arrResultado['lb_valido'];
			}
			if($lb_valido)
			{
				$arrResultado=$io_report->uf_buscar_cestaticket_personal($ls_codper,$ls_anio,$ls_mes,$ls_cestaticket,$li_cestaticket,$li_nrocestaticket,$li_cestaticketanual,$li_nrocestaticketanual);
				$li_cestaticket=$arrResultado['ad_valor'];
				$li_nrocestaticket=$arrResultado['ad_cantidad'];
				$li_cestaticketanual=$arrResultado['ad_valoranual'];
				$li_nrocestaticketanual=$arrResultado['ad_cantidadanual'];
				$lb_valido=$arrResultado['lb_valido'];
			}
			...
			$la_conceptos=explode(",",$ls_conceptos);
			$li_totconcepto=count((Array)$la_conceptos);
			for($li_cont=0;($li_cont<$li_totconcepto)&&($lb_valido);$li_cont++)
			{
				$ls_concepto=$la_conceptos[$li_cont];
				if($ls_concepto<>"''")
				{
					$arrResultado=$io_report->uf_buscar_valor_concepto_personal($ls_codnom,$ls_codper,$ls_anio,$ls_mes,$ls_concepto,$ld_valor);
					$ld_valor=$arrResultado['ad_valor'];
					$lb_valido=$arrResultado['lb_valido'];
					if($lb_valido)
					{
