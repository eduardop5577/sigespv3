...
***********************************************************************************/
	...
	$lo_archivo = tempnam("/tmp", "listadotipopersonal_ipsfa.xls");
	...
	$ls_titulo="Parte del Personal Civil Activo, Pensionados y Jubilados";
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_tipo_personalunidadadmin(" AND sno_ubicacionfisica.codubifis='0540'"," AND sno_nomina.codnom IN ('0001','0002','0003','0004')"); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		$ld_totempleadosfijos=0;
		$ld_totobrerosfijos=0;
		$ld_totempleadoscontratados=0;
		$ld_totobreroscontratados=0;
		$ld_totgeneral=0;
		...
		$ld_totalgeneral=0;
		$li_totrow=$io_report->DS_detalle->getRowCount("minorguniadm");
		...
		$ld_totempleadosfijos=$ld_totempleadosfijos+$ld_totalempleadosfijos;
		$ld_totobrerosfijos=$ld_totobrerosfijos+$ld_totalobrerosfijos;
		$ld_totempleadoscontratados=$ld_totempleadoscontratados+$ld_totalempleadoscontratados;
		$ld_totobreroscontratados=$ld_totobreroscontratados+$ld_totobreroscontratados;
		$ld_totalgeneral=number_format($ld_totalempleadosfijos+$ld_totalobrerosfijos+$ld_totalempleadoscontratados+$ld_totalobreroscontratados,2,",",".");
		...
		$li_row=$li_row+1;
		$lo_hoja->write($li_row, 0, "SUB-TOTAL", $lo_titulo);
		...
		$li_row=$li_row+1;
		$li_row=$li_row+1;
		$lb_valido=$io_report->uf_tipo_personal(" AND sno_ubicacionfisica.codubifis<>'0540'"); // Cargar el DS con los datos de la cabecera del reporte
		$ld_totalempleadosfijos=0;
		$ld_totalobrerosfijos=0;
		$ld_totalempleadoscontratados=0;
		$ld_totalobreroscontratados=0;
		...
		$li_row=$li_row+1;
		$li_row=$li_row+1;
		$lb_valido=$io_report->uf_tipo_personaljubilado(); // Cargar el DS con los datos de la cabecera del reporte
		$ld_totalempleadosfijos=0;
		$ld_totalobrerosfijos=0;
		$ld_totalempleadoscontratados=0;
		$ld_totalobreroscontratados=0;
		...
		$ld_totempleadoscontratados=$ld_totempleadoscontratados+$ld_totalempleadoscontratados;
		$ld_totobreroscontratados=$ld_totobreroscontratados+$ld_totobreroscontratados;
		$ld_totgeneral=$ld_totgeneral+$ld_totempleadosfijos+$ld_totobrerosfijos+$ld_totempleadoscontratados+$ld_totobreroscontratados;
		$ld_totalgeneral=number_format($ld_totalempleadosfijos+$ld_totalobrerosfijos+$ld_totalempleadoscontratados+$ld_totalobreroscontratados,2,",",".");
		$ld_totalempleadosfijos=number_format($ld_totalempleadosfijos,2,",",".");
		$ld_totalobrerosfijos=number_format($ld_totalobrerosfijos,2,",",".");
		$ld_totalempleadoscontratados=number_format($ld_totalempleadoscontratados,2,",",".");
		...
		$ld_totgeneral=number_format($ld_totgeneral,2,",",".");
		$ld_totempleadosfijos=number_format($ld_totempleadosfijos,2,",",".");
		$ld_totobrerosfijos=number_format($ld_totobrerosfijos,2,",",".");
		$ld_totempleadoscontratados=number_format($ld_totempleadoscontratados,2,",",".");
		$ld_totobreroscontratados=number_format($ld_totobreroscontratados,2,",",".");
		
		$li_row=$li_row+1;
		$li_row=$li_row+1;
		$lo_hoja->write($li_row, 0, "TOTAL GENERAL", $lo_titulo);
		$lo_hoja->write($li_row, 1, $ld_totempleadosfijos, $lo_dataright);
		$lo_hoja->write($li_row, 2, $ld_totobrerosfijos, $lo_dataright);
		$lo_hoja->write($li_row, 3, $ld_totempleadoscontratados, $lo_dataright);
		$lo_hoja->write($li_row, 4, $ld_totobreroscontratados, $lo_dataright);
		$lo_hoja->write($li_row, 5, $ld_totgeneral, $lo_dataright);
		
		
		
		$lo_libro->close();
		header("Content-Type: application/x-msexcel; name=\"listadotipopersonal_ipsfa.xls\"");
		header("Content-Disposition: inline; filename=\"listadotipopersonal_ipsfa.xls\"");
