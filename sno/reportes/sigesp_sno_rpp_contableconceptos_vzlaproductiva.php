...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_titulo2,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/cintillo_vzlaproductiva.jpg',50,720,530,50);
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,700,11,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo2);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,690,11,$as_titulo2); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(11,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,680,11,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(10,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,670,10,$as_desnom); // Agregar el título
		$io_pdf->addText(500,720,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(506,713,7,date("h:i a")); // Agregar la Hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_firmas($io_pdf){
		...
		$io_pdf->addText(80,121,7,"OFICINA DE TALENTO HUMANO"); // Agregar el título
		$io_pdf->addText(60,105,8,"Elaborado por:");
		$io_pdf->addText(60,65,8,"Fecha/Hora:"); // Agregar el título
		$io_pdf->line(220,60,220,130);	//VERTICAL		
		$io_pdf->addText(240,121,7,"COORDINACIÓN DE CONTABILIDAD"); // Agregar el título
		$io_pdf->addText(230,105,8,"Aprobado por:"); // Agregar el título
		$io_pdf->addText(230,65,8,"Fecha/Hora:"); // Agregar el título
		$io_pdf->line(380,60,380,130);	//VERTICAL		
		$io_pdf->addText(390,121,7,"OFICINA DE PLANIFICACIÓN Y PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(390,105,8,"Imputación por:"); // Agregar el título
		$io_pdf->addText(390,65,8,"Fecha/Hora:"); // Agregar el título
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		$io_pdf->ezSetCmMargins(3.3,4.5,3,3); // Configuración de los margenes en centímetros
