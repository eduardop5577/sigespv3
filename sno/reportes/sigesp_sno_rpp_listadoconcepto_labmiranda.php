		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($lb_valido)
			{
				...
				while(!$io_report->rs_data_detalle->EOF)
				{
					...
					$li_cantidad=$io_fun_nomina->uf_formatonumerico(number_format($io_report->rs_data_detalle->fields["unidad"],2));
					$la_data[$li_s]=array('nro'=>$li_s,'cedula'=>$ls_cedper,'nombre'=>$ls_apenomper,'cargo'=>$ls_descar,'cantidad'=>$li_cantidad,'monto'=>$li_valsal);					
					$li_s++;
