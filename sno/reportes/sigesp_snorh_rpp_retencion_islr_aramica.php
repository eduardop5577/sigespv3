...
***********************************************************************************/
	...
	$ls_retencioncero=$io_fun_nomina->uf_obtenervalor_get("retencioncero","");
	...
	else  // Imprimimos el reporte
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$li_porisr=$io_fun_nomina->uf_formatonumerico($io_report->DS->data["porisr"][$li_i]);
			
			$li_arc=$io_fun_nomina->uf_formatonumerico($io_report->DS->data["arc"][$li_i]+abs($io_report->DS->data["islr"][$li_i]));
			$li_islr=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS->data["islr"][$li_i]));
			
			if($ls_conceptocero==1)
			{
				if($ls_retencioncero==1)
				{
					if(($li_arc<>"0,00")&&($li_islr<>"0,00"))
					{
						$li_s=$li_s+1;
						$la_data[$li_s]=array('nro'=>$li_s,'cedula'=>$ls_cedper,'nombre'=>$ls_nomper,'monto'=>$li_arc,'porcentaje'=>$li_porisr,'retencion'=>$li_islr);
					}
				}
				else
				{
