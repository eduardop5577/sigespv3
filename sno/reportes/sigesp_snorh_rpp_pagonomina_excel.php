...
***********************************************************************************/
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo)
	{
		...
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_pagonomina.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	$ls_codperdes=$io_fun_nomina->uf_obtenervalor_get("perdes","");
	$ls_codperhas=$io_fun_nomina->uf_obtenervalor_get("perhas","");
	...
	$ls_pagocheque=$io_fun_nomina->uf_obtenervalor_get("pagocheque","");		
        $ls_incunicon=$io_fun_nomina->uf_obtenervalor_get("incunicon","");
	//--------------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	$ls_titulo="Reporte General de Pago";
	$ls_desnom="Nomina Desde ".$ls_codnomdes." Hasta ".$ls_codnomhas."";
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_pagonomina_personal($ls_codnomdes,$ls_codnomhas,$ls_codperdes,$ls_codperhas,$ls_conceptocero,$ls_conceptoreporte,$ls_conceptop2,
	...
	else // Imprimimos el reporte
	{
		...
		$lb_valido=$io_report->uf_pagonominaconsolidado_concepto_excel($ls_tituloconcepto," AND (sigcon='A' OR sigcon='B' OR sigcon='X' OR sigcon='I')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
		if($lb_valido)
		{
			...
			while(!$io_report->rs_data_detalle->EOF)
			{
				...
				$io_report->DS_detalle2->insertRow("columna",$li_col);
                                ...
                                {
                                    $li_col++;
                                    $lo_hoja->write(4, $li_col, "UNIDAD",$lo_titulo);
                                }                                
				$io_report->rs_data_detalle->MoveNext();
			}
		}
		// Buscamos los conceptos de Deducción
		$lb_valido=$io_report->uf_pagonominaconsolidado_concepto_excel($ls_tituloconcepto," AND (sigcon='D' OR sigcon='E')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
		if($lb_valido)
		{
		...
		$lb_valido=$io_report->uf_pagonominaconsolidado_concepto_excel($ls_tituloconcepto," AND sigcon='P'",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
		if($lb_valido)
		{
			while(!$io_report->rs_data_detalle->EOF)
			{
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$lb_valido=$io_report->uf_pagonominaconsolidado_conceptopersonal_excel($ls_codper,$ls_tituloconcepto," AND (".$ls_tabla.".tipsal='A' OR ".$ls_tabla.".tipsal='V1' OR ".$ls_tabla.".tipsal='W1')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				while(!$io_report->rs_data_detalle->EOF)
				{
					...
					$lo_hoja->write($li_row, $li_col, $li_valsal, $lo_dataright);
                                        ...
                                        {
                                            $li_col++;
                                            $li_unidad=abs($io_report->rs_data_detalle->fields["unidad"]);
                                            $lo_hoja->write($li_row, $li_col, $li_unidad,$lo_dataright);
                                        }                                        
					$io_report->rs_data_detalle->MoveNext();
				}
			}
			// Buscamos las Deducciones
			$lb_valido=$io_report->uf_pagonominaconsolidado_conceptopersonal_excel($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='D' OR ".$ls_tabla.".tipsal='V2' OR ".$ls_tabla.".tipsal='W2')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				while(!$io_report->rs_data_detalle->EOF)
				{
			...
			$lb_valido=$io_report->uf_pagonominaconsolidado_conceptopersonal_excel($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='P1' OR ".$ls_tabla.".tipsal='V3' OR ".$ls_tabla.".tipsal='W3')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				while(!$io_report->rs_data_detalle->EOF)
				{
			...
			$lb_valido=$io_report->uf_pagonominaconsolidado_conceptopersonal_excel($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='P2' OR ".$ls_tabla.".tipsal='V4' OR ".$ls_tabla.".tipsal='W4')",$ls_codnomdes,$ls_codperdes,$ls_codperhas); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				while(!$io_report->rs_data_detalle->EOF)
				{
