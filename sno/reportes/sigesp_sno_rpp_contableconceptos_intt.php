...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina($as_titulo,$as_titulo2,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,750,9,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_titulo2);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,740,9,$as_titulo2); // Agregar el título
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,720,8,$as_desnom); // Agregar el título
		$io_pdf->addText(500,760,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(506,753,6,date("h:i a")); // Agregar la Hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	else // Imprimimos el reporte
	{
		...
		$io_pdf->ezStartPageNumbers(550,40,10,'','',1); // Insertar el número de página
