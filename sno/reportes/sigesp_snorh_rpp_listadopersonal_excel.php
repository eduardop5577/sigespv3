...
***********************************************************************************/
	...
	$ls_codestper=$io_fun_nomina->uf_obtenervalor_get("codestper","");
	$ls_codmunper=$io_fun_nomina->uf_obtenervalor_get("codmunper","");
	$ls_codparper=$io_fun_nomina->uf_obtenervalor_get("codparper","");
	//---------------------------------------------------------------------------------------------------------------------------
	//Busqueda de la data 
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_listadopersonal_personal($ls_codnomdes,$ls_codnomhas,$ls_codperdes,$ls_codperhas,$ls_activo,
														   ...
														   $ls_codmun,$ls_codpar,$ls_orden,$ls_uniadmin,$ls_codestper,$ls_codmunper,$ls_codparper); // Obtenemos el detalle del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->set_column(8,11,50);
		$lo_hoja->set_column(12,13,50);
		$lo_hoja->set_column(14,14,15);
		$lo_hoja->set_column(15,16,50);
		$lo_hoja->set_column(17,18,20);
		$lo_hoja->set_column(19,22,50);
		...
		$lo_hoja->write(3, 8, "Pais",$lo_titulo);
		$lo_hoja->write(3, 9, "Estado",$lo_titulo);
		$lo_hoja->write(3, 10, "Municipio",$lo_titulo);
		$lo_hoja->write(3, 11, "Parroquia",$lo_titulo);
		$lo_hoja->write(3, 12, "Nómina",$lo_titulo);
		$lo_hoja->write(3, 13, "Cargo",$lo_titulo);
		$lo_hoja->write(3, 14, "Sueldo",$lo_titulo);
		$lo_hoja->write(3, 15, "Dedicación",$lo_titulo);
		$lo_hoja->write(3, 16, "Tipo de Personal",$lo_titulo);
		$lo_hoja->write(3, 17, "Fecha de Ingreso a la Nómina",$lo_titulo);
		$lo_hoja->write(3, 18, "Estatus en Nómina",$lo_titulo);
		$lo_hoja->write(3, 19, "Ubicacion Fisica",$lo_titulo);
		$lo_hoja->write(3, 20, "Estado Ubicacion Fisica",$lo_titulo);
		$lo_hoja->write(3, 21, "Municipio Ubicacion Fisica",$lo_titulo);
		$lo_hoja->write(3, 22, "Parroquia Ubicacion Fisica",$lo_titulo);
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_despaiper=$io_report->DS->data["despaiper"][$li_i];
			$ls_desestper=$io_report->DS->data["desestper"][$li_i];
			$ls_desmunper=$io_report->DS->data["desmunper"][$li_i];
			$ls_desparper=$io_report->DS->data["desparper"][$li_i];
			...
			switch ($ls_nivacaper)
			{
				...
				case "7":
					...
					break;
				case "8":
					$ls_nivacaper="Especialización";
					break;
				case "9":
					$ls_nivacaper="Tecnico Medio";
					break;
			}
				$li_row=$li_row+1;
				...
				$lo_hoja->write($li_row, 8, $ls_despaiper, $lo_dataleft);
				$lo_hoja->write($li_row, 9, $ls_desestper, $lo_dataleft);
				$lo_hoja->write($li_row, 10, $ls_desmunper, $lo_dataleft);
				$lo_hoja->write($li_row, 11, $ls_desparper, $lo_dataleft);
				$lo_hoja->write($li_row, 12, $ls_nomina, $lo_dataleft);
				$lo_hoja->write($li_row, 13, $ls_cargo, $lo_dataleft);
				$lo_hoja->write($li_row, 14, $li_sueper, $lo_dataright);
				$lo_hoja->write($li_row, 15, $ls_desded, $lo_dataleft);
				$lo_hoja->write($li_row, 16, $ls_destipper, $lo_dataleft);
				$lo_hoja->write($li_row, 17, $ld_fechano, $lo_datacenter);
				$lo_hoja->write($li_row, 18, $ls_estnom, $lo_datacenter);
				$lo_hoja->write($li_row, 19, $ls_desubifis, $lo_dataleft);
				$lo_hoja->write($li_row, 20, $ls_desest, $lo_dataleft);
				$lo_hoja->write($li_row, 21, $ls_desmun, $lo_dataleft);
				$lo_hoja->write($li_row, 22, $ls_despar, $lo_dataleft);
		}
