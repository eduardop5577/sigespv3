...
***********************************************************************************/
	...
	function uf_print_detalle($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('fecha'=>array('justification'=>'center','width'=>60, // Justificación y ancho de la columna
						 			   'codigo'=>array('justification'=>'center','width'=>220), // Justificación y ancho de la columna
						 			   'nomina'=>array('justification'=>'left','width'=>220)))); // Justificación y ancho de la columna
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_historicocargospersonal($ls_codperdes,$ls_codperhas); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else  // Imprimimos el reporte
	{
		...
		while ((!$io_report->rs_data->EOF) &&($lb_valido))
		{
			...
			uf_print_cabecera($ls_codper,$ls_apenomper,$io_cabecera,$io_pdf); // Imprimimos la cabecera del registro
			$lb_valido=$io_report->uf_historicocargospersonal_cargo($ls_codper); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				$li_i=0;
				while ((!$io_report->rs_detalle->EOF) && ($lb_valido))
				{
					...
					$ls_codcar=$io_report->rs_detalle->fields["codcar"];
					$ls_descar=$io_report->rs_detalle->fields["descar"];
					$ls_codasicar=$io_report->rs_detalle->fields["codasicar"];
					$ls_desasicar=$io_report->rs_detalle->fields["desasicar"];
					if (trim($ls_codcar)=='0000000000')
					{
						$ls_cargo = $ls_codasicar." ".$ls_desasicar;
					}
					else
					{
						$ls_cargo = $ls_codcar." ".$ls_descar;
					}
					$ls_nomina=$io_report->rs_detalle->fields["codnom"]." ".$io_report->rs_detalle->fields["desnom"];
					$la_data[$li_i]=array('fecha'=>$ld_fecmov,'codigo'=>$ls_cargo,'nomina'=>$ls_nomina);
