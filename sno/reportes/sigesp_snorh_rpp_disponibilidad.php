...
***********************************************************************************/
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_seguridad($as_titulo,$as_titulo2,$as_periodo)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_descripcion="Generó el Reporte ".$as_titulo." ".$as_titulo2.".".$as_periodo;
                $lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_disponibilidad.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------

	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina_general($as_titulo,$as_titulo2,$as_periodo,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_detalle_presupuesto_general($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('estructura'=>array('justification'=>'center','width'=>93), // Justificación y ancho de la columna
						 			   'cuenta'=>array('justification'=>'center','width'=>60), // Justificación y ancho de la columna
						 			   'denominacion'=>array('justification'=>'left','width'=>202), // Justificación y ancho de la columna
									   'gastonomina'=>array('justification'=>'right','width'=>65), // Justificación y ancho de la columna
									   'disponible'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
									   'saldo'=>array('justification'=>'right','width'=>60))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	}// end function uf_print_detalle_presupuesto
	...
	$ld_fecpro=$io_fun_nomina->uf_obtenervalor_get("fecpro","");
        $ls_nominas=$io_fun_nomina->uf_obtenervalor_get("nominas","");
	$li_len1=0;
	$li_len2=0;
	$li_len3=0;
	$li_len4=0;
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=uf_insert_seguridad($ls_titulo,$ls_titulo2,$ls_periodo); // Seguridad de Reporte
	if($lb_valido) // Buscamos la información que afecta el presupuesto
	{
            $lb_valido=$io_report->uf_contableconceptos_presupuesto($ls_nominas);
	}
	...
	else // Imprimimos el reporte
	{
		...
		uf_print_encabezado_pagina_general($ls_titulo,$ls_titulo2,$ls_periodo,$io_pdf);
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$adec_causado = 0;
			$adec_pagado = 0;
			$arrResultado=$io_intspg->uf_spg_saldo_select($ls_codemp, $estprog, $ls_spg_cuenta, $ls_status, $adec_asignado, 
			...
			$li_resto=$li_disponibilidad-$li_monto;
			...
			$la_datag[$li_j]=array('estructura'=>$ls_codestpro,'cuenta'=>$ls_spg_cuenta,'denominacion'=>$ls_denominacion,'gastonomina'=>$li_monto,'disponible'=>$li_disponibilidad,'saldo'=>$li_resto);
			$li_j++;
		}
