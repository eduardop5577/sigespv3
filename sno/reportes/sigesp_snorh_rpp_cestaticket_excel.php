	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			{   
				...
				$li_col=3;
			    $lb_valido=$io_report->uf_pagonomina_concepto_excel(" AND (sigcon='A')",$ls_codperi, $ls_codnomdes, $ls_codnomhas,$ls_codconcdes,$ls_codconchas); 
				if($lb_valido)
				{
				    $columna=0;
				    $columna=$li_col+1;
				...
				$lb_valido=$io_report->uf_pagonomina_concepto_excel(" AND (sigcon='D')",$ls_codperi, $ls_codnomdes, $ls_codnomhas,$ls_codconcdes,$ls_codconchas); 
				if($lb_valido)
				{   
					$li_totrow2=$io_report->DS_detalle->getRowCount("codconc");
	        ...
	        $lb_valido=$io_report->uf_pagonomina_conceptopersonal_excel($ls_codper,$ls_codperi,$ls_codnomdes, $ls_codnomhas," AND (sigcon='A')"); 
			if($lb_valido)
			{   
			    $li_total=0;
				$li_total2=0;
				...
				for($li_k=1;$li_k<=$li_totrow3;$li_k++)
				{
					...
					$li_ticket=number_format(abs($io_report->DS_detalle->data["valsal"][$li_k])/abs($ls_monto_cesta),0);				   				
			...
			$lb_valido1=$io_report->uf_pagonomina_conceptopersonal_excel($ls_codper,$ls_codperi,$ls_codnomdes, $ls_codnomhas," AND (sigcon='D')"); 
			if($lb_valido1)
			{   
			    $li_total=0;
				$li_total3=0;
