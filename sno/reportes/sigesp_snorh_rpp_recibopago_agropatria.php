...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,370,11,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,360,9,$as_periodo); // Agregar el título
		$io_pdf->addText($tm+260,360,10,""); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,345,9,$as_desnom); // Agregar el título
		$ls_rif="Rif.:".$_SESSION['la_empresa']['rifemp'];
		$io_pdf->addText(50,355,9,$ls_rif); // Agregar el título		                                
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,$as_fecingper,$as_sueper,$as_desuniadm,$io_cabecera,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>150), // Justificación y ancho de la columna
									   ...
									   'Sueldo'=>array('justification'=>'left','width'=>70))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
	...
	function uf_print_cabecera2($as_cedper,$as_nomper,$as_descar,$as_fecingper,$as_sueper,$as_desuniadm,$io_cabecera,$io_pdf)
	{
		...
		$io_pdf->ezSetDy(-98);
		$la_data=array(array('nombre'=>'Apellidos y Nombres:', 'cedula'=>'Cédula:', 'cargo'=>'Cargo:', 'unidad'=>'Unidad Organizativa:', 'Sueldo'=>'Sueldo:', 'fecing'=>'Fec. de Ingreso:' ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'','unidad'=>'','Sueldo'=>'','fecing'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						 'fontSize' => 7, // Tamaño de Letras
