...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],50,720,($_SESSION["ls_width"]-20),$_SESSION["ls_height"]); // Agregar Logo
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera1($as_codnom,$as_cedper,$as_nomper,$as_descar,$as_gerencia,$io_cabecera,$ld_fechaingreso,$ls_peractnom,$ld_fecdesper,$ld_fechasper,$ls_codgerencia,$ls_codigocar,$ls_codcueban,$ls_suelper,$ls_rifper,$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data[4]=array('trabajador'=>'Código Nómina:  '.$as_codnom);
		...
		unset($la_config);
		$io_pdf->ezSetDy(48);
		$la_data[1]=array('trabajador'=>'               Fecha de Ingreso.           '.$ld_fechaingreso);
		$la_data[2]=array('trabajador'=>'               Nro. Cuenta Bancaria      '.$ls_codcueban);
		$la_data[3]=array('trabajador'=>'               Sueldo Básico Mensual   '.$ls_suelper);
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('concepto'=>'<b> Concepto                                    Descripción                                                                   Asignaciones                          Deducciones                                </b>'));
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$as_suelper,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('neto'=>array('justification'=>'left','width'=>104))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(440,410,'7','');
		$io_pdf->addText(410,420,'7','');
		$io_pdf->addText(50,410,'7','Sello de la Dirección de Talento Humano');
		$io_pdf->addText(40,420,'7','_________________________________________');
		$io_pdf->addText(200,410,'5','');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	else // Imprimimos el reporte
	{
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($ls_consolidar=="0")
			{
				$ls_peractnom=$io_report->rs_data->fields["codperi"];
			...
			uf_print_cabecera1($ls_codnom,$ls_cedper,$ls_nomper,$ls_descar,$ls_gerencia,$io_cabecera,$ld_fechaingreso,$ls_peractnom,$ld_fecdesper,$ld_fechasper,$ls_codgerencia,$ls_codigocar,$ls_codcueban,$ls_suelper,$ls_rifper,$io_pdf); // Imprimimos la cabecera del registro
				...
				else// No utiliza adelanto de quincena
				{
					for($li_s=1;$li_s<=$li_totrow_det;$li_s++) 
					{
						...
						if(($ls_tipsal=="A") || ($ls_tipsal=="V1") || ($ls_tipsal=="V2") || ($ls_tipsal=="R")) // Buscamos las asignaciones
						{
							...
							$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS_detalle->data["valsal"][$li_s]));
							$la_data_a[$li_asig]=array('concasig'=>$ls_codconc,'denomasig'=>$ls_nomcon,'valorasig'=>$li_valsal);
							$ls_checka=1;
						}
						else // Buscamos las deducciones y aportes
						{
							...
							$li_totded=$li_totded+abs($io_report->DS_detalle->data["valsal"][$li_s]);
							$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS_detalle->data["valsal"][$li_s]));
							$la_data_d[$li_dedu]=array('concdedu'=>$ls_codconc,'denomdedu'=>$ls_nomcon,'valordedu'=>$li_valsal);
							$ls_checkb=1;
						}
					}
				}
