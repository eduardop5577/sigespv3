...
***********************************************************************************/
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$ls_encabezado="<b>REPÚBLICA BOLIVARIANA DE VENEZUELA</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,775,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>ESTADO CARABOBO - MUNICIPIO DE VALENCIA</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,766,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>CONTRALORÍA MUNICIPAL DE VALENCIA</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,757,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>RIF:G-20004957-0</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,748,8,$ls_encabezado); // Agregar el título
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,720,8,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(8,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,705,8,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,352,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>ESTADO CARABOBO - MUNICIPIO DE VALENCIA</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,343,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>CONTRALORÍA MUNICIPAL DE VALENCIA</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,334,8,$ls_encabezado); // Agregar el título
		$ls_encabezado="<b>RIF:G-20004957-0</b>";
		$li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,325,8,$ls_encabezado); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,310,10,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(8,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,300,8,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(8,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,285,8,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,$io_cabecera,$as_fecingper,$as_sueper,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>170), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'left','width'=>330))); // Justificación y ancho de la columna
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>170), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'left','width'=>330))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$la_data=array(array('nombre'=>'Fecha Ing:  '.$as_fecingper, 'cedula'=>'Sueldo:  '.$as_sueper, 'cargo'=>'Compensación: ' ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						 'fontSize' => 8, // Tamaño de Letras
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>180), // Justificación y ancho de la columna
						 			   'cedula'=>array('justification'=>'center','width'=>170), // Justificación y ancho de la columna
		...
		$io_pdf->ezSetY(668);
		$la_data=array(array('denomasig'=>'<b>DENOMINACIÓN</b>', 'valorasig'=>'<b>ASIGNACIÓN</b>', 'denomdedu'=>'<b>DENOMINACIÓN</b>','valordedu'=>'<b>DEDUCCIÓN</b>','saldo'=>'<b>SALDO</b>'));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('denomasig'=>array('justification'=>'center','width'=>120), // Justificación y ancho de la columna
	...
	function uf_print_detalle($la_data,$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('denomasig'=>array('justification'=>'left','width'=>120), // Justificación y ancho de la columna
						 			   'valorasig'=>array('justification'=>'right','width'=>100), // Justificación y ancho de la columna
						...
						case "2": // segunda quincena;
							while(!$io_report->rs_data_detalle->EOF)
							{
								...
								else // Buscamos las deducciones y aportes
								{
									...
									$ls_monto=$io_report->rs_data_detalle->fields["valsal"];
									$ls_prestamo=$io_report->uf_prestamo_personal($ls_codper,$ls_codconc,"D");
