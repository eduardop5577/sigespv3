...
***********************************************************************************/
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo)
	{
		...
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_contableconceptosaportes.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
...
//		$io_pdf->line(50,73,552,73);		
		$io_pdf->line(50,117,552,117);		
		$io_pdf->line(175,60,175,130);		
...
//		$io_pdf->addText(42,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(200,122,7,"AUTORIZADO POR"); // Agregar el título
//		$io_pdf->addText(145,63,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->addText(330,122,7,"REVISADO POR"); // Agregar el título
...
//		$io_pdf->addText(445,63,7,"FIRMA / SELLO / FECHA"); // Agregar el título
	...
	{
		$ls_titulo2="<b>Presupuestario de Conceptos y Aportes</b>";
	}
	$ls_periodo="<b>Período ".$ld_fecdes." - ".$ld_fechas."</b>";
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	{
		$lb_valido=$io_report->uf_contableconceptosaportes_presupuesto($ls_codnom,$ls_codperides,$ls_codperihas,$ls_tipo);
	}
	...
	else // Imprimimos el reporte
	{
		...
		{			
			$ls_programatica=$io_report->rs_data->fields["codestpro1"].$io_report->rs_data->fields["codestpro2"].
							 $io_report->rs_data->fields["codestpro3"].$io_report->rs_data->fields["codestpro4"].
							 $io_report->rs_data->fields["codestpro5"];
			$ls_codest1="";
			$ls_codest2="";
			$ls_codest3="";
			$ls_codest4="";
			...
			$ls_spg_cuenta=$io_report->rs_data->fields["spg_cuenta"];
			$ls_denominacion=$io_report->rs_data->fields["denominacion"];
			$li_total=abs($io_report->rs_data->fields["monto"]);
		...
		{
			$lb_valido=$io_report->uf_contableconceptosaportes_contabilidad($ls_codnom,$ls_codperides,$ls_codperihas,$ls_tipo);
			$li_i=0;
			$li_totalcontadebe=0;
			$li_totalcontahaber=0;
			while((!$io_report->rs_data->EOF)&&($lb_valido))
			{			
				$ls_cueconpatcon=trim($io_report->rs_data->fields["cuenta"]);
				$ls_denominacion=$io_report->rs_data->fields["denominacion"];
				$ls_operacion=$io_report->rs_data->fields["debhab"];
				if($ls_operacion=="D")
				{
					$li_debe=abs($io_report->rs_data->fields["monto"]);
					$li_haber=0;
				...
				{
					$li_debe=0;
					$li_haber=abs($io_report->rs_data->fields["monto"]);
					$li_totalcontadebe=$li_totalcontadebe+$li_debe;
					$li_totalcontahaber=$li_totalcontahaber+$li_haber;
					$li_debe=$io_fun_nomina->uf_formatonumerico($li_debe);
					$li_haber=$io_fun_nomina->uf_formatonumerico($li_haber);
