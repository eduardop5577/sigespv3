...
***********************************************************************************/
	...
	$la_seguridad=Array();
	$la_permisos=Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad("SNR","sigesp_snorh_p_aprobacion_fideicomiso.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_cargarnomina()
   {
		...
		$ls_sql="SELECT sno_nomina.codnom, sno_nomina.desnom ".
			...
			"   AND sss_permisos_internos.codusu='".$_SESSION["la_logusr"]."'".
                        "   AND sno_nomina.codnom IN (SELECT codnom  ".
                        "                               FROM sno_dt_scg ".
                        "                              WHERE sno_dt_scg.codemp = '".$ls_codemp."' ".
			"                                AND sno_dt_scg.estatus = 0 ".
			"                                AND sno_dt_scg.estaprfid = '0' ".
                        "                                AND (sno_dt_scg.tipnom = 'P' OR sno_dt_scg.tipnom='K') ".
                        "                                AND sno_dt_scg.codemp = sno_nomina.codemp ".
                        "                                AND sno_dt_scg.codnom = sno_nomina.codnom) ".
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					{
						$io_fideicomiso->io_mensajes->message("No se pudo aprobar el comprobante ".$ls_comprobante);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
		    $arrResultado= $io_fideicomiso->uf_select_fidei_aprobar($ls_codnom,$lo_object,$li_totrows,0);
