...
***********************************************************************************/
   ...
   session_start();
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad_nomina("SNO","sigesp_sno_d_personaproyecto.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
...
<?php 
	...
	{
		print("<script language=JavaScript>");
		print(" alert('Esta definición esta desactiva para nóminas que no utilizan contabilización por proyectos.');");
	...
	switch ($ls_operacion) 
	{
		...
		case "GUARDAR":
			...
			$arrResultado=$io_proyecto->uf_load_personalproyecto($ls_codper,$li_totdiaper,$li_porcentaje,$li_totrows,$lo_object);
			$li_totdiaper=$arrResultado['ai_totdiaper'];
			$li_porcentaje=$arrResultado['ai_porcentaje'];
		...
		case "ELIMINARPROYECTO":
			...
			for($li_i=1;$li_i<$li_totrows;$li_i++)
			{
				...
				if($ls_proyecto==$ls_codproy)
				{
					...
					{
						$io_proyecto->io_mensajes->message("La Nómina ya se proceso. Reverse la Nómina para eliminar el Proyecto al Personal.");
						$li_j++;
						$ls_nomproy=$_POST["txtnomproy".$li_i];  
						$li_totdiames=$_POST["txttotdiames".$li_i]; 
						$li_pordiames=$_POST["txtpordiames".$li_i]; 
...
function uf_sumar(li_row)
{
	...
	{
	   alert("El valor no es válido, la suma del porcentaje es mayor a 100 % ");
	   eval("f.txtpordiames"+li_row+".value=0"); 
	}
	else
	{
