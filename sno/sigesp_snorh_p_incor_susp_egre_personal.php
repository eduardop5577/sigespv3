...
***********************************************************************************/
	...
	$la_seguridad = Array();
	$la_permisos = Array();	
	$arrResultado=$io_fun_nomina->uf_load_seguridad("SNR","sigesp_snorh_p_incor_susp_egre_personal.php",$ls_permisos,$la_seguridad,$la_permisos);
...
/***********************************************************CARGAR************************************************************************/	
	if ($ls_operacion=="CARGAR")
	{
      $rs_data=0;
      $ls_estper=$_POST["estper"];	
	  $arrResultado=$io_c_incorsuspegr->uf_load_grid_personal($ls_estper,$ls_cmbnomina,$rs_data);
	  $rs_data=$arrResultado['rs_data'];
	  $lb_valido=$arrResultado['lb_valido'];
	  if($lb_valido)
	  {
