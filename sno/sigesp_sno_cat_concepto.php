...
***********************************************************************************/
   ...
   //--------------------------------------------------------------
   function uf_print($as_codconc, $as_nomcon, $as_codnomdes, $as_codnomhas, $as_tipo, $as_sigcon)
   {
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_print
		//		   Access: public
		...
		if(($as_tipo=="copiarconcepto"))
		{		
			print "<table width=500 border=0 cellpadding=1 cellspacing=1 class=fondo-tabla align=center>";
			print "<tr class=titulo-celda>";
			print "<td width=60>Nomina</td>";
			print "<td width=60>Código</td>";
			print "<td width=260>Nombre</td>";
		...
		if(($as_tipo=="copiarconcepto"))
		{
			$ls_sql="  SELECT codnom, codconc, MAX(nomcon) as nomcon,                          ".
					"		  MAX(sigcon) as sigcon, MAX(forcon) as forcon,            ".
					"		  MAX(forpatcon) as forpatcon,MAX(intprocon) as intprocon  ". 
					"	 FROM sno_concepto                                             ".
					"   WHERE codemp='".$ls_codemp."'                                  ".
					"     AND sigcon='".$as_sigcon."' 								   ".
					"   GROUP BY codnom,codconc                                        ".
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					...
					case "copiarconcepto": // 
						$ls_nomina=$row["codnom"];
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarcopiarconcepto('$ls_nomina','$ls_codconc');\">".$ls_nomina."</a></td>";
...
<?php
	...
	$ls_sigcon=$io_fun_nomina->uf_obtenervalor_get("sigcon","");
	if($ls_operacion=="BUSCAR")
	{
		$ls_codconc="%".$_POST["txtcodconc"]."%";
		$ls_nomcon="%".$_POST["txtnomcon"]."%";
		uf_print($ls_codconc, $ls_nomcon, $ls_codnomdes, $ls_codnomhas, $ls_tipo, $ls_sigcon);
	}
	else
	{
		$ls_codconc="%%";
...
function aceptarvacacion(codconc,nomcon,sigcon,forcon,forpatcon)
{
	...
	if (forpatcon=="")
	{
		opener.document.form1.txtforpatsalvac.value=forcon;
		opener.document.form1.txtforpatreivac.value=forcon;
	}
	else
	{
...
function aceptarcopiarconcepto(codnom,codconc)
{
  	opener.document.form1.operacion.value="COPIARCONCEPTO";
  	opener.document.form1.action="sigesp_sno_d_concepto.php?nomina="+codnom+"&concepto="+codconc;
  	opener.document.form1.submit();
	close();
}
